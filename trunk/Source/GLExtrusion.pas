...
const
  ...
  procedure RenderSides(prevRow, curRow: PRowData; TextCoordTileS, TextCoordTileT: Single);
  var
  ...
  begin
    ...
    if NodesColorMode <> pncmNone then
      GL.Color4fv(@prevRow^.color);
    GL.TexCoord2f(TextCoordTileS, prevRow^.textcoordT * TextCoordTileT); //0
...
begin
  ...
  begin
    ...
    begin
      RenderDisk(@rows[0], Nodes[0].AsVector, normal, False, TexCoordTileS);
      FTriangleCount := FTriangleCount + Slices * 2; //Slices+1;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ppOutside in Parts then
          RenderSides(@rows[curRow xor 1], @rows[curRow], TexCoordTileS, TexCoordTileT);
        if ppInside in Parts then
          RenderSides(@rows[curRow], @rows[curRow xor 1], TexCoordTileS, TexCoordTileT);
      end;
  ...
  begin
    ...
    if ppInside in Parts then
    begin
      RenderDisk(@rows[0], Nodes[i].AsVector, normal, True, TexCoordTileS);
      FTriangleCount := FTriangleCount + Slices * 2; //Slices+1;
    end;
  end;
  if SplineMode = lsmCubicSpline then
