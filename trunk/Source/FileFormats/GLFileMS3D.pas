...
begin
    ...
    try
        ...
        if (not(copy(path,length(path),1)='\')) then path:=path+'\';
        ...
        ms3d_norm.X:=0; ms3d_norm.Y:=0; ms3d_norm.Z:=0;
        setLength(ms3d_norm_Array,nNumVertices);
        for i:=0 to nNumVertices-1 do ms3d_norm_Array[i]:=ms3d_norm;
        ...
        for i := 0 to nNumGroups - 1 do begin
            ...
            if Group.materialIndex = -1 then begin
        ...
        for i := 0 to nNumMaterials-1 do begin
            ...
            if Assigned(Owner.MaterialLibrary) then begin
                ...
                // colors above
                if ms3d_material.transparency<1 then begin
                ...
                for j := 0 to GroupList.Count-1 do begin
                    ...
                    if Group.materialIndex = i then
                        for k := 0 to Group.numtriangles - 1 do begin
        ...
        for i := 0 to nNumJoints-1 do begin
            ...
            if (i=0) then Assert(ms3d_joints^[i].base.numKeyFramesRot=iTotalFrames,'This importer only works if the number of key frames = the number of total frames. i.e. Every frame must be a key frame');
            if ms3d_joints^[i].base.numKeyFramesRot>0 then begin
            ...
            if ms3d_joints^[i].base.numKeyFramesTrans>0 then begin
        ...
        if (nNumGroupComments>0) then groupCommentList.Capacity:=nNumGroupComments;
        for i:=0 to nNumGroupComments-1 do begin
        ...
        if (nNumMaterialComments>0) then MaterialCommentList.Capacity:=nNumMaterialComments;
        for i:=0 to nNumMaterialComments-1 do begin
        ...
        if (nNumJointComments>0) then JointCommentList.Capacity:=nNumJointComments;
        for i:=0 to nNumJointComments-1 do begin
        ...
        for i:=0 to nHasModelComment-1 do begin
        ...
        if Owner is TGLActor then begin
            for i:=0 to nNumVertices-1 do begin
                ...
                if (vertexWeight.boneIds[0]<>255) then begin
                ...
                if (vertexWeight.boneIds[1]<>255) then begin
                ...
                if (vertexWeight.boneIds[2]<>255) then begin
        ...
        if (Owner is TGLActor) and (nNumJoints>0) then begin
            // Bone names are added to a list initally to sort out parents
            bonelist:=TStringList.Create;
            for i := 0 to nNumJoints-1 do bonelist.Add(ms3d_joints^[i].Base.Name);
            ...
            for i:=0 to nNumJoints-1 do begin
                for j:=0 to ms3d_joints^[i].Base.NumKeyFramesRot-1 do begin
        ...
        if Assigned(ms3d_vertices) then FreeMem(ms3d_vertices);
        if Assigned(ms3d_triangles) then FreeMem(ms3d_triangles);
        if Assigned(ms3d_joints) then begin
            // Free the internal storage of the joint
            for i := 0 to nNumJoints-1 do begin
                if Assigned(ms3d_joints^[i].keyFramesRot) then FreeMem(ms3d_joints^[i].keyFramesRot);
                if Assigned(ms3d_joints^[i].keyFramesTrans) then FreeMem(ms3d_joints^[i].keyFramesTrans);
            end;
            FreeMem(ms3d_joints);
        end;

        // Finalize
        for i := 0 to GroupList.Count-1 do TMS3DGroup(GroupList[i]).Free;
        GroupList.Free;
        if (assigned(groupCommentList)) then groupCommentList.free;
        if (assigned(materialCommentList)) then materialCommentList.free;
        if (assigned(jointCommentList)) then jointCommentList.free;
        if (assigned(modelCommentList)) then modelCommentList.free;
    end;
end;
