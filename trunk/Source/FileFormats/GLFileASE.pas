    ...
    for i := 0 to aASEMesh.Faces.Count - 1 do begin
      ...
      if tex then begin
        ...
        if light then begin
          ...
          aMesh.LightMapTexCoords.Add(lmt[0].Coord[0], lmt[0].Coord[1]);
          aMesh.LightMapTexCoords.Add(lmt[1].Coord[0], lmt[1].Coord[1]);
          aMesh.LightMapTexCoords.Add(lmt[2].Coord[0], lmt[2].Coord[1]);
        end;
      end;
...
begin
  ...
  Value := GetFirstValue(Data);
  Result.Coord[0] := StringToFloatRegular(Value);

  Start := GetEndOfFirstValue(Data) + 1;
  Data := Copy(Data, Start, Length(Data) - Start + 1);
  Value := GetFirstValue(Data);
  Result.Coord[1] := StringToFloatRegular(Value);

  Start := GetEndOfFirstValue(Data) + 1;
  Data := Copy(Data, Start, Length(Data) - Start + 1);
  Value := GetFirstValue(Data);
  Result.Coord[2] := StringToFloatRegular(Value);
end;
...
begin
  ...
  while not IsEndOfSection(Data) do begin
    ...
    face.FTextChannels.ChanelTexture[chanelIdx].Idx0 := Round(v3D.Coord[0]);
    face.FTextChannels.ChanelTexture[chanelIdx].Idx1 := Round(v3D.Coord[1]);
    face.FTextChannels.ChanelTexture[chanelIdx].Idx2 := Round(v3D.Coord[2]);
...
begin
  ...
  while not IsEndOfSection(Data) do begin
    ...
    case GetTagOnData(Data) of
      ...
      ASCII_ROW_I:          aMesh.FMatrix.Coord[0] := VectorMake(GetValue3D(Data));
      ASCII_ROW1_I:         aMesh.FMatrix.Coord[1] := VectorMake(GetValue3D(Data));
      ASCII_ROW2_I:         aMesh.FMatrix.Coord[2] := VectorMake(GetValue3D(Data));
      ASCII_ROW3_I:         aMesh.FMatrix.Coord[3] := PointMake(GetValue3D(Data));
