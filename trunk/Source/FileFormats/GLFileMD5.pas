...
procedure TGLMD5VectorFile.LoadFromStream(aStream : TStream);
  ...
  begin
    ...
    pos.Coord[0]:=GLUtils.StrToFloatDef(FTempString[2]);
    pos.Coord[1]:=GLUtils.StrToFloatDef(FTempString[4]);
    pos.Coord[2]:=GLUtils.StrToFloatDef(FTempString[3]);
    ...
    for i:=0 to FJointFlags.Count-1 do begin
      ...
      if FJointFlags[i] > 0 then begin
        ...
        if FJointFlags[i] and 1 > 0 then begin
          pos.Coord[0]:=GLUtils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 2 > 0 then begin
          pos.Coord[1]:=GLUtils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 4 > 0 then begin
          pos.Coord[2]:=GLUtils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;

        if FJointFlags[i] and 8 > 0 then begin
          quat.ImagPart.Coord[0]:=GLUtils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 16 > 0 then begin
          quat.ImagPart.Coord[1]:=GLUtils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 32 > 0 then
          quat.ImagPart.Coord[2]:=GLUtils.StrToFloatDef(FTempString[j]);
      end;
