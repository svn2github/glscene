...
begin
  ...
  FLightSrc.Diffuse.Color := VectorScale(FLightSrc.Diffuse.Color, AData.Multiplier);  //надо потестить
          ...
          if Trim(material.Bump.Map.NameStr) <> '' then
            try
              ...
              begin
                ...
                Texture.Disabled := False;
                Texture.TextureMode := tmModulate;
                //Texture.TextureFormat:=tfNormalMap;  //≈сли этот параметр активен, то должна быть карта высот(как в макс-е)
                TextureIndex := 2;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //≈сли матрица не нормализована, т.е. третий столбец не равен векторному произведению первых двух столбцов,
        //то значит еЄ нужно повернуть на -pi вокруг оси Y.
        m := Result;
        v4 := m[3];
