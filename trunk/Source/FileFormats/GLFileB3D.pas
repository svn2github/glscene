  ...
  try
    ...
    begin
      ...
      begin
        ...
        RotQuat := QuaternionMake([Node^.Rotation.Coord[2], Node^.Rotation.Coord[1],
          Node^.Rotation.Coord[3]], Node^.Rotation.Coord[0]);
        ...
        if Pos('ENT_', UpperCase(Mo.Name)) = 0 then
          V := AffineVectorMake(Node^.Position.Coord[1],
            Node^.Position.Coord[0], Node^.Position.Coord[2])
        else
        begin
          V := AffineVectorMake(0.0, 0.0, 0.0);
        end;

        V1 := AffineVectorMake(Node^.Scale.Coord[1], Node^.Scale.Coord[0], Node^.Scale.Coord[2]);
