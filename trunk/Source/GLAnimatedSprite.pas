...
// Create
//
constructor TSpriteAnimFrame.Create(aOwner : TXCollection);
begin
  inherited;
  FFrameIndex:=0;
end;
...
// SetFrameIndex
//
procedure TSpriteAnimFrame.SetFrameIndex(const Value: Integer);
begin
  ...
  if Assigned(Owner) then begin
    if Assigned(Owner.Owner) then
      if Owner.Owner is TGLBaseSceneObject then
        TGLBaseSceneObject(Owner.Owner).NotifyChange(nil);
  end;
end;
...
// DoRender
//
procedure TGLAnimatedSprite.DoRender(var rci : TRenderContextInfo;
...
begin
  if renderSelf and (FAnimationIndex<>-1) and (FAnimationIndex<Animations.Count) then begin
    Anim:=TSpriteAnimation(Animations[FAnimationIndex]);
    if (Anim.CurrentFrame<>-1) and (Anim.CurrentFrame<Anim.Frames.Count) then begin
      ...
      if Assigned(libMat) then begin
        ...
        if Anim.Dimensions = sfdManual then begin
          ...
          if TexWidth>0 then begin
            x0:=Anim.FrameWidth*(Frame.FrameIndex mod (TexWidth div Anim.FrameWidth));
            y0:=Anim.FrameHeight*(Frame.FrameIndex div (TexWidth div Anim.FrameWidth));
          end else begin
            x0:=0;
            y0:=0;
          end;
...
begin
  ...
  currentFrame:=TSpriteAnimation(Animations[FAnimationIndex]).CurrentFrame;
  endFrame:=TSpriteAnimation(Animations[FAnimationIndex]).Frames.Count-1;
  ...
  case AnimationMode of
    ...
    samBounceBackward, samLoopBackward : begin
      Dec(CurrentFrame);
      if (currentFrame = 0) and Assigned(FOnStartFrameReached) then
  ...
  TSpriteAnimation(Animations[FAnimationIndex]).CurrentFrame:=currentFrame;
end;
