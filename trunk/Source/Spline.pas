...
begin
 ...
 tciStart:
   begin
     Result.V[0]:=fSplineX.D[1];
     Result.V[1]:=fSplineY.D[1];
     Result.V[2]:=fSplineZ.D[1];
     Result.V[3]:=fSplineW.D[1];
   end;
 TciEnd:
   begin
     Result.V[0]:=fSplineX.D[fSplineX.FSize];
     Result.V[1]:=fSplineY.D[fSplineY.FSize];
     Result.V[2]:=fSplineZ.D[fSplineZ.FSize];
     Result.V[3]:=fSplineW.D[fSplineW.FSize];
   end;
 end;
...
begin
   Result.V[0]:=fSplineX.U[Index+1];
   Result.V[1]:=fSplineY.U[Index+1];
   Result.V[2]:=fSplineZ.U[Index+1];
   Result.V[3]:=fSplineW.U[Index+1];
end;
...
//         if Tau[i-1]=tau[i-1] then Tau[i]:=ti+1.e-6;
         if ti=tau[i-1] then Tau[i]:=ti+1.0e-6
         else Tau[i]:=ti
       end;
     dcmSqLinear:
       begin
...
begin
 ...
 tciStart:
   begin
     fSplineX.D[1]:=Value.V[0];
     fSplineY.D[1]:=Value.V[1];
     fSplineZ.D[1]:=Value.V[2];
     fSplineW.D[1]:=Value.V[3];
   end;
 TciEnd:
   begin
     fSplineX.D[fSplineX.FSize]:=Value.V[0];
     fSplineY.D[fSplineY.FSize]:=Value.V[1];
     fSplineZ.D[fSplineZ.FSize]:=Value.V[2];
     fSplineW.D[fSplineW.FSize]:=Value.V[3];
   end;
 end;
 finally
  EndUpdate;
...
begin
 BeginUpdate;
 try
     fSplineX.U[index+1]:=Value.V[0];
     fSplineY.U[index+1]:=Value.V[1];
     fSplineZ.U[index+1]:=Value.V[2];
     fSplineW.U[index+1]:=Value.V[3];
 except
   Endupdate;
 end
...
begin
   tau:=gettau(t);
   vector.V[0]:=FsplineX[tau];
   vector.V[1]:=FsplineY[tau];
   vector.V[2]:=FsplineZ[tau];
end;
...
begin
   tau:=gettau(t);
   result.V[0]:=fSplineX[tau];
   result.V[1]:=FsplineY[tau];
   result.V[2]:=FsplineZ[tau];
end;
...
begin
   tau:=getTau(t);
   Result.V[0]:=self.fSplineX.Convex[Tau];
   Result.V[1]:=self.fSplineY.Convex[Tau];
   Result.V[2]:=self.fSplineZ.Convex[Tau];
end;
...
begin
   tau:=getTau(t);
   Result.V[0]:=self.fSplineX.Slope[Tau];
   Result.V[1]:=self.fSplineY.Slope[Tau];
   Result.V[2]:=self.fSplineZ.Slope[Tau];
end;
