    ...
    case SelAxis of
      GaX:
        if not Viewer.Buffer.ScreenVectorIntersectWithPlaneXZ(V,
          SelectedObj.AbsolutePosition.Coord[1], Result) then
          MakeVector(Result, X / 5, 0, 0);

      GaY:
        if not Viewer.Buffer.ScreenVectorIntersectWithPlaneYZ(V,
          SelectedObj.AbsolutePosition.Coord[0], Result) then
      ...
      GaXY:
        begin
          Viewer.Buffer.ScreenVectorIntersectWithPlaneXY(V,
            SelectedObj.AbsolutePosition.Coord[2], Result);
        end;
      GaXZ:
        begin
          Viewer.Buffer.ScreenVectorIntersectWithPlaneXZ(V,
            SelectedObj.AbsolutePosition.Coord[1], Result);
        end;
      GaYZ:
        begin
          Viewer.Buffer.ScreenVectorIntersectWithPlaneYZ(V,
            SelectedObj.AbsolutePosition.Coord[0], Result);
        end;
    end;
...
var
  ...
  begin
    for T := 0 to 3 do
    begin
      QuantizedMousePos.Coord[T] := (Round(MousePos.Coord[T] / MoveCoef)) * MoveCoef;
      QuantizedMousePos2.Coord[T] := (Round(LastMousePos.Coord[T] / MoveCoef)) * MoveCoef;
    end;
    case SelAxis of
      GaX:
        begin
  ...
  begin
    ...
    begin
      if RotationCoef > 1 then
        Vec1.Coord[0] := RotationCoef * (Round((X - Rx) / (RotationCoef)))
      else
        Vec1.Coord[0] := RotationCoef * (X - Rx);
      Rx := X;
    end;
    if Abs(Y - Ry) >= RotationCoef then
    begin
      if RotationCoef > 1 then
        Vec1.Coord[1] := RotationCoef * (Round((Y - Ry) / (RotationCoef)))
      else
        Vec1.Coord[1] := RotationCoef * (Y - Ry);
      Ry := Y;
    end;
      ...
      GaX:
        begin
          ...
          RotateAroundArbitraryAxis(SelectedObj, RotV,
            AffineVectorMake(SelectedObj.Position.AsVector), Vec1.Coord[1]);
        end;
      GaY:
        begin
          RotV := VectorTransform(YVector, Pmat);
          RotateAroundArbitraryAxis(SelectedObj, RotV,
            AffineVectorMake(SelectedObj.Position.AsVector), Vec1.Coord[0]);
        end;
      GaZ:
        begin
  ...
  begin
    ...
    begin
      ...
      QuantizedMousePos2.Coord[T] := (Round(LastMousePos.Coord[T] / FScaleCoef)) *
        FScaleCoef;
    end;
    case SelAxis of
      GaX:
        begin
          ...
          begin
            MakeVector(Vec1, QuantizedMousePos.Coord[0], QuantizedMousePos.Coord[0],
              QuantizedMousePos.Coord[0]);
            MakeVector(Vec2, QuantizedMousePos2.Coord[0], QuantizedMousePos2.Coord[0],
              QuantizedMousePos2.Coord[0]);
          end
          else
          begin
            MakeVector(Vec1, QuantizedMousePos.Coord[0], 0, 0);
      ...
      GaY:
        begin
          if FForceUniformScale then
          begin
            MakeVector(Vec1, QuantizedMousePos.Coord[1], QuantizedMousePos.Coord[1],
              QuantizedMousePos.Coord[1]);
            MakeVector(Vec2, QuantizedMousePos2.Coord[1], QuantizedMousePos2.Coord[1],
              QuantizedMousePos2.Coord[1]);
          end
          else
          begin
            MakeVector(Vec1, 0, QuantizedMousePos.Coord[1], 0);
      ...
      GaZ:
        begin
          if FForceUniformScale then
          begin
            MakeVector(Vec1, QuantizedMousePos.Coord[2], QuantizedMousePos.Coord[2],
              QuantizedMousePos.Coord[2]);
            MakeVector(Vec2, QuantizedMousePos2.Coord[2], QuantizedMousePos2.Coord[2],
              QuantizedMousePos2.Coord[2]);
          end
          else
          begin
            MakeVector(Vec1, 0, 0, QuantizedMousePos.Coord[2]);
