...
begin
  ...
  begin
    for J := 0 to 3 do
      if C1.BBox[CBBFront[J]].Coord[0] < C2.BBox[I].Coord[0] then
        SetPlanBB(C1, 0, C2.BBox[I].Coord[0]);
    for J := 0 to 3 do
      if C1.BBox[CBBBack[J]].Coord[0] > C2.BBox[I].Coord[0] then
        SetPlanBB(C1, 1, C2.BBox[I].Coord[0]);
    for J := 0 to 3 do
      if C1.BBox[CBBLeft[J]].Coord[1] < C2.BBox[I].Coord[1] then
        SetPlanBB(C1, 2, C2.BBox[I].Coord[1]);
    for J := 0 to 3 do
      if C1.BBox[CBBRight[J]].Coord[1] > C2.BBox[I].Coord[1] then
        SetPlanBB(C1, 3, C2.BBox[I].Coord[1]);
    for J := 0 to 3 do
      if C1.BBox[CBBTop[J]].Coord[2] < C2.BBox[I].Coord[2] then
        SetPlanBB(C1, 4, C2.BBox[I].Coord[2]);
    for J := 0 to 3 do
      if C1.BBox[CBBBottom[J]].Coord[2] > C2.BBox[I].Coord[2] then
        SetPlanBB(C1, 5, C2.BBox[I].Coord[2]);
  end;
  Result := C1;
end;
...
procedure AddAABB(var Aabb: TAABB; const Aabb1: TAABB);
begin
  if Aabb1.Min.Coord[0] < Aabb.Min.Coord[0] then
    Aabb.Min.Coord[0] := Aabb1.Min.Coord[0];
  if Aabb1.Min.Coord[1] < Aabb.Min.Coord[1] then
    Aabb.Min.Coord[1] := Aabb1.Min.Coord[1];
  if Aabb1.Min.Coord[2] < Aabb.Min.Coord[2] then
    Aabb.Min.Coord[2] := Aabb1.Min.Coord[2];
  if Aabb1.Max.Coord[0] > Aabb.Max.Coord[0] then
    Aabb.Max.Coord[0] := Aabb1.Max.Coord[0];
  if Aabb1.Max.Coord[1] > Aabb.Max.Coord[1] then
    Aabb.Max.Coord[1] := Aabb1.Max.Coord[1];
  if Aabb1.Max.Coord[2] > Aabb.Max.Coord[2] then
    Aabb.Max.Coord[2] := Aabb1.Max.Coord[2];
end;
...
begin
  ...
  Bb.Max := Bb.Min;
  AABBInclude(Bb, VectorTransform(AffineVectorMake(OldMin.Coord[0],
    OldMin.Coord[1], OldMax.Coord[2]), M));
  AABBInclude(Bb, VectorTransform(AffineVectorMake(OldMin.Coord[0],
    OldMax.Coord[1], OldMin.Coord[2]), M));
  AABBInclude(Bb, VectorTransform(AffineVectorMake(OldMin.Coord[0],
    OldMax.Coord[1], OldMax.Coord[2]), M));
  AABBInclude(Bb, VectorTransform(AffineVectorMake(OldMax.Coord[0],
    OldMin.Coord[1], OldMin.Coord[2]), M));
  AABBInclude(Bb, VectorTransform(AffineVectorMake(OldMax.Coord[0],
    OldMin.Coord[1], OldMax.Coord[2]), M));
  AABBInclude(Bb, VectorTransform(AffineVectorMake(OldMax.Coord[0],
    OldMax.Coord[1], OldMin.Coord[2]), M));
...
procedure AABBInclude(var Bb: TAABB; const P: TAffineVector);
begin
  if P.Coord[0] < Bb.Min.Coord[0] then
    Bb.Min.Coord[0] := P.Coord[0];
  if P.Coord[0] > Bb.Max.Coord[0] then
    Bb.Max.Coord[0] := P.Coord[0];
  if P.Coord[1] < Bb.Min.Coord[1] then
    Bb.Min.Coord[1] := P.Coord[1];
  if P.Coord[1] > Bb.Max.Coord[1] then
    Bb.Max.Coord[1] := P.Coord[1];
  if P.Coord[2] < Bb.Min.Coord[2] then
    Bb.Min.Coord[2] := P.Coord[2];
  if P.Coord[2] > Bb.Max.Coord[2] then
    Bb.Max.Coord[2] := P.Coord[2];
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        // check point in Wire
        if IsInRange(P.Coord[0], Pt1[CWires[I, 0]].Coord[0],
          Pt1[CWires[I, 1]].Coord[0]) and
          IsInRange(P.Coord[1], Pt1[CWires[I, 0]].Coord[1],
          Pt1[CWires[I, 1]].Coord[1]) and
          IsInRange(P.Coord[2], Pt1[CWires[I, 0]].Coord[2],
          Pt1[CWires[I, 1]].Coord[2]) then
        begin
          // check point in Plane
          if IsInRange(P.Coord[0], Pt2[CPlanes[T, 0]].Coord[0],
            Pt2[CPlanes[T, 2]].Coord[0]) and
            IsInRange(P.Coord[1], Pt2[CPlanes[T, 0]].Coord[1],
            Pt2[CPlanes[T, 2]].Coord[1]) and
            IsInRange(P.Coord[2], Pt2[CPlanes[T, 0]].Coord[2],
            Pt2[CPlanes[T, 2]].Coord[2]) then
          begin
            Result := True;
            Exit;
          end;
...
begin
  ...
  if (AABB2.Min.Coord[0] > AABB1.Max.Coord[0]) or
    (AABB2.Min.Coord[1] > AABB1.Max.Coord[1]) then
    Exit
  else if (AABB2.Max.Coord[0] < AABB1.Min.Coord[0]) or
    (AABB2.Max.Coord[1] < AABB1.Min.Coord[1]) then
    Exit
  else
    Result := True;
...
function IntersectAABBsAbsoluteXZ(const Aabb1, Aabb2: TAABB): Boolean;
begin
  Result := ((AABB1.Min.Coord[0] < AABB2.Max.Coord[0]) and
...
function IntersectAABBsAbsolute(const Aabb1, Aabb2: TAABB): Boolean;
begin
  Result := not((AABB1.Min.Coord[0] > AABB2.Max.Coord[0]) or
...
begin
  ...
  Result := (AABB1.Min.Coord[0] >= AABB2.Min.Coord[0]) and
...
begin
  Result := (P.Coord[0] <= Aabb.Max.Coord[0]) and
    (P.Coord[0] >= Aabb.Min.Coord[0]) and (P.Coord[1] <= Aabb.Max.Coord[1]) and
    (P.Coord[1] >= Aabb.Min.Coord[1]) and (P.Coord[2] <= Aabb.Max.Coord[2]) and
    (P.Coord[2] >= Aabb.Min.Coord[2]);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      vec.Coord[j] := box[index[j]].Coord[j];
    end;
    ...
    begin
      ...
      begin
        temp := vec;
        temp.Coord[j mod 3] := box[(index[j mod 3] + 1) mod 2].Coord[j mod 3];
        if (j div 3) > 0 then
        begin
          temp.Coord[(j+1) mod 3] := box[(index[(j+1) mod 3] + 1) mod 2].Coord[(j+1) mod 3];
          if (j div 3) > 1 then
          begin
            temp.Coord[(j+2) mod 3] := box[(index[(j+2) mod 3] + 1) mod 2].Coord[(j+2) mod 3];
          end;
        end;
        V[j+1] := temp;
      end;
      for j := 0 to 2 do
        vec.Coord[j] := box[(index[j]+1) mod 2].Coord[j];
      V[7] := vec;
    end;
  end;
...
begin
  ...
  for Q := 0 to 2 do
  begin
    if (Normal.Coord[Q] > 0.0) then
    begin
      Vmin.Coord[Q] := -Maxbox.Coord[Q];
      Vmax.Coord[Q] := Maxbox.Coord[Q];
    end
    else
    begin
      Vmin.Coord[Q] := Maxbox.Coord[Q];
...
begin
  ...
  P0 := E0.Coord[2] * Tv0.Coord[1] - E0.Coord[1] * Tv0.Coord[2];
  P2 := E0.Coord[2] * Tv2.Coord[1] - E0.Coord[1] * Tv2.Coord[2];
  Min := MinFloat(P0, P2);
  Max := MaxFloat(P0, P2);
  Rad := Fez * Boxhalfsize.Coord[1] + Fey * Boxhalfsize.Coord[2];
  if (Min > Rad) or (Max < -Rad) then
    Exit;

  // AXISTEST_Y02(e0[Z], e0[X], fez, fex);
  P0 := -E0.Coord[2] * Tv0.Coord[0] + E0.Coord[0] * Tv0.Coord[2];
  P2 := -E0.Coord[2] * Tv2.Coord[0] + E0.Coord[0] * Tv2.Coord[2];
  Min := MinFloat(P0, P2);
  Max := MaxFloat(P0, P2);
  Rad := Fez * Boxhalfsize.Coord[0] + Fex * Boxhalfsize.Coord[2];
  if (Min > Rad) or (Max < -Rad) then
    Exit;

  // AXISTEST_Z12(e0[Y], e0[X], fey, fex);
  P1 := E0.Coord[1] * Tv1.Coord[0] - E0.Coord[0] * Tv1.Coord[1];
  P2 := E0.Coord[1] * Tv2.Coord[0] - E0.Coord[0] * Tv2.Coord[1];
  Min := MinFloat(P1, P2);
  Max := MaxFloat(P1, P2);
  Rad := Fey * Boxhalfsize.Coord[0] + Fex * Boxhalfsize.Coord[1];
  if (Min > Rad) or (Max < -Rad) then
    Exit;
  ...
  P0 := E1.Coord[2] * Tv0.Coord[1] - E1.Coord[1] * Tv0.Coord[2];
  P2 := E1.Coord[2] * Tv2.Coord[1] - E1.Coord[1] * Tv2.Coord[2];
  Min := MinFloat(P0, P2);
  Max := MaxFloat(P0, P2);
  Rad := Fez * Boxhalfsize.Coord[1] + Fey * Boxhalfsize.Coord[2];
  if (Min > Rad) or (Max < -Rad) then
  ...
  P0 := -E1.Coord[2] * Tv0.Coord[0] + E1.Coord[0] * Tv0.Coord[2];
  P2 := -E1.Coord[2] * Tv2.Coord[0] + E1.Coord[0] * Tv2.Coord[2];
  Min := MinFloat(P0, P2);
  Max := MaxFloat(P0, P2);
  Rad := Fez * Boxhalfsize.Coord[0] + Fex * Boxhalfsize.Coord[2];
  if (Min > Rad) or (Max < -Rad) then
  ...
  P0 := E1.Coord[1] * Tv0.Coord[0] - E1.Coord[0] * Tv0.Coord[1];
  P1 := E1.Coord[1] * Tv1.Coord[0] - E1.Coord[0] * Tv1.Coord[1];
  Min := MinFloat(P0, P1);
  Max := MaxFloat(P0, P1);
  Rad := Fey * Boxhalfsize.Coord[0] + Fex * Boxhalfsize.Coord[1];
  if (Min > Rad) or (Max < -Rad) then
  ...
  P0 := E2.Coord[2] * Tv0.Coord[1] - E2.Coord[1] * Tv0.Coord[2];
  P1 := E2.Coord[2] * Tv1.Coord[1] - E2.Coord[1] * Tv1.Coord[2];
  Min := MinFloat(P0, P1);
  Max := MaxFloat(P0, P1);
  Rad := Fez * Boxhalfsize.Coord[1] + Fey * Boxhalfsize.Coord[2];
  if (Min > Rad) or (Max < -Rad) then
  ...
  P0 := -E2.Coord[2] * Tv0.Coord[0] + E2.Coord[0] * Tv0.Coord[2];
  P1 := -E2.Coord[2] * Tv1.Coord[0] + E2.Coord[0] * Tv1.Coord[2];
  Min := MinFloat(P0, P1);
  Max := MaxFloat(P0, P1);
  Rad := Fez * Boxhalfsize.Coord[0] + Fex * Boxhalfsize.Coord[2];
  if (Min > Rad) or (Max < -Rad) then
  ...
  P1 := E2.Coord[1] * Tv1.Coord[0] - E2.Coord[0] * Tv1.Coord[1];
  P2 := E2.Coord[1] * Tv2.Coord[0] - E2.Coord[0] * Tv2.Coord[1];
  Min := MinFloat(P1, P2);
  Max := MaxFloat(P1, P2);
  Rad := Fey * Boxhalfsize.Coord[0] + Fex * Boxhalfsize.Coord[1];
  if (Min > Rad) or (Max < -Rad) then
  ...
  // test in X-direction
  FindMinMax(Tv0.Coord[0], Tv1.Coord[0], Tv2.Coord[0], Min, Max);
  if (Min > Boxhalfsize.Coord[0]) or (Max < -Boxhalfsize.Coord[0]) then
    Exit;

  // test in Y-direction
  FindMinMax(Tv0.Coord[1], Tv1.Coord[1], Tv2.Coord[1], Min, Max);
  if (Min > Boxhalfsize.Coord[1]) or (Max < -Boxhalfsize.Coord[1]) then
    Exit;

  // test in Z-direction
  FindMinMax(Tv0.Coord[2], Tv1.Coord[2], Tv2.Coord[2], Min, Max);
  if (Min > Boxhalfsize.Coord[2]) or (Max < -Boxhalfsize.Coord[2]) then
    Exit;
...
procedure ExtractAABBCorners(const AABB: TAABB; var AABBCorners: TAABBCorners);
begin
  MakeVector(AABBCorners[0], AABB.Min.Coord[0], AABB.Min.Coord[1],
    AABB.Min.Coord[2]);
  MakeVector(AABBCorners[1], AABB.Min.Coord[0], AABB.Min.Coord[1],
    AABB.Max.Coord[2]);
  MakeVector(AABBCorners[2], AABB.Min.Coord[0], AABB.Max.Coord[1],
    AABB.Min.Coord[2]);
  MakeVector(AABBCorners[3], AABB.Min.Coord[0], AABB.Max.Coord[1],
    AABB.Max.Coord[2]);

  MakeVector(AABBCorners[4], AABB.Max.Coord[0], AABB.Min.Coord[1],
    AABB.Min.Coord[2]);
  MakeVector(AABBCorners[5], AABB.Max.Coord[0], AABB.Min.Coord[1],
    AABB.Max.Coord[2]);
  MakeVector(AABBCorners[6], AABB.Max.Coord[0], AABB.Max.Coord[1],
    AABB.Min.Coord[2]);
  MakeVector(AABBCorners[7], AABB.Max.Coord[0], AABB.Max.Coord[1],
    AABB.Max.Coord[2]);
end;
...
begin
  ...
  if ((MainAABB.Min.Coord[0] < TestAABB.Max.Coord[0]) and
...
begin
  ...
  if Result.Coord[0] < AABB.Min.Coord[0] then
    Result.Coord[0] := AABB.Min.Coord[0];
  if Result.Coord[1] < AABB.Min.Coord[1] then
    Result.Coord[1] := AABB.Min.Coord[1];
  if Result.Coord[2] < AABB.Min.Coord[2] then
    Result.Coord[2] := AABB.Min.Coord[2];

  if Result.Coord[0] > AABB.Max.Coord[0] then
    Result.Coord[0] := AABB.Max.Coord[0];
  if Result.Coord[1] > AABB.Max.Coord[1] then
    Result.Coord[1] := AABB.Max.Coord[1];
  if Result.Coord[2] > AABB.Max.Coord[2] then
    Result.Coord[2] := AABB.Max.Coord[2];
end;
...
begin
  ...
  begin
    ...
    if I > 0 then
      IncludeInClipRect(Result, ViewportSizeX * (Vt.Coord[0] + 1) * 0.5,
        ViewportSizeY * (Vt.Coord[1] + 1) * 0.5)
    else
    begin
...
begin
  ...
  for P := 0 to 2 do
  begin
    if (RayDirection.Coord[P] = 0) then
    begin
      if ((RayOrigin.Coord[P] < Aabb.Min.Coord[P]) or
        (RayOrigin.Coord[P] > Aabb.Max.Coord[P])) then
        Exit;
    end
    else
    begin
      InvDir := 1 / RayDirection.Coord[P];
      T0 := (Aabb.Min.Coord[P] - RayOrigin.Coord[P]) * InvDir;
      T1 := (Aabb.Max.Coord[P] - RayOrigin.Coord[P]) * InvDir;
