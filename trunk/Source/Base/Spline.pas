...
{: Spline<p>
   ...
   Cubic spline interpolation functions<p>
	...
	<b>History : </b><font size=-1><ul>
           ...
           <li>16/06/03 - Bob - Added methods to access convex and curvature
...
begin
 ...
 tciStart:
   begin
     Result[0]:=fSplineX.D[1];
     Result[1]:=fSplineY.D[1];
     Result[2]:=fSplineZ.D[1];
     Result[3]:=fSplineW.D[1];
   end;
 TciEnd:
   begin
     Result[0]:=fSplineX.D[fSplineX.FSize];
     Result[1]:=fSplineY.D[fSplineY.FSize];
     Result[2]:=fSplineZ.D[fSplineZ.FSize];
     Result[3]:=fSplineW.D[fSplineW.FSize];
   end;
 end;
...
begin
   Result[0]:=fSplineX.U[Index+1];
   Result[1]:=fSplineY.U[Index+1];
   Result[2]:=fSplineZ.U[Index+1];
   Result[3]:=fSplineW.U[Index+1];
end;
...
begin
 ...
 tciStart:
   begin
     fSplineX.D[1]:=Value[0];
     fSplineY.D[1]:=Value[1];
     fSplineZ.D[1]:=Value[2];
     fSplineW.D[1]:=Value[3];
   end;
 TciEnd:
   begin
     fSplineX.D[fSplineX.FSize]:=Value[0];
     fSplineY.D[fSplineY.FSize]:=Value[1];
     fSplineZ.D[fSplineZ.FSize]:=Value[2];
     fSplineW.D[fSplineW.FSize]:=Value[3];
   end;
 end;
 finally
  EndUpdate;
...
begin
 BeginUpdate;
 try
     fSplineX.U[index+1]:=Value[0];
     fSplineY.U[index+1]:=Value[1];
     fSplineZ.U[index+1]:=Value[2];
     fSplineW.U[index+1]:=Value[3];
 except
   Endupdate;
 end
...
begin
   tau:=gettau(t);
   vector[0]:=FsplineX[tau];
   vector[1]:=FsplineY[tau];
   vector[2]:=FsplineZ[tau];
end;
...
begin
   tau:=gettau(t);
   result[0]:=fSplineX[tau];
   result[1]:=FsplineY[tau];
   result[2]:=FsplineZ[tau];
end;
...
begin
   tau:=getTau(t);
   Result[0]:=self.fSplineX.Convex[Tau];
   Result[1]:=self.fSplineY.Convex[Tau];
   Result[2]:=self.fSplineZ.Convex[Tau];
end;
...
begin
   tau:=getTau(t);
   Result[0]:=self.fSplineX.Slope[Tau];
   Result[1]:=self.fSplineY.Slope[Tau];
   Result[2]:=self.fSplineZ.Slope[Tau];
end;
