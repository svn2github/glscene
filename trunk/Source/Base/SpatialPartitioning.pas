...
begin
  ...
  // Upper / Lower
  if Location.Coord[1] < FBSphere.Center.Coord[1] then
    ChildNodeIndex := 4;

  // Left / Right
  if Location.Coord[2] < FBSphere.Center.Coord[2] then
    ChildNodeIndex := ChildNodeIndex or 2;

  // Fore / Back
  if Location.Coord[0] > FBSphere.Center.Coord[0] then
...
var
  ...
  begin
    ...
    begin
      case Flags[N] of
        CMIN:
          Result.Coord[N] := FAABB.Min.Coord[N];
        CMID:
          Result.Coord[N] := (FAABB.Max.Coord[N] + FAABB.Min.Coord[N]) / 2;
        CMAX:
          Result.Coord[N] := FAABB.Max.Coord[N];
      end;
    end;
  end;
...
begin
  Result := (AAABB.Min.Coord[0] >= FAABB.Min.Coord[0]) and
...
begin
  ...
  begin
    NewMin := FChildren[0].AABB.Min.Coord[1];
    NewMax := FChildren[0].AABB.Max.Coord[1];
  end
  else
  begin
  ...
  for I := 0 to FChildCount - 1 do
  begin
    NewMin := Min(NewMin, FChildren[I].AABB.Min.Coord[1]);
    NewMax := Max(NewMax, FChildren[I].AABB.Max.Coord[1]);
  end;
