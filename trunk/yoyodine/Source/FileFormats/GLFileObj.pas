...
uses OpenGL1x, MeshUtils, XOpenGL, VectorTypes, GLUtils;
...
var Mesh:TMeshObject;
  ...
  begin
    ...
    with Mesh.Vertices do
      for i:=0 to Count-1 do
        begin
          s:=Format('v %g %g %g',[List[i].Coord[0],List[i].Coord[1],List[i].Coord[2]]);
          Writeln(s);
        end;
    WriteLn('');
  end;
  ...
  begin
    ...
    with Mesh.Normals do
      for i:=0 to Count-1 do
        begin
          s:=Format('vn %g %g %g',[List[i].Coord[0],List[i].Coord[1],List[i].Coord[2]]);
          Writeln(s);
        end;
    WriteLn('');
  end;
...
begin
   ...
   try
      ...
      if sl.Count>0 then begin
         ...
         for i:=0 to 3 do
            if sl.Count>i then
               Result.Coord[i]:=GLUtils.StrToFloatDef(sl[i])
            else Break;
