...
procedure TGLGRDVectorFile.HeightFieldGetHeight(const X, Y: Single; var Z: Single;
  var Color: TVector4f; var TexPoint: TTexPoint);
...
begin
   ...
   try
      ...
      begin
        ...
        Nx := StrToInt(tl[0]);  Ny := StrToInt(tl[1]);  // Nx Ny

        tl.DelimitedText := sl[2];
        Xo := StrToInt(tl[0]);  Xe := StrToInt(tl[1]);  // Xo Xe

        tl.DelimitedText := sl[3];
        Yo := StrToInt(tl[0]);  Ye := StrToInt(tl[1]);  // Yo Ye

        tl.DelimitedText := sl[4];
        Zo := StrToInt(tl[0]);  Ze := StrToInt(tl[1]);  // Zo Ze
