...
begin
  ...
  repeat                // see ReadBlocks for details
    ...
    case code of
      0 :;
      2 : BlockName:=ReadLine;
      10: insertpoint.X := ReadDouble;
      20: insertpoint.Y := ReadDouble;
      30: insertpoint.Z := ReadDouble;
      41: scale.X := ReadDouble;
      42: scale.Y := ReadDouble;
      43: scale.Z := ReadDouble;
      else
        s:=ReadLine;
    end;
  until code=0;
...
begin
  ...
  repeat
    ...
    case code of
      ...
      10: pts[0].X := ReadDouble;
      11: pts[1].X := ReadDouble;
      12: pts[2].X := ReadDouble;
      13: begin pts[3].X := ReadDouble; isquad:=TRUE end;
      20: pts[0].Y := ReadDouble;
      21: pts[1].Y := ReadDouble;
      22: pts[2].Y := ReadDouble;
      23: begin pts[3].Y := ReadDouble; isquad:=TRUE end;
      30: pts[0].Z := ReadDouble;
      31: pts[1].Z := ReadDouble;
      32: pts[2].Z := ReadDouble;
      33: begin pts[3].Z := ReadDouble; isquad:=TRUE end;
      62: Color:=ReadLine; // Color
      else ReadLine;
    end;
  until code=0;
  PushCode(0);
  isquad:=isquad and ((pts[2].X<>pts[3].X) or (pts[2].Y<>pts[3].Y) or (pts[2].Z<>pts[3].Z));
  if isquad then
