...
procedure SetTexImageName(ml : TGLMaterialLibrary; const matName, fileName : String);
var
...
//   FDebugMode:=True;
   ...
   {$WARNINGS ON}
   repeat
      GLSoundLibrary.Samples.AddFile('Sounds\'+searchRec.Name, ChangeFileExt(searchRec.Name, ''))
...
begin
   with GameMenu do begin
      Items.CommaText:='"Instant Action","Enter Tournament","Tutorial","Options","Exit Game"';
      Selected:=0;
...
begin
   ...
   iTeam:=0;
   nbDeaths:=0;
   for i:=0 to FGameEngine.TeamCount-1 do begin
      if FGameEngine.Teams[i].Name='Player' then begin
         k:=k+FGameEngine.Teams[i].Score;
         iTeam:=i;
      end else nbDeaths:=nbDeaths+FGameEngine.Teams[i].Score;
   end;
   totalScore:=k*1000;
   buf:=IntToStr(k*1000);
   if k>=0 then buf:='+'+buf;
   ...
   with FGameEngine.Teams[iTeam] do begin
      totalScore:=totalScore-FireRecord[frtMissile]*100;
      items.Add(PaddedConcat('Missiles Fired (x100)', '-'+IntToStr(FireRecord[frtMissile]*100)));
      totalScore:=totalScore-FireRecord[frtGunRound];
      items.Add(PaddedConcat('Gun Rounds (x1)', '-'+IntToStr(FireRecord[frtGunRound])));
      totalScore:=totalScore-FireRecord[frtDecoy]*25;
      items.Add(PaddedConcat('Decoys Dropped (x25)', '-'+IntToStr(FireRecord[frtDecoy]*25)));
   end;
   items.Add('');
...
begin
   ...
   with FGameEngine.AddViewerCam do begin
...
procedure TMain.GLCadencerProgress(Sender: TObject; const deltaTime,
  newTime: Double);
begin
   if not SceneViewer.Visible then Exit;
   if not GameOverMenu.Visible then begin
      if HSCover.Visible then begin
      ...
      while FDeltaTimeBuffer>=0.01 do begin
         FGameEngine.Progress(0.01);
         FDeltaTimeBuffer:=FDeltaTimeBuffer-0.01;
      end;
   end else FGameEngine.Progress(0);

   if FGameEngine.Status=gesCompleted then begin
      if not GameOverMenu.Visible then begin
         if FInTutorial then begin
...
begin
   ...
   if playerMobile<>nil then
      Caption:=Format('%.1f, %.1f, %.1f / ', [playerMobile.Position[0],playerMobile.Position[1],playerMobile.Position[2]])
   else Caption:='';
   Caption:=Caption+SceneViewer.FramesPerSecondText+' / '+IntToStr(TerrainRenderer.LastTriangleCount)+' tris'
...
begin
   case Key of
      ...
      VK_ESCAPE : begin
         if DCMenu.Visible then begin
            if AbortGameMenu.Visible then begin
      ...
      Ord('X'), VK_F4 : if ssAlt in Shift then Close;
      222 : if FDebugMode then begin
         // WayPoint log helper
         sl:=TStringList.Create;
         if FileExists('path.log') then
            sl.LoadFromFile('path.log');
         if Trim(sl.text)='' then sl.Clear;
         sl.Add('WP'+IntToStr(sl.Count)+'='+Vector3ToString(FGameEngine.MobileByName('Player').position));
         sl.SaveToFile('path.log'); sl.Free;
      end;
   end;
   if DCMenu.Visible then begin
      if GameMenu.Visible then begin
         case Key of
            ...
            VK_RETURN : begin
               ...
               if GameMenu.SelectedText='Options' then
                  DoOptionsDlg
               else if GameMenu.SelectedText='Exit Game' then begin
                  TIMenuVoiceDelay.Enabled:=False;
                  FGameEngine.StopVoice('GameMenu');
                  Close;
               end else if GameMenu.SelectedText='Back' then
                  GameMenuMainMenu
               else if GameMenu.SelectedText='Instant Action' then begin
                  GameMenu.Items.CommaText:='"Back","1 vs 1","1 vs 1 Furball","1 vs 1 Firestorm","Endurance"';
               end else if GameMenu.SelectedText='Tutorial' then begin
                  StartGame('Tutorial.mis');
                  FInTutorial:=True;
               end else if GameMenu.SelectedText='1 vs 1' then begin
                  StartGame('1vs1.mis');
                  FGameEngine.PlayVoice('GameMenu', '1 vs 1');
                  FGameEngine.PlayVoice('GameMenu', 'Match Beginning');
               end else if GameMenu.SelectedText='1 vs 1 Furball' then begin
                  StartGame('1vs1-Furball.mis');
                  FGameEngine.PlayVoice('GameMenu', '1 vs 1 Furball');
                  FGameEngine.PlayVoice('GameMenu', 'Match Beginning');
               end else if GameMenu.SelectedText='1 vs 1 Firestorm' then begin
                  StartGame('1vs1-Firestorm.mis');
                  FGameEngine.PlayVoice('GameMenu', '1 vs 1 Firestorm');
                  FGameEngine.PlayVoice('GameMenu', 'Match Beginning');
               end else if GameMenu.SelectedText='Endurance' then begin
                  StartGame('Endurance.mis');
               end;
            end;
         end;
      end;
      if GameOverMenu.Visible then begin
         case Key of
      ...
      if AbortGameMenu.Visible then begin
         case Key of
            ...
            VK_RETURN : begin
               ...
               if AbortGameMenu.SelectedText='Yes' then begin
                  StartGame('Demo.mis');
                  GameMenuMainMenu;
               end else begin
                  DCMenu.Visible:=False;
