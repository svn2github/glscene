...
#include "fFurBall.h"
#include "dynodegl.hpp"        // Pascal unit
#include "GLVerletClasses.hpp"
//---------------------------------------------------------------------------
#pragma package(smart_init)
#pragma link "VerletHairClasses"
#pragma link "VerletClasses"
...
void __cdecl nearCallback(void *data, PdxGeom o1, PdxGeom o2)
{
  ...
  if(numc > 0)
  {
    ...
    {
      c =
        dJointCreateContact(frmFurBall->world, frmFurBall->contactgroup,
                            &contact[i]);
...
//---------------------------------------------------------------------------
__fastcall TfrmFurBall::TfrmFurBall(TComponent * Owner):TForm(Owner)
{
  Show();
  ...
  CheckBox_FurGravityClick(this);
  CheckBox_WindResistenceClick(this);
...
void __fastcall TfrmFurBall::FormClose(TObject * Sender, TCloseAction & Action)
{
...
double angle = 0;
void __fastcall TfrmFurBall::GLCadencer1Progress(TObject * Sender,
...
void __fastcall TfrmFurBall::DC_LightHolderProgress(TObject * Sender,
                                                    const double deltaTime,
                                                    const double newTime)
{
...
int FoldMouseX, FoldMouseY;
void __fastcall TfrmFurBall::GLSceneViewer1MouseMove(TObject * Sender,
...
void TfrmFurBall::CreateRandomHair()
{
  int i;
  TAffineVector Dir;
  TVerletHair *Hair;
...
void __fastcall TfrmFurBall::FormMouseWheel(TObject * Sender, TShiftState Shift,
                                            int WheelDelta, TPoint & MousePos,
                                            bool & Handled)
{
  GLCamera1->
    AdjustDistanceToTarget(Power
                           ((float)1.1, (float)((float)WheelDelta / 120.0)));
}

//---------------------------------------------------------------------------

void __fastcall TfrmFurBall::CheckBox_FurGravityClick(TObject * Sender)
{
...
void __fastcall TfrmFurBall::CheckBox_WindResistenceClick(TObject * Sender)
{
...
void __fastcall TfrmFurBall::TrackBar_WindForceChange(TObject * Sender)
{
  if(AirResistance)
    AirResistance->WindMagnitude =
      (float)(TrackBar_WindForce->Position) / 100.0 * cMaxWindMag;
...
void __fastcall TfrmFurBall::CheckBox_BaldClick(TObject * Sender)
{
...
void __fastcall TfrmFurBall::CheckBox_ShadowsClick(TObject * Sender)
{
...
void __fastcall TfrmFurBall::CheckBox_InertiaClick(TObject * Sender)
{
