...
type
  ...
  TVKSCUDACompilerSourceProperty = class(TStringProperty)
  private
  ...
  TVKSCUDADeviceProperty = class(TStringProperty)
  private
...
procedure Register;
begin
  RegisterComponents('GLScene Computing', [TVKSCUDA, TVKSCUDADevice,
    TVKSCUDACompiler]);
  RegisterComponentEditor(TVKSCUDA, TVKSCUDAEditor);
  RegisterComponentEditor(TVKSCUDACompiler, TVKSCUDACompilerEditor);
  RegisterPropertyEditor(TypeInfo(string), TVKSCUDACompiler, 'ProjectModule',
    TVKSCUDACompilerSourceProperty);
  RegisterPropertyEditor(TypeInfo(string), TVKSCUDADevice, 'SelectDevice',
    TVKSCUDADeviceProperty);
  ...
  ObjectManager.RegisterSceneObject(TVKFeedBackMesh, 'GPU generated mesh', 'Computing', HInstance);
end;
...
procedure TVKSCUDAEditor.Edit;
begin
  with GLSCUDAEditorForm do
  begin
    SetCUDAEditorClient(TVKSCUDA(Self.Component), Self.Designer);
    Show;
  end;
end;

procedure TVKSCUDAEditor.ExecuteVerb(Index: Integer);
begin
  case Index of
    0: Edit;
  end;
...
function TVKSCUDAEditor.GetVerb(Index: Integer): string;
begin
  case Index of
...
function TVKSCUDAEditor.GetVerbCount: Integer;
begin
  Result := 1;
end;

// ------------------
// ------------------ TVKSCUDACompilerEditor ------------------
// ------------------

procedure TVKSCUDACompilerEditor.Edit;
var
  CUDACompiler: TVKSCUDACompiler;
  I, J: Integer;
  func: TCUDAFunction;
  tex: TCUDATexture;
  cnst: TCUDAConstant;
...
begin
  CUDACompiler := TVKSCUDACompiler(Self.Component);
...
procedure TVKSCUDACompilerEditor.ExecuteVerb(Index: Integer);
begin
  case Index of
    0: Edit;
  end;
...
function TVKSCUDACompilerEditor.GetVerb(Index: Integer): string;
begin
  case Index of
...
function TVKSCUDACompilerEditor.GetVerbCount: Integer;
begin
  Result := 1;
end;
...
constructor TVKSCUDACompilerSourceProperty.Create(
...
destructor TVKSCUDACompilerSourceProperty.Destroy;
begin
  FModuleList.Destroy;
  inherited;
end;
...
procedure TVKSCUDACompilerSourceProperty.RefreshModuleList;
var
  proj: IOTAProject;
  I: Integer;
  LModule: IOTAModuleInfo;
...
function TVKSCUDACompilerSourceProperty.GetAttributes;
begin
  Result := [paValueList];
end;
...
// GetValues
//
procedure TVKSCUDACompilerSourceProperty.GetValues(Proc: TGetStrProc);
var
   I : Integer;
begin
  RefreshModuleList;
...
// SetValue
//
procedure TVKSCUDACompilerSourceProperty.SetValue(const Value: String);
var
  I, J: Integer;
begin
  RefreshModuleList;
  ...
  if J > 0 then
  begin
    TVKSCUDACompiler(GetComponent(0)).SetSourceCodeFile(FModuleList[J]);
...
// ------------------ TVKSCUDADeviceProperty ------------------
// ------------------
constructor TVKSCUDADeviceProperty.Create(const ADesigner: IDesigner; APropCount: Integer);
begin
  inherited;
  FDeviceList := TStringList.Create;
end;

destructor TVKSCUDADeviceProperty.Destroy;
begin
  FDeviceList.Destroy;
  inherited;
end;

function TVKSCUDADeviceProperty.GetAttributes: TPropertyAttributes;
begin
  Result := [paValueList];
end;

procedure TVKSCUDADeviceProperty.GetValues(Proc: TGetStrProc);
begin
  CUDAContextManager.FillUnusedDeviceList(FDeviceList);
end;

procedure TVKSCUDADeviceProperty.SetValue(const Value: String);
var
  I: Integer;
begin
