...
type
  ...
  private
    ...
    procedure DrawNode(var rci : TVKRenderContextInfo; Node: TLineNode; LineWidth: Single);
...
begin
   if (GetOwner<>nil) then
      TVKMeshLines((GetOwner as TLineItem).Collection.Owner).StructureChanged;
end;
...
begin
  if TVKMeshLines(Collection.Owner).SelectedLineItem = self then
    TVKMeshLines(Collection.Owner).DeSelectLineItem;
  FNodes.Free;
  inherited;
end;
...
destructor TVKMeshLines.Destroy;
begin
  FLines.Free;
...
procedure TVKMeshLines.Loaded;
begin
  DoChanged;
end;

procedure TVKMeshLines.BeginUpdate;
begin
  inc(FUpdating);
end;

procedure TVKMeshLines.EndUpdate;
begin
  Dec(FUpdating);
  if FUpdating < 1 then
  begin
...
procedure TVKMeshLines.BuildList(var rci : TVKRenderContextInfo);
var
  i,j: Integer;
begin
  inherited;
...
procedure TVKMeshLines.DoRender(var rci : TVKRenderContextInfo; renderSelf, renderChildren : Boolean);
begin
  if FNoZWrite then
  begin
...
procedure TVKMeshLines.SetShowNodes(const Value: Boolean);
begin
  FShowNodes := Value;
  DoChanged;
end;

procedure TVKMeshLines.SetNoZWrite(const Value: Boolean);
begin
  FNoZWrite := Value;
  DoChanged;
end;

procedure TVKMeshLines.SetLightmapIndex(const value: Integer);
begin
  FLightmapIndex := Value;
  DoChanged;
end;

procedure TVKMeshLines.SetLightmapMaterialName(const value: String);
var
  lLibMaterial: TVKLibMaterial;
begin
  if Value <> '' then
  begin
...
procedure TVKMeshLines.SetLightmapBounds( const value: TLightmapBounds );
begin
  FLightmapBounds.SetVector(value.X, value.Y,value.Z,value.W);
  DoChanged;
end;

procedure TVKMeshLines.DoChanged;
begin
...
procedure TVKMeshLines.BuildGeometry;
var
  i: Integer;
...
procedure TVKMeshLines.DrawNode(var rci : TVKRenderContextInfo; Node: TLineNode; LineWidth: Single);
var
  lNodeSize: Single;
begin
...
function TVKMeshLines.SelectLineItem(LineItem: TLineItem): TLineItem;
begin
  Result := nil;
...
function TVKMeshLines.SelectLineItem(LineNode: TLineNode): TLineItem;
begin
...
procedure TVKMeshLines.DeselectLineItem;
begin
  FSelectedLineItem := nil;
  FSelectedNode := nil;
  DoChanged;
...
procedure TVKMeshLines.DeselectLineNode;
begin
  FSelectedNode := nil;
  DoChanged;
end;

function TVKMeshLines.SelectLineItem(const X,Z: Single; Tolerance: single = 1): TLineItem;
var
  i: Integer;
  lStartPoint: Integer;
  lNode: TLineNode;
...
function TVKMeshLines.GetUpdating: Boolean;
begin
  Result := FUpdating > 0;
end;

function TVKMeshLines.PointNearLine(const LineItem: TLineItem; const X,Z: Single; Tolerance: single = 1): boolean;
var
  i: Integer;
...
function TVKMeshLines.PointNearSegment(const StartNode, EndNode: TLineNode; const X,Z: Single; LineWidth: single; Tolerance: single = 1): boolean;
var
  xt, yt, u, len: single;
  xp, yp: single;
  lDist: Single;
...
procedure TVKMeshLines.StitchStrips(idx: TIntegerList);
var
  i: integer;
  i0, i1, i2: integer;
begin
...
procedure TVKMeshLines.AddStitchMarker(idx: TIntegerList);
begin
  idx.Add(-1);
  idx.Add(-2);
  idx.Add(-2);
...
procedure TVKMeshLines.NotifyChange(Sender : TObject);
begin
  inherited;
  DoChanged;
end;

procedure TVKMeshLines.AddIndex;
begin
  FFaceGroup.Add(FIndex);
  inc(FIndex);
end;

procedure TVKMeshLines.AddVertices(Up,Inner,Outer: TAffineVector; S: Single; Correction: Single; UseDegenerate: Boolean; LineItem: TLineItem);
begin
...
procedure TVKMeshLines.BuildLineItem(LineItem: TLineItem);
var
  Seg1: TAffineVector;
  Seg2: TAffineVector;
  NSeg1: TAffineVector;
