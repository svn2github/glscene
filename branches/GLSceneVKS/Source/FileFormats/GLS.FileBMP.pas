...
type
  ...
  private
    ...
    function ExpandColor(Value: longword): TVKPixel32;
...
procedure TVKBMPImage.LoadFromFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TVKBMPImage.SaveToFile(const filename: string);
var
  fs: TStream;
begin
...
function TVKBMPImage.CountBits(Value: byte): shortint;
var
  i, bits: shortint;
begin
  bits := 0;
...
function TVKBMPImage.ShiftCount(Mask: longword): shortint;
var
  tmp: shortint;
begin
  tmp := 0;
...
function TVKBMPImage.ExpandColor(Value: longword): TVKPixel32;
var
...
function TVKBMPImage.Monochrome(N: Integer): Integer;
begin
  Result := (FLineBuffer[N div 8] shr (7 - (N and 7))) and 1;
end;

function TVKBMPImage.Quadrochrome(N: Integer): Integer;
begin
  Result := (FLineBuffer[N div 2] shr (((N + 1) and 1) * 4)) and $0F;
end;

function TVKBMPImage.Octochrome(N: Integer): Integer;
begin
...
procedure TVKBMPImage.LoadFromStream(stream: TStream);
type
...
begin
  ...
  begin
    ...
    if LInfo.ClrUsed > 0 then
      Stream.Read(LPalette[0], LInfo.ClrUsed * SizeOf(TVKPixel32))
    else // Seems to me that this is dangerous.
      Stream.Read(LPalette[0], nPalette * SizeOf(TVKPixel32));
  end
...
procedure TVKBMPImage.ExpandRLE4ScanLine(Row: Integer; Stream: TStream);
var
  i, j, tmpsize: integer;
  b0, b1: byte;
  nibline: PByteArray;
...
procedure TVKBMPImage.ExpandRLE8ScanLine(Row: Integer; Stream: TStream);
var
  i, j: integer;
  b0, b1: byte;
begin
...
procedure TVKBMPImage.SaveToStream(stream: TStream);
begin
  {$Message Hint 'TVKBMPImage.SaveToStream not yet implemented' }
end;
...
class function TVKBMPImage.Capabilities: TVKDataFileCapabilities;
begin
...
initialization
  ...
  RegisterRasterFormat('bmp', 'Bitmap Image File', TVKBMPImage);
