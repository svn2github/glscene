...
{$ELSE}
  begin
    raise Exception.Create('GLS.Keyboard.IsKeyDown(c : Char) not yet implemented for your platform!');
  end;
{$ENDIF}
...
begin
   ...
   {$IFDEF MSWINDOWS}
      Result := (GetAsyncKeyState(vk) < 0);
   {$ELSE}
      raise Exception.Create('GLS.Keyboard.IsKeyDown(vk : TVirtualKeyCode) not fully yet implemented for your platform!');
   {$ENDIF}
   end;
end;
...
{$ELSE}
begin
  raise Exception.Create('GLS.Keyboard.KeyPressed not yet implemented for your platform!');
end;
{$ENDIF}
...
begin
   ...
   case vk of
      ...
      SetLength(Result, nSize);
      {$ELSE}
      raise Exception.Create('GLS.Keyboard.VirtualKeyCodeToKeyName not yet fully implemented for your platform!');
      {$ENDIF}
   end;
end;
...
begin
  ...
  if (Result = -1) and lExceptionWasRaised then
    raise Exception.Create('GLS.Keyboard.KeyNameToVirtualKeyCode not yet fully implemented for your platform or keyName wasn''t found!');
end;
{$ENDIF}
...
{$ELSE}
   raise Exception.Create('GLS.Keyboard.CharToVirtualKeyCode not yet implemented for your platform!');
{$ENDIF}
end;
