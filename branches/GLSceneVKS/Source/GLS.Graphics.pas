...
type
  ...
  // TVKPixel24
  //
  TVKPixel24 = packed record
    r, g, b: Byte;
  end;
  PGLPixel24 = ^TVKPixel24;

  // TVKPixel32
  //
  TVKPixel32 = packed record
    r, g, b, a: Byte;
  end;
  PGLPixel32 = ^TVKPixel32;

  TVKPixel32Array = array[0..MaxInt shr 3] of TVKPixel32;
  PGLPixel32Array = ^TVKPixel32Array;

  TVKLODStreamingState = (ssKeeping, ssLoading, ssLoaded, ssTransfered);

  // TVKImageLevelDesc
  //
  TVKImageLevelDesc = record
    Width: Integer;
    Height: Integer;
    Depth: Integer;
    PBO: TVKUnpackPBOHandle;
    MapAddress: Pointer;
    Offset: LongWord;
    StreamOffset: LongWord;
    Size: LongWord;
    State: TVKLODStreamingState;
  end;

  TVKImageLODRange = 0..15;

  TVKImagePiramid = array[TVKImageLODRange] of TVKImageLevelDesc;
...
{$ENDIF}
  protected
    ...
    function GetLevelWidth(ALOD: TVKImageLODRange): Integer;
    function GetLevelHeight(ALOD: TVKImageLODRange): Integer;
    function GetLevelDepth(ALOD: TVKImageLODRange): Integer;
    function GetLevelPBO(ALOD: TVKImageLODRange): TVKUnpackPBOHandle;
    function GetLevelOffset(ALOD: TVKImageLODRange): Integer;
    function GetLevelSizeInByte(ALOD: TVKImageLODRange): Integer;
    function GetLevelStreamingState(ALOD: TVKImageLODRange): TVKLODStreamingState;
    procedure SetLevelStreamingState(ALOD: TVKImageLODRange; AState: TVKLODStreamingState);
  ...
  public
    ...
    property LevelWidth[ALOD: TVKImageLODRange]: Integer
      read GetLevelWidth;
    property LevelHeight[ALOD: TVKImageLODRange]: Integer
      read GetLevelHeight;
    property LevelDepth[ALOD: TVKImageLODRange]: Integer
      read GetLevelDepth;
  ...
  TVKBaseImageClass = class of TVKBaseImage;
...
{$ENDIF}
  ...
  public
    ...
    procedure Add(const Ext, Desc: string; DescID: Integer; AClass:
      TVKBaseImageClass);
    function FindExt(ext: string): TVKBaseImageClass;
    function FindFromFileName(const fileName: string): TVKBaseImageClass;
    function FindFromStream(const AStream: TStream): TVKBaseImageClass;
    procedure Remove(AClass: TVKBaseImageClass);
    procedure BuildFilterStrings(imageFileClass: TVKBaseImageClass;
...
function TRasterFileFormatsList.FindExt(ext: string): TVKBaseImageClass;
var
  i: Integer;
begin
...
procedure TRasterFileFormatsList.Remove(AClass: TVKBaseImageClass);
var
  i: Integer;
begin
  for i := Count - 1 downto 0 do
...
procedure TRasterFileFormatsList.BuildFilterStrings(
  imageFileClass: TVKBaseImageClass;
  var descriptions, filters: string;
  formatsThatCanBeOpened: Boolean = True;
  formatsThatCanBeSaved: Boolean = False);
var
...
procedure CalcImagePiramid(var APiramid: TVKImagePiramid);
begin
...
constructor TVKBaseImage.Create;
begin
  inherited Create(Self);
  FillChar(FLOD, SizeOf(TVKImagePiramid), $00);
...
destructor TVKBaseImage.Destroy;
var
  level: TVKImageLODRange;
begin
  ...
  for level := 0 to High(TVKImageLODRange) do
  begin
    FLOD[level].PBO.Free;
  end;
  FSourceStream.Free;
...
procedure TVKBaseImage.Assign(Source: TPersistent);
var
  img: TVKBaseImage;
  size: integer;
begin
  if Source is TVKBaseImage then
  begin
    img := Source as TVKBaseImage;
    FLOD := img.FLOD;
...
function TVKBaseImage.GetTextureTarget: TVKTextureTarget;
begin
  Result := ttTexture2D;
...
function TVKBaseImage.DataSize: PtrUint;
var
  l: TVKImageLODRange;
  s: PtrUint;
begin
...
function TVKBaseImage.GetWidth: Integer;
begin
  Result := FLOD[0].Width;
end;

function TVKBaseImage.GetDepth: Integer;
begin
  Result := FLOD[0].Depth;
end;

function TVKBaseImage.GetHeight: Integer;
begin
  Result := FLOD[0].Height;
end;

function TVKBaseImage.GetLevelAddress(ALevel: Byte): Pointer;
begin
  Result := FData;
  Inc(PByte(Result), FLOD[ALevel].Offset);
end;

function TVKBaseImage.GetLevelAddress(ALevel, AFace: Byte): Pointer;
begin
  Result := FData;
  Inc(PByte(Result), FLOD[ALevel].Offset);
...
function TVKBaseImage.GetLevelDepth(ALOD: TVKImageLODRange): Integer;
begin
  Result := FLOD[ALOD].Depth;
end;

function TVKBaseImage.GetLevelHeight(ALOD: TVKImageLODRange): Integer;
begin
  Result := FLOD[ALOD].Height;
end;

function TVKBaseImage.GetLevelOffset(ALOD: TVKImageLODRange): Integer;
begin
  Result := FLOD[ALOD].Offset;
end;

function TVKBaseImage.GetLevelPBO(ALOD: TVKImageLODRange): TVKUnpackPBOHandle;
begin
  Result := FLOD[ALOD].PBO;
end;

function TVKBaseImage.GetLevelSizeInByte(ALOD: TVKImageLODRange): Integer;
begin
  Result := FLOD[ALOD].Size;
end;

function TVKBaseImage.GetLevelStreamingState(ALOD: TVKImageLODRange): TVKLODStreamingState;
begin
  Result := FLOD[ALOD].State;
end;

function TVKBaseImage.GetLevelWidth(ALOD: TVKImageLODRange): Integer;
begin
...
function TVKBaseImage.IsEmpty: Boolean;
begin
...
function TVKBaseImage.IsCompressed: Boolean;
begin
...
function TVKBaseImage.IsVolume: boolean;
begin
...
function TVKBaseImage.ConvertCrossToCubemap: Boolean;
var
...
function TVKBaseImage.ConvertToVolume(const col, row: Integer; const MakeArray:
  Boolean): Boolean;
var
...
procedure TVKBaseImage.SetErrorImage;
const
{$I TextureError.inc}
begin
  UnMipmap;
...
procedure TVKBaseImage.SetLevelStreamingState(ALOD: TVKImageLODRange;
  AState: TVKLODStreamingState);
begin
...
procedure TVKBaseImage.GenerateMipmap(AFilter: TImageFilterFunction);
var
...
begin
  ...
  begin
    ...
    UpdateLevelsInfo;
    ReallocMem(FData, DataSize);
    {Message Hint 'TVKBaseImage.GenerateMipmap not yet implemented for volume images' }
  end
  else
  begin
...
procedure TVKBaseImage.UnMipmap;
var
  level: TVKImageLODRange;
begin
  for level := 1 to High(TVKImageLODRange) do
  begin
    FLOD[level].Width := 0;
    FLOD[level].Height := 0;
    FLOD[level].Depth := 0;
...
procedure TVKBaseImage.UpdateLevelsInfo;
var
  level: TVKImageLODRange;
  w, h, d: Integer;
...
function TVKBaseImage.GetData: PGLPixel32Array;
begin
  Result := fData;
end;
...
begin
  ...
  try
    ...
    if Self is TVKImage then
    begin
      TVKImage(Self).FBlank := FLevelCount = 0;
      if FLevelCount = 0 then
      begin
        UnMipmap;
        FreeMem(fData);
...
procedure TVKBaseImage.SaveHeader;
var
  Temp: Integer;
  LStream: TStream;
begin
  ...
  try
    ...
    begin
      ...
      Write(fLevelCount, SizeOf(TVKImageLODRange));
...
procedure TVKBaseImage.LoadHeader;
var
  Temp: Integer;
  LStream: TStream;
begin
...
procedure TVKBaseImage.StartStreaming;
var
  level: TVKImageLODRange;
begin
...
begin
  ...
  begin
    if Source is TVKImage then
      FBlank := TVKImage(Source).fBlank
    else
      FBlank := false;
    ...
    begin
      FLOD := TVKImage(Source).FLOD;
      FLevelCount := TVKImage(Source).FLevelCount;
      fCubeMap := TVKImage(Source).fCubeMap;
      fColorFormat := TVKImage(Source).fColorFormat;
      fInternalFormat := TVKImage(Source).fInternalFormat;
      fDataType := TVKImage(Source).fDataType;
      fElementSize := TVKImage(Source).fElementSize;
      fTextureArray := TVKImage(Source).fTextureArray;
    end;
  end
...
procedure TVKImage.AssignFromTexture2D(textureHandle: Cardinal);
var
  oldTex: Cardinal;
begin
  UnMipmap;
...
procedure TVKImage.AssignFromTexture2D(textureHandle: TVKTextureHandle);
var
...
// SetWidth
//
procedure TVKImage.SetWidth(val: Integer);
begin
...
procedure TVKImage.SetHeight(const val: Integer);
begin
...
procedure TVKImage.SetDepth(const val: Integer);
begin
...
procedure TVKImage.SetCubeMap(const val: Boolean);
begin
...
procedure TVKImage.SetArray(const val: Boolean);
begin
...
procedure TVKImage.SetColorFormatDataType(const AColorFormat, ADataType: GLenum);
begin
  if fBlank then
  begin
    fDataType := ADataType;
...
function TVKImage.GetScanLine(index: Integer): PGLPixel32Array;
begin
  Narrow;
...
procedure TVKImage.SetAlphaFromIntensity;
var
  i: Integer;
begin
  Narrow;
...
procedure TVKImage.SetAlphaTransparentForColor(const aColor: TColor);
var
  color: TVKPixel24;
begin
...
procedure TVKImage.SetAlphaTransparentForColor(const aColor: TVKPixel32);
var
  color: TVKPixel24;
begin
  color.r := aColor.r;
...
procedure TVKImage.SetAlphaTransparentForColor(const aColor: TVKPixel24);
var
  i: Integer;
  intCol: Integer;
begin
...
procedure TVKImage.SetAlphaToValue(const aValue: Byte);
var
  i: Integer;
begin
  Narrow;
...
procedure TVKImage.SetAlphaToFloatValue(const aValue: Single);
begin
...
procedure TVKImage.InvertAlpha;
var
  i: Integer;
begin
  Narrow;
...
procedure TVKImage.BrightnessCorrection(const factor: Single);
begin
  if Assigned(FData) then
  begin
    Narrow;
...
procedure TVKImage.GammaCorrection(const gamma: Single);
begin
  if Assigned(FData) then
  begin
    Narrow;
...
procedure TVKImage.DownSampleByFactor2;
type
...
procedure TVKImage.ReadPixels(const area: TVKRect);
begin
  UnMipmap;
...
procedure TVKImage.DrawPixels(const x, y: Single);
begin
...
procedure TVKImage.GrayScaleToNormalMap(const scale: Single;
  wrapX: Boolean = True; wrapY: Boolean = True);
var
  x, y: Integer;
  dcx, dcy: Single;
...
procedure TVKImage.NormalizeNormalMap;
var
  x, y: Integer;
  sr, sg, sb: Single;
  invLen: Single;
...
procedure TVKImage.SetBlank(const Value: boolean);
begin
...
procedure TVKImage.GenerateMipmap(AFilter: TImageFilterFunction);
begin
  if not FBlank then
...
procedure TVKImage.DataConvertTask;
var
  oldLOD: TVKImagePiramid;
  newData: Pointer;
  ptr: PByte;
  L: TVKImageLODRange;
  d: Integer;
begin
  ...
  if IsVolume then
  begin
    {Message Hint 'TVKImage.DataConvertTask not yet implemented for volume images' }
  end
  else
  begin
