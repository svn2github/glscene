...
uses
  GLScene_Material, GLScene_Base_Context_Info, GLScene_Base_Vector_Geometry;
...
begin
  ...
  if CBAdditive.Checked then
    ...
    OrbitingSphere1.Material.BlendingMode := bmTransparency;
  OrbitingSphere2.Material.BlendingMode :=
    OrbitingSphere1.Material.BlendingMode;
end;
...
begin
  ...
  if CBSorting.Checked then
    ...
    BaseDummyCube.ObjectsSorting := osNone;
end;
...
begin
  ...
  OrbitingSphere1.Position.SetPoint(1.5 * cos(alpha), 1.5 * sin(alpha),
    1.5 * sin(alpha));
  alpha := alpha + PI / 2;
  OrbitingSphere2.Position.SetPoint(1.5 * cos(alpha), 1.5 * sin(alpha),
    1.5 * sin(alpha));
end;
