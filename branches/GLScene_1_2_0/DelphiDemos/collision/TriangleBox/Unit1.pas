...
type
  ...
  private
    mdx, mdy: Integer;
    BoxPos, BoxScale, MinExtend, MaxExtend: TAffineVector;
...
begin
  ...
  if CheckBox1.Checked then
    BoxPos := AffineVectorMake(Random * SizePos - SizePos / 2,
      Random * SizePos - SizePos / 2, Random * SizePos - SizePos / 2);
  // Change scale.
  if CheckBox2.Checked then
    BoxScale := AffineVectorMake(Random * ScaleSize + ScaleSize / 2,
      Random * ScaleSize + ScaleSize / 2, Random * ScaleSize + ScaleSize / 2);
  // Change triange.
  if CheckBox3.Checked then
    for i := 0 to 2 do
      TriangePos[i] := AffineVectorMake(Random * TrigRect - TrigRect / 2,
        Random * TrigRect - TrigRect / 2, Random * TrigRect - TrigRect / 2);
  // Calc extends.
