    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            xyz[0] := (baseframe.BaseVertices[k, 0] * MDC_BASEVERTEX_FACTOR) +
              borderframes[j].localorigin[0];
            xyz[1] := (baseframe.BaseVertices[k, 1] * MDC_BASEVERTEX_FACTOR) +
              borderframes[j].localorigin[1];
            xyz[2] := (baseframe.BaseVertices[k, 2] * MDC_BASEVERTEX_FACTOR) +
            ...
            begin
              xyz[0] := xyz[0] + ((compframe.CompVertices[k, 0] - 128) *
                MDC_COMPVERTEX_FACTOR);
              xyz[1] := xyz[1] + ((compframe.CompVertices[k, 1] - 128) *
                MDC_COMPVERTEX_FACTOR);
              xyz[2] := xyz[2] + ((compframe.CompVertices[k, 2] - 128) *
                MDC_COMPVERTEX_FACTOR);
              // FIXME:
            ...
            morphTarget.Vertices.Add(xyz[0], xyz[2], -xyz[1]);
            morphTarget.Normals.Add(normal[0], normal[2], -normal[1]);
          end;
        end;
      end;
