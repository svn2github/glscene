...
unit GLScene_Files_VRML_Parser;
...
begin
  if unknown_token = '{' then
    ...
    ReadUnknownArray
  else if (unknown_token <> '}') and (unknown_token <> ']') and
    AllowUnknownNodes then
  begin
...
begin
  Result := GLScene_Utils.StrToFloatDef(ReadToken, 0);
end;
  ...
  repeat
    ...
    if token = '' then
      ...
      TVRMLSingleArray(FCurrentNode)
        .Values.Add(GLScene_Utils.StrToFloatDef(token, 0));
  until token = ']';
