...
type
  ...
  TGLShaderUnAplyEvent = procedure(Shader: TGLCustomShader;
  ...
  IGLShaderDescription = interface
    ...
    procedure SetShaderColorParams(const AAmbientColor, ADiffuseColor,
      ASpecularcolor: TVector4f);
    procedure GetShaderColorParams(var AAmbientColor, ADiffuseColor,
      ASpecularcolor: TVector4f);

    procedure SetShaderMiscParameters(const ACadencer: TGLCadencer;
      const AMatLib: TGLMaterialLibrary;
      const ALightSources: TGLLightSourceSet);
    procedure GetShaderMiscParameters(var ACadencer: TGLCadencer;
  ...
  protected
    ...
    property FragmentProgram: TGLFragmentProgram read FFragmentProgram
      write SetFragmentProgram stored StoreFragmentProgram;
    property VertexProgram: TGLVertexProgram read FVertexProgram
      write SetVertexProgram stored StoreVertexProgram;
    property GeometryProgram: TGLGeometryProgram read FGeometryProgram
      write SetGeometryProgram stored StoreGeometryProgram;
  ...
  published
    ...
    property InputPrimitiveType: TGLgsInTypes read FInputPrimitiveType
      write SetInputPrimitiveType default gsInPoints;
    property OutputPrimitiveType: TGLgsOutTypes read FOutputPrimitiveType
      write SetOutputPrimitiveType default gsOutPoints;
    property VerticesOut: TGLint read FVerticesOut write SetVerticesOut
      default 0;
  end;
  ...
  protected
    ...
    procedure SetAsCustomTexture(const TextureIndex: Integer;
      TextureTarget: TGLTextureTarget; const Value: Cardinal); virtual;
      abstract;
  ...
  public
    ...
    procedure SetToTextureOf(const LibMaterial: TGLLibMaterial;
      const TextureIndex: Integer); overload;
    procedure SetToTextureOf(const Texture: TGLTexture;
      const TextureIndex: Integer); overload;

    // : GLScene-friendly properties.
    property AsVector: TVector read GetAsVector4f write SetAsVector4f;
    property AsAffineVector: TAffineVector read GetAsVector3f
      write SetAsVector3f;
    ...
    // : Texture Types.
    property AsTexture[const TextureIndex: Integer]: TGLTexture
      write SetAsTexture;
    property AsTexture1D[const TextureIndex: Integer]: TGLTexture
      write SetAsTexture1D;
    property AsTexture2D[const TextureIndex: Integer]: TGLTexture
      write SetAsTexture2D;
    property AsTexture3D[const TextureIndex: Integer]: TGLTexture
      write SetAsTexture3D;
    property AsTextureRect[const TextureIndex: Integer]: TGLTexture
      write SetAsTextureRect;
    property AsTextureCube[const TextureIndex: Integer]: TGLTexture
      write SetAsTextureCube;

    property AsCustomTexture[const TextureIndex: Integer;
      TextureTarget: TGLTextureTarget]: Cardinal read GetAsCustomTexture
      write SetAsCustomTexture;

    property AsUniformBuffer: GLenum read GetAsUniformBuffer
      write SetAsUniformBuffer;
  end;
  ...
  TGLBlendingModeEx = (bmxOpaque, bmxTransparency, bmxAdditive, bmxAlphaTest50,
    bmxAlphaTest100, bmxModulate, bmxDestColorOne, bmxDestAlphaOne);

  // Exported procedures.
procedure ApplyBlendingModeEx(const BlendingMode: TGLBlendingModeEx);
procedure UnApplyBlendingModeEx;
procedure InitTexture(const TextureHandle: Cardinal; const TextureSize: TGLSize;
...
procedure CopyScreentoTexture(const ViewPortSize: TGLSize;
  const TextureTarget: Word = GL_TEXTURE_2D);
procedure CopyScreentoTexture2(const ViewPortSize: TGLSize;
  const TextureTarget: Word = GL_TEXTURE_2D);

function IsFogEnabled(const AFogSupportMode: TGLShaderFogSupport;
...
begin
  case AFogSupportMode of
    ...
    sfsDisabled:
      Result := False;
    sfsAuto:
      Result := TGLSceneBuffer(rci.buffer).FogEnable;
  else
    Result := False;
...
procedure CopyScreentoTexture(const ViewPortSize: TGLSize;
  const TextureTarget: Word = GL_TEXTURE_2D);
begin
  GL.CopyTexSubImage2D(TextureTarget, 0, 0, 0, 0, 0, ViewPortSize.cx,
    ViewPortSize.cy);
end;
...
procedure CopyScreentoTexture2(const ViewPortSize: TGLSize;
  const TextureTarget: Word = GL_TEXTURE_2D);
begin
  GL.CopyTexImage2D(TextureTarget, 0, GL_RGB, 0, 0, ViewPortSize.cx,
    ViewPortSize.cy, 0);
end;
...
begin
  ...
  GL.DepthMask(False);
  GL.Begin_(GL_QUADS);
  GL.TexCoord2f(0.0, ViewPortSize.cy);
  GL.Vertex2f(0, 0);
  GL.TexCoord2f(0.0, 0.0);
  GL.Vertex2f(0, ViewPortSize.cy);
  GL.TexCoord2f(ViewPortSize.cx, 0.0);
  GL.Vertex2f(ViewPortSize.cx, ViewPortSize.cy);
  GL.TexCoord2f(ViewPortSize.cx, ViewPortSize.cy);
  GL.Vertex2f(ViewPortSize.cx, 0);
  GL.End_;
  GL.DepthMask(True);
  GL.Enable(GL_DEPTH_TEST);
  GL.MatrixMode(GL_PROJECTION);
...
begin
  ...
  GL.TexCoord2f(0, 0);
  GL.Vertex2f(-1, -1);
  GL.TexCoord2f(ViewPortSize.cx, 0);
  GL.Vertex2f(1, -1);
  GL.TexCoord2f(ViewPortSize.cx, ViewPortSize.cy);
  GL.Vertex2f(1, 1);
  GL.TexCoord2f(0, ViewPortSize.cy);
  GL.Vertex2f(-1, 1);
  GL.End_;
end;
...
begin
  ...
  GL.CopyTexImage2D(glTarget, 0, GL_RGBA8, 0, 0, TextureSize.cx,
    TextureSize.cy, 0);
end;
...
procedure TGLCustomShader.LoadShaderPrograms(const VPFilename,
  FPFilename: string; GPFilename: string = '');
begin
  If VPFilename <> '' then
...
procedure TGLCustomShaderParameter.SetAsTexture(const TextureIndex: Integer;
  const Value: TGLTexture);
begin
  SetAsCustomTexture(TextureIndex, Value.Image.NativeTextureTarget,
    Value.Handle);
end;

procedure TGLCustomShaderParameter.SetAsTexture1D(const TextureIndex: Integer;
  const Value: TGLTexture);
begin
  SetAsCustomTexture(TextureIndex, ttTexture1D, Value.Handle);
end;

procedure TGLCustomShaderParameter.SetAsTexture2D(const TextureIndex: Integer;
  const Value: TGLTexture);
begin
  SetAsCustomTexture(TextureIndex, ttTexture2D, Value.Handle);
end;

procedure TGLCustomShaderParameter.SetAsTexture3D(const TextureIndex: Integer;
  const Value: TGLTexture);
begin
  SetAsCustomTexture(TextureIndex, ttTexture3D, Value.Handle);
end;

procedure TGLCustomShaderParameter.SetAsTextureCube(const TextureIndex: Integer;
  const Value: TGLTexture);
begin
  SetAsCustomTexture(TextureIndex, ttTextureCube, Value.Handle);
end;

procedure TGLCustomShaderParameter.SetAsTextureRect(const TextureIndex: Integer;
  const Value: TGLTexture);
begin
...
procedure TGLCustomShaderParameter.SetToTextureOf(const LibMaterial
  : TGLLibMaterial; const TextureIndex: Integer);
begin
  SetToTextureOf(LibMaterial.Material.Texture, TextureIndex);
end;

procedure TGLCustomShaderParameter.SetToTextureOf(const Texture: TGLTexture;
  const TextureIndex: Integer);
begin
  SetAsCustomTexture(TextureIndex, Texture.Image.NativeTextureTarget,
    Texture.Handle);
end;
...
RegisterClasses([TGLCustomShader, TGLShaderProgram, TGLVertexProgram,
  TGLFragmentProgram, TGLGeometryProgram]);
