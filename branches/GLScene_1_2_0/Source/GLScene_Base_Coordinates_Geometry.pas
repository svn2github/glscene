...
unit GLScene_Base_Coordinates_Geometry;
...
begin
  GLScene_Base_Vector_Geometry.sincos(theta, r, y, x);
  z := z1;
end;
...
begin
  r := sqrt(x * x + y * y);
  theta := GLScene_Base_Vector_Geometry.arctan2(y, x);
  z := z1;
end;
...
begin
  GLScene_Base_Vector_Geometry.sincos(phi, r, a, z);
  // z = r*cos(phi), a=r*sin(phi)
  GLScene_Base_Vector_Geometry.sincos(theta, a, y, x);
...
begin
  r := sqrt((x * x) + (y * y) + (z * z));
  theta := GLScene_Base_Vector_Geometry.arctan2(y, x);
  phi := GLScene_Base_Vector_Geometry.arccos(z / r);
end;
...
begin
  GLScene_Base_Vector_Geometry.sincos(eta, a, sn, cs);
  GLScene_Base_Vector_Geometry.sincos(phi, snphi, csphi);
  shx := sinh(xi);
  chx := cosh(xi);
...
begin
  GLScene_Base_Vector_Geometry.sincos(u, sn, cs);
  shx := sinh(v);
  chx := cosh(v);
