...
begin
  GLScene.Base.Vector.Geometry.sincos(theta, r, y, x);
  z := z1;
end;
...
begin
  r := sqrt(x * x + y * y);
  theta := GLScene.Base.Vector.Geometry.arctan2(y, x);
  z := z1;
end;
...
begin
  GLScene.Base.Vector.Geometry.sincos(phi, r, a, z);
  // z = r*cos(phi), a=r*sin(phi)
  GLScene.Base.Vector.Geometry.sincos(theta, a, y, x);
...
begin
  r := sqrt((x * x) + (y * y) + (z * z));
  theta := GLScene.Base.Vector.Geometry.arctan2(y, x);
  phi := GLScene.Base.Vector.Geometry.arccos(z / r);
end;
...
begin
  GLScene.Base.Vector.Geometry.sincos(eta, a, sn, cs);
  GLScene.Base.Vector.Geometry.sincos(phi, snphi, csphi);
  shx := sinh(xi);
  chx := cosh(xi);
...
begin
  GLScene.Base.Vector.Geometry.sincos(u, sn, cs);
  shx := sinh(v);
  chx := cosh(v);
