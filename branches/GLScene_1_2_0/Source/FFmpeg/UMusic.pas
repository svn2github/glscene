...
type
  ...
  protected
    ...
    procedure FillBufferWithFrame(Buffer: PByteArray; BufferSize: integer;
      Frame: PByteArray; FrameSize: integer);
  public
      (**
...
type
  ...
  IAudioPlayback = interface(IGenericPlayback)
    ...
    function CreateVoiceStream(ChannelMap: integer;
      FormatInfo: TAudioFormatInfo): TAudioVoiceStream;
  end;
  ...
  *)
  ...
  IAudioDecoder = interface(IGenericDecoder)
    ...
    function Open(const Filename: Utf8String): TAudioDecodeStream;
  end;
...
type
  ...
  public
    function Init(SrcFormatInfo: TAudioFormatInfo;
      DstFormatInfo: TAudioFormatInfo): boolean; virtual;
    destructor Destroy(); override;
       ...
       *)
    function Convert(InputBuffer: PByteArray; OutputBuffer: PByteArray;
      var InputSize: integer): integer; virtual; abstract;
...
constructor TAudioFormatInfo.Create(Channels: byte; SampleRate: double;
...
begin
  ...
  GLSLogger.LogInfo('InterfaceList' + IntToStr(InterfaceList.Count - 1));
  ...
  begin
    ...
    begin
      GLSLogger.LogError('Initialize failed, Removing - ' + CurrentAudioDecoder.GetName);
  ...
  begin
    ...
    GLSLogger.LogError('Initialize failed, Removing - ' + CurrentAudioPlayback.GetName);
  ...
  begin
    ...
    GLSLogger.LogError('Initialize failed, Removing - ' + CurrentAudioInput.GetName);
...
begin
  ...
  begin
    ...
    GLSLogger.LogError('Initialize failed, Removing - ' + VideoInterface.GetName);
...
function TAudioConverter.Init(SrcFormatInfo: TAudioFormatInfo;
 ...
 *)
function TAudioPlaybackStream.Synchronize(BufferSize: integer;
  FormatInfo: TAudioFormatInfo): integer;
var
  TimeDiff: double;
  FrameDiff: double;
  FrameSkip: integer;
...
begin
  ...
  {$IFDEF LOG_SYNC}
  GLSLogger.LogError(Format('c:%.3f | p:%.3f | d:%.3f | a:%.3f',
  ...
  begin
    {$IFDEF LOG_SYNC}
    GLSLogger.LogError(Format('ReposSynch: %.3f > %.3f',
  ...
  begin
    {$IFDEF LOG_SYNC}
    GLSLogger.LogError(Format('SoftSynch: %.3f > %.3f',
 ...
 *)
procedure TAudioPlaybackStream.FillBufferWithFrame(Buffer: PByteArray;
  BufferSize: integer; Frame: PByteArray; FrameSize: integer);
var
  i: integer;
...
function TAudioVoiceStream.Open(ChannelMap: integer;
  FormatInfo: TAudioFormatInfo): boolean;
begin
