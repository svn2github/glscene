...
begin
  ...
  begin
    GLSLogger.LogError('Unhandled sample-format'+ 'TBassPlaybackStream.Open');
    Exit;
  end;
  ...
  if (Handle = 0) then
  begin
    GLSLogger.LogError('BASS_StreamCreate failed: ' + BassCore.ErrorGetString(BASS_ErrorGetCode())+
...
begin
  ...
  begin
    GLSLogger.LogError('BASS_ChannelGetAttribute: ' + BassCore.ErrorGetString() +
...
begin
  ...
  case State of
    ...
    begin
      GLSLogger.LogError('Unknown status'+ 'TBassPlaybackStream.GetStatus');
      Result := ssStopped;
    end;
  end;
end;
...
begin
  ...
  begin
    GLSLogger.LogError('TSoundEffect.engineData already set'+ 'TBassPlaybackStream.AddSoundEffect');
    Exit;
  end;
  ...
  if (DspHandle = 0) then
  begin
    GLSLogger.LogError(BassCore.ErrorGetString()+ 'TBassPlaybackStream.AddSoundEffect');
    Exit;
  end;
...
begin
  ...
  begin
    GLSLogger.LogError('TSoundEffect.engineData invalid'+ 'TBassPlaybackStream.RemoveSoundEffect');
    Exit;
  end;

  if not BASS_ChannelRemoveDSP(Handle, PHDSP(Effect.EngineData)^) then
  begin
    GLSLogger.LogError(BassCore.ErrorGetString()+ 'TBassPlaybackStream.RemoveSoundEffect');
    Exit;
  end;
...
begin
  ...
  begin
    GLSLogger.LogError('Could not initialize BASS'+ 'TAudioPlayback_Bass.InitializePlayback');
    Exit;
  end;
