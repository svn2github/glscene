...
const
  DCEBox: array [0 .. 35] of TAffineVector = ((1, -1, -1), (1, 1, -1),
...
var
  N: TAffineVector;
begin
  N[0] := Abs(MovePack.Velocity[0]) + Abs(MovePack.Gravity[0]) +
    (MovePack.Radius[0]);
  N[1] := Abs(MovePack.Velocity[1]) + Abs(MovePack.Gravity[1]) +
    (MovePack.Radius[1]);
  N[2] := Abs(MovePack.Velocity[2]) + Abs(MovePack.Gravity[2]) +
    (MovePack.Radius[2]);
