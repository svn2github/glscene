...
begin
  ...
  begin
    ...
    begin
      ...
      if (BoneVertex[0] > max[0]) then
        max[0] := BoneVertex[0];
      if (BoneVertex[1] > max[1]) then
        max[1] := BoneVertex[1];
      if (BoneVertex[2] > max[2]) then
        max[2] := BoneVertex[2];

      if (BoneVertex[0] < min[0]) then
        min[0] := BoneVertex[0];
      if (BoneVertex[1] < min[1]) then
        min[1] := BoneVertex[1];
      if (BoneVertex[2] < min[2]) then
        min[2] := BoneVertex[2];
    end;
