      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              if PointInTriangle(PolyVerts[temp[j]], v[0], v[1], v[2],
                PolyCW) then
              begin
...
var
  ...
  begin
    repeat
      ...
      Inc(uniqueMatID);
      libMat := TGLMaterialLibrary(Owner.MaterialLibrary)
        .Materials.GetLibMaterialByName(Result);
  ...
  begin
    ...
    MatLibrary := Owner.MaterialLibrary as TGLMaterialLibrary;
    Result := MatLibrary.Materials.GetLibMaterialByName(matname);
    if not Assigned(Result) then
    begin
      Result := MatLibrary.Materials.Add;
      Result.Name := matname;
    end;
    ...
    begin
      if VRMLMaterial.HasDiffuse then
        Diffuse.Color := VectorMake(VRMLMaterial.DiffuseColor,
          Diffuse.Color[3]);
      if VRMLMaterial.HasAmbient then
        Ambient.Color := VectorMake(VRMLMaterial.AmbientColor,
          Ambient.Color[3]);
  ...
  begin
    ...
    for i := 0 to node.Count - 1 do
      ...
      begin
        if not VectorEquals(TVRMLTransform(node[i]).Rotation,
          NullHMGVector) then
        begin
      ...
      begin
        ...
        begin
          with Owner.MaterialLibrary as TGLMaterialLibrary do
            tempLibMat := Materials.GetLibMaterialByName
