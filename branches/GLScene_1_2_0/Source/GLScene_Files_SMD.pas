  ...
  try
    ...
    begin
      ...
      begin
        ...
        with Owner.Skeleton do
          if (tl[2] <> '-1') then
            bone := TSkeletonBone.CreateOwned
              (RootBones.BoneByID(StrToInt(tl[2])))
          else
            bone := TSkeletonBone.CreateOwned(RootBones);
    ...
    begin
      ...
      begin
        ...
        frame.Position.Add(GLScene_Utils.StrToFloatDef(tl[1]),
          GLScene_Utils.StrToFloatDef(tl[2]),
          GLScene_Utils.StrToFloatDef(tl[3]));
        v := AffineVectorMake(GLScene_Utils.StrToFloatDef(tl[4]),
          GLScene_Utils.StrToFloatDef(tl[5]),
          GLScene_Utils.StrToFloatDef(tl[6]));
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                boneIDs[j].Weight := GLScene_Utils.StrToFloatDef
                  (tl[11 + j * 2]);
              end;

              nVert := FindOrAdd(boneIDs,
                AffineVectorMake(GLScene_Utils.StrToFloatDef(tl[1]),
                GLScene_Utils.StrToFloatDef(tl[2]),
                GLScene_Utils.StrToFloatDef(tl[3])),
                AffineVectorMake(GLScene_Utils.StrToFloatDef(tl[4]),
                GLScene_Utils.StrToFloatDef(tl[5]),
                GLScene_Utils.StrToFloatDef(tl[6])));
              nTex := TexCoords.FindOrAdd
                (AffineVectorMake(GLScene_Utils.StrToFloatDef(tl[7]),
                GLScene_Utils.StrToFloatDef(tl[8]), 0));
...
var
  ...
  procedure GetNodesFromBonesRecurs(bone: TSkeletonBone; ParentID: Integer;
    bl: TStrings);
  var
    i: Integer;
  begin
    ...
    begin
      ...
      for i := 0 to Owner.MeshObjects.Count - 1 do
        if Owner.MeshObjects[i] is TSkeletonMeshObject then
          with TSkeletonMeshObject(Owner.MeshObjects[i]) do
          begin
