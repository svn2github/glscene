...
begin
  ...
  try
    ...
    for i := 0 to sl.Count - 1 do
      if Copy(sl[i], 1, 2) = 't ' then
      begin
        tl.CommaText := Trim(Copy(sl[i], 3, MaxInt));
        if tl.Count = 9 then
        begin
          SetVector(v1, GLScene_Utils.StrToFloatDef(tl[0], 0),
            GLScene_Utils.StrToFloatDef(tl[1], 0),
            GLScene_Utils.StrToFloatDef(tl[2], 0));
          SetVector(v2, GLScene_Utils.StrToFloatDef(tl[3], 0),
            GLScene_Utils.StrToFloatDef(tl[4], 0),
            GLScene_Utils.StrToFloatDef(tl[5], 0));
          SetVector(v3, GLScene_Utils.StrToFloatDef(tl[6], 0),
            GLScene_Utils.StrToFloatDef(tl[7], 0),
