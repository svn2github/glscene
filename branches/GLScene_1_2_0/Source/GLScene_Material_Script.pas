...
begin
  ...
  if pos('(', Val) > 0 then
  begin
    tmpcolor.Alpha := GLScene_Utils.StrToFloatDef(copy(Val, pos('(', Val) + 1,
      pos(';', Val) - 2));
    delete(Val, 1, pos(';', Val));
    tmpcolor.Red := GLScene_Utils.StrToFloatDef
      (copy(Val, 1, pos(';', Val) - 1));
    delete(Val, 1, pos(';', Val));
    tmpcolor.Green := GLScene_Utils.StrToFloatDef
      (copy(Val, 1, pos(';', Val) - 1));
    delete(Val, 1, pos(';', Val));
    tmpcolor.Blue := GLScene_Utils.StrToFloatDef
      (copy(Val, 1, pos(')', Val) - 1));
  end;
end;
...
begin
  ...
  if pos('(', Val) > 0 then
  begin
    tmpcoords.X := GLScene_Utils.StrToFloatDef(copy(Val, pos('(', Val) + 1,
      pos(';', Val) - 2));
    delete(Val, 1, pos(';', Val));
    tmpcoords.Y := GLScene_Utils.StrToFloatDef(copy(Val, 1, pos(';', Val) - 1));
    delete(Val, 1, pos(';', Val));
    tmpcoords.Z := GLScene_Utils.StrToFloatDef(copy(Val, 1, pos(')', Val) - 1));
  end;
end;
...
begin
  ...
  if pos('(', Val) > 0 then
  begin
    tmpcoords4.W := GLScene_Utils.StrToFloatDef(copy(Val, pos('(', Val) + 1,
      pos(';', Val) - 2));
    delete(Val, 1, pos(';', Val));
    tmpcoords4.X := GLScene_Utils.StrToFloatDef
      (copy(Val, 1, pos(';', Val) - 1));
    delete(Val, 1, pos(';', Val));
    tmpcoords4.Y := GLScene_Utils.StrToFloatDef
      (copy(Val, 1, pos(';', Val) - 1));
    delete(Val, 1, pos(';', Val));
    tmpcoords4.Z := GLScene_Utils.StrToFloatDef
      (copy(Val, 1, pos(')', Val) - 1));
  end;
end;
  ...
  if ClassExists('imagebrightness') then
    if ExtractValue <> '' then
      NewMat.Material.Texture.ImageBrightness :=
        GLScene_Utils.StrToFloatDef(ExtractValue);
end;
...
begin
  if ClassExists('imagegamma') then
    if ExtractValue <> '' then
      NewMat.Material.Texture.ImageGamma := GLScene_Utils.StrToFloatDef
        (ExtractValue);
end;
