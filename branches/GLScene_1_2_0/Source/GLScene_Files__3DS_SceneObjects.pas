...
unit GLScene_Files__3DS_SceneObjects;
...
begin
  ...
  FTargetPos := TGLCoordinates.CreateInitialized(self,
    VectorMake(NullVector), csPoint);
  FHotSpot := 1;
  FMultipler := 1;
end;

procedure TGLFile3DSLight.DoRender(var rci: TRenderContextInfo;
...
begin
  ...
  if (Sender = FTargetPos) or (Sender = Position) then
    SpotDirection.SetVector
      (VectorNormalize(VectorSubtract(FTargetPos.AsAffineVector,
...
procedure TGLFile3DSCamera.DoRender(var rci: TRenderContextInfo;
  renderSelf, renderChildren: Boolean);
...
begin
  ...
  v := VectorNormalize(VectorSubtract(FTargetPos.AsAffineVector,
    Position.AsAffineVector));

  v1 := AffineVectorMake(v[0], v[1], 0);
  NormalizeVector(v1);
  ang := GLScene_Base_Vector_Geometry.arccos(VectorDotProduct(v, v1));
...
begin
  ...
  FS_Rot3DS := TGLCoordinates4.CreateInitialized(self, VectorMake(1, 0, 0),
    csVector);
  FRot3DS := TGLCoordinates4.CreateInitialized(self, VectorMake(1, 0, 0),
    csVector);
  FScale3DS := TGLCoordinates4.CreateInitialized(self, VectorMake(1, 1, 1),
    csVector);
...
RegisterClasses([TGLFile3DSLight, TGLFile3DSCamera, TGLFile3DSActor,
  TGLFile3DSFreeForm]);
