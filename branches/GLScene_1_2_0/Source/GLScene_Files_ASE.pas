...
type
  ...
  TGLASEFaceTexureChannels = record
    Count: Integer;
    ChanelTexture: array [0 .. GL_ASE_MAX_TEXURE_CHANNELS - 1]
      of TGLASEFaceTexure;
  end;
  ...
  public
    ...
    property TextChannels: TGLASEFaceTexureChannels read FTextChannels;
    // channels texture coordinates
    property Smoothing: TGLASESmoothingGroups read FSmoothing;
  ...
  private
    ...
    FTexChannels: array [0 .. GL_ASE_MAX_TEXURE_CHANNELS - 1]
      of TAffineVectorList;
  ...
  public
    ...
    property TextChannel[Channel: Integer]: TAffineVectorList
      read GetTextChannel;
  ...
  private
    ...
    function GetValue4D(const aData: string; var Value0: Integer)
      : TAffineVector;
    ...
    procedure ParseFaceString(const aData: string;
      var Index, A, B, C, AB, BC, CA, MatID: Integer;
  ...
  private
    ...
    procedure ParseMeshOptions(var aLineIndex: Integer;
      aMesh: TGLASEMeshObject);
    procedure ParseMeshGeom(var aLineIndex: Integer; aMesh: TGLASEMeshObject);
    procedure ParseMappingChannel(var aLineIndex: Integer;
      aMesh: TGLASEMeshObject);
    procedure ParseMeshVertices(var aLineIndex: Integer;
      aMesh: TGLASEMeshObject; VerticesCount: Integer);
    procedure ParseMeshFaces(var aLineIndex: Integer; aMesh: TGLASEMeshObject;
      FacesCount: Integer);
    procedure ParseMeshNormals(var aLineIndex: Integer; aMesh: TGLASEMeshObject;
      FacesCount: Integer);
    procedure ParseMeshTextureVertices(var aLineIndex: Integer;
      aMesh: TGLASEMeshObject; TextureVerticesCount: Integer);
    procedure ParseMeshTextureFaces(var aLineIndex: Integer;
      aMesh: TGLASEMeshObject; TextureFacesCount: Integer);
    ...
    procedure ParseSubMaterial(var aLineIndex: Integer;
      aMaterial: TGLASEMaterial);
    function CheckTextureMap(var aLineIndex: Integer;
      var aMaps: TGLASEMaterialTextureMaps): Boolean;
    procedure ParseTextureMap(var aLineIndex: Integer;
      var aMaps: TGLASEMaterialTextureMaps; const aMapKind: string);
  ...
  TASETextureMap = (tmGeneric, tmAmbient, tmDiffuse, tmSpecular, tmShine,
    tmShinestrength, tmSelfillum, tmOpacity, tmFiltercolor, tmBump, tmReflect,
    tmRefract);
...
procedure ASESetPreferredTexture(aMap: TASETextureMap;
  aSubMaterialIndex: Integer = -1);
procedure ASESetPreferredLightmap(aMap: TASETextureMap;
  aSubMaterialIndex: Integer = -1);
...
const
  vTagIdx: array [0 .. 88] of TTagIdx = ((Idx: ASCII_COMMENT_I;
    Name: ASCII_COMMENT_S), (Idx: ASCII_SCENE_I; Name: ASCII_SCENE_S),
    (Idx: ASCII_GEOMOBJECT_I; Name: ASCII_GEOMOBJECT_S),
    (Idx: ASCII_NODE_NAME_I; Name: ASCII_NODE_NAME_S), (Idx: ASCII_NODE_TM_I;
    Name: ASCII_NODE_TM_S), (Idx: ASCII_INHERIT_POS_I;
    Name: ASCII_INHERIT_POS_S), (Idx: ASCII_INHERIT_ROT_I;
    Name: ASCII_INHERIT_ROT_S), (Idx: ASCII_INHERIT_SCL_I;
    Name: ASCII_INHERIT_SCL_S), (Idx: ASCII_ROW_I; Name: ASCII_ROW_S),
    (Idx: ASCII_POS_I; Name: ASCII_POS_S), (Idx: ASCII_ROTAXIS_I;
    Name: ASCII_ROTAXIS_S), (Idx: ASCII_ROTANGLE_I; Name: ASCII_ROTANGLE_S),
    (Idx: ASCII_SCALE_I; Name: ASCII_SCALE_S), (Idx: ASCII_SCALEAXIS_I;
    Name: ASCII_SCALEAXIS_S), (Idx: ASCII_SCALEAXISANG_I;
    Name: ASCII_SCALEAXISANG_S), (Idx: ASCII_MESH_I; Name: ASCII_MESH_S),
    ...
    (Idx: ASCII_ROW1_I; Name: ASCII_ROW1_S), (Idx: ASCII_ROW2_I;
    Name: ASCII_ROW2_S), (Idx: ASCII_ROW3_I; Name: ASCII_ROW3_S),
    ...
    (Idx: ASCII_MAP_BUMP_I; Name: ASCII_MAP_BUMP_S), (Idx: ASCII_MAP_REFLECT_I;
    Name: ASCII_MAP_REFLECT_S), (Idx: ASCII_MAP_REFRACT_I;
    Name: ASCII_MAP_REFRACT_S), (Idx: ASCII_MAP_NAME_I; Name: ASCII_MAP_NAME_S),
    (Idx: ASCII_MAP_CLASS_I; Name: ASCII_MAP_CLASS_S), (Idx: ASCII_MAP_SUBNO_I;
    Name: ASCII_MAP_SUBNO_S), (Idx: ASCII_MAP_AMOUNT_I;
    Name: ASCII_MAP_AMOUNT_S), (Idx: ASCII_BITMAP_I; Name: ASCII_BITMAP_S),
    (Idx: ASCII_MAP_TYPE_I; Name: ASCII_MAP_TYPE_S), (Idx: ASCII_UVW_U_OFFSET_I;
    Name: ASCII_UVW_U_OFFSET_S), (Idx: ASCII_UVW_V_OFFSET_I;
    Name: ASCII_UVW_V_OFFSET_S), (Idx: ASCII_UVW_U_TILING_I;
    Name: ASCII_UVW_U_TILING_S), (Idx: ASCII_UVW_V_TILING_I;
    Name: ASCII_UVW_V_TILING_S), (Idx: ASCII_UVW_ANGLE_I;
    Name: ASCII_UVW_ANGLE_S), (Idx: ASCII_UVW_BLUR_I; Name: ASCII_UVW_BLUR_S),
...
procedure CopyASEToMesh(aASEMesh: TGLASEMeshObject; aMesh: TMeshObject;
  aASEMaterials: TGLASEMaterialList);

const
  ASETextureMapKinds: array [TASETextureMap] of string = (ASCII_MAP_GENERIC_S,
    ASCII_MAP_AMBIENT_S, ASCII_MAP_DIFFUSE_S, ASCII_MAP_SPECULAR_S,
    ASCII_MAP_SHINE_S, ASCII_MAP_SHINESTRENGTH_S, ASCII_MAP_SELFILLUM_S,
    ASCII_MAP_OPACITY_S, ASCII_MAP_FILTERCOLOR_S, ASCII_MAP_BUMP_S,
    ASCII_MAP_REFLECT_S, ASCII_MAP_REFRACT_S);

  function FindTextureMap(aMaterial: TGLASEMaterial; aMapKind: string;
    aSubMaterialIndex: Integer; out TM: TGLASEMaterialTextureMap): Boolean;
    ...
    begin
      ...
      for i := 0 to aMaps.Count - 1 do
        if aMaps.Map[i].Kind = aMapKind then
        begin
          TM := aMaps.Map[i];
          Result := True;
          Break;
  ...
  begin
    ...
    else if aSubMaterialIndex < aMaterial.SubMaterials.Count then
      Result := FindInMaps(aMaterial.SubMaterials.SubMaterial[aSubMaterialIndex]
        .TextureMaps)
    else
      Result := False;
  end;
  ...
  begin
    ...
    if Assigned(aMesh.Owner) and Assigned(aMesh.Owner.Owner) then
    begin
      matLib := aMesh.Owner.Owner.MaterialLibrary as TGLMaterialLibrary;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Shininess :=
              MaxInteger(0, Integer(Round((1 - Material.ShineStrength) * 128)));
          end;

          if FindTextureMap(Material, ASETextureMapKinds[vPrepTexMap],
            vPrepTexSubM, TM) and (Trim(TM.Bitmap) <> '') then
          begin
            try
  ...
  begin
    ...
    begin
      ...
      if Assigned(matLib) then
      begin
        if FindTextureMap(Material, ASETextureMapKinds[vPrepLightMap],
          vPrepLightSubM, TM) and (Trim(TM.Bitmap) <> '') then
        begin
...
var
  ...
  function GetFaceGroup(aMaterialID, aSubMaterialID: Integer)
    : TFGVertexIndexList;
  var
    i: Integer;
    name: string;
  begin
    ...
    if aMaterialID >= 0 then
      name := aASEMaterials[aMaterialID].Name + IntToStr(aMaterialID) +
        IntToStr(aSubMaterialID)
    else
      name := '';
    ...
    begin
      ...
      for i := 0 to aMesh.FaceGroups.Count - 1 do
        if aMesh.FaceGroups.Items[i].MaterialName = name then
        begin
      ...
      begin
        ...
        if aMaterialID > -1 then
        begin
          Result.MaterialName := GetOrAllocateMaterial(aMaterialID,
            aSubMaterialID);
          Result.LightMapIndex := GetOrAllocateLightMap(aMaterialID,
            aSubMaterialID);
        end;
        vLastFG := Result;
      end;
...
begin
  ...
  begin
    ...
    vLastFG := nil;
    for i := 0 to aASEMesh.Faces.Count - 1 do
    begin
      ...
      begin
        ...
        aMesh.TexCoords.Add(aASEMesh.TextChannel[0]
          [aseFace.TextChannels.ChanelTexture[0].Idx0],
        ...
        if light then
        begin
          lmt[0] := aASEMesh.TextChannel[1]
            [aseFace.TextChannels.ChanelTexture[1].Idx0];
          lmt[1] := aASEMesh.TextChannel[1]
            [aseFace.TextChannels.ChanelTexture[1].Idx1];
          lmt[2] := aASEMesh.TextChannel[1]
            [aseFace.TextChannels.ChanelTexture[1].Idx2];
  ...
  begin
    ...
    begin
      aseFace := aASEMesh.Faces[i];
      fg.VertexIndices.Add(aseFace.VertIdx1, aseFace.VertIdx2,
        aseFace.VertIdx3);
    end;
  end;
...
begin
  Assert(FTexChannelsCount < GL_ASE_MAX_TEXURE_CHANNELS,
    'texture channels count maximum reached');
...
begin
  ...
  for Index := High(vTagIdx) downto Low(vTagIdx) do
    if ContainString(aData, vTagIdx[Index].Name) then
    begin
...
function TGLASEVectorFile.GetValue4D(const aData: string; var Value0: Integer)
  : TAffineVector;
var
  Start: Integer;
  Data: string;
...
begin
  ...
  while not IsEndOfSection(Data) do
  begin
...
begin
  LineIndex := 0;
  while LineIndex < FStringData.Count do
  begin
...
procedure TGLASEVectorFile.ParseFaceString(const aData: string;
  var Index, A, B, C, AB, BC, CA, MatID: Integer;
  var Smooth: TGLASESmoothingGroups);
var
  Data: string;
...
begin
  ...
  if Pos(ASCII_MESH_MTLID_S, Data) > 0 then
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      case GetTagOnData(Data) of
        ASCII_NODE_NAME_I:
          obj.Name := GetStringValue(Data);
        ASCII_NODE_TM_I:
          ParseMeshOptions(aLineIndex, aseMesh);
        ASCII_MESH_I:
          ParseMeshGeom(aLineIndex, aseMesh);
        ASCII_MATERIAL_REF_I:
          aseMesh.FMaterialID := Round(GetDoubleValue(Data));
      else
        begin
          B := True;
          CheckUnknownData(aLineIndex);
...
procedure TGLASEVectorFile.ParseMeshFaces(var aLineIndex: Integer;
  aMesh: TGLASEMeshObject; FacesCount: Integer);
var
  Data: string;
...
procedure TGLASEVectorFile.ParseMeshNormals(var aLineIndex: Integer;
  aMesh: TGLASEMeshObject; FacesCount: Integer);
var
  Data: string;
  Index: Integer;
...
procedure TGLASEVectorFile.ParseMeshTextureFaces(var aLineIndex: Integer;
  aMesh: TGLASEMeshObject; TextureFacesCount: Integer);
var
  Data: string;
  faceIndex: Integer;
...
procedure TGLASEVectorFile.ParseMeshTextureVertices(var aLineIndex: Integer;
  aMesh: TGLASEMeshObject; TextureVerticesCount: Integer);
var
  Data: string;
  Index: Integer;
...
begin
  ...
  begin
    ...
    Assert(Index = (Channel.Count - 1),
      'unexpected unsorted texture coordinates');
...
procedure TGLASEVectorFile.ParseMeshGeom(var aLineIndex: Integer;
  aMesh: TGLASEMeshObject);
var
  Data: string;
...
begin
  ...
  begin
    ...
    case GetTagOnData(Data) of
      ASCII_TIMEVALUE_I:
        ; // aMesh.TimeValue := Round(GetDoubleValue(Data));
      ASCII_MESH_NUM_VERTEX_I:
        VerticesCount := Round(GetDoubleValue(Data));
      ASCII_NUM_FACES_I:
        FacesCount := Round(GetDoubleValue(Data));
      ASCII_VERTEX_LIST_I:
        ParseMeshVertices(aLineIndex, aMesh, VerticesCount);
      ASCII_MESH_FACE_LIST_I:
        ParseMeshFaces(aLineIndex, aMesh, FacesCount);
      ASCII_MESH_NORMALS_I:
        ParseMeshNormals(aLineIndex, aMesh, FacesCount);
      ASCII_MESH_NUMTVERTEX_I:
        TextureVerticesCount := Round(GetDoubleValue(Data));
      ASCII_MESH_TVERTLIST_I:
        ParseMeshTextureVertices(aLineIndex, aMesh, TextureVerticesCount);
      ASCII_MESH_NUMTVFACES_I:
        TextureFacesCount := Round(GetDoubleValue(Data));
      ASCII_MESH_TFACELIST_I:
        ParseMeshTextureFaces(aLineIndex, aMesh, TextureFacesCount);
      ASCII_MESH_MAPPINGCHANNEL_I:
        ParseMappingChannel(aLineIndex, aMesh);
    else
      begin
        B := True;
        CheckUnknownData(aLineIndex);
...
procedure TGLASEVectorFile.ParseMeshVertices(var aLineIndex: Integer;
  aMesh: TGLASEMeshObject; VerticesCount: Integer);
var
  Data: string;
  VertIndex: Integer;
begin
  ...
  begin
    ...
    Assert(VertIndex = (aMesh.Vertices.Count - 1),
      'unexpeted unsorted vertices');
...
procedure TGLASEVectorFile.ParseMappingChannel(var aLineIndex: Integer;
  aMesh: TGLASEMeshObject);
var
  Data: string;
  TextureVerticesCount: Integer;
...
procedure TGLASEVectorFile.ParseMeshOptions(var aLineIndex: Integer;
  aMesh: TGLASEMeshObject);
var
  Data: string;
  B: Boolean;
begin
  ...
  begin
    ...
    case GetTagOnData(Data) of
      ASCII_INHERIT_POS_I:
        aMesh.FInheritedPosition := GetValue3D(Data);
      ASCII_INHERIT_ROT_I:
        aMesh.FInheritedRotation := GetValue3D(Data);
      ASCII_INHERIT_SCL_I:
        aMesh.FInheritedScale := GetValue3D(Data);
      ASCII_ROW_I:
        aMesh.FMatrix[0] := VectorMake(GetValue3D(Data));
      ASCII_ROW1_I:
        aMesh.FMatrix[1] := VectorMake(GetValue3D(Data));
      ASCII_ROW2_I:
        aMesh.FMatrix[2] := VectorMake(GetValue3D(Data));
      ASCII_ROW3_I:
        aMesh.FMatrix[3] := PointMake(GetValue3D(Data));
      ASCII_POS_I:
        aMesh.FPosition := PointMake(GetValue3D(Data));
      ASCII_ROTAXIS_I:
        aMesh.FRotationAxis := GetValue3D(Data);
      ASCII_ROTANGLE_I:
        aMesh.FRotationAngle := GetDoubleValue(Data);
      ASCII_SCALE_I:
        aMesh.FScale := GetValue3D(Data);
      ASCII_SCALEAXIS_I:
        aMesh.FScaleAxis := GetValue3D(Data);
      ASCII_SCALEAXISANG_I:
        aMesh.FScaleAxisAngle := GetDoubleValue(Data);
    else
      begin
        B := True;
        CheckUnknownData(aLineIndex);
...
function TGLASEVectorFile.CheckTextureMap(var aLineIndex: Integer;
  var aMaps: TGLASEMaterialTextureMaps): Boolean;
var
  Data: string;
begin
  ...
  case GetTagOnData(Data) of
    ASCII_MAP_AMBIENT_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_AMBIENT_S);
    ASCII_MAP_DIFFUSE_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_DIFFUSE_S);
    ASCII_MAP_GENERIC_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_GENERIC_S);
    ASCII_MAP_SPECULAR_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_SPECULAR_S);
    ASCII_MAP_SHINE_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_SHINE_S);
    ASCII_MAP_SHINESTRENGTH_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_SHINESTRENGTH_S);
    ASCII_MAP_SELFILLUM_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_SELFILLUM_S);
    ASCII_MAP_OPACITY_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_OPACITY_S);
    ASCII_MAP_FILTERCOLOR_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_FILTERCOLOR_S);
    ASCII_MAP_BUMP_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_BUMP_S);
    ASCII_MAP_REFLECT_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_REFLECT_S);
    ASCII_MAP_REFRACT_I:
      ParseTextureMap(aLineIndex, aMaps, ASCII_MAP_REFRACT_S);
  else
    Result := False;
  end;
end;

procedure TGLASEVectorFile.ParseMaterial(var aLineIndex: Integer;
  aMaterial: TGLASEMaterial);
var
  Data: string;
  B: Boolean;
begin
  ...
  begin
    B := IsSectionBegingin(Data);
    case GetTagOnData(Data) of
      ASCII_MATERIAL_NAME_I:
        aMaterial.FName := GetStringValue(Data);
      ASCII_MATERIAL_AMBIENT_I:
        aMaterial.FAmbient := GetValue3D(Data);
      ASCII_MATERIAL_DIFFUSE_I:
        aMaterial.FDiffuse := GetValue3D(Data);
      ASCII_MATERIAL_SPECULAR_I:
        aMaterial.FSpecular := GetValue3D(Data);
      ASCII_MATERIAL_SHINE_I:
        aMaterial.FShiness := GetDoubleValue(Data);
      ASCII_MATERIAL_SHINESTRENGTH_I:
        aMaterial.FShineStrength := GetDoubleValue(Data);
      ASCII_MATERIAL_TRANSPARENCY_I:
        aMaterial.FTransparency := GetDoubleValue(Data);
      ASCII_MATERIAL_WIRESIZE_I:
        aMaterial.FWireSize := GetDoubleValue(Data);
      ASCII_SUBMATERIAL_I:
        ParseSubMaterial(aLineIndex, aMaterial);
    else
      if not CheckTextureMap(aLineIndex, aMaterial.FTextureMaps) then
      begin
        B := True;
        CheckUnknownData(aLineIndex);
      end;
...
procedure TGLASEVectorFile.ParseSubMaterial(var aLineIndex: Integer;
  aMaterial: TGLASEMaterial);
var
  Data: string;
  Index: Integer;
...
begin
  ...
  begin
    ...
    case GetTagOnData(Data) of
      ASCII_MATERIAL_NAME_I:
        aMaterial.FSubMaterials.SubMaterial[Index].Name := GetStringValue(Data);
      ASCII_MATERIAL_AMBIENT_I:
        aMaterial.FSubMaterials.SubMaterial[Index].Ambient := GetValue3D(Data);
      ASCII_MATERIAL_DIFFUSE_I:
        aMaterial.FSubMaterials.SubMaterial[Index].Diffuse := GetValue3D(Data);
      ASCII_MATERIAL_SPECULAR_I:
        aMaterial.FSubMaterials.SubMaterial[Index].Specular := GetValue3D(Data);
      ASCII_MATERIAL_SHINE_I:
        aMaterial.FSubMaterials.SubMaterial[Index].Shiness :=
          GetDoubleValue(Data);
      ASCII_MATERIAL_SHINESTRENGTH_I:
        aMaterial.FSubMaterials.SubMaterial[Index].ShineStrength :=
          GetDoubleValue(Data);
      ASCII_MATERIAL_TRANSPARENCY_I:
        aMaterial.FSubMaterials.SubMaterial[Index].Transparency :=
          GetDoubleValue(Data);
      ASCII_MATERIAL_WIRESIZE_I:
        aMaterial.FSubMaterials.SubMaterial[Index].WireSize :=
          GetDoubleValue(Data);
      ASCII_MATERIAL_SELFILLUM_I:
        aMaterial.FSubMaterials.SubMaterial[Index].SelfIllumination :=
          GetDoubleValue(Data);
    else
      if not CheckTextureMap(aLineIndex, aMaterial.FSubMaterials.SubMaterial
        [Index].TextureMaps) then
      begin
        B := True;
        CheckUnknownData(aLineIndex);
      end;
...
procedure TGLASEVectorFile.ParseTextureMap(var aLineIndex: Integer;
  var aMaps: TGLASEMaterialTextureMaps; const aMapKind: string);
var
  Data: string;
  Index: Integer;
...
begin
  ...
  begin
    ...
    case GetTagOnData(Data) of
      ASCII_MAP_NAME_I:
        aMaps.Map[Index].Name := GetStringValue(Data);
      ASCII_MAP_CLASS_I:
        aMaps.Map[Index]._Class := GetStringValue(Data);
      ASCII_MAP_SUBNO_I:
        aMaps.Map[Index].No := Round(GetDoubleValue(Data));
      ASCII_MAP_AMOUNT_I:
        aMaps.Map[Index].Amount := GetDoubleValue(Data);
      ASCII_BITMAP_I:
        aMaps.Map[Index].Bitmap := GetStringValue(Data);
      ASCII_UVW_U_OFFSET_I:
        aMaps.Map[Index].UOffset := GetDoubleValue(Data);
      ASCII_UVW_V_OFFSET_I:
        aMaps.Map[Index].VOffset := GetDoubleValue(Data);
      ASCII_UVW_U_TILING_I:
        aMaps.Map[Index].UTiling := GetDoubleValue(Data);
      ASCII_UVW_V_TILING_I:
        aMaps.Map[Index].VTiling := GetDoubleValue(Data);
      ASCII_UVW_ANGLE_I:
        aMaps.Map[Index].Angle := GetDoubleValue(Data);
      ASCII_UVW_BLUR_I:
        aMaps.Map[Index].Blur := GetDoubleValue(Data);
      ASCII_UVW_BLUR_OFFSET_I:
        aMaps.Map[Index].BlurOffset := GetDoubleValue(Data);
      ASCII_UVW_NOUSE_AMT_I:
        aMaps.Map[Index].NouseAmount := GetDoubleValue(Data);
      ASCII_UVW_NOISE_SIZE_I:
        aMaps.Map[Index].NoiseSize := GetDoubleValue(Data);
      ASCII_UVW_NOISE_LEVEL_I:
        aMaps.Map[Index].NoiseLevel := Round(GetDoubleValue(Data));
      ASCII_UVW_NOISE_PHASE_I:
        aMaps.Map[Index].NoisePhase := GetDoubleValue(Data);
    else
      begin
        B := True;
        CheckUnknownData(aLineIndex);
