...
procedure TGLMD5VectorFile.LoadFromStream(aStream: TStream);
  ...
  begin
    ...
    pos[0] := GLScene.Utils.StrToFloatDef(FTempString[2]);
    pos[1] := GLScene.Utils.StrToFloatDef(FTempString[4]);
    pos[2] := GLScene.Utils.StrToFloatDef(FTempString[3]);

    quat := QuaternionMakeFromImag(GLScene.Utils.StrToFloatDef(FTempString[5]),
      GLScene.Utils.StrToFloatDef(FTempString[7]),
      GLScene.Utils.StrToFloatDef(FTempString[6]));
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            mesh.TexCoords[vnum] :=
              AffineVectorMake(GLScene.Utils.StrToFloatDef(FTempString[2]),
              1 - GLScene.Utils.StrToFloatDef(FTempString[3]), 0);
        ...
        begin
          ...
          begin
            ...
            VertexWeight[wnum] := GLScene.Utils.StrToFloatDef(FTempString[3]);
            VertexWeighted[wnum] :=
              AffineVectorMake(GLScene.Utils.StrToFloatDef(FTempString[4]),
              GLScene.Utils.StrToFloatDef(FTempString[6]),
              GLScene.Utils.StrToFloatDef(FTempString[5]));
          end;
        end;
      end;
    until temp = '}';
  ...
  begin
    ...
    repeat
      ...
      if FTempString.Count >= 6 then
      begin
        pos := AffineVectorMake(GLScene.Utils.StrToFloatDef(FTempString[0]),
          GLScene.Utils.StrToFloatDef(FTempString[1]),
          GLScene.Utils.StrToFloatDef(FTempString[2]));
        quat := QuaternionMakeFromImag
          (GLScene.Utils.StrToFloatDef(FTempString[3]),
          GLScene.Utils.StrToFloatDef(FTempString[4]),
          GLScene.Utils.StrToFloatDef(FTempString[5]));
        FFramePositions.Add(pos);
        FFrameQuaternions.Add(quat);
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          pos[0] := GLScene.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 2 > 0 then
        begin
          pos[1] := GLScene.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 4 > 0 then
        begin
          pos[2] := GLScene.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;

        if FJointFlags[i] and 8 > 0 then
        begin
          quat.ImagPart[0] := GLScene.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 16 > 0 then
        begin
          quat.ImagPart[1] := GLScene.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 32 > 0 then
          quat.ImagPart[2] := GLScene.Utils.StrToFloatDef(FTempString[j]);
      end;

      pos := AffineVectorMake(pos[0], pos[2], pos[1]);
      quat := QuaternionMakeFromImag(quat.ImagPart[0], quat.ImagPart[2],
        quat.ImagPart[1]);
