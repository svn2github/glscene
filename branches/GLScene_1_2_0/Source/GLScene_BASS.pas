...
{$IFDEF FPC}
  if not BASS_Init(1, OutputFrequency, BASS_DEVICE_3D,
    TWinControl(Owner).Handle, nil) then
  begin
{$ELSE}
  if not BASS_Init(1, OutputFrequency, BASS_DEVICE_3D, Application.Handle,
    nil) then
  begin
{$ENDIF}
    Result := False;
...
begin
  ...
  begin
    if aSource.Mute then
      ...
      res := BASS_ChannelSetAttributes(p.channel, aSource.Frequency,
        Round(aSource.Volume * 100), -101);
    assert(res);
  end
  else
    aSource.Free;
...
begin
  ...
  begin
    ...
    if muted then
      ...
      res := BASS_ChannelSetAttributes(p.channel, -1,
        Round(aSource.Volume * 100), -101);
    assert(res);
  end;
end;
