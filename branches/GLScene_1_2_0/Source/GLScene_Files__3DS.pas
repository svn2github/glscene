...
uses
  ...
  GLScene_Files__3DS_Base,
  GLScene_Files__3DS_Types,
  GLScene_Base_Context,
  GLScene_Base_PersistentClasses,
  GLScene_Base_Strings,
  GLScene_Files__3DS_SceneObjects,
  GLScene_Platform,
...
type
  ...
  public
    ...
    procedure Apply(var DataTransf: TGLFile3DSAnimationData;
      const AFrame: real); virtual; abstract;
  ...
  protected
    procedure ApplyAnimKeys(var DataTransf: TGLFile3DSAnimationData;
      const AFrame: real);
  public
  ...
  TGLFile3DSAnimKeysClassType = (ctScale, ctRot, ctPos, ctCol, ctTPos, ctFall,
    ctHot, ctRoll);
  ...
  private
    ...
    FStatic: Boolean;
    // Static tag used in BuildList to not apply animation matrix
  public
    ...
    property RefrenceTransf: TGLFile3DSAnimationData read FRefTranf
      write FRefTranf;
  end;
  ...
  public
    constructor Create; override;
    procedure LoadData(const AOwner: TGLBaseMesh;
      const AData: PLight3DS); virtual;
    procedure LoadAnimation(const AData: Pointer); override;
  ...
  public
    procedure LoadData(const AOwner: TGLBaseMesh;
      const AData: PLight3DS); override;
    procedure LoadAnimation(const AData: Pointer); override;
    procedure SetFrame(const AFrame: real); override;
  end;
...
function AnimKeysClassTypeToClass(const AAnimKeysClassType
  : TGLFile3DSAnimKeysClassType): TClass;
begin
...
function ClassToAnimKeysClassType(const AAnimKeysClass: TClass)
  : TGLFile3DSAnimKeysClassType;
begin
  if AAnimKeysClass.InheritsFrom(TGLFile3DSScaleAnimationKeys) then
    ...
    Result := ctTPos
  else if AAnimKeysClass.InheritsFrom
    (TGLFile3DSSpotLightCutOffAnimationKeys) then
    Result := ctFall
...
function MakeRotationQuaternion(const axis: TAffineVector; angle: single)
  : TQuaternion;
var
  v: TVector;
...
procedure TGLFile3DSAnimationKeys.InterpolateFrame(var I: integer; var w: real;
  const AFrame: real);
begin
  w := 1;
  I := 0;
...
function TGLFile3DSAnimationKeys.InterpolateValue(const AValues
  : array of single; const AFrame: real): single;
var
  I: integer;
  w: real;
  start, stop: single;
...
function TGLFile3DSAnimationKeys.InterpolateValue(const AValues
  : array of TAffineVector; const AFrame: real): TAffineVector;
var
  I: integer;
  w: real;
...
function TGLFile3DSAnimationKeys.InterpolateValue(const AValues
  : array of TKFRotKey3DS; const AFrame: real): TMatrix;
var
  I: integer;
  w: real;
begin
  ...
  begin
    with AValues[I] do
      Result := MatrixMultiply(Result, CreateRotationMatrix(AffineVectorMake(X,
        Y, Z), angle));
    Inc(I);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      AffVect := VectorRotateAroundX(AffVect,
        cGLFILE3DS_FIXDEFAULTUPAXISY_ROTATIONVALUE);
...
procedure TGLFile3DSScaleAnimationKeys.Apply(var DataTransf
  : TGLFile3DSAnimationData; const AFrame: real);
begin
  if FNumKeys > 0 then
...
procedure TGLFile3DSRotationAnimationKeys.Apply(var DataTransf
  : TGLFile3DSAnimationData; const AFrame: real);
begin
  if FNumKeys > 0 then
    DataTransf.ModelMatrix := MatrixMultiply(DataTransf.ModelMatrix,
...
begin
  ...
  begin
    ...
    if vGLFile3DS_FixDefaultUpAxisY then
    begin
      FPos[I] := VectorRotateAroundX(FPos[I],
        cGLFILE3DS_FIXDEFAULTUPAXISY_ROTATIONVALUE);
    end;
  end;
end;

procedure TGLFile3DSPositionAnimationKeys.Apply(var DataTransf
  : TGLFile3DSAnimationData; const AFrame: real);
begin
  if FNumKeys > 0 then
    DataTransf.ModelMatrix[3] := VectorAdd(DataTransf.ModelMatrix[3],
      VectorMake(InterpolateValue(FPos, AFrame)));
end;
...
procedure TGLFile3DSColorAnimationKeys.Apply(var DataTransf
  : TGLFile3DSAnimationData; const AFrame: real);
begin
  if FNumKeys > 0 then
...
begin
  ...
  begin
    ...
    if vGLFile3DS_FixDefaultUpAxisY then
    begin
      FTPos[I] := VectorRotateAroundX(FTPos[I],
        cGLFILE3DS_FIXDEFAULTUPAXISY_ROTATIONVALUE);
    end;
  end;
end;

procedure TTGLFile3DSPositionAnimationKeys.Apply(var DataTransf
  : TGLFile3DSAnimationData; const AFrame: real);
begin
  if FNumKeys > 0 then
...
procedure TTGLFile3DSPositionAnimationKeys.ReadFromFiler
...
procedure TGLFile3DSSpotLightCutOffAnimationKeys.LoadData(const ANumKeys
  : integer; const Keys: PKeyHeaderList; const AData: Pointer);
var
  I: integer;
begin
  inherited;
...
procedure TGLFile3DSSpotLightCutOffAnimationKeys.Apply(var DataTransf
  : TGLFile3DSAnimationData; const AFrame: real);
begin
  if FNumKeys > 0 then
    DataTransf.SpotLightCutOff := DataTransf.SpotLightCutOff +
      InterpolateValue(FFall, AFrame);
end;
...
procedure TGLFile3DSSpotLightCutOffAnimationKeys.WriteToFiler
...
procedure TGLFile3DSSpotLightCutOffAnimationKeys.ReadFromFiler
  (Reader: TVirtualReader);
begin
  inherited;
...
procedure TGLFile3DSLightHotSpotAnimationKeys.Apply(var DataTransf
  : TGLFile3DSAnimationData; const AFrame: real);
begin
  if FNumKeys > 0 then
...
procedure TGLFile3DSLightHotSpotAnimationKeys.WriteToFiler
  (Writer: TVirtualWriter);
begin
  inherited;
...
procedure TGLFile3DSLightHotSpotAnimationKeys.ReadFromFiler
  (Reader: TVirtualReader);
begin
  inherited;
...
procedure TGLFile3DSRollAnimationKeys.Apply(var DataTransf
  : TGLFile3DSAnimationData; const AFrame: real);
begin
  if FNumKeys > 0 then
...
procedure TGLFile3DSAnimationKeyList.AddKeys(const AItem
  : TGLFile3DSAnimationKeys);
var
  ind: integer;
begin
...
procedure TGLFile3DSAnimationKeyList.ApplyAnimKeys(var DataTransf
  : TGLFile3DSAnimationData; const AFrame: real);
var
  I: integer;
begin
...
begin
  ...
  begin
    ClearKeys;
    for I := 0 to Length(TGLFile3DSAnimationKeyList(Source)
      .FAnimKeysList) - 1 do
    begin
      item := (TGLFile3DSAnimationKeyList(Source).FAnimKeysList[I]
        .ClassType.Create as TGLFile3DSAnimationKeys);
...
begin
  ...
  begin
    ...
    FAnimKeysList[I] := AnimKeysClassTypeToClass(Val)
      .Create as TGLFile3DSAnimationKeys;
...
begin
  ...
  if (FParentName <> '') then
  begin
    FParent := Owner.FindMeshByName(string(FParentName))
      as TGLFile3DSDummyObject;
    FParentName := '';
  end;
...
procedure TGLFile3DSDummyObject.Lerp(morphTargetIndex1, morphTargetIndex2
  : integer; lerpFactor: single);
begin
  if (Owner.Owner is TGLActor) and ((Owner.Owner as TGLActor).AnimationMode
    in [aamBounceBackward, aamLoopBackward]) then
...
begin
  ...
  FLightSrc.Diffuse.Color := VectorMake(AData.Color.R, AData.Color.G,
    AData.Color.B);
  FLightSrc.Specular.Color := VectorMake(AData.Color.R, AData.Color.G,
    AData.Color.B);
  FLightSrc.Diffuse.Color := VectorScale(FLightSrc.Diffuse.Color,
    AData.Multiplier);
  // ﳥ鳼
...
procedure TGLFile3DSCameraObject.LoadData(Owner: TGLBaseMesh;
  AData: PCamera3DS);
begin
...
{$IFDEF GLS_REGIONS}{$REGION 'TGL3DSVectorFiles.LoadFromStream Local functions'}{$ENDIF}
  // --------------- local functions -------------------------------------------

  function GetOrAllocateMaterial(materials: TMaterialList;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Shininess :=
              MaxInteger(0, integer(round((Material.Shininess) * 128)));
          ...
          if Trim(string(Material.Texture.Map.NameStr)) <> '' then
            try
              if vGLFile3DS_UseTextureEx then
                ...
                begin
                  Texture.Image.LoadFromFile
                    (string(Material.Texture.Map.NameStr));
                  ...
                  begin
                    ...
                    TextureOffset.SetPoint((1 - frac(UOffset)) * UScale,
                      (frac(VOffset)) * VScale, 0);
                  end;
                end
              else
              ...
              begin
                if not Owner.IgnoreMissingTextures then
                  raise EGLFile3DS.CreateFmt(gls3DSMapNotFound,
                    ['diffuse', E.Message, matLib.TexturePaths]);
              end
              else
                raise;
            end;
            ...
            try
              if vGLFile3DS_UseTextureEx then
                ...
                begin
                  ...
                  Texture.TextureMode := tmModulate;
                  Texture.Image.LoadFromFile
                    (string(Material.Opacity.Map.NameStr));
                  Texture.Disabled := False;
                  TextureIndex := 1;
                ...
                begin
                  ...
                  SecondMaterial.Material.Texture.ImageAlpha :=
              ...
              begin
                if not Owner.IgnoreMissingTextures then
                  raise EGLFile3DS.CreateFmt(gls3DSMapNotFound,
                    ['opacity', E.Message, matLib.TexturePaths]);
              end
              else
                raise;
            end;
            ...
            try
              if vGLFile3DS_UseTextureEx then
                ...
                begin
                  SecondMaterial := matLib.materials.Add;
                  SecondMaterial.Material.Texture.Image.LoadFromFile
                    (string(Material.Bump.Map.NameStr));
                  SecondMaterial.Material.Texture.Disabled := False;
                  SecondMaterial.Material.Texture.ImageAlpha :=
                    tiaAlphaFromIntensity;
                  SecondMaterial.Material.Texture.TextureMode := tmModulate;
              ...
              begin
                if not Owner.IgnoreMissingTextures then
                  raise EGLFile3DS.CreateFmt(gls3DSMapNotFound,
                    ['bump', E.Message, matLib.TexturePaths]);
              end
              else
                raise;
            end;
          ...
          begin
            ...
            try
              ...
              begin
                if not Owner.IgnoreMissingTextures then
                  raise EGLFile3DS.CreateFmt(gls3DSMapNotFound,
                    ['light', E.Message, matLib.TexturePaths]);
              end
              else
                raise;
            end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if boolY then
          Result := MatrixMultiply(Result,
            CreateRotationMatrix(AffineVectorMake(0, 1, 0), -PI));
...
{$ENDIF}
  ...
  begin
    ...
    ReallocMem(SmoothIndices, (CurrentVertexCount + 1) *
      SizeOf(TSmoothIndexEntry));
...
// ---------------------------------------------------------------------------
  ...
  function FindMotionIndex(KeyFramer: TKeyFramer;
    const ObjectName: AnsiString): integer;
  ...
  begin
    ...
    with KeyFramer do
      for I := 0 to MeshMotionCount - 1 do
        if CompareText(string(MeshMotion[I].NameStr), string(ObjectName)
          ) = 0 then
        begin
          Result := I;
          Break;
    ...
    try
      ...
      for I := 0 to Objects.MeshCount - 1 do
        ...
        begin
          ...
          // dummy targets
          for X := KeyFramer.Settings.Seg.SegBegin to KeyFramer.Settings.
            Seg.SegEnd do
...
{$IFDEF RangeCheck3DS} {$R+} {$UNDEF RangeCheck3DS} {$ENDIF}
                    ...
                    begin
                      ...
                      TargetVertex := GetSmoothIndex(CurrentIndex,
                        SmoothingGroup, SmoothIndices);
...
{$IFDEF RangeCheck3DS} {$R+} {$UNDEF RangeCheck3DS} {$ENDIF}
                        ...
                        StoreSmoothIndex(CurrentVertexCount - 1, SmoothingGroup,
                          CurrentVertexCount - 1, SmoothIndices);
                        // mark new vertex also as touched
                        MarkVertex(Marker, CurrentVertexCount - 1);
                      end
...
{$IFDEF RangeCheck3DS} {$R+} {$UNDEF RangeCheck3DS} {$ENDIF}
          ...
          begin
            ...
            begin
              ...
              with aFaceGroup do
              begin
                MaterialName := GetOrAllocateMaterial(materials,
                  string(MatArray[iMaterial].NameStr));
                LightMapIndex := GetOrAllocateLightMap(materials,
                  string(MatArray[iMaterial].NameStr));
          ...
          if hasLightmap then
            for j := 0 to mesh.texCoords.Count - 1 do
              mesh.LightMapTexCoords.Add(mesh.texCoords[j][0],
                mesh.texCoords[j][1]);
        end;

      // Adding non-mesh objects (for example, dummies).
      for I := 0 to KeyFramer.MeshMotionCount - 1 do
        if (Owner.MeshObjects.FindMeshByName(string(KeyFramer.MeshMotion[I]
          .NameStr)) = nil) then
        begin
          mesh := TGLFile3DSMeshObject.CreateOwned(Owner.MeshObjects);
      ...
      for I := 0 to Objects.MeshCount - 1 do
        with PMesh3DS(Objects.mesh[I])^ do
        begin
          mesh := Owner.MeshObjects.FindMeshByName(string(NameStr))
            as TGLFile3DSMeshObject;
        ...
        begin
          ...
          begin
            ...
            for j := 0 to mesh.Vertices.Count - 1 do
              mesh.Vertices[j] := VectorTransform(mesh.Vertices[j],
                mesh.FAnimTransf.ModelMatrix);
          end;
        end;
      end;
