...
function TFFmpegDecodeStream.Open(const Filename: Utf8String): boolean;
var
...
begin
  ...
  begin
    GLSLogger.LogError('Audio-file does not exist: "' + Filename + '"'+ 'UAudio_FFmpeg');
    Exit;
  end;
  ...
  if (av_open_input_file(fFormatCtx, PAnsiChar('ufile:'+FileName), nil, 0, nil) <> 0) then
  begin
    GLSLogger.LogError('av_open_input_file failed: "' + Filename + '"'+ 'UAudio_FFmpeg');
    Exit;
  end;
  ...
  begin
    GLSLogger.LogError('av_find_stream_info failed: "' + Filename + '"'+ 'UAudio_FFmpeg');
    Close();
    Exit;
  end;
  ...
  begin
    GLSLogger.LogError('FindAudioStreamIndex: No Audio-stream found "' + Filename + '"'+ 'UAudio_FFmpeg');
    Close();
    Exit;
  end;
  ...
  *)
  ...
  if (fCodec = nil) then
  begin
    GLSLogger.LogError('Unsupported codec!'+ 'UAudio_FFmpeg');
    fCodecCtx := nil;
    Close();
    Exit;
  end;
  ...
  if (AVResult < 0) then
  begin
    GLSLogger.LogError('avcodec_open failed!'+ 'UAudio_FFmpeg');
    Close();
    Exit;
  end;
  ...
  if fCodecCtx^.channels > 255 then
    GLSLogger.LogInfo('Error: CodecCtx^.channels > 255'+ 'TFFmpegDecodeStream.Open');
      ...
      begin
        ...
        try
          ...
          begin
            // seeking failed
            fErrorState := true;
            GLSLogger.LogError('Seek Error in "'+fFormatCtx^.filename+'"'+ 'UAudioDecoder_FFmpeg');
          end
          else
          begin
            if (fSeekFlush) then
  ...
  begin
    ...
    begin
      ...
      case (fAudioPaket.flags) of
        ...
        begin
          SetError(true);
          GLSLogger.LogInfo('I/O Error'+ 'TFFmpegDecodeStream.DecodeFrame');
          Exit;
        end;
        ...
        begin
          GLSLogger.LogInfo('Unknown status'+ 'TFFmpegDecodeStream.DecodeFrame');
        end;
      end;
...
begin
  GLSLogger.LogInfo('InitializeDecoder'+ ' UAudioDecoder_FFmpeg');
...
function TAudioDecoder_FFmpeg.Open(const Filename: UTF8String): TAudioDecodeStream;
var
  Stream: TFFmpegDecodeStream;
begin
  Result := nil;
