 ...
 * $URL: https://ultrastardx.svn.sourceforge.net/svnroot/ultrastardx/trunk/src/media/UMediaCore_FFmpeg.pas $
 *}
...
begin
  ...
  begin
    GLSLogger.LogError(Format('%s header (%s) and DLL (%s) versions do not match.',
...
begin
  ...
  try
    Stream := TFileStream.Create(FilePath, Mode);
    h.priv_data := Stream;
  except
