 ...
 * $URL: https://ultrastardx.svn.sourceforge.net/svnroot/ultrastardx/trunk/src/media/UAudioDecoder_Bass.pas $
 *}
...
begin
  ...
  begin
    GLSLogger.LogError('BASS_ChannelFlags: ' + BassCore.ErrorGetString()+ ' TBassDecodeStream.GetLoop');
    Result := false;
    Exit;
  end;
...
begin
  ...
  begin
    GLSLogger.LogError('BASS_ChannelFlags: ' + BassCore.ErrorGetString()+ 'TBassDecodeStream.SetLoop');
    Exit;
  end;
end;
...
function TAudioDecoder_Bass.Open(const Filename: UTF8string): TAudioDecodeStream;
var
  Stream: HSTREAM;
  ChannelInfo: BASS_CHANNELINFO;
  FileExt: string;
...
begin
  ...
  Stream := BASS_StreamCreateFile(False, PWideChar(aFilename), 0, 0, BASS_STREAM_DECODE or BASS_UNICODE);
  {$ELSE}
  ...
  if (Stream = 0) then
  begin
    GLSLogger.LogInfo(BassCore.ErrorGetString()+ ' TAudioDecoder_Bass.Open');
