...
//
{
  ...
  - You can choose the shape of your static object
    - csEllipsoid, csBox
    - csFreeform MUST BE A TVKFreeform, otherwise will raise errors
    - csTerrain MUST BE A TVKTerrainRenderer, same condition above
...
type
  ...
  TDCECollisionEvent = procedure (Sender : TObject; object1, object2 : TVKBaseSceneObject;
                                       CollisionInfo: TDCECollision) of object;
  TDCEObjectCollisionEvent = procedure (Sender : TObject; ObjectCollided : TVKBaseSceneObject;
                                       CollisionInfo: TDCECollision) of object;
  ...
  private
    ...
    FDynamics : TList;
    FGravity: Single;
    FWorldDirection: TVKCoordinates; //Used to calculate jumps f.i.
    FWorldScale: Single;
    FMovimentScale: Single;
    FStandardiseLayers : TDCECollisionSelection;
    FManualStep: Boolean;
    FOnCollision : TDCECollisionEvent;
    procedure SetWorldDirection(const Value: TVKCoordinates);
  ...
  protected
    ...
    procedure RegisterDynamic(aClient : TVKDCEDynamic);
  ...
  public
    ...
    function MoveByDistance(var Body: TVKDCEDynamic; deltaS, deltaAbsS: TAffineVector): Single;
  ...
  published
    ...
    property Size : TVKCoordinates read FSize write SetSize;
  end;
...
function GetOrCreateDCEStatic(behaviours : TVKBehaviours) : TVKDCEStatic; overload;
function GetOrCreateDCEStatic(obj : TVKBaseSceneObject) : TVKDCEStatic; overload;

function GetOrCreateDCEDynamic(behaviours : TVKBehaviours) : TVKDCEDynamic; overload;
function GetOrCreateDCEDynamic(obj : TVKBaseSceneObject) : TVKDCEDynamic; overload;

implementation

function RotateVectorByObject(Obj: TVKBaseSceneObject; v: TAffineVector): TAffineVector;
var v2: TVector;
begin
  SetVector(v2,v);
...
constructor TVKDCEManager.Create(AOwner: TComponent);
begin
	inherited Create(AOwner);
  FStatics:=TList.Create;
  FDynamics:=TList.Create;
  FGravity:=0;
  FWorldDirection:=TVKCoordinates.CreateInitialized(Self, YHmgVector, csVector);
  FWorldScale := 1;
  FMovimentScale := 1;
...
// Destroy
//
destructor TVKDCEManager.Destroy;
begin
...
function TVKDCEManager.GetDynamicCount: Integer;
begin
  result := FDynamics.Count;
end;

function TVKDCEManager.GetStaticCount: Integer;
begin
  result := FStatics.Count;
end;

function TVKDCEManager.MoveByDistance(var Body: TVKDCEDynamic; deltaS, deltaAbsS: TAffineVector): Single;
var
    //Friction and bounce
...
begin
  ...
  while deltaCR>0 do begin
    ...
    with TVKDCEStatic(FStatics[i]) do
    begin
      ...
      begin
        case Shape of
          ...
          csTerrain: ECAddTerrain(MP,TVKTerrainRenderer(OwnerBaseSceneObject),FWorldScale*2,Solid,i);
        end;
      end;
    ...
    with TVKDCEDynamic(FDynamics[i]) do
    begin
      CanCollide := False;
      if (Active) and (TVKDCEDynamic(FDynamics[i]) <> Body) then
      case FStandardiseLayers of
        ccsDCEStandard: CanCollide := (Layer <= Body.Layer);
        ccsCollisionStandard: CanCollide := (layer = 0) or (body.layer = 0) or (layer <> body.layer);
        ccsHybrid: CanCollide := ( (layer = 0) or (body.layer = 0) or (Layer <= Body.Layer) ) and (layer>=0) and (body.layer>=0);
  ...
  begin
    ...
    if oi < 0 then
    begin
      tFriction := TVKDCEDynamic(FDynamics[abs(oi) - 1]).Friction;
      tBounceFactor := TVKDCEDynamic(FDynamics[abs(oi) - 1]).BounceFactor;
      tObject := TVKDCEDynamic(FDynamics[abs(oi) - 1]).OwnerBaseSceneObject;
    end else
    begin
      tFriction := TVKDCEStatic(FStatics[oi]).Friction;
      tBounceFactor := TVKDCEStatic(FStatics[oi]).BounceFactor;
      tObject := TVKDCEStatic(FStatics[oi]).OwnerBaseSceneObject;
    end;
    ...
    if (oi >= 0) and Assigned(TVKDCEStatic(FStatics[oi]).FOnCollision) then
      TVKDCEStatic(FStatics[oi]).FOnCollision(Self,Body.OwnerBaseSceneObject,ColInfo);
    end;
  finally
    ContactList.Free;
  end;
...
procedure TVKDCEManager.Step(deltaTime: Double);
var i: Integer;
begin
...
procedure TVKDCEManager.SetWorldDirection(const Value: TVKCoordinates);
begin
  FWorldDirection := Value;
  FWorldDirection.Normalize;
end;

procedure TVKDCEManager.SetWorldScale(const Value: Single);
begin
...
procedure TVKDCEManager.RegisterStatic(aClient : TVKDCEStatic);
begin
...
procedure TVKDCEManager.DeRegisterStatic(aClient : TVKDCEStatic);
begin
...
procedure TVKDCEManager.DeRegisterAllStatics;
var
   i : Integer;
begin
   // Fast deregistration
   for i:=0 to FStatics.Count-1 do
      TVKDCEStatic(FStatics[i]).FManager:=nil;
   FStatics.Clear;
end;

//Register Dynamics

procedure TVKDCEManager.RegisterDynamic(aClient : TVKDCEDynamic);
begin
   if Assigned(aClient) then
...
procedure TVKDCEManager.DeRegisterDynamic(aClient : TVKDCEDynamic);
begin
   if Assigned(aClient) then begin
      aClient.FManager:=nil;
...
procedure TVKDCEManager.DeRegisterAllDynamics;
var
   i : Integer;
begin
   // Fast deregistration
   for i:=0 to FDynamics.Count-1 do
      TVKDCEDynamic(FDynamics[i]).FManager:=nil;
   FDynamics.Clear;
end;

{ TVKDCEStatic }

procedure TVKDCEStatic.Assign(Source: TPersistent);
begin
   if Source is TVKDCEStatic then begin
      Active := TVKDCEStatic(Source).Active;
      Manager:=TVKDCEStatic(Source).Manager;
      Shape := TVKDCEStatic(Source).Shape;
      Layer := TVKDCEStatic(Source).Layer;
      Solid := TVKDCEStatic(Source).Solid;
      Size.Assign(TVKDCEStatic(Source).Size);
      Friction := TVKDCEStatic(Source).Friction;
      BounceFactor := TVKDCEStatic(Source).BounceFactor;
   end;
...
begin
   inherited Create(aOwner);
   FActive := True;
   FSize:=TVKCoordinates.CreateInitialized(Self, XYZHmgVector, csVector);
   FShape := csEllipsoid;
   FSolid := True;
   FFriction := 1;
   FBounceFactor := 0;
...
destructor TVKDCEStatic.Destroy;
begin
   Manager:=nil;
   FSize.Free;
   inherited Destroy;
...
class function TVKDCEStatic.FriendlyDescription: String;
begin
   Result:='Static Collision-detection registration';
end;

class function TVKDCEStatic.FriendlyName: String;
begin
   Result:='DCE Static Collider';
end;

procedure TVKDCEStatic.Loaded;
var
   mng : TComponent;
begin
   inherited;
   if FManagerName<>'' then begin
      mng:=FindManager(TVKDCEManager, FManagerName);
      if Assigned(mng) then
         Manager:=TVKDCEManager(mng);
      FManagerName:='';
   end;
end;

procedure TVKDCEStatic.WriteToFiler(writer: TWriter);
begin
...
procedure TVKDCEStatic.ReadFromFiler(reader: TReader);
var
...
procedure TVKDCEStatic.SetBounceFactor(const Value: Single);
begin
  FBounceFactor := Value;
...
procedure TVKDCEStatic.SetFriction(const Value: Single);
begin
  FFriction := Value;
...
procedure TVKDCEStatic.SetManager(const val: TVKDCEManager);
begin
...
procedure TVKDCEStatic.SetShape(const Value: TDCEShape);
begin
  FShape := Value;
end;

procedure TVKDCEStatic.SetSize(const Value: TVKCoordinates);
begin
  FSize.Assign(Value);
...
procedure TVKDCEDynamic.ApplyAccel(NewAccel: TAffineVector);
begin
  AddVector(FAccel, NewAccel);
end;

procedure TVKDCEDynamic.ApplyAccel(x,y,z: Single);
begin
  AddVector(FAccel, AffineVectorMake(x,y,z));
end;

procedure TVKDCEDynamic.ApplyAbsAccel(NewAccel: TAffineVector);
begin
  AddVector(FAbsAccel, NewAccel);
end;

procedure TVKDCEDynamic.ApplyAbsAccel(x,y,z: Single);
begin
  AddVector(FAbsAccel, AffineVectorMake(x,y,z));
end;

procedure TVKDCEDynamic.StopAccel;
begin
  SetVector(FAccel, NullVector);
end;

procedure TVKDCEDynamic.StopAbsAccel;
begin
  SetVector(FAbsAccel, NullVector);
end;

procedure TVKDCEDynamic.Assign(Source: TPersistent);
begin
  if Source is TVKDCEDynamic then begin
    Manager:=TVKDCEDynamic(Source).Manager;
    Active := TVKDCEDynamic(Source).Active;
    UseGravity := TVKDCEDynamic(Source).UseGravity;
    Layer := TVKDCEDynamic(Source).Layer;
    Solid := TVKDCEDynamic(Source).Solid;
    Size.Assign(TVKDCEDynamic(Source).Size);
    Friction := TVKDCEDynamic(Source).Friction;
    BounceFactor := TVKDCEDynamic(Source).BounceFactor;
    SlideOrBounce := TVKDCEDynamic(Source).SlideOrBounce;
    MaxRecursionDepth := TVKDCEDynamic(Source).MaxRecursionDepth;
  end;
  inherited Assign(Source);
end;
...
destructor TVKDCEDynamic.Destroy;
begin
  Manager:=nil;
  FSize.Free;
  inherited Destroy;
...
procedure TVKDCEDynamic.DoMove(deltaTime: Double);
...
procedure TVKDCEDynamic.DoProgress(const progressTime: TProgressTimes);
begin
...
class function TVKDCEDynamic.FriendlyDescription: String;
begin
  Result:='Dynamic Collision-detection registration';
end;

class function TVKDCEDynamic.FriendlyName: String;
begin
  Result:='DCE Dynamic Collider';
end;

procedure TVKDCEDynamic.Jump(jHeight, jSpeed: Single);
begin
...
procedure TVKDCEDynamic.Loaded;
var
   mng : TComponent;
begin
   inherited;
...
procedure TVKDCEDynamic.Move(deltaS: TAffineVector; deltaTime: Double);
begin
  ScaleVector(deltaS, deltaTime);
  FManager.MoveByDistance(Self, NullVector, deltaS);
end;

procedure TVKDCEDynamic.MoveTo(Position: TAffineVector; Amount: Single);
begin
...
procedure TVKDCEDynamic.WriteToFiler(writer: TWriter);
begin
   with writer do begin
      // ArchiveVersion 1, added inherited call
      WriteInteger(1);
...
procedure TVKDCEDynamic.ReadFromFiler(reader: TReader);
var
   archiveVersion : Integer;
begin
   with reader do begin
...
procedure TVKDCEDynamic.SetBounceFactor(const Value: Single);
begin
  FBounceFactor := Value;
  if FBounceFactor < 0 then FBounceFactor := 0;
  if FBounceFactor > 1 then FBounceFactor := 1;
...
procedure TVKDCEDynamic.SetFriction(const Value: Single);
begin
  FFriction := Value;
  if FFriction < 0 then FFriction := 0;
  if FFriction > 100 then FFriction := 100;
...
procedure TVKDCEDynamic.SetManager(const val: TVKDCEManager);
begin
   if val<>FManager then begin
      if Assigned(FManager) then
...
procedure TVKDCEDynamic.SetSize(const Value: TVKCoordinates);
begin
  FSize.Assign(Value);
  if FSize.X <= 0 then FSize.X := 0.1;
  if FSize.Y <= 0 then FSize.Y := 0.1;
...
function GetOrCreateDCEStatic(behaviours : TVKBehaviours) : TVKDCEStatic;
var
	i : Integer;
begin
	i:=behaviours.IndexOfClass(TVKDCEStatic);
	if i>=0 then
		Result:=TVKDCEStatic(behaviours[i])
	else Result:=TVKDCEStatic.Create(behaviours);
end;

function GetOrCreateDCEStatic(obj : TVKBaseSceneObject) : TVKDCEStatic;
begin
	Result:=GetOrCreateDCEStatic(obj.Behaviours);
end;

function GetOrCreateDCEDynamic(behaviours : TVKBehaviours) : TVKDCEDynamic;
var
	i : Integer;
begin
	i:=behaviours.IndexOfClass(TVKDCEDynamic);
	if i>=0 then
		Result:=TVKDCEDynamic(behaviours[i])
	else Result:=TVKDCEDynamic.Create(behaviours);
end;

function GetOrCreateDCEDynamic(obj : TVKBaseSceneObject) : TVKDCEDynamic;
begin
...
// ------------------------------------------------------------------
	...
	// class registrations
	RegisterXCollectionItemClass(TVKDCEStatic);
	RegisterXCollectionItemClass(TVKDCEDynamic);

finalization

	UnregisterXCollectionItemClass(TVKDCEStatic);
	UnregisterXCollectionItemClass(TVKDCEDynamic);
