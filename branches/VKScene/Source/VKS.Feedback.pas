...
uses
  ...
  VKS.VectorGeometry, VKS.VectorLists, VKS.Scene, VKS.VectorFileObjects,
  VKS.Texture, VKS.RenderContextInfo, VKS.Context, VKS.State, Winapi.OpenGL, Winapi.OpenGLext, 
  VKS.MeshUtils, VKS.VectorTypes;
...
constructor TVKFeedback.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TVKFeedback.Destroy;
begin
  FBuffer.Free;
...
procedure TVKFeedback.BuildMeshFromBuffer(
  Vertices: TAffineVectorList = nil;
  Normals: TAffineVectorList = nil;
  Colors: TVectorList = nil;
  TexCoords: TAffineVectorList = nil;
...
procedure TVKFeedback.SetMaxBufferSize(const Value: Cardinal);
begin
...
procedure TVKFeedback.SetMode(const Value: TFeedbackMode);
begin
  if Value <> FMode then
  begin
    FMode := Value;
