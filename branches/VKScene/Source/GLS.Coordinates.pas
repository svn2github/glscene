...
uses
  ...
  //GLS
  GLS.VectorGeometry, GLS.VectorTypes, GLS.BaseClasses,
  GLS.CrossPlatform;
...
begin
  Result := GLS.VectorGeometry.MaxXYZComponent(FCoords);
end;
...
begin
  ...
  GLS.VectorGeometry.MakePoint(FCoords, X, Y, Z);
  NotifyChange(Self);
end;
...
begin
  Assert(FStyle = CsPoint, CsPointHelp);
  GLS.VectorGeometry.MakePoint(FCoords, V);
  NotifyChange(Self);
end;
...
begin
  Assert(FStyle = CsPoint2D, CsPoint2DHelp);
  GLS.VectorGeometry.MakeVector(FCoords, Vector);
  NotifyChange(Self);
end;
...
begin
  Assert(FStyle = CsPoint2D, CsPoint2DHelp);
  GLS.VectorGeometry.MakeVector(FCoords, Vector.X, Vector.Y, 0);
  NotifyChange(Self);
end;
