...
begin
  PositionSceneObject(TVKBaseSceneObject(PdxGeom(FGeom.data)), FGeom);
  RagDoll.Owner.Skeleton.BoneByID(BoneID).SetGlobalMatrixForRagDoll(TVKBaseSceneObject(PdxGeom(FGeom.data)).AbsoluteMatrix);
end;
...
constructor TODERagdoll.Create(AOwner : TVKBaseMesh);
begin
