...
{: VKS.VectorFileObjects<p>
 ...
 <b>History :</b><font size=-1><ul>
      ...
      <li>30/06/11 - DaStr - TVKBaseMesh.BarycenterAbsolutePosition() now uses caching
      ...
      <li>11/06/10 - Yar - Bugfixed binary reading TVKMeshObject for FPC
                           Replace OpenGL1x functions to OpenGLAdapter.
                           Fixes for Linux x64
      <li>22/04/10 - Yar - Fixes after VKS.State revision
      <li>11/04/10 - Yar - Replaced function InsideList to VKS.State.InsideList
      <li>05/03/10 - DanB - More state added to TVKStateCache
      <li>25/12/09 - DaStr - Separated TVKActor.DoAnimate() from TVKActor.BuildList()
      ...
      <li>01/05/08 - DaStr - Implemented TVKBaseMesh.BarycenterAbsolutePosition()
                             Bugfixed TVKBaseMesh.AxisAlignedDimensionsUnscaled()
      ...
      <li>21/02/07 - DaStr - Added TMeshObjectList.BuildTangentSpace, UseVBO
                             Added TVKActor.SetCurrentFrameDirect
      <li>19/02/07 - LC - Added some VBO support
      <li>19/10/06 - LC - Fixed bug in TVKActor.SetCurrentFrame. Bugtracker ID=1580511
      <li>04/10/06 - PhP - fixed TVKActor.SetCurrentFrame (thanks dikoe)
      <li>05/12/05 - PhP - fixed TFGIndexTexCoordList.BuildList (thanks fig)
      <li>10/11/05 - Mathx - Added LastLoadedFilename to TVKBaseMesh (RFE 955083).
      <li>09/11/05 - Mathx - Added isSwitchingAnimation to TVKActor.
      ...
      <li>11/01/05 - SG - Another fix for TVKBaseMesh.Assign (dikoe Kenguru)
      <li>11/01/05 - SG - Fix for TVKBaseMesh.Assign when assigning actors
      <li>26/11/04 - MRQZZZ - by Uwe Raabe : fixed TBaseMeshObject.BuildNormals
      <li>26/11/04 - MRQZZZ - Added "Rendered" property to TVKBaseMesh in order
      ...
      <li>03/10/04 - MRQZZZ - Fixed memory leak (FAutoScaling.Free) in TVKBaseMesh.Destroy; (thanks Jan Zizka)
      ...
      <li>07/05/03 - SG - Added TVKSMDVectorFile.SaveToFile method and [read,write] capabilities
      <li>17/04/03 - SG - Added TMeshObjectList.FindMeshByName method
      <li>01/04/03 - SG - Fixed TVKBaseMesh.Assign
      ...
      <li>17/10/02 - EG - TVKSTLVectorFile moved to new GLFileSTL unit
      <li>04/09/02 - EG - Fixed TVKBaseMesh.AxisAlignedDimensions
      <li>23/08/02 - EG - Added TVKBaseMesh.Visible
      <li>23/07/02 - EG - TVKBaseMesh.LoadFromStream fix (D. Angilella)
      <li>13/07/02 - EG - AutoCenter on barycenter
      <li>22/03/02 - EG - TVKAnimationControler basics now functional
      ...
      <li>17/12/01 - EG - Upgraded TVKActor.Synchronize (smooth transitions support)
      ...
      <li>12/08/01 - EG - Completely rewritten handles management,
                          ...
                          Fixed persistence
      <li>08/08/01 - EG - Added TVKBaseMesh.AxisAlignedDimensions
      <li>19/07/01 - EG - AutoCentering is now a property of TVKBaseMesh,
      ...
      <li>25/03/01 - EG - Added TVKAnimationControler
      ...
      <li>08/03/01 - EG - TVK3DSVectorFile now loads materials for TVKBaseMesh
      ...
      <li>10/01/01 - EG - Fixed in TVKBaseMesh.DoRender for RenderChildren states
      <li>08/01/01 - EG - Fixed TVKBaseMesh.BuildList messup of attrib states
      <li>22/12/00 - EG - Fixed non-interpolated TVKActor animation (was freezing),
                          Fixed TVKBaseMesh.DoRender messup of attrib states
      <li>18/12/00 - EG - TFGIndexTexCoordList now supports normals (automatically),
                          NormalsOrientation code moved to TVKBaseMesh
      <li>11/12/00 - EG - Fix for NormalOrientation (3DS importer)
      <li>06/12/00 - EG - Added PrepareBuildList mechanism
      <li>08/10/00 - EG - Removed TVKOBJVectorFile, use GLFileOBJ instead
      ...
      <li>21/07/00 - EG - Vastly improved memory use and mechanisms for MD2/TVKActor
      ...
      <li>15/07/00 - EG - FreeForm can now handle 3DS files with multiple textures,
                          Added TVKBaseMesh.GetExtents
      <li>28/06/00 - EG - Support for "ObjectStyle"
      <li>23/06/00 - EG - Reversed "t" texture coord for MD2,
                          TActorAnimations can now load/save
      <li>21/06/00 - EG - Added frame change events to TVKActor,
                          Added TActorAnimations collection
      <li>19/06/00 - EG - Completed smooth movement interpolation for TVKActor
      <li>07/06/00 - EG - TVectorFile now longers assumes a TVKFreeForm as Owner,
                          Added generic TVectorFile.LoadFromFile
      <li>26/05/00 - EG - Removed dependency to VKS.Objects,
                          TVKFreeForm now may use InterleavedArrays instead of
                          IndexedArrays (better BuildList compatibility)
      <li>22/04/00 - EG - Fixed Material handlings in TVKFreeForm, inverted CCW/CW
                          convention for 3DS Release3
      <li>11/04/00 - EG - Removed unnecessary code in finalization (thanks Uwe)
      <li>09/02/00 - EG - Creation from split of VKS.Objects,
...
uses
  ...
  VKS.Scene, VKS.OpenGLTokens, VKS.VectorGeometry,  VKS.Texture,
  VKS.Material, VKS.Mesh, VKS.VectorLists, VKS.PersistentClasses, VKS.Octree, VKS.GeometryBB,
  VKS.ApplicationFileIO, VKS.Silhouette, VKS.Context, VKS.Color, VKS.RenderContextInfo,
  VKS.Coordinates, VKS.BaseClasses, VKS.TextureFormat;
...
type
  ...
  TVKGLSMVectorFile = class(TVectorFile)
  public
    { Public Declarations }
...
begin
  ...
  begin
    if Owner.Owner is TSkeleton then
      if TSkeleton(Owner.Owner).Owner is TVKBaseSceneObject then
        mat := MatrixMultiply(FBone.GlobalMatrix,
          TVKBaseSceneObject(TSkeleton(Owner.Owner).Owner).AbsoluteMatrix)
      else
...
constructor TSkeleton.CreateOwned(AOwner: TVKBaseMesh);
begin
  FOwner := aOwner;
  Create;
end;
...
procedure TMeshObject.PrepareMaterialLibraryCache(matLib: TVKMaterialLibrary);
begin
...
constructor TMeshObjectList.CreateOwned(aOwner: TVKBaseMesh);
begin
  FOwner := AOwner;
  Create;
end;
...
procedure TFaceGroup.AttachLightmap(lightMap: TVKTexture; var mrci:
  TRenderContextInfo);
begin
  if GL.ARB_multitexture then
    with lightMap do
...
procedure TFaceGroup.PrepareMaterialLibraryCache(matLib: TVKMaterialLibrary);
begin
...
procedure TFaceGroups.PrepareMaterialLibraryCache(matLib: TVKMaterialLibrary);
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
...
function TFaceGroups.MaterialLibrary: TVKMaterialLibrary;
var
  mol: TMeshObjectList;
  bm: TVKBaseMesh;
begin
...
function TVectorFile.Owner: TVKBaseMesh;
begin
...
class function TVKGLSMVectorFile.Capabilities: TDataFileCapabilities;
begin
...
procedure TVKBaseMesh.PrepareVectorFile(aFile: TVectorFile);
begin
...
procedure TVKBaseMesh.PrepareBuildList(var mrci: TRenderContextInfo);
begin
...
procedure TVKBaseMesh.BuildList(var rci: TRenderContextInfo);
begin
...
procedure TVKBaseMesh.DoRender(var rci: TRenderContextInfo;
...
function TActorAnimation.OwnerActor: TVKActor;
begin
  Result := ((Collection as TActorAnimations).GetOwner as TVKActor);
end;
...
constructor TActorAnimations.Create(AOwner: TVKActor);
begin
  FOwner := AOwner;
...
procedure TVKActor.SetReference(val: TActorAnimationReference);
begin
...
procedure TVKActor.SetAnimations(const val: TActorAnimations);
begin
...
procedure TVKActor.BuildList(var rci: TRenderContextInfo);
begin
  DoAnimate;
  inherited;
...
procedure TVKActor.PrepareBuildList(var mrci: TRenderContextInfo);
begin
...
procedure TVKActor.SwitchToAnimation(anAnimation: TActorAnimation; smooth:
  Boolean = False);
begin
...
initialization
  ...
  RegisterClasses([TVKFreeForm, TVKActor, TSkeleton, TSkeletonFrame,
    TSkeletonBone,
