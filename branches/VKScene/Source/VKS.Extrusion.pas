...
type
  ...
  published
    ...
    property Parts: TRevolutionSolidParts read FParts write SetParts
      default [rspOutside];
    property StartAngle: Single read FStartAngle write SetStartAngle;
    property StopAngle: Single read FStopAngle write SetStopAngle
      stored StoreStopAngle;
    ...
    property YOffsetPerTurn: Single read FYOffsetPerTurn
      write SetYOffsetPerTurn;
  ...
  public
    ...
    procedure StructureChanged; override;
  published
    property Parts: TExtrusionSolidParts read FParts write SetParts
      default [espOutside];
    ...
    property NormalDirection: TNormalDirection read FNormalDirection
      write SetNormalDirection default ndOutside;
    property MinSmoothAngle: Single read FMinSmoothAngle
      write SetMinSmoothAngle;
  end;
  ...
  published
    property RadiusFactor: Single read FRadiusFactor write SetRadiusFactor
      stored StoreRadiusFactor;
    property Color: TVKColor read FColor write SetColor;
    property TexCoordT: Single read FTexCoordT write FTexCoordT
      stored StoreTexCoordT;
  end;
  ...
  public
    ...
    property Items[index: Integer]: TVKPipeNode read GetItems
  ...
  published
    ...
    property NodesColorMode: TPipeNodesColorMode read FNodesColorMode
      write SetNodesColorMode default pncmNone;
    ...
    property NormalMode: TPipeNormalMode read FNormalMode write SetNormalMode
      default pnmDefault;
    property NormalSmoothAngle: Single read FNormalSmoothAngle
      write SetNormalSmoothAngle;
  end;
...
var
  ...
  procedure CalcNormal(const ptTop, ptBottom: PAffineVector;
    var normal: TAffineVector);
  var
    tb: TAffineVector;
    mx, mz: Single;
  begin
  ...
  var
    i: Integer;
    topBase, topNext, bottomBase, bottomNext, normal, topNormal,
      bottomNormal: TAffineVector;
...
const
  ...
  procedure CalculateRow(row: PRowData; const center, normal: TAffineVector;
    Radius: Single);
  var
    i: Integer;
    vx, vy: TAffineVector;
  ...
  procedure RenderDisk(row: PRowData; const center: TVector;
    const normal: TAffineVector; invert: Boolean; TextCoordTileS: Single);
  var
    i: Integer;
  begin
    begin
                ...
                begin
                  ...
                  begin
                    curRow.node[j].normal :=
                      VectorCrossProduct
                      (VectorNormalize
                      (VectorCrossProduct(VectorNormalize(VectorSubtract
                      (curRow.node[j].pos, curRow.center)),
                      curRow.node[j].sidedir)), curRow.node[j].sidedir);
                    prevRow.node[j].normal :=
                      VectorCrossProduct
                      (VectorNormalize
                      (VectorCrossProduct(VectorNormalize(VectorSubtract
                      (prevRow.node[j].pos, prevRow.center)),
                      curRow.node[j].sidedir)), curRow.node[j].sidedir);
                  end
                  else
                  begin
  ...
  procedure RenderSides(prevRow, curRow: PRowData;
    TextCoordTileS, TextCoordTileT: Single; outside: Boolean);
  var
    j: Integer;
  begin
    begin
      ...
      begin
        ...
        begin
          ...
          if NodesColorMode <> pncmNone then
            glColor4fv(@curRow^.Color);
          glTexCoord2f((j + 1) / Slices * TextCoordTileS,
      ...
      begin
        ...
        begin
          if NodesColorMode <> pncmNone then
            glColor4fv(@curRow^.Color);
          glTexCoord2f(j / Slices * TextCoordTileS,
            curRow^.textcoordT * TextCoordTileT);
          ...
          if NodesColorMode <> pncmNone then
            glColor4fv(@prevRow^.Color);
          glTexCoord2f((j + 1) / Slices * TextCoordTileS,
            prevRow^.textcoordT * TextCoordTileT);
...
begin
  ...
  case TexCoordMode of
    ptcmDefault:
      rows[0].textcoordT := 0;
    ptcmManual:
      rows[0].textcoordT := TVKPipeNode(Nodes[0]).TexCoordT;
  end;
        ...
        begin
          ...
          if k < Nodes.Count - 1 then
            rows[curRow].Color := VectorLerp(TVKPipeNodes(Nodes)[k].Color.Color,
              TVKPipeNodes(Nodes)[k + 1].Color.Color, Frac(T))
          else
            ...
            begin
              ...
              if k < Nodes.Count - 1 then
                rows[curRow].textcoordT := Lerp(TVKPipeNode(Nodes[k]).TexCoordT,
                  TVKPipeNode(Nodes[k + 1]).TexCoordT, Frac(T))
              else
    ...
    begin
      ...
      begin
        ...
        case TexCoordMode of
          ...
          ptcmManual:
            rows[curRow].textcoordT := TVKPipeNode(Nodes[i]).TexCoordT;
        end;
...
var
  ...
  procedure CalcNormal(const Top, Bottom: TAffineVector;
    var normal: TAffineVector);
  ...
  var
    step: Integer;
    topBase, topNext, bottomBase, bottomNext, normal, normTop,
      normBottom: TAffineVector;
    topTPBase, topTPNext, bottomTPBase, bottomTPNext: TTexPoint;
...
begin
  ...
  begin
    ...
    FAxisAlignedDimensionsCache.Z := MaxFloat(Abs(dMin.Z),
      Abs(dMax.Z + Height));
  end;
  SetVector(Result, FAxisAlignedDimensionsCache);
end;
