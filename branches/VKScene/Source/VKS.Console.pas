...
{: VKS.Console <p>
   ...
   What is different compared to the original component?
     ...
     3) TVKConsoleCommand.UnknownCommand added
...
uses
  ...
  VKS.Scene, VKS.Objects, VKS.HUDObjects, VKS.SceneViewer, VKS.BitmapFont,
  VKS.PersistentClasses, VKS.Context, VKS.Texture, VKS.Utils, VKS.Strings,
  VKS.CrossPlatform, VKS.Material, VKS.VectorTypes;
...
type
  ...
  TVKConsoleOption = (coAutoCompleteCommandsOnKeyPress,
  ...
  TVKConsoleOptions = set of TVKConsoleOption;

  TVKCustomConsole = class;
  ...
  TVKConsoleMatchList = set of 0..CONSOLE_MAX_COMMANDS {Byte};

  { A class that checks for duplicates. }
  TVKConsoleStringList = class(TStringList)
  private
    FConsole: TVKCustomConsole;
  protected
    procedure Changed; override;
    function GetOwner: TPersistent; override;
  public
    function CommandExists(const Command: string): Boolean;
    constructor Create(const Owner: TVKCustomConsole);
  end;

  { A wrapper for a console command. }
  TVKConsoleCommand = class(TCollectionItem)
  private
    FVisible: Boolean;
    FEnabled: Boolean;
    FSilentDisabled: Boolean;
    FCommandList: TVKConsoleCommandList;
    FCommandName: string;
    FShortHelp: string;
    FLongHelp: TStringList;
    FOnCommand: TVKlConsoleEvent;
  ...
  protected
    ...
    procedure ShowInvalidNumberOfArgumentsError(const ShowHelpAfter: Boolean =
      True); virtual;
    procedure DoOnCommand(var UserInputCommand: TVKUserInputCommand); virtual;
  ...
  TVKConsoleCommandList = class(TCollection)
  private
    FConsole: TVKCustomConsole;
    function GetItems(const Index: Integer): TVKConsoleCommand;
  protected
    function GetOwner: TPersistent; override;
  public
    ...
    // General list stuff.
    function LastConsoleCommand: TVKConsoleCommand;
    function Add: TVKConsoleCommand; overload;

    // Standard stuff.
    constructor Create(const AOwner: TVKCustomConsole);
    destructor Destroy; override;

    property Items[const Index: Integer]: TVKConsoleCommand read GetItems;
      default;
  end;

  TVKConsoleControls = class(TPersistent)
  private
    FOwner: TPersistent;
  ...
  private
    ...
    FCommands: TVKConsoleCommandList;
    FAdditionalCommands: TVKConsoleStringList;
    FTypedCommands: TStringList;
    FControls: TVKConsoleControls;

    FOnCommandIssued: TVKlConsoleEvent;

    FOptions: TVKConsoleOptions;
    FHint: string;

    procedure SetSize(const Value: Single);
    procedure SetSceneViewer(const Value: TVKSceneViewer);
  ...
  protected
    ...
    procedure ProcessInternalCommandConsoleClearTypedCommands(const
      ConsoleCommand: TVKConsoleCommand; const Console: TVKCustomConsole; var
      Command: TVKUserInputCommand); virtual;
  ...
  public
    ...
    property Font: TVKCustomBitmapFont read GetFont write SetFont stored False;

    property Options: TVKConsoleOptions read FOptions write FOptions;
  ...
  TVKConsole = class(TVKCustomConsole)
  published
    property FontColor;
...
procedure TVKCustomConsole.ProcessInternalCommandClearScreen(const
  ConsoleCommand: TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
begin
  Console.FInputLine := '';
...
procedure TVKCustomConsole.ProcessInternalCommandConsoleHide(const
  ConsoleCommand: TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
begin
...
procedure TVKCustomConsole.ProcessInternalCommandConsoleColor(const
  ConsoleCommand: TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
var
  NewColor: TColor;
...
procedure TVKCustomConsole.ProcessInternalCommandConsoleRename(const
  ConsoleCommand: TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
var
...
procedure TVKCustomConsole.ProcessInternalCommandConsoleClearTypedCommands(const
  ConsoleCommand: TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
begin
  if (Command.CommandCount = 1) then
...
procedure TVKCustomConsole.ProcessInternalCommandSystemDate(const
  ConsoleCommand: TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
begin
  if (Command.CommandCount = 1) then
...
procedure TVKCustomConsole.ProcessInternalCommandHelp(const ConsoleCommand:
  TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
var
  MainCommand: string;
...
procedure TVKCustomConsole.ProcessInternalCommandSystemTime(const
  ConsoleCommand: TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
begin
  if Command.CommandCount = 1 then
...
procedure TVKCustomConsole.GetHelpInternalCommandRename(Sender: TObject);
begin
  with TVKConsoleCommand(Sender) do
  begin
...
procedure TVKCustomConsole.ProcessInternalCommandViewerFPS(const ConsoleCommand:
  TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
begin
  if Command.CommandCount = 1 then
...
procedure
  TVKCustomConsole.ProcessInternalCommandViewerResetPerformanceMonitor(const
  ConsoleCommand: TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
begin
  if Command.CommandCount = 1 then
...
procedure TVKCustomConsole.ProcessInternalCommandViewerVSync(const
  ConsoleCommand: TVKConsoleCommand; const Console: TVKCustomConsole; var Command:
  TVKUserInputCommand);
const
...
procedure TVKCustomConsole.ProcessInternalCommandViewerAntiAliasing(
  const ConsoleCommand: TVKConsoleCommand;
  const Console: TVKCustomConsole; var Command: TVKUserInputCommand);
var
  Temp: Integer;
begin
  if Console.FSceneViewer <> nil then
  begin
    if Command.CommandCount = 1 then
      AddLine(' - Current SceneViewer AntiAliasing = ' +
        GetEnumName(TypeInfo(TVKAntiAliasing),
        Ord(Console.FSceneViewer.Buffer.AntiAliasing)))
    else if (Command.CommandCount = 2) then
    begin
      Temp := GetEnumValue(TypeInfo(TVKAntiAliasing), Command.Strings[1]);
      if Temp = -1 then
      begin
        AddLine(' - ' + STR_UNRECOGNIZED_PARAMETER + Command.Strings[1]);
      end
      ...
      begin
        Console.FSceneViewer.Buffer.AntiAliasing := TVKAntiAliasing(Temp);
...
procedure TVKCustomConsole.FixCommand(var UserInputCommand:
  TVKUserInputCommand);
var
  nCount, I: Integer;
  openq: Boolean;
...
constructor TVKCustomConsole.Create(AOwner: TComponent);
begin
  inherited;
  FColsoleLog := TStringList.Create;
  FTypedCommands := TStringList.Create;
  FCommands := TVKConsoleCommandList.Create(Self);
  FAdditionalCommands := TVKConsoleStringList.Create(Self);
  FControls := TVKConsoleControls.Create(Self);

  FHudSprite := TVKHudSprite.Create(Self);
  ...
  FHudText := TVKHudText.Create(Self);
...
destructor TVKCustomConsole.Destroy;
begin
  Controls.Destroy;
  FCommands.Destroy;
...
procedure TVKCustomConsole.ProcessKeyPress(const c: Char);
begin
  if not Visible then
    Exit;
...
procedure TVKCustomConsole.ProcessKeyDown(const key: word);
var
  MatchCount: Integer;
  AdditionalCommandsMatchList: TVKConsoleMatchList;
  CommandsMatchList: TVKConsoleMatchList;
...
procedure TVKCustomConsole.RefreshHud;
var
  outStr: string;
  endLine, I: Integer;
begin
...
function TVKCustomConsole.NumLines: Integer;
begin
  if GetFont = nil then
...
procedure TVKCustomConsole.ProcessInput;
var
  info: TVKUserInputCommand;
begin
...
procedure TVKCustomConsole.ExecuteCommands(const Commands: TStrings);
var
  I: Integer;
begin
...
procedure TVKCustomConsole.ExecuteCommand(const Command: string);
begin
  FInputLine := Command;
  ProcessInput;
end;

procedure TVKCustomConsole.AddLine(const str: string);
begin
...
procedure TVKCustomConsole.CommandIssued(var UserInputCommand:
  TVKUserInputCommand);
var
  MainCommand: string;
  I: Integer;
...
procedure TVKCustomConsole.RefreshHudSize;
begin
...
procedure TVKCustomConsole.SetFontColor(const Color: TColor);
begin
  FHUDText.ModulateColor.AsWinColor := Color;
  FHUDText.Material.FrontProperties.Ambient.AsWinColor := Color;
end;

procedure TVKCustomConsole.ShowConsoleHelp;
var
  I: Integer;
...
procedure TVKCustomConsole.ClearTypedCommands;
begin
...
procedure TVKCustomConsole.AutoCompleteCommand(var MatchCount: Integer;
  var AdditionalCommandsMatchList: TVKConsoleMatchList;
  var CommandsMatchList: TVKConsoleMatchList);
var
  I: Integer;
  HasEnterKey: Boolean;
...
procedure TVKCustomConsole.AutoCompleteCommand;
var
  MatchCount: Integer;
  AdditionalCommandsMatchList: TVKConsoleMatchList;
  CommandsMatchList: TVKConsoleMatchList;
...
procedure TVKCustomConsole.RegisterBuiltInCommands;
begin
  { Special commands }
...
procedure TVKCustomConsole.HandleUnknownCommand(const Command: string);
begin
...
procedure TVKCustomConsole.NavigateDown;
begin
  Inc(FStartLine);
  if FStartLine > FColsoleLog.Count - numlines then
    FStartLine := FColsoleLog.Count - numlines;
...
procedure TVKCustomConsole.NavigatePageDown;
begin
  Inc(FStartLine, NumLines);
  if FStartLine > FColsoleLog.Count - numlines then
    FStartLine := FColsoleLog.Count - numlines;
...
procedure TVKCustomConsole.NavigatePageUp;
begin
  Dec(FStartLine, NumLines);
  if FStartLine > FColsoleLog.Count - numlines then
    FStartLine := FColsoleLog.Count - numlines;
...
procedure TVKCustomConsole.NavigateUp;
begin
  Dec(FStartLine);
  if FStartLine > FColsoleLog.Count - numlines then
    FStartLine := FColsoleLog.Count - numlines;
...
function TVKCustomConsole.GetFontColor: TColor;
begin
  Result := FHUDText.ModulateColor.AsWinColor;
end;

function TVKCustomConsole.GetHUDSpriteColor: TColor;
begin
  if Assigned(HUDSprite.Material.MaterialLibrary)
    and (HUDSprite.Material.MaterialLibrary is TVKMaterialLibrary)
    and (HUDSprite.Material.LibMaterialName <> '') then
    Result :=
      TVKMaterialLibrary(HUDSprite.Material.MaterialLibrary).LibMaterialByName(HUDSprite.Material.LibMaterialName).Material.FrontProperties.Ambient.AsWinColor
  else
    Result := HUDSprite.Material.FrontProperties.Ambient.AsWinColor;
end;

procedure TVKCustomConsole.SetHUDSpriteColor(const Color: TColor);
begin
  if Assigned(HUDSprite.Material.MaterialLibrary)
    and (HUDSprite.Material.MaterialLibrary is TVKMaterialLibrary)
    and (HUDSprite.Material.LibMaterialName <> '') then
      TVKMaterialLibrary(HUDSprite.Material.MaterialLibrary).LibMaterialByName(HUDSprite.Material.LibMaterialName).Material.FrontProperties.Ambient.AsWinColor := Color
  else
    HUDSprite.Material.FrontProperties.Ambient.AsWinColor := Color;
end;

procedure TVKCustomConsole.SetSize(const Value: Single);
begin
...
procedure TVKCustomConsole.DoOnCommandIssued(var UserInputCommand:
  TVKUserInputCommand);
begin
...
procedure TVKCustomConsole.Notification(AComponent: TComponent;
...
procedure TVKCustomConsole.SetSceneViewer(
...
function TVKCustomConsole.GetFont: TVKCustomBitmapFont;
begin
  Result := FHudText.BitmapFont;
end;

procedure TVKCustomConsole.SetFont(const Value: TVKCustomBitmapFont);
begin
  FHudText.BitmapFont := Value;
end;

procedure TVKCustomConsole.SetName(const Value: TComponentName);
begin
  inherited;
...
procedure TVKConsoleControls.Assign(Source: TPersistent);
begin
  if Source is TVKConsoleControls then
  begin
    FNavigateUp := TVKConsoleControls(Source).FNavigateUp;
    FNavigateDown := TVKConsoleControls(Source).FNavigateDown;
    FNavigatePageUp := TVKConsoleControls(Source).FNavigatePageUp;
    FNavigatePageDown := TVKConsoleControls(Source).FNavigatePageDown;
    FNextCommand := TVKConsoleControls(Source).FNextCommand;
    FPreviousCommand := TVKConsoleControls(Source).FPreviousCommand;
    FAutoCompleteCommand := TVKConsoleControls(Source).FAutoCompleteCommand;
    FDblClickDelay := TVKConsoleControls(Source).FDblClickDelay;
  end;
end;

constructor TVKConsoleControls.Create(AOwner: TPersistent);
begin
  FOwner := AOwner;
...
function TVKConsoleControls.GetOwner: TPersistent;
begin
  Result := FOwner;
end;
...
procedure TVKConsoleCommand.Assign(Source: TPersistent);
begin
  Assert(Source <> nil);
  inherited;
  SetCommandName(TVKConsoleCommand(Source).FCommandName);
  FShortHelp := TVKConsoleCommand(Source).FShortHelp;
  FLongHelp.Assign(TVKConsoleCommand(Source).FLongHelp);

  FVisible := TVKConsoleCommand(Source).FVisible;
  FEnabled := TVKConsoleCommand(Source).FEnabled;
  FSilentDisabled := TVKConsoleCommand(Source).FSilentDisabled;
end;

constructor TVKConsoleCommand.Create(Collection: TCollection);
begin
  inherited;
  Assert((Collection is TVKConsoleCommandList) or (Collection = nil));

  FCommandList := TVKConsoleCommandList(Collection);
...
destructor TVKConsoleCommand.Destroy;
begin
  FLongHelp.Destroy;
  inherited;
end;

procedure TVKConsoleCommand.ShowInvalidUseOfCommandError;
begin
  FCommandList.FConsole.AddLine('   - Invalid use of command!');
end;

procedure TVKConsoleCommand.ShowInvalidNumberOfArgumentsError(const
...
procedure TVKConsoleCommand.SetCommandName(const Value: string);
begin
...
procedure TVKConsoleCommand.ShowHelp;
var
  I: Integer;
begin
...
procedure TVKConsoleCommand.DoOnCommand(var UserInputCommand:
  TVKUserInputCommand);
begin
...
procedure TVKConsoleCommand.ShowShortHelp;
begin
  if FVisible then
    FCommandList.FConsole.AddLine('   - ' + FCommandName + ' - ' + FShortHelp);
end;

function TVKConsoleCommand.GetDisplayName: string;
begin
...
{ TVKConsoleCommandList }

function TVKConsoleCommandList.Add: TVKConsoleCommand;
begin
  Result := TVKConsoleCommand(inherited Add);
end;

constructor TVKConsoleCommandList.Create(const AOwner: TVKCustomConsole);
begin
  Assert(AOwner <> nil);
  FConsole := TVKCustomConsole(AOwner);
  inherited Create(TVKConsoleCommand);
end;

destructor TVKConsoleCommandList.Destroy;
begin
  Clear;
  inherited;
end;

function TVKConsoleCommandList.GetItems(const Index: Integer):
  TVKConsoleCommand;
begin
  Result := TVKConsoleCommand(inherited Items[Index]);
end;

function TVKConsoleCommandList.LastConsoleCommand: TVKConsoleCommand;
begin
  Result := GetItems(Count - 1);
end;

procedure TVKConsoleCommandList.SortCommands(const Ascending: Boolean);
begin
  Assert(False, 'Not implemented yet....');
end;

function TVKConsoleCommandList.CommandExists(const Command: string): Boolean;
var
  I: Integer;
begin
  Result := True;
...
function TVKConsoleCommandList.GetCommandIndex(const Command: string): Integer;
begin
  if Count <> 0 then
...
function TVKConsoleCommandList.GetOwner: TPersistent;
begin
  Result := FConsole;
end;

{ TVKConsoleStringList }

procedure TVKConsoleStringList.Changed;
begin
  inherited;
...
function TVKConsoleStringList.CommandExists(const Command: string): Boolean;
begin
  Result := IndexOf(Command) <> -1;
end;

constructor TVKConsoleStringList.Create(const Owner: TVKCustomConsole);
begin
  Assert(Owner <> nil);
  Duplicates := dupError;
  FConsole := Owner;
...
function TVKConsoleStringList.GetOwner: TPersistent;
begin
  Result := FConsole;
end;

initialization
  RegisterClasses([TVKCustomConsole, TVKConsole, TVKConsoleStringList,
    TVKConsoleCommand, TVKConsoleCommandList, TVKConsoleControls]);
