...
uses
  ...
  VKS.VectorGeometry, VKS.PerlinBase, VKS.HeightData,
  VKS.CrossPlatform;

Type
  TVKPerlinInterpolation = (pi_none, pi_simple, pi_linear, pi_Smoothed,
    pi_Cosine, pi_cubic);

  TVKBasePerlinOctav = class
  private
    FAmplitude: Double;
    FScale: Double;
  ...
  public
    ...
    property Interpolation: TVKPerlinInterpolation read FInterpolation
      write FInterpolation;
    property Smoothing: TVKPerlinInterpolation read FSmoothing write FSmoothing;
    property Amplitude: Double read FAmplitude write FAmplitude;
    property Scale: Double read FScale write FScale;
  End;

  TVKPerlinOctav = class of TVKBasePerlinOctav;

  TVKBasePerlin = class(TComponent)
  private
    FPersistence: Double;
    FNumber_Of_Octaves: Integer;
    FOctaves: TList;
    FOctavClass: TVKPerlinOctav;
    FInterpolation: TVKPerlinInterpolation;
    FSmoothing: TVKPerlinInterpolation;
  protected
    function PerlinNoise_1D(x: Double): Double;
    function PerlinNoise_2D(x, y: Double): Double;
    function GetOctave(index: Integer): TVKBasePerlinOctav;
    Procedure SetPersistence(val: Double);
    Procedure Set_Number_Of_Octaves(val: Integer);
  public
    Constructor Create(AOwner: TComponent); override;
    Procedure Generate; dynamic; abstract;
    Property Octaves[index: Integer]: TVKBasePerlinOctav read GetOctave;
  ...
  TVK1DPerlin = class(TVKBasePerlin)
    Function GetPerlinValue_1D(x: Double): Double;
  published
  End;

  TVK2DPerlinOctav = class(TVKBasePerlinOctav)
  public
    Data: T2DPerlinArray;
  ...
  TVK2DPerlin = class(TVKBasePerlin)
  private
  public
    Width, Height: Integer;
    XStart, YStart: Integer;
...
implementation
//---------------------------------------------------------------------
function TVKBasePerlin.PerlinNoise_1D(x: Double): Double;
...
Begin
  ...
  case Interpolation of
    ...
    raise exception.Create
      ('TVKBasePerlin.PerlinNoise_1D, Interpolation not implemented!');
  End;
End;

function TVKBasePerlin.PerlinNoise_2D(x, y: Double): Double;
Var
  int_x, int_y: Integer;
  // frac_y,
  frac_x: Double;
...
function TVKBasePerlin.GetOctave(index: Integer): TVKBasePerlinOctav;
Begin
  Result := TVKBasePerlinOctav(FOctaves[index]);
End;

Procedure TVKBasePerlin.Set_Number_Of_Octaves(val: Integer);

Var
  XC: Integer;
  NewScale: Integer;
  Octav: TVKBasePerlinOctav;
Begin
...
Procedure TVKBasePerlin.SetPersistence(val: Double);
...
Constructor TVKBasePerlin.Create(AOwner: TComponent);
...
Function TVK1DPerlin.GetPerlinValue_1D(x: Double): Double;
Var
...
Procedure TVK2DPerlinOctav.Generate;
...
Function TVK2DPerlinOctav.GetPerling(x, y: Double): Double;
...
Procedure TVK2DPerlinOctav.Generate_CubicInterpolate;
...
Procedure TVK2DPerlinOctav.Generate_SmoothInterpolate;
...
Procedure TVK2DPerlinOctav.Generate_NonInterpolated;
...
Function TVK2DPerlinOctav.GetDataSmoothed(x, y: Integer): Double;
...
Function TVK2DPerlinOctav.GetData(x, y: Integer): Double;

Begin
  Result := Data[y][x];
End;

Function TVK2DPerlinOctav.GetCubic(x, y: Double): Double;
...
Function TVK2DPerlinOctav.GetCosine(x, y: Double): Double;
...
Constructor TVK2DPerlin.Create(AOwner: TComponent);
...
Begin
  ...
  XStep := 1;
  YStep := 1;
  FOctavClass := TVK2DPerlinOctav;
End;

Procedure TVK2DPerlin.Generate;
...
Begin
  For i := 0 to Number_Of_Octaves - 1 do
    With TVK2DPerlinOctav(Octaves[i]) do
    Begin
...
Function TVK2DPerlin.GetPerlinValue_2D(x, y: Double): Double;
Var
  ...
  Begin
    ...
    Begin
      ...
      For Octaver := 0 to FNumber_Of_Octaves - 1 do
        With TVK2DPerlinOctav(Octaves[Octaver]) do
...
Constructor TVKPerlinHDS.Create(AOwner: TComponent);
...
Begin
  ...
  Perlin := TVK2DPerlin.Create(self);
  ...
  Begin
    Thread := TVKPerlinHDSThread.Create(True);
...
procedure TVKPerlinHDS.WaitFor;
...
Procedure TVKPerlinHDSThread.Execute;
...
Procedure TVKPerlinHDSThread.OpdateOutSide;
