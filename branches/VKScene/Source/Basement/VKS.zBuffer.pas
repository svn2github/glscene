...
{: VKS.zBuffer<p>
 ...
 <b>History : </b><font size=-1><ul>
      ...
      <li>29/05/08 - DaStr - Removed unused variables in TVKZShadows.CalcShadowTexture()
      <li>20/01/08 - DaStr - Bugfixed TVKZShadows.HardSet(Bugtracker ID = 1875708)
                             Removed some old commented out code
                             Removed TVKZShadows.Trnc() procedure - there is a
                              similar procedure in VKS.VectorGeometry.pas
      ...
      <li>07/09/01 - Lin - Restructure zBuffer code, to support the new TVKMemoryViewer
      <li>06/09/01 - Lin - Created TVKZShadows object, for casting shadows
...
uses
  ...
  VKS.Scene, VKS.VectorGeometry, VKS.Graphics, VKS.Objects, VKS.Context,
  VKS.SceneViewer, VKS.Color, VKS.RenderContextInfo, VKS.State, VKS.TextureFormat,
  VKS.OpenGLTokens, VKS.XOpenGL, VKS.VectorTypes;
...
type
  ...
  published
    ...
    property Caster: TVKMemoryViewer read GetCaster write SetCaster;
...
procedure TVKzBuffer.LinkToViewer(viewer: TVKSceneViewer); // overload;
begin
...
procedure TVKzBuffer.LinkToViewer(viewer: TVKMemoryViewer); // overload;
begin
...
procedure TVKzBuffer.PrepareBufferMemory;
var
  i: Integer;
begin
...
function TVKzBuffer.GetDepthBuffer(CalcVectors: Boolean; ContextIsActive:
  boolean): PZArray;
begin
...
function TVKzBuffer.GetPixelzDepth(x, y: integer): Single;
begin
...
function TVKzBuffer.PixelToDistance_OLD(x, y: integer): Single;
var
...
function TVKzBuffer.PixelToDistance(x, y: integer): Single;
var
  z, dst: single;
  xx, yy, zz: single;
  fy: integer;
...
procedure TVKzBuffer.Refresh;
begin
  if assigned(Buffer) then
    GetDepthBuffer(True, False);
end;

procedure TVKzBuffer.DoCalcVectors;
var
  axs: TAffineVector;
  Hnorm, hcvec: TVector;
  vec: TAffineVector;
...
function TVKzBuffer.FastScreenToVector(x, y: integer): TAffineVector;
var
  w, h: integer;
  Rlerp, Ulerp: single;
begin
...
function TVKzBuffer.FastVectorToScreen(Vec: TAffineVector): TAffineVector;
var
  v0, v1, x, y, z: Single;
begin
  x := vec.V[0];
...
function TVKzBuffer.PixelToWorld(const x, y: Integer): TAffineVector;
var
  z, dst: single;
  fy: integer;
  camvec: TVector;
...
function TVKzBuffer.WorldToPixel(const aPoint: TAffineVector; out pixX, pixY:
...
function TVKzBuffer.WorldToPixelZ(const aPoint: TAffineVector; out pixX, pixY:
...
function TVKzBuffer.OrthWorldToPixelZ(const aPoint: TAffineVector; out pixX,
...
begin
  ...
  if TVKSceneBuffer(ARci.buffer).Camera <> FViewer.Camera then
    exit;
  if not assigned(ViewerZBuf) then
  begin //Create viewer zbuffer
    ViewerZBuf := TVKZBuffer.Create;
  ...
  GL.PushMatrix;
  GL.LoadMatrixf(@TVKSceneBuffer(ARci.buffer).BaseProjectionMatrix);
...
function TVKZShadows.SoftTest(const x, y: integer): Byte;
begin
  result := FDataInvIdx[y]^[x];
end;

function TVKZShadows.GetViewer: TVKSceneViewer;
begin
  result := FViewer;
end;

procedure TVKZShadows.SetViewer(const val: TVKSceneViewer);
begin
  FViewer := Val;
...
function TVKZShadows.GetCaster: TVKMemoryViewer;
begin
  result := FCaster;
end;

procedure TVKZShadows.SetCaster(const val: TVKMemoryViewer);
begin
  FCaster := Val;
end;

function TVKZShadows.CastShadow: Boolean;
begin
  if Caster <> nil then
  begin
    if not assigned(CasterZBuf) then
    begin
      CasterZBuf := TVKZBuffer.Create;
...
procedure TVKZShadows.SetWidth(const val: integer);
begin
  FWidth := val;
  SetXRes(val);
end;

procedure TVKZShadows.SetHeight(const val: integer);
begin
  FHeight := val;
  SetYRes(val);
end;

procedure TVKZShadows.SetXRes(const val: integer);
var
  i: integer;
begin
  i := 2;
...
procedure TVKZShadows.SetYRes(const val: integer);
var
  i: integer;
begin
  i := 2;
...
procedure TVKZShadows.SetSoft(const val: boolean);
begin
  FSoft := val;
  NotifyChange(Self);
end;
