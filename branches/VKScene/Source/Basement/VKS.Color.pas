...
uses
  ...
  VKS.VectorTypes, VKS.VectorGeometry, VKS.CrossPlatform,
  VKS.PersistentClasses, VKS.BaseClasses;
...
function ColorManager: TVKColorManager;
...
var
   // Specifies if TVKColor should allocate memory for
...
begin
	if not Assigned(vColorManager) then begin
		vColorManager:=TVKColorManager.Create;
...
// Create
//
constructor TVKColor.Create(AOwner: TPersistent);
begin
   inherited;
   Initialize(clrBlack);
end;
...
// CreateInitialized
//
constructor TVKColor.CreateInitialized(AOwner : TPersistent; const color : TColorVector;
                                       changeEvent : TNotifyEvent = nil);
begin
   Create(AOwner);
   Initialize(color);
...
// Initialize
//
procedure TVKColor.Initialize(const color : TColorVector);
begin
...
// SetColorVector
//
procedure TVKColor.SetColorVector(const aColor : TColorVector);
begin
   SetVector(FColor, AColor);
	NotifyChange(Self);
end;

procedure TVKColor.SetDirectColorVector(const AColor: TColorVector);
begin
  SetVector(FColor, AColor);
end;
...
// SetColorComponent
//
procedure TVKColor.SetColorComponent(index : Integer; value : Single);
begin
...
// SetAsWinColor
//
procedure TVKColor.SetAsWinColor(const val : TColor);
begin
...
// GetAsWinColor
//
function TVKColor.GetAsWinColor : TColor;
begin
...
// GetColorComponent
//
function TVKColor.GetColorComponent(const Index: Integer): Single;
begin
...
// Assign
//
procedure TVKColor.Assign(Source: TPersistent);
begin
   if Assigned(Source) and (Source is TVKColor) then begin
		FColor:=TVKColor(Source).FColor;
      NotifyChange(Self);
   end else inherited;
end;
...
// DefineProperties
//
procedure TVKColor.DefineProperties(Filer: TFiler);
begin
  inherited;
...
// ReadData
//
procedure TVKColor.ReadData(Stream: TStream);
begin
...
// WriteData
//
procedure TVKColor.WriteData(Stream: TStream);
begin
...
// NotifyChange
//
procedure TVKColor.NotifyChange(Sender : TObject);
var
  intf: IGLNotifyable;
begin
...
//      if Owner is TVKBaseSceneObject then
//         TVKBaseSceneObject(Owner).StructureChanged;
      inherited;
   end;
end;
...
// RandomColor
//
procedure TVKColor.RandomColor;
begin
   Red:=Random;
   Green:=Random;
   Blue:=Random;
...
// GetHSVA
//
function TVKColor.GetHSVA : TVector;
var
   delta, min : Single;
const
   H = 0;
...
// SetHSVA
//
procedure TVKColor.SetHSVA(const hsva : TVector);
var
...
// Find Color
//
function TVKColorManager.FindColor(const aName: String): TColorVector;
var
   i : Integer;
begin
   Result:=clrBlack;
...
// GetColor
//
function TVKColorManager.GetColor(const aName : String): TColorVector;
var
   workCopy  : String;
   delimiter : Integer;
begin
...
function TVKColorManager.GetColorName(const aColor: TColorVector): String;
...
// Destroy
//
destructor TVKColorManager.Destroy;
var
   i : Integer;
begin
   for i:=0 to Count-1 do
...
// AddColor
//
procedure TVKColorManager.AddColor(const aName: String; const aColor: TColorVector);
var
...
// EnumColors
//
procedure TVKColorManager.EnumColors(Proc: TGetStrProc);
var
   i : Integer;
begin
   for i:=0 to Count-1 do
...
// EnumColors
//
procedure TVKColorManager.EnumColors(AValues: TStrings);
var
   i : Integer;
begin
   for i:=0 to Count-1 do
...
//
procedure TVKColorManager.RegisterDefaultColors;
begin
   Capacity:=150;
...
// RemoveColor
//
procedure TVKColorManager.RemoveColor(const aName: String);
var
   i : Integer;
begin
