...
uses
  VKS.VectorFileObjects, VKS.EllipseCollision, VKS.VectorGeometry, VKS.VectorLists,
  VKS.Scene, VKS.TerrainRenderer, VKS.ProxyObjects, VKS.MultiProxy, VKS.VectorTypes;
...
procedure ECAddFreeForm(var MovePack: TECMovePack; FreeForm: TVKBaseSceneObject;
  Solid: Boolean; ObjectID: Integer);

//Add a TriMesh box
procedure ECAddBox(var MovePack: TECMovePack;
  BoxObj: TVKBaseSceneObject; BoxSize: TAffineVector;
  Solid: Boolean; ObjectID: Integer);

//Add the terrain as a TriMesh
procedure ECAddTerrain(var MovePack: TECMovePack;
  TerrainRenderer: TVKTerrainRenderer; Resolution: Single;
  Solid: Boolean; ObjectID: Integer);
...
begin
  Master := FreeForm;
  if Master is TVKFreeFormProxy then
    Master := TVKFreeFormProxy(Master).MasterObject;
  if Master is TVKMultiProxy then
    if TVKMultiProxy(Master).MasterObjects.Count > 0 then
      Master := TVKMultiProxy(Master).MasterObjects[0].MasterObject;
  Assert((Master is TVKFreeForm), 'Object must be freeform, freeformproxy or freeformbased Multiproxy.');
  Assert(Assigned(TVKFreeForm(Master).Octree), 'Octree must have been prepared and setup before use.');
  ...
  with TVKFreeForm(Master).Octree do
  begin
    ...
    MovePack.Freeforms[count].InvertedNormals := TVKFreeForm(Master).NormalsOrientation = mnoInvert;
  end;
end;
