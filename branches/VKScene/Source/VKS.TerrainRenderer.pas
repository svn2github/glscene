...
uses
  ...
  VKS.Scene, VKS.HeightData, VKS.Material, VKS.VectorGeometry, VKS.Context,
  VKS.ROAMPatch, VKS.VectorLists, VKS.RenderContextInfo, VKS.OpenGLTokens,
  VKS.XOpenGL, VKS.Utils, VKS.VectorTypes;
...
procedure TVKTerrainRenderer.DestroyHandle;
begin
  inherited;
  ReleaseAllTiles;
...
function TVKTerrainRenderer.RayCastIntersect(const rayStart, rayVector: TVector;
...
procedure TVKTerrainRenderer.OnTileDestroyed(Sender: TObject);
var
  list: TList;
begin
...
function TVKTerrainRenderer.InterpolatedHeight(const p: TVector): single;
var
  pLocal: TVector;
begin
  if Assigned(HeightDataSource) then
...
function TVKTerrainRenderer.InterpolatedHeight(const p: TAffineVector): single;
begin
...
procedure TVKTerrainRenderer.BuildList(var rci: TRenderContextInfo);
var
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if deltaY > 0 then
              patch.ConnectToTheNorth(TVKROAMPatch(prevRow.Items[n]))
            else
              TVKROAMPatch(prevRow.Items[n]).ConnectToTheNorth(patch);
          end;
  ...
  begin
    ...
    begin
      patch := TVKROAMPatch(patchList[n]);
    ...
    begin
      patch := TVKROAMPatch(patchList[n - rpIdxDelta]);
      if Assigned(patch) then
      begin
        ApplyMaterial(patch.HeightData.materialName);
...
procedure TVKTerrainRenderer.MarkAllTilesAsUnused;
var
  i, j, zero: Integer;
begin
...
procedure TVKTerrainRenderer.ReleaseAllUnusedTiles;
var
  i, j: Integer;
  hashList: TList;
  hd: THeightData;
...
function TVKTerrainRenderer.HashedTileCount: Integer;
var
  i: Integer;
  hashList: TList;
  cnt: Integer;
...
begin
  ...
  begin
    patch := TVKROAMPatch(tile.ObjectTag);
    if not Assigned(patch) then
    begin
      // spawn ROAM patch
      patch := TVKROAMPatch.Create;
...
procedure TVKTerrainRenderer.SetTileSize(const val: Integer);
begin
...
procedure TVKTerrainRenderer.SetTilesPerTexture(const val: single);
begin
...
procedure TVKTerrainRenderer.SetMaterialLibrary(const val: TVKMaterialLibrary);
begin
...
procedure TVKTerrainRenderer.SetQualityStyle(const val: TTerrainHighResStyle);
begin
...
// class registrations
RegisterClass(TVKTerrainRenderer);
