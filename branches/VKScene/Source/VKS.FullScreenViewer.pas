...
uses
  ...
  VKS.Scene, VKS.SceneViewer, VKS.Context, VKS.SceneContext, VKS.OpenGLTokens,
  VKS.Crossplatform,  VKS.OpenGLAdapter, VKS.Screen;

type

  // TVKScreenDepth
  //
  TVKScreenDepth = (sd8bits, sd16bits, sd24bits, sd32bits);
  ...
  private
    ...
    procedure SetScreenDepth(const val: TVKScreenDepth);
...
// Create
//
constructor TVKFullScreenViewer.Create(AOwner: TComponent);
begin
...
// Destroy
//
destructor TVKFullScreenViewer.Destroy;
begin
  Active := False;
  inherited Destroy;
end;
...
// DoBeforeRender
//
procedure TVKFullScreenViewer.DoBeforeRender(Sender: TObject);
begin
  SetupVSync(VSync);
end;
...
// DoBufferChange
//
procedure TVKFullScreenViewer.DoBufferChange(Sender: TObject);
begin
...
//
procedure TVKFullScreenViewer.DoBufferStructuralChange(Sender: TObject);
begin
...
// Render
//
procedure TVKFullScreenViewer.Render(baseObject: TVKBaseSceneObject = nil);
begin
...
// BeginUpdate
//
procedure TVKFullScreenViewer.BeginUpdate;
begin
  Inc(FUpdateCount);
end;
...
// EndUpdate
//
procedure TVKFullScreenViewer.EndUpdate;
begin
  Dec(FUpdateCount);
...
procedure TVKFullScreenViewer.ReActivate;
begin
  Shutdown;
  Startup;
end;

procedure TVKFullScreenViewer.Notification(AComponent: TComponent;
...
function TVKFullScreenViewer.LastFrameTime: Single;
begin
  Result := Buffer.LastFrameTime;
end;

function TVKFullScreenViewer.FramesPerSecond: Single;
begin
  Result := Buffer.FramesPerSecond;
end;

function TVKFullScreenViewer.FramesPerSecondText(decimals: Integer): String;
begin
  Result := Format('%.*f FPS', [decimals, Buffer.FramesPerSecond]);
end;

procedure TVKFullScreenViewer.ResetPerformanceMonitor;
begin
...
//
procedure TVKFullScreenViewer.UseCurrentResolution;
begin
  BeginUpdate;
  try
    Width := Screen.Width;
...
// SetActive
//
procedure TVKFullScreenViewer.SetActive(const val: Boolean);
begin
  if val <> FActive then
  begin
...
// Startup
//
procedure TVKFullScreenViewer.Startup;
var
  res: TResolution;
begin
  if FActive then
...
// Shutdown
//
procedure TVKFullScreenViewer.Shutdown;
begin
  if not FActive then
    Exit;
  Assert(FForm <> nil);
...
// BindFormEvents
//
procedure TVKFullScreenViewer.BindFormEvents;
begin
...
// DoCloseQuery
//
procedure TVKFullScreenViewer.DoCloseQuery(Sender: TObject;
...
// DoPaint
//
procedure TVKFullScreenViewer.DoPaint(Sender: TObject);
begin
...
procedure TVKFullScreenViewer.DoActivate(Sender: TObject);
begin
...
procedure TVKFullScreenViewer.DoDeactivate(Sender: TObject);
begin
...
procedure TVKFullScreenViewer.DoFormDestroy(Sender: TObject);
begin
  Active := False;
end;
...
// SetScreenDepth
//
procedure TVKFullScreenViewer.SetScreenDepth(const val: TVKScreenDepth);
begin
...
// SetStayOnTop
//
procedure TVKFullScreenViewer.SetStayOnTop(const val: Boolean);
begin
...
// SetOnCloseQuery
//
procedure TVKFullScreenViewer.SetOnCloseQuery(const val: TCloseQueryEvent);
begin
...
// SetOnClose
//
procedure TVKFullScreenViewer.SetOnClose(const val: TCloseEvent);
begin
  If Form <> nil then
    Form.OnClose := val;
  FOnClose := val;
...
// SetOnKeyPress
//
procedure TVKFullScreenViewer.SetOnKeyPress(const val: TKeyEvent); //VCL - TKeyPressEvent
begin
  If Form <> nil then
    Form.OnKeyDown := val;
  FOnKeyPress := val;
...
// SetOnKeyUp
//
procedure TVKFullScreenViewer.SetOnKeyUp(const val: TKeyEvent);
begin
  If Form <> nil then
    Form.OnKeyUp := val;
  FOnKeyUp := val;
...
// SetOnKeyDown
//
procedure TVKFullScreenViewer.SetOnKeyDown(const val: TKeyEvent);
begin
  If Form <> nil then
    Form.OnKeyDown := val;
  FOnKeyDown := val;
...
// SetOnMouseWheel
//
procedure TVKFullScreenViewer.SetOnMouseWheel(const val: TMouseWheelEvent);
begin
  If Form <> nil then
...
// SetOnMouseWheelDown
//
procedure TVKFullScreenViewer.SetOnMouseWheelDown
...
// SetOnMouseWheelUp
//
procedure TVKFullScreenViewer.SetOnMouseWheelUp(const val: TMouseWheelEvent);
begin
  If Form <> nil then
    Form.OnMouseWheel := val;
  FOnMouseWheelUp := val;
...
// SetOnClick
//
procedure TVKFullScreenViewer.SetOnClick(const val: TNotifyEvent);
begin
  If Form <> nil then
...
// SetOnDblClick
//
procedure TVKFullScreenViewer.SetOnDblClick(const val: TNotifyEvent);
begin
  If Form <> nil then
...
// SetOnMouseMove
//
procedure TVKFullScreenViewer.SetOnMouseMove(const val: TMouseMoveEvent);
begin
  If Form <> nil then
...
// SetOnMouseDown
//
procedure TVKFullScreenViewer.SetOnMouseDown(const val: TMouseEvent);
begin
  If Form <> nil then
...
// SetOnMouseUp
//
procedure TVKFullScreenViewer.SetOnMouseUp(const val: TMouseEvent);
begin
  If Form <> nil then
    Form.OnMouseUp := val;
  FOnMouseUp := val;
...
//
(*
procedure TVKFullScreenViewer.SetCursor(const val: TCursor);
begin
  if val <> FCursor then
  begin
    FCursor := val;
...
// SetPopupMenu
//
procedure TVKFullScreenViewer.SetPopupMenu(const val: TPopupMenu);
begin
...
procedure TVKFullScreenViewer.SetForm(aVal: TForm);
begin
  FForm := aVal;
end;

procedure TVKFullScreenViewer.SetManualRendering(const val: Boolean);
begin
...
// GetHandle
//
function TVKFullScreenViewer.GetHandle: TWindowHandle;
begin
  if Form <> nil then
    Result := FForm.Handle
  else
...
RegisterClasses([TVKFullScreenViewer]);
