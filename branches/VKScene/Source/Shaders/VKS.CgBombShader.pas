...
uses
  ...
  //VKS
  VKS.Texture, VKS.Cadencer, VKS.Context, Winapi.OpenGL, Winapi.OpenGLext,  VKS.Strings, VKS.Material,
...
type
  ...
  private
    ...
    FGradientTexture: TVKTexture;
    FMainTexture:     TVKTexture;
    FMainTextureName:     TVKLibMaterialName;
    FGradientTextureName: TVKLibMaterialName;
...
{$ENDIF}
    ...
    procedure SetMainTexture(const Value: TVKTexture);

    function GetMainTextureName: TVKLibMaterialName;
    procedure SetMainTextureName(const Value: TVKLibMaterialName);
    function GetGradientTextureName: TVKLibMaterialName;
    procedure SetGradientTextureName(const Value: TVKLibMaterialName);
  ...
  public
    ...
    property MainTexture: TVKTexture read FMainTexture write SetMainTexture;
    property MainTextureName: TVKLibMaterialName read GetMainTextureName write SetMainTextureName;

    property GradientTexture: TVKTexture read FGradientTexture write SetGradientTexture;
    property GradientTextureName: TVKLibMaterialName read GetGradientTextureName write SetGradientTextureName;
...
{ TVKCustomCGBombShader }

constructor TVKCustomCGBombShader.Create(AOwner: TComponent);
begin
  inherited;
...
{$ELSE}
  case FMainTextureSource of
    stsPrimaryTexture: FragmentProgram.ParamByName('MainTextureSampler').SetAsTexture2D(rci.VKStates.TextureBinding[0, ttTexture2D]);
    stsSecondadyTexture: FragmentProgram.ParamByName('MainTextureSampler').SetAsTexture2D(rci.VKStates.TextureBinding[1, ttTexture2D]);
    stsThirdTexture: FragmentProgram.ParamByName('MainTextureSampler').SetAsTexture2D(rci.VKStates.TextureBinding[2, ttTexture2D]);
...
begin
  ...
  try
    FGradientTexture := TVKMaterialLibrary(FMaterialLibrary).TextureByName(FGradientTextureName);
  except
    Enabled := False;
    raise;
  end;
  if FMainTexture = nil then
  try
    FMainTexture := TVKMaterialLibrary(FMaterialLibrary).TextureByName(FMainTextureName);
  except end;
...
function TVKCustomCGBombShader.GetGradientTextureName: TVKLibMaterialName;
begin
  Result := TVKMaterialLibrary(FMaterialLibrary).GetNameOfTexture(FGradientTexture);
  if Result = '' then Result := FGradientTextureName;
end;

function TVKCustomCGBombShader.GetMainTextureName: TVKLibMaterialName;
begin
  Result := TVKMaterialLibrary(FMaterialLibrary).GetNameOfTexture(FMainTexture);
  if Result = '' then Result := FMainTextureName;
end;

function TVKCustomCGBombShader.GetMaterialLibrary: TVKAbstractMaterialLibrary;
begin
  Result := FMaterialLibrary;
end;

procedure TVKCustomCGBombShader.Notification(AComponent: TComponent;
  ...
  if Operation = opRemove then
    if AComponent = FMaterialLibrary then
      ...
      begin
        ...
        if FGradientTexture <> nil then
        begin
          Index := TVKMaterialLibrary(FMaterialLibrary).Materials.GetTextureIndex(FGradientTexture);
          if Index <> -1 then
...
procedure TVKCustomCGBombShader.OnApplyFP(CgProgram: TCgProgram; Sender: TObject);
begin
...
procedure TVKCustomCGBombShader.OnApplyVP(CgProgram: TCgProgram; Sender: TObject);
begin
...
procedure TVKCustomCGBombShader.OnUnApplyFP(CgProgram: TCgProgram);
begin
...
procedure TVKCustomCGBombShader.SetGradientTexture(const Value: TVKTexture);
begin
...
procedure TVKCustomCGBombShader.SetGradientTextureName(
...
procedure TVKCustomCGBombShader.SetMainTexture(
...
procedure TVKCustomCGBombShader.SetMainTextureName(
  const Value: TVKLibMaterialName);
begin
...
procedure TVKCustomCGBombShader.SetMaterialLibrary(
  const Value: TVKAbstractMaterialLibrary);
begin
...
function TVKCustomCGBombShader.StoreColorRange: Boolean;
begin
  Result := Abs(FColorRange - 0.24) > EPS;
end;

function TVKCustomCGBombShader.StoreColorSharpness: Boolean;
begin
  Result := Abs(FColorSharpness - 1) > EPS;
end;

function TVKCustomCGBombShader.StoreDisplacement: Boolean;
begin
  Result := Abs(FDisplacement - 0.3) > EPS;
end;

function TVKCustomCGBombShader.StoreGradientTextureShare: Boolean;
begin
  Result := Abs(FGradientTextureShare - 0.7) > EPS;
end;

function TVKCustomCGBombShader.StoreMainTextureShare: Boolean;
begin
  Result := Abs(FMainTextureShare - 0.7) > EPS;
end;

function TVKCustomCGBombShader.StoreSharpness: Boolean;
begin
  Result := Abs(FSharpness - 3) > EPS;
end;

function TVKCustomCGBombShader.StoreSpeed: Boolean;
begin
  Result := Abs(FSpeed - 0.3) > EPS;
end;

function TVKCustomCGBombShader.StoreTurbDensity: Boolean;
begin
...
initialization
  RegisterClasses([TVKCustomCGBombShader, TVKCgBombShader]);
