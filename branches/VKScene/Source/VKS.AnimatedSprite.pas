...
type
  TVKSpriteAnimFrame = class;
  TVKSpriteAnimFrameList = class;
  TVKSpriteAnimation = class;
  TVKSpriteAnimationList = class;
  TVKAnimatedSprite = class;
  ...
  { Used by the SpriteAnimation when Dimensions are set manual. The animation
     ...
     for this frame. }
  TVKSpriteAnimFrame = class(TVKXCollectionItem)
  private
    FOffsetX,
      FOffsetY,
      FWidth,
  ...
  // TVKSpriteAnimFrameList
  { The XCollection used for the TSpriteAnimFrame object. }
  TVKSpriteAnimFrameList = class(TVKXCollection)
  public
  ...
  TVKSpriteFrameDimensions = (sfdAuto, sfdManual);

  // TVKSpriteAnimMargins
  { Used to mask the auto generated frames. The Left, Top, Right and
     ...
     auto dimensions. }
  TVKSpriteAnimMargins = class(TPersistent)
  private
    FOwner: TVKSpriteAnimation;
    FLeft,
      FTop,
      FRight,
      FBottom: Integer;
  ...
  public
    constructor Create(Animation: TVKSpriteAnimation);

    property Owner: TVKSpriteAnimation read FOwner;
  ...
  TVKSpriteAnimation = class(TVKXCollectionItem, IGLMaterialLibrarySupported)
  private
    FCurrentFrame,
      FStartFrame,
      FEndFrame,
      ...
      FFrameHeight,
      FInterval: Integer;
    FFrames: TVKSpriteAnimFrameList;
    FLibMaterialName: TVKLibMaterialName;
    FLibMaterialCached: TVKLibMaterial;
    FDimensions: TVKSpriteFrameDimensions;
    FMargins: TVKSpriteAnimMargins;
  ...
  protected
    ...
    procedure WriteToFiler(writer: TWriter); override;
    procedure ReadFromFiler(reader: TReader); override;
    procedure SetDimensions(const Value: TVKSpriteFrameDimensions);
    procedure SetLibMaterialName(const val: TVKLibMaterialName);
    function GetLibMaterialCached: TVKLibMaterial;
  ...
  public
    ...
    property LibMaterialCached: TVKLibMaterial read GetLibMaterialCached;
  ...
  published
    ...
    property Frames: TVKSpriteAnimFrameList read FFrames;
    // Automatic or manual texture coordinate generation.
    property Dimensions: TVKSpriteFrameDimensions read FDimensions write
      SetDimensions;
    { The number of milliseconds between each frame in the animation.
       Will automatically calculate the FrameRate value when set.
       Will override the TVKAnimatedSprite Interval is greater than
       zero. }
    ...
    property Margins: TVKSpriteAnimMargins read FMargins;

  end;

  // TVKSpriteAnimationList
  { A collection for storing TVKSpriteAnimation objects. }
  TVKSpriteAnimationList = class(TVKXCollection)
  public
    constructor Create(aOwner: TPersistent); override;
    class function ItemsClass: TVKXCollectionItemClass; override;
  ...
  TVKSpriteAnimationMode = (samNone, samPlayOnce, samLoop, samBounceForward,
    samBounceBackward, samLoopBackward);

  // TVKAnimatedSprite
  { An animated version of the TVKSprite using offset texture
     coordinate animation. }
  TVKAnimatedSprite = class(TVKBaseSceneObject)
  private
    FAnimations: TVKSpriteAnimationList;
    ...
    FMirrorU,
      FMirrorV: Boolean;
    FAnimationMode: TVKSpriteAnimationMode;
  ...
  protected
    ...
    procedure SetAnimationMode(const val: TVKSpriteAnimationMode);
  ...
  published
    { A collection of animations. Stores the settings for animating
       then sprite. }
    property Animations: TVKSpriteAnimationList read FAnimations;
    ...
    property AnimationMode: TVKSpriteAnimationMode read FAnimationMode write
      SetAnimationMode;
...
procedure TVKSpriteAnimFrame.DoChanged;
begin
  if Assigned(Owner) then
  begin
    if Assigned(Owner.Owner) then
      if Owner.Owner is TVKSpriteAnimation then
        TVKSpriteAnimation(Owner.Owner).DoChanged;
  end;
end;
...
class function TVKSpriteAnimFrame.FriendlyName: string;
begin
  Result := 'Frame';
end;
...
class function TVKSpriteAnimFrame.FriendlyDescription: string;
begin
...
procedure TVKSpriteAnimFrame.WriteToFiler(writer: TWriter);
begin
  inherited;
...
procedure TVKSpriteAnimFrame.ReadFromFiler(reader: TReader);
var
...
procedure TVKSpriteAnimFrame.SetOffsetX(const Value: Integer);
begin
...
procedure TVKSpriteAnimFrame.SetOffsetY(const Value: Integer);
begin
...
procedure TVKSpriteAnimFrame.SetWidth(const Value: Integer);
begin
...
procedure TVKSpriteAnimFrame.SetHeight(const Value: Integer);
begin
...
constructor TVKSpriteAnimFrameList.Create(aOwner: TPersistent);
begin
  inherited;
end;
...
class function TVKSpriteAnimFrameList.ItemsClass: TVKXCollectionItemClass;
begin
  Result := TVKSpriteAnimFrame;
end;

// ----------
// ---------- TVKSpriteAnimationMargins ----------
// ----------

// Create
//

constructor TVKSpriteAnimMargins.Create(Animation: TVKSpriteAnimation);
begin
  inherited Create;
  FOwner := Animation;
end;
...
procedure TVKSpriteAnimMargins.SetLeft(const Value: Integer);
begin
  if Value <> FLeft then
  begin
    FLeft := Value;
...
procedure TVKSpriteAnimMargins.SetTop(const Value: Integer);
begin
  if Value <> FTop then
  begin
    FTop := Value;
...
procedure TVKSpriteAnimMargins.SetRight(const Value: Integer);
begin
...
procedure TVKSpriteAnimMargins.SetBottom(const Value: Integer);
begin
...
procedure TVKSpriteAnimMargins.DoChanged;
begin
  if Assigned(Owner) then
    Owner.DoChanged;
end;
...
constructor TVKSpriteAnimation.Create(aOwner: TVKXCollection);
begin
  inherited;
  FFrames := TVKSpriteAnimFrameList.Create(Self);
  FMargins := TVKSpriteAnimMargins.Create(Self);
end;

// Destroy
//

destructor TVKSpriteAnimation.Destroy;
begin
  FFrames.Free;
  FMargins.Free;
  inherited;
...
function TVKSpriteAnimation.GetMaterialLibrary: TVKAbstractMaterialLibrary;
begin
  if not (Owner is TVKSpriteAnimationList) then
    Result := nil
  else
  begin
    if not (TVKSpriteAnimationList(Owner).Owner is TVKAnimatedSprite) then
      Result := nil
    else
      Result :=
        TVKAnimatedSprite(TVKSpriteAnimationList(Owner).Owner).FMaterialLibrary;
  end;
end;
...
class function TVKSpriteAnimation.FriendlyName: string;
begin
  Result := 'Animation';
end;
...
class function TVKSpriteAnimation.FriendlyDescription: string;
begin
...
procedure TVKSpriteAnimation.WriteToFiler(writer: TWriter);
begin
  inherited;
...
procedure TVKSpriteAnimation.ReadFromFiler(reader: TReader);
var
  archiveVersion: Integer;
begin
  inherited;
  ...
  begin
    ...
    Dimensions := TVKSpriteFrameDimensions(ReadInteger);
...
procedure TVKSpriteAnimation.DoChanged;
begin
  if Assigned(Owner) then
  begin
    if Assigned(Owner.Owner) then
      if Owner.Owner is TVKBaseSceneObject then
        TVKBaseSceneObject(Owner.Owner).NotifyChange(Self);
  end;
end;
...
procedure TVKSpriteAnimation.SetCurrentFrame(const Value: Integer);
begin
...
procedure TVKSpriteAnimation.SetFrameWidth(const Value: Integer);
begin
...
procedure TVKSpriteAnimation.SetFrameHeight(const Value: Integer);
begin
...
procedure TVKSpriteAnimation.SetDimensions(
  const Value: TVKSpriteFrameDimensions);
begin
...
procedure TVKSpriteAnimation.SetLibMaterialName(const val: TVKLibMaterialName);
begin
...
function TVKSpriteAnimation.GetLibMaterialCached: TVKLibMaterial;
begin
  Result := nil;
  ...
  if not Assigned(FLibMaterialCached) then
    if Assigned(Owner) then
      if Assigned(Owner.Owner) then
        if Owner.Owner is TVKAnimatedSprite then
          if Assigned(TVKAnimatedSprite(Owner.Owner).MaterialLibrary) then
            FLibMaterialCached :=
              TVKAnimatedSprite(Owner.Owner).MaterialLibrary.Materials.GetLibMaterialByName(FLibMaterialName);
...
procedure TVKSpriteAnimation.SetInterval(const Value: Integer);
begin
...
procedure TVKSpriteAnimation.SetFrameRate(const Value: Single);
begin
  if Value > 0 then
...
function TVKSpriteAnimation.GetFrameRate: Single;
begin
  if Interval > 0 then
...
constructor TVKSpriteAnimationList.Create(aOwner: TPersistent);
begin
  inherited;
end;
...
class function TVKSpriteAnimationList.ItemsClass: TVKXCollectionItemClass;
begin
  Result := TVKSpriteAnimation;
end;
...
constructor TVKAnimatedSprite.Create(AOwner: TComponent);
begin
  inherited;

  FAnimations := TVKSpriteAnimationList.Create(Self);
  FAnimationIndex := -1;
  FInterval := 100;
  FPixelRatio := 100;
  FRotation := 0;
...
destructor TVKAnimatedSprite.Destroy;
begin
  FAnimations.Free;
  inherited;
end;
...
procedure TVKAnimatedSprite.DoProgress(const progressTime: TProgressTimes);
var
  i, intr: Integer;
begin
  inherited;
  if (AnimationIndex = -1) then
    exit;
  intr := TVKSpriteAnimation(Animations[AnimationIndex]).Interval;
  if intr = 0 then
    intr := Interval;
...
procedure TVKAnimatedSprite.Notification(AComponent: TComponent; Operation:
  TOperation);
begin
...
procedure TVKAnimatedSprite.DefineProperties(Filer: TFiler);
begin
  inherited;
...
procedure TVKAnimatedSprite.WriteAnimations(Stream: TStream);
var
  writer: TWriter;
begin
...
procedure TVKAnimatedSprite.ReadAnimations(Stream: TStream);
var
  reader: TReader;
begin
...
procedure TVKAnimatedSprite.NextFrame;
var
  currentFrame,
    startFrame,
    endFrame: Integer;
...
procedure TVKAnimatedSprite.SetInterval(const val: Integer);
begin
...
procedure TVKAnimatedSprite.SetFrameRate(const Value: Single);
begin
  if Value > 0 then
    Interval := Round(1000 / Value)
  else
...
function TVKAnimatedSprite.GetFrameRate: Single;
begin
  if Interval > 0 then
    Result := 1000 / Interval
  else
...
procedure TVKAnimatedSprite.SetAnimationIndex(const val: Integer);
begin
  ...
  begin
    ...
    if FAnimationIndex < 0 then
      FAnimationIndex := -1;
    if (FAnimationIndex <> -1) and (FAnimationIndex < Animations.Count) then
      with TVKSpriteAnimation(Animations[FAnimationIndex]) do
        case AnimationMode of
...
procedure TVKAnimatedSprite.SetAnimationMode(const val: TVKSpriteAnimationMode);
begin
...
procedure TVKAnimatedSprite.SetMaterialLibrary(const val: TVKMaterialLibrary);
var
  i: Integer;
begin
  ...
  begin
    ...
    for i := 0 to Animations.Count - 1 do
      TVKSpriteAnimation(Animations[i]).FLibMaterialCached := nil;
    NotifyChange(Self);
  end;
end;
...
procedure TVKAnimatedSprite.SetPixelRatio(const val: Integer);
begin
...
procedure TVKAnimatedSprite.SetRotation(const val: Integer);
begin
...
procedure TVKAnimatedSprite.SetMirrorU(const val: Boolean);
begin
...
procedure TVKAnimatedSprite.SetMirrorV(const val: Boolean);
begin
...
initialization
  ...
  RegisterClasses([TVKAnimatedSprite,
    TVKSpriteAnimFrame, TVKSpriteAnimFrameList,
      TVKSpriteAnimation, TVKSpriteAnimationList]);

  RegisterXCollectionItemClass(TVKSpriteAnimFrame);
  RegisterXCollectionItemClass(TVKSpriteAnimation);

finalization

  UnregisterXCollectionItemClass(TVKSpriteAnimFrame);
  UnregisterXCollectionItemClass(TVKSpriteAnimation);
