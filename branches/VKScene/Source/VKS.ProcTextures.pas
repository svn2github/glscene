...
unit VKS.ProcTextures;

interface

uses
  System.Classes, System.SysUtils,
  VKS.Texture, VKS.Graphics, Winapi.OpenGL, Winapi.OpenGLext, 
  VKS.CrossPlatform, VKS.TextureFormat, VKS.VectorGeometry;
...
type
  TVKProcTextureNoise = class(TVKTextureImage)
  private
    FNoiseMap: TVKBitmap32;
...
constructor TVKProcTextureNoise.Create(AOwner: TPersistent);
...
destructor TVKProcTextureNoise.Destroy;
begin
  ReleaseBitmap32;
  inherited;
end;

procedure TVKProcTextureNoise.UpdateNoise;
var
  X, Y, C: Integer;
...
function TVKProcTextureNoise.GetBitmap32: TVKBitmap32;
begin
  if not Assigned(FNoiseMap) then
  begin
    FNoiseMap := TVKBitmap32.Create;
...
class function TVKProcTextureNoise.FriendlyName: string;
begin
...
class function TVKProcTextureNoise.FriendlyDescription: string;
begin
  Result := 'Procedural Noise (Animated)';
end;

procedure TVKProcTextureNoise.SetSeamless(const val: Boolean);
begin
...
procedure TVKProcTextureNoise.LoadFromFile(const fileName: string);
begin
  Assert(False, 'TVKProcTextureNoise.LoadFromFile not implemented');
end;

procedure TVKProcTextureNoise.ReleaseBitmap32;
begin
...
procedure TVKProcTextureNoise.SaveToFile(const fileName: string);
begin
  {Nothing here}
end;

function TVKProcTextureNoise.GetHeight: Integer;
begin
  Result := FHeight;
end;

function TVKProcTextureNoise.GetWidth: Integer;
begin
  Result := FWidth;
end;

function TVKProcTextureNoise.GetDepth: Integer;
begin
  Result := 1;
end;
...
function TVKProcTextureNoise.GetTextureTarget: TVKTextureTarget;
begin
  Result := ttTexture2D;
end;

procedure TVKProcTextureNoise.SetHeight(const val: Integer);
begin
  if val <> FHeight then
  begin
    FHeight := val;
...
procedure TVKProcTextureNoise.SetWidth(const val: Integer);
begin
  if val <> FWidth then
  begin
    FWidth := val;
...
procedure TVKProcTextureNoise.SetMinCut(const val: Byte);
begin
  if val <> FMinCut then
  begin
    FMinCut := val;
...
procedure TVKProcTextureNoise.SetNoiseSharpness(const val: Single);
begin
...
procedure TVKProcTextureNoise.SetNoiseRandSeed(const val: Longint);
var
  i: Integer;
  seedBackup: Longint;
  z, r, theta: Single;
...
procedure TVKProcTextureNoise.Assign(Source: TPersistent);
begin
  if Assigned(Source) and (Source is TVKProcTextureNoise) then
  begin
    FWidth := TVKProcTextureNoise(Source).FWidth;
    FHeight := TVKProcTextureNoise(Source).FHeight;
    FMinCut := TVKProcTextureNoise(Source).FMinCut;
    FNoiseSharpness := TVKProcTextureNoise(Source).FNoiseSharpness;
    FNoiseRandSeed := TVKProcTextureNoise(Source).FNoiseRandSeed;
    Invalidate;
  end
  else
    inherited;
...
procedure TVKProcTextureNoise.NoiseAnimate(speed: Single);
begin
  FNoiseAnimate := FNoiseAnimate + speed;
  Invalidate;
end;

function TVKProcTextureNoise.Noise(x, y: Single): Single;
var
  ix, iy, iz: Integer;
...
procedure TVKProcTextureNoise.SetPermFromData(inPERM: array of Byte);
var
  I: Integer;
begin
  for I := 0 to 255 do
...
procedure TVKProcTextureNoise.SetPermToDefault;
begin
