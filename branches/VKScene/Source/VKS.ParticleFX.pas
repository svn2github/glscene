...
uses
  ...
  VKS.Scene, Winapi.OpenGL, Winapi.OpenGLext,  VKS.CrossPlatform, VKS.State,
  VKS.VectorTypes, VKS.PersistentClasses, VKS.VectorGeometry,
  VKS.XCollection, VKS.Material, VKS.Cadencer, VKS.VectorLists,
  VKS.Graphics, VKS.Context, VKS.Color, VKS.BaseClasses,
  VKS.Coordinates, VKS.RenderContextInfo, VKS.Manager, VKS.TextureFormat;
...
type
  ...
  TVKParticleList = class;
  TVKParticleFXManager = class;
  TVKParticleFXEffect = class;
  ...
  TVKParticleClass = class of TVKParticle;
  TVKParticleArray = array[0..MaxInt shr 4] of TVKParticle;
  PGLParticleArray = ^TVKParticleArray;
  ...
  TVKParticleFXRenderer = class;
  TPFXCreateParticleEvent = procedure(Sender: TObject; aParticle: TVKParticle) of object;
  ...
  // TVKSourcePFXVelocityMode
  //
  TVKSourcePFXVelocityMode = (svmAbsolute, svmRelative);

  // TVKSourcePFXPositionMode
  //
  TVKSourcePFXPositionMode = (spmAbsoluteOffset, spmRelative);

  // TVKSourcePFXDispersionMode
  //
  TVKSourcePFXDispersionMode = (sdmFast, sdmIsotropic);
  ...
  private
    ...
    FVelocityMode: TVKSourcePFXVelocityMode;
    FPositionMode: TVKSourcePFXPositionMode;
    FDispersionMode: TVKSourcePFXDispersionMode;
    FEnabled: Boolean;
  ...
  published
    ...
    property PositionDispersionRange: TVKCoordinates read FPositionDispersionRange write SetPositionDispersionRange;
    property ParticleInterval: Single read FParticleInterval write SetParticleInterval;
    property VelocityMode: TVKSourcePFXVelocityMode read FVelocityMode write FVelocityMode default svmAbsolute;
    property PositionMode: TVKSourcePFXPositionMode read FPositionMode write FPositionMode default spmAbsoluteOffset;
    property DispersionMode: TVKSourcePFXDispersionMode read FDispersionMode write FDispersionMode default sdmFast;
  ...
  TPFXParticleProgress = procedure(Sender: TObject; const progressTime: TProgressTimes;
    aParticle: TVKParticle; var killParticle: Boolean) of object;
  ...
  private
    ...
    FAspectRatio: Single;
    FRotation: Single;
    FShareSprites: TVKBaseSpritePFXManager;
  ...
  //
  TPFXPrepareTextureImageEvent = procedure(Sender: TObject; destBmp32: TVKBitmap32; var texFormat: Integer) of object;
...
begin
  ...
  begin
    if name = '' then
    begin
      i := IndexOfClass(TVKSourcePFXEffect);
      if i >= 0 then
        Result := TVKSourcePFXEffect(Items[i])
      else
        Result := TVKSourcePFXEffect.Create(obj.Effects);
    end
    else
    begin
      i := IndexOfName(name);
      if i >= 0 then
        Result := (Items[i] as TVKSourcePFXEffect)
      else
      begin
        Result := TVKSourcePFXEffect.Create(obj.Effects);
        Result.Name := name;
...
procedure RndVector(const dispersion: TVKSourcePFXDispersionMode;
  var v: TAffineVector; var f: Single;
  dispersionRange: TVKCoordinates);
var
  f2, fsq: Single;
  p: TVector;
begin
...
// Create
//
constructor TVKParticle.Create;
begin
  FEffectScale := 1;
  inherited Create;
end;
...
// Destroy
//
destructor TVKParticle.Destroy;
begin
  inherited Destroy;
end;

function TVKParticle.GetPosition(const Index: Integer): Single;
begin
  Result := FPosition.V[Index];
end;

procedure TVKParticle.WritePosition(const Index: Integer; const aValue: Single);
begin
...
// WriteToFiler
//
procedure TVKParticle.WriteToFiler(writer: TVirtualWriter);
begin
...
procedure TVKParticle.ReadFromFiler(reader: TVirtualReader);
var
...
constructor TVKParticleList.Create;
begin
  inherited Create;
...
destructor TVKParticleList.Destroy;
begin
  FItemList.CleanFree;
  inherited Destroy;
end;
...
procedure TVKParticleList.WriteToFiler(writer: TVirtualWriter);
begin
  inherited WriteToFiler(writer);
  with writer do
  begin
...
procedure TVKParticleList.ReadFromFiler(reader: TVirtualReader);
var
  archiveVersion: integer;
begin
  inherited ReadFromFiler(reader);
...
function TVKParticleList.GetItems(index: Integer): TVKParticle;
begin
  Result := TVKParticle(FItemList[index]);
end;

// SetItems
//

procedure TVKParticleList.SetItems(index: Integer; val: TVKParticle);
begin
  FItemList[index] := val;
end;
...
procedure TVKParticleList.AfterItemCreated(Sender: TObject);
begin
  (Sender as TVKParticle).Manager := Self.Owner;
end;

// ItemCount
//

function TVKParticleList.ItemCount: Integer;
begin
...
function TVKParticleList.AddItem(aItem: TVKParticle): Integer;
begin
...
procedure TVKParticleList.RemoveAndFreeItem(aItem: TVKParticle);
var
  i: Integer;
begin
...
function TVKParticleList.IndexOfItem(aItem: TVKParticle): Integer;
begin
...
procedure TVKParticleList.Pack;
begin
  FItemList.Pack;
  FDirectList := PGLParticleArray(FItemList.List);
end;
...
constructor TVKParticleFXManager.Create(aOwner: TComponent);
begin
  inherited;
  FUsers := TList.create;
  FParticles := TVKParticleList.Create;
...
destructor TVKParticleFXManager.Destroy;
var
  i: integer;
begin
  inherited Destroy;
  for i := FUsers.Count - 1 downto 0 do
    TVKParticleFXEffect(FUsers[i]).managerNotification(self, opRemove);
...
procedure TVKParticleFXManager.NotifyChange(Sender: TObject);
begin
...
procedure TVKParticleFXManager.DoProgress(const progressTime: TProgressTimes);
begin
  inherited;
...
class function TVKParticleFXManager.ParticlesClass: TVKParticleClass;
begin
  Result := TVKParticle;
end;
...
function TVKParticleFXManager.CreateParticle: TVKParticle;
begin
...
procedure TVKParticleFXManager.CreateParticles(nbParticles: Integer);
var
  i: Integer;
begin
...
procedure TVKParticleFXManager.SetRenderer(const val: TVKParticleFXRenderer);
begin
...
procedure TVKParticleFXManager.SetParticles(const aParticles: TVKParticleList);
begin
...
function TVKParticleFXManager.ParticleCount: Integer;
begin
...
procedure TVKParticleFXManager.ApplyBlendingMode;
begin
...
procedure TVKParticleFXManager.UnapplyBlendingMode;
begin
  if Renderer.BlendingMode <> BlendingMode then
  begin
    // case disjunction to minimize Vulkan State changes
...
procedure TVKParticleFXManager.registerUser(obj: TVKParticleFXEffect);
begin
...
procedure TVKParticleFXManager.unregisterUser(obj: TVKParticleFXEffect);
begin
  FUsers.Remove(obj);
end;
...
destructor TVKParticleFXEffect.Destroy;
begin
  Manager := nil;
  inherited Destroy;
end;
...
procedure TVKParticleFXEffect.WriteToFiler(writer: TWriter);
var
  st: string;
begin
  with writer do
...
procedure TVKParticleFXEffect.ReadFromFiler(reader: TReader);
var
  archiveVersion: integer;
begin
  with reader do
...
procedure TVKParticleFXEffect.Loaded;
var
  mng: TComponent;
begin
  inherited;
  ...
  begin
    mng := FindManager(TVKParticleFXManager, FManagerName);
    if Assigned(mng) then
      Manager := TVKParticleFXManager(mng);
    FManagerName := '';
  end;
end;
...
procedure TVKParticleFXEffect.SetManager(val: TVKParticleFXManager);
begin
...
procedure TVKParticleFXEffect.SetEffectScale(const Value: single);
begin
  FEffectScale := Value;
end;
...
procedure TVKParticleFXEffect.managerNotification(
  aManager: TVKParticleFXManager; Operation: TOperation);
begin
...
constructor TVKParticleFXRenderer.Create(aOwner: TComponent);
begin
  inherited;
...
destructor TVKParticleFXRenderer.Destroy;
var
  i: Integer;
begin
...
procedure TVKParticleFXRenderer.RegisterManager(aManager: TVKParticleFXManager);
begin
...
procedure TVKParticleFXRenderer.UnRegisterManager(aManager: TVKParticleFXManager);
begin
...
procedure TVKParticleFXRenderer.UnRegisterAll;
begin
  while FManagerList.Count > 0 do
    TVKParticleFXManager(FManagerList[FManagerList.Count - 1]).Renderer := nil;
end;
...
var
  ...
  curManager: TVKParticleFXManager;
  curList: PGLParticleArray;
  curParticle: TVKParticle;
...
begin
  ...
  try
    ...
    begin
      curManager := TVKParticleFXManager(FManagerList[managerIdx]);
    ...
    try
      // Initialize managers
      for managerIdx := 0 to FManagerList.Count - 1 do
        TVKParticleFXManager(FManagerList.Items[managerIdx]).InitializeRendering(rci);
      ...
      try
        ...
        // Finalize managers
        for managerIdx := 0 to FManagerList.Count - 1 do
          TVKParticleFXManager(FManagerList.Items[managerIdx]).FinalizeRendering(rci);
      end;
    finally
...
function TVKParticleFXRenderer.StoreZMaxDistance: Boolean;
begin
...
begin
  ...
  FInitialPosition := TVKCoordinates.CreateInitialized(Self, NullHmgVector, csPoint);
  FPositionDispersionRange := TVKCoordinates.CreateInitialized(Self, XYZHmgVector, csPoint);
...
destructor TVKSourcePFXEffect.Destroy;
begin
...
class function TVKSourcePFXEffect.FriendlyName: string;
begin
  Result := 'PFX Source';
end;
...
class function TVKSourcePFXEffect.FriendlyDescription: string;
begin
...
procedure TVKSourcePFXEffect.WriteToFiler(writer: TWriter);
begin
  inherited;
  with writer do
  begin
...
procedure TVKSourcePFXEffect.ReadFromFiler(reader: TReader);
var
  archiveVersion: Integer;
begin
  inherited;
  ...
  begin
    ...
    FVelocityMode := TVKSourcePFXVelocityMode(ReadInteger);
    if archiveVersion >= 1 then
      FDispersionMode := TVKSourcePFXDispersionMode(ReadInteger);
    ...
    if archiveVersion >= 6 then
      FPositionMode := TVKSourcePFXPositionMode(ReadInteger);
  end;
end;
...
procedure TVKSourcePFXEffect.SetInitialVelocity(const val: TVKCoordinates);
begin
...
procedure TVKSourcePFXEffect.SetInitialPosition(const val: TVKCoordinates);
begin
...
procedure TVKSourcePFXEffect.SetPositionDispersionRange(const val: TVKCoordinates);
begin
...
procedure TVKSourcePFXEffect.SetParticleInterval(const val: Single);
begin
...
procedure TVKSourcePFXEffect.DoProgress(const progressTime: TProgressTimes);
var
  n: Integer;
begin
...
function TVKSourcePFXEffect.ParticleAbsoluteInitialPos: TAffineVector;
begin
...
procedure TVKSourcePFXEffect.Burst(time: Double; nb: Integer);
...
begin
  ...
  //   if FManager is TVKDynamicPFXManager then
  //     TVKDynamicPFXManager(FManager).FRotationCenter := pos;
...
procedure TVKSourcePFXEffect.RingExplosion(time: Double;
  minInitialSpeed, maxInitialSpeed: Single;
  nbParticles: Integer);
var
  particle: TVKParticle;
...
begin
  ...
  FColorInner := TVKColor.CreateInitialized(Self, NullHmgVector);
  FColorOuter := TVKColor.CreateInitialized(Self, NullHmgVector);
  FLifeTime := 1;
  FInvLifeTime := 1;
  FSizeScale := 1;
  FRotateAngle := 0;
...
procedure TPFXLifeColor.SetColorInner(const val: TVKColor);
begin
...
procedure TPFXLifeColor.SetColorOuter(const val: TVKColor);
begin
...
constructor TVKDynamicPFXManager.Create(aOwner: TComponent);
begin
  inherited;
...
destructor TVKDynamicPFXManager.Destroy;
begin
  FAcceleration.Free;
  inherited Destroy;
end;
...
procedure TVKDynamicPFXManager.DoProgress(const progressTime: TProgressTimes);
var
  i: Integer;
  curParticle: TVKParticle;
  maxAge: Double;
...
procedure TVKDynamicPFXManager.SetAcceleration(const val: TVKCoordinates);
begin
...
constructor TVKLifeColoredPFXManager.Create(aOwner: TComponent);
begin
  inherited;
  FLifeColors := TPFXLifeColors.Create(Self);
  FColorInner := TVKColor.CreateInitialized(Self, clrYellow);
  FColorOuter := TVKColor.CreateInitialized(Self, NullHmgVector);
...
destructor TVKLifeColoredPFXManager.Destroy;
begin
  FLifeColors.Free;
  FColorInner.Free;
  FColorOuter.Free;
...
procedure TVKLifeColoredPFXManager.SetColorInner(const val: TVKColor);
begin
  FColorInner.Assign(val);
end;
...
procedure TVKLifeColoredPFXManager.SetColorOuter(const val: TVKColor);
begin
  FColorOuter.Assign(val);
end;
...
procedure TVKLifeColoredPFXManager.SetLifeColors(const val: TPFXLifeColors);
begin
...
function TVKLifeColoredPFXManager.MaxParticleAge: Single;
begin
...
procedure TVKLifeColoredPFXManager.ComputeColors(var lifeTime: Single; var inner, outer: TColorVector);
var
  i, k, n: Integer;
  f: Single;
...
procedure TVKLifeColoredPFXManager.ComputeInnerColor(var lifeTime: Single; var inner: TColorVector);
var
  i, k, n: Integer;
  f: Single;
  lck, lck1: TPFXLifeColor;
...
procedure TVKLifeColoredPFXManager.ComputeOuterColor(var lifeTime: Single; var outer: TColorVector);
var
  i, k, n: Integer;
  f: Single;
  lck, lck1: TPFXLifeColor;
...
function TVKLifeColoredPFXManager.ComputeSizeScale(var lifeTime: Single; var sizeScale: Single): Boolean;
var
  i, k, n: Integer;
  f: Single;
  lck, lck1: TPFXLifeColor;
...
function TVKLifeColoredPFXManager.ComputeRotateAngle(var lifeTime: Single; var rotateAngle: Single): Boolean;
var
  i, k, n: Integer;
  f: Single;
  lck, lck1: TPFXLifeColor;
...
procedure TVKLifeColoredPFXManager.RotateVertexBuf(buf: TAffineVectorList;
...
procedure TVKCustomPFXManager.DoProgress(const progressTime: TProgressTimes);
var
  i: Integer;
  list: PGLParticleArray;
  curParticle: TVKParticle;
...
function TVKCustomPFXManager.TexturingMode: Cardinal;
begin
  Result := 0;
end;
...
function TVKCustomPFXManager.ParticleCount: Integer;
begin
...
constructor TVKPolygonPFXManager.Create(aOwner: TComponent);
begin
  inherited;
  FNbSides := 6;
end;
...
destructor TVKPolygonPFXManager.Destroy;
begin
  inherited Destroy;
end;
...
procedure TVKPolygonPFXManager.SetNbSides(const val: Integer);
begin
...
function TVKPolygonPFXManager.TexturingMode: Cardinal;
begin
  Result := 0;
end;
...
constructor TVKBaseSpritePFXManager.Create(aOwner: TComponent);
begin
  inherited;
...
destructor TVKBaseSpritePFXManager.Destroy;
begin
  FTexHandle.Free;
  FShareSprites := nil;
  inherited Destroy;
...
procedure TVKBaseSpritePFXManager.SetSpritesPerTexture(const val: TSpritesPerTexture);
begin
...
procedure TVKBaseSpritePFXManager.SetColorMode(const val: TSpriteColorMode);
begin
...
procedure TVKBaseSpritePFXManager.SetAspectRatio(const val: Single);
begin
...
function TVKBaseSpritePFXManager.StoreAspectRatio: Boolean;
begin
...
procedure TVKBaseSpritePFXManager.SetRotation(const val: Single);
begin
...
procedure TVKBaseSpritePFXManager.SetShareSprites(const val: TVKBaseSpritePFXManager);
begin
...
function TVKBaseSpritePFXManager.TexturingMode: Cardinal;
begin
...
// ------------------
// ------------------ TVKCustomSpritePFXManager ------------------
// ------------------

// Create
//

constructor TVKCustomSpritePFXManager.Create(aOwner: TComponent);
begin
  inherited;
  FColorMode := scmInner;
  FSpritesPerTexture := sptOne;
...
destructor TVKCustomSpritePFXManager.Destroy;
begin
  inherited Destroy;
end;
...
procedure TVKCustomSpritePFXManager.PrepareImage(bmp32: TVKBitmap32; var texFormat: Integer);
begin
...
constructor TVKPointLightPFXManager.Create(aOwner: TComponent);
begin
  inherited;
  FTexMapSize := 5;
  FColorMode := scmInner;
...
destructor TVKPointLightPFXManager.Destroy;
begin
  inherited Destroy;
end;
...
procedure TVKPointLightPFXManager.SetTexMapSize(const val: Integer);
begin
...
procedure TVKPointLightPFXManager.PrepareImage(bmp32: TVKBitmap32; var texFormat: Integer);
var
  s: Integer;
  x, y, d, h2: Integer;
  ih2, f, fy: Single;
...
initialization
  ...
  // ------------------------------------------------------------------
     ...
     // class registrations
  RegisterClasses([TVKParticle, TVKParticleList,
    TVKParticleFXEffect, TVKParticleFXRenderer,
      TVKCustomPFXManager,
      TVKPolygonPFXManager,
      TVKCustomSpritePFXManager,
      TVKPointLightPFXManager]);
  RegisterXCollectionItemClass(TVKSourcePFXEffect);

finalization

  UnregisterXCollectionItemClass(TVKSourcePFXEffect);
