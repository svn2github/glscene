...
type
  ...
  TOnOpenGLInteropInit = procedure(out Context: TVKContext) of object;
...
constructor TVKSCUDADevice.Create(AOwner: TComponent);
var
  LDevice: TCUDADevice;
begin
  inherited Create(AOwner);
...
destructor TVKSCUDADevice.Destroy;
var
  Device: TCUDADevice;
begin
  inherited;
...
function TVKSCUDADevice.GetDevice: TCUDADevice;
begin
  Result := CUDAContextManager.GetDeviceByName(FSelectDeviceName);
end;

function TVKSCUDADevice.Suitable: Boolean;
var
  LDevice: TCUDADevice;
begin
  LDevice := GetDevice;
...
procedure TVKSCUDADevice.SetDevice(AValue: TCUDADevice);
begin
end;

procedure TVKSCUDADevice.SetDeviceName(const AName: string);
begin
...
// ------------------------------------------------------------------
  ...
  RegisterClasses([TVKSCUDADevice]);
