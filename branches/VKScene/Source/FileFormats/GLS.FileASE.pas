...
type
  ...
  private
    ...
    FSmoothing: TVKASESmoothingGroups;
    FSubMaterialID: Integer;                    // submaterial ID
    FTextChannels: TVKASEFaceTexureChannels;
  public
    constructor Create;
    ...
    property TextChannels: TVKASEFaceTexureChannels read FTextChannels; // channels texture coordinates
    property Smoothing: TVKASESmoothingGroups read FSmoothing;   // smoothing group list
  ...
  private
    FItems: TList;
    function GetFace(Index: Integer): TVKASEFace;
    function GetCount: Integer;
  public
    constructor Create;
    destructor Destroy; override;
    function Add: TVKASEFace;
    procedure Delete(aIndex: Integer);
    procedure Clear;
    property Face[Index: Integer]: TVKASEFace read GetFace; default;
  ...
  public
    constructor Create;
    destructor Destroy; override;
    property Faces: TVKASEFaceList read FFaces;
  ...
  TVKASEMaterialTextureMap = record
    Kind: string;
    Name: string;
    _Class: string;
    No: Integer;
  ...
  TVKASEMaterialTextureMaps = record
    Map: array [0..GL_ASE_MAX_TEXTURE_MAPS - 1] of TVKASEMaterialTextureMap;
    Count: Integer;
  end;

  TVKASESubMaterial = record
    Name: string;
    ...
    TextureMaps: TVKASEMaterialTextureMaps;
  end;

  TVKASESubMaterialList = record
    SubMaterial: array [0..GL_ASE_MAX_SUBMATERIALS - 1] of TVKASESubMaterial;
    Count: Integer;
  end;

  TVKASEMaterial = class(TObject)
  private
    FWireSize: Single;
    ...
    FSubMaterials: TVKASESubMaterialList;
    FTextureMaps: TVKASEMaterialTextureMaps;
  public
    constructor Create;
    ...
    property TextureMaps: TVKASEMaterialTextureMaps read FTextureMaps;
    property SubMaterials: TVKASESubMaterialList read FSubMaterials;
  end;

  TVKASEMaterialList = class(TObject)
  private
    FItems: TList;
    function GetCount: Integer;
    function GetMaterial(Index: Integer): TVKASEMaterial;
  public
    constructor Create;
    destructor Destroy; override;
    function Add: TVKASEMaterial;
    procedure Delete(aIndex: Integer);
    procedure Clear;
    property Material[Index: Integer]: TVKASEMaterial read GetMaterial; default;
    property Count: Integer read GetCount;
  end;
  ...
  private
    ...
    FCastShadow: Boolean;
    FMotionBlur: Boolean;
    FMaterialList: TVKASEMaterialList;
    ...
    procedure ParseFaceString(const aData: string; var Index, A, B, C, AB, BC, CA, MatID: Integer; var Smooth: TVKASESmoothingGroups);
  private
    procedure ParseScene(var aLineIndex: Integer);
    procedure ParseGeomObject(var aLineIndex: Integer);
    procedure ParseMeshOptions(var aLineIndex: Integer; aMesh: TVKASEMeshObject);
    procedure ParseMeshGeom(var aLineIndex: Integer; aMesh: TVKASEMeshObject);
    procedure ParseMappingChannel(var aLineIndex: Integer; aMesh: TVKASEMeshObject);
    procedure ParseMeshVertices(var aLineIndex: Integer; aMesh: TVKASEMeshObject; VerticesCount: Integer);
    procedure ParseMeshFaces(var aLineIndex: Integer; aMesh: TVKASEMeshObject; FacesCount: Integer);
    procedure ParseMeshNormals(var aLineIndex: Integer; aMesh: TVKASEMeshObject; FacesCount: Integer);
    procedure ParseMeshTextureVertices(var aLineIndex: Integer; aMesh: TVKASEMeshObject; TextureVerticesCount: Integer);
    procedure ParseMeshTextureFaces(var aLineIndex: Integer; aMesh: TVKASEMeshObject; TextureFacesCount: Integer);
    procedure ParseMaterialList(var aLineIndex: Integer);
    procedure ParseMaterial(var aLineIndex: Integer; aMaterial: TVKASEMaterial);
    procedure ParseSubMaterial(var aLineIndex: Integer; aMaterial: TVKASEMaterial);
    function CheckTextureMap(var aLineIndex: Integer; var aMaps: TVKASEMaterialTextureMaps): Boolean;
    procedure ParseTextureMap(var aLineIndex: Integer; var aMaps: TVKASEMaterialTextureMaps; const aMapKind: string);
...
procedure CopyASEToMesh(aASEMesh: TVKASEMeshObject; aMesh: TVKMeshObject; aASEMaterials: TVKASEMaterialList);
  ...
  function FindTextureMap(aMaterial: TVKASEMaterial; aMapKind: string; aSubmaterialIndex: Integer;
    out TM: TVKASEMaterialTextureMap): Boolean;

    function FindInMaps(const aMaps: TVKASEMaterialTextureMaps): Boolean;
    var
      i: Integer;
    begin
      Result := False;
  ...
  var
    material : TVKASEMaterial;
    specColor : TVector;
    matLib : TVKMaterialLibrary;
    libMat : TVKLibMaterial;
    TM: TVKASEMaterialTextureMap;
  begin
...
constructor TVKASEFaceList.Create;
begin
  FItems := TList.Create;
end;

destructor TVKASEFaceList.Destroy;
begin
  Clear;
  FreeAndNil(FItems);
  inherited;
...
function TVKASEFaceList.Add: TVKASEFace;
begin
  Result := TVKASEFace.Create;
  FItems.Add(Result);
end;

procedure TVKASEFaceList.Delete(aIndex: Integer);
begin
  Face[aIndex].Free;
  FItems.Delete(aIndex);
end;

procedure TVKASEFaceList.Clear;
var
  i: Integer;
begin
...
function TVKASEFaceList.GetFace(Index: Integer): TVKASEFace;
begin
  Result := FItems[Index];
end;

function TVKASEFaceList.GetCount: Integer;
begin
  Result := FItems.Count;
end;
...
constructor TVKASEMaterial.Create;
begin
...
constructor TVKASEMeshObject.Create;
begin
  FFaces := TVKASEFaceList.Create;
...
destructor TVKASEMeshObject.Destroy;
var
  i: Integer;
begin
  FreeAndNil(FFaces);
...
function TVKASEMeshObject.AddTexChannel: TAffineVectorList;
begin
...
function TVKASEMeshObject.GetTextChannel(Channel: Integer): TAffineVectorList;
begin
...
constructor TVKASEMaterialList.Create;
begin
  FItems := TList.Create;
end;

destructor TVKASEMaterialList.Destroy;
begin
  Clear;
  FreeAndNil(FItems);
  inherited;
...
function TVKASEMaterialList.Add: TVKASEMaterial;
begin
  Result := TVKASEMaterial.Create;
  FItems.Add(Result);
end;

procedure TVKASEMaterialList.Delete(aIndex: Integer);
begin
  Material[aIndex].Free;
  FItems.Delete(aIndex);
end;

procedure TVKASEMaterialList.Clear;
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
...
function TVKASEMaterialList.GetMaterial(Index: Integer): TVKASEMaterial;
begin
  Result := FItems[Index];
end;

function TVKASEMaterialList.GetCount: Integer;
begin
  Result := FItems.Count;
end;
...
constructor TVKASEVectorFile.Create;
begin
  inherited;
  FStringData := TStringList.Create;
  FMaterialList := TVKASEMaterialList.Create;
end;

destructor TVKASEVectorFile.Destroy;
begin
  FMaterialList.Free;
  FStringData.Free;
  inherited;
...
procedure TVKASEVectorFile.LoadFromStream(aStream: TStream);
begin
  FStringData.LoadFromStream(aStream);
  Parse;
end;

function TVKASEVectorFile.ContainString(const aData, aString: string): Boolean;
begin
  Result := (Length(aData) >= Length(aString)) and (Pos(AString, aData) > 0);
end;

function TVKASEVectorFile.GetDoubleValue(const aData: string): Double;
var
  Start: Integer;
  Data: string;
  Value: string;
...
function TVKASEVectorFile.GetEndOfFirstValue(const aData: string): Integer;
var
  Value: string;
begin
...
function TVKASEVectorFile.GetFirstValue(aData: string): string;
const
  sep1 = #32;
var
  Index: Integer;
...
function TVKASEVectorFile.GetPropCastShadow(const aData: string): Boolean;
begin
  Result := Pos('1', aData) > 0;
end;

function TVKASEVectorFile.GetPropMBlur(const aData: string): Boolean;
begin
  Result := Pos('1', aData) > 0;
end;

function TVKASEVectorFile.GetPropRECVShadow(const aData: string): Boolean;
begin
  Result := Pos('1', aData) > 0;
end;

function TVKASEVectorFile.GetStringValue(const aData: string): string;
const
  Breaker = '"';
var
  Start: Integer;
...
function TVKASEVectorFile.GetTagOnData(const aData: string): Integer;
var
  Index: Integer;
begin
  Result := -1;
...
function TVKASEVectorFile.GetValue3D(const aData: string): TAffineVector;
var
  Start: Integer;
  Data: string;
  Value: String;
...
function TVKASEVectorFile.GetValue4D(const aData: string; var Value0: Integer): TAffineVector;
var
  Start: Integer;
  Data: string;
  Value: String;
...
function TVKASEVectorFile.IsEndOfSection(const aData: string): Boolean;
begin
...
function TVKASEVectorFile.IsSectionBegingin(const aData: string): Boolean;
begin
  Result := Pos('{', aData) > 0;
end;

function TVKASEVectorFile.CheckUnknownData(var aLineIndex: Integer): Boolean;
begin
...
procedure TVKASEVectorFile.SkipSection(var aLineIndex: Integer);
var
  Data: string;
begin
  Inc(aLineIndex);
...
procedure TVKASEVectorFile.Parse;
var
  LineIndex: Integer;
  Data: string;
  b: Boolean;
...
procedure TVKASEVectorFile.ParseFaceString(const aData: string; var Index, A, B, C, AB, BC, CA, MatID: Integer;
  var Smooth: TVKASESmoothingGroups);
var
  Data: string;
  Start: Integer;
  Value: string;
  ...
  procedure GetNextValues(var aValues: TVKASESmoothingGroups);
...
procedure TVKASEVectorFile.ParseMeshFaces(var aLineIndex: Integer; aMesh: TVKASEMeshObject; FacesCount: Integer);
var
  Data: string;
  Index, A, B, C, AB, BC, CA, ID: Integer;
  face: TVKASEFace;
  smooth: TVKASESmoothingGroups;
begin
  Inc(aLineIndex);
  Data := FStringData[aLineIndex];
  while not IsEndOfSection(Data) do begin
...
procedure TVKASEVectorFile.ParseMeshNormals(var aLineIndex: Integer; aMesh: TVKASEMeshObject; FacesCount: Integer);
var
  Data: string;
  Index: Integer;
  Counter: Integer;
...
procedure TVKASEVectorFile.ParseMeshTextureFaces(var aLineIndex: Integer; aMesh: TVKASEMeshObject;
...
procedure TVKASEVectorFile.ParseMeshTextureVertices(var aLineIndex: Integer; aMesh: TVKASEMeshObject;
...
procedure TVKASEVectorFile.ParseMeshGeom(var aLineIndex: Integer; aMesh: TVKASEMeshObject);
var
  Data: string;
...
procedure TVKASEVectorFile.ParseMeshVertices(var aLineIndex: Integer; aMesh: TVKASEMeshObject;
  VerticesCount: Integer);
var
  Data: string;
  VertIndex: Integer;
...
procedure TVKASEVectorFile.ParseMappingChannel(var aLineIndex: Integer; aMesh: TVKASEMeshObject);
var
  Data: string;
  TextureVerticesCount: Integer;
  TextureFacesCount: Integer;
...
procedure TVKASEVectorFile.ParseMeshOptions(var aLineIndex: Integer; aMesh: TVKASEMeshObject);
var
  Data: string;
  b: Boolean;
begin
...
procedure TVKASEVectorFile.ParseScene(var aLineIndex: Integer);
begin
  CheckUnknownData(aLineIndex);
end;

procedure TVKASEVectorFile.ParseMaterialList(var aLineIndex: Integer);
var
  Data: string;
  material: TVKASEMaterial;
begin
...
function TVKASEVectorFile.CheckTextureMap(var aLineIndex: Integer; var aMaps: TVKASEMaterialTextureMaps): Boolean;
var
  Data: string;
begin
  Result := True;
...
procedure TVKASEVectorFile.ParseMaterial(var aLineIndex: Integer; aMaterial: TVKASEMaterial);
var
  Data: string;
  b: Boolean;
begin
...
procedure TVKASEVectorFile.ParseSubMaterial(var aLineIndex: Integer; aMaterial: TVKASEMaterial);
var
  Data: string;
  Index: Integer;
  b: Boolean;
...
procedure TVKASEVectorFile.ParseTextureMap(var aLineIndex: Integer; var aMaps: TVKASEMaterialTextureMaps; const aMapKind: string);
var
  Data: string;
  Index: Integer;
  b: Boolean;
...
initialization
  RegisterVectorFileFormat('ase', 'ASCII files', TVKASEVectorFile);
