...
{: VKS.VfsPAK<p>
   ...
   VKS.ApplicationFileIO mechanism will be pointed into PAK file.<p>
   You can change current PAK file by ActivePak variable.<p>
	...
	<b>History :</b><font size=-1><ul>
      ...
      <li>26/10/2006 - LC - Fixed memory leak in TVKVfsPAK.LoadFromFile. Bugtracker ID=1585215 (thanks Da Stranger)
...
uses
  System.Classes, System.Contnrs, System.SysUtils, VKS.ApplicationFileIO
...
procedure TVKVfsPAK.SetStreamNumber(i:integer);
begin
   FStream:=TFileStream(FStreamList[i]);
end;
...
function TVKVfsPAK.GetFileCount: integer;
begin
   Result := FHeader.DirLength div SizeOf(TFileSection);
end;

procedure TVKVfsPAK.MakeFileList;
var
   I: integer;
begin
...
procedure TVKVfsPAK.LoadFromFile(FileName: string; Mode: word);
var
   l: integer;
begin
   FFileName := FileName;
...
procedure TVKVfsPAK.ClearPakFiles;
begin
   SetLength(FHeaderList, 0);
   FPakFiles.Clear;
   // Objects are automatically freed by TObjectList
...
function TVKVfsPAK.GetFile(index: integer): TStream;
...
function TVKVfsPAK.FileExists(FileName: string): boolean;
begin
   Result := (FFiles.IndexOf(FileName) > -1);
end;

function TVKVfsPAK.GetFile(FileName: string): TStream;
begin
   Result := nil;
...
function TVKVfsPAK.GetFileSize(index: integer): integer;
begin
   FStream.Seek(FHeader.DirOffset + SizeOf(TFileSection) * index, soFromBeginning);
...
function TVKVfsPAK.GetFileSize(FileName: string): integer;
begin
   Result := -1;
   if Self.FileExists(FileName) then
...
{$WARNINGS OFF}
procedure TVKVfsPAK.AddFromStream(FileName, Path: string; F: TStream);
var
...
procedure TVKVfsPAK.AddFromFile(FileName, Path: string);
var
   F: TFileStream;
begin
...
procedure TVKVfsPAK.AddEmptyFile(FileName, Path: string);
var
   F: TMemoryStream;
begin
...
procedure TVKVfsPAK.RemoveFile(index: integer);
var
   Temp: TMemoryStream;
   i:    integer;
   f:    TFileSection;
...
procedure TVKVfsPAK.RemoveFile(FileName: string);
begin
   if Self.FileExists(FileName) then
      RemoveFile(FFiles.IndexOf(FileName));
end;

procedure TVKVfsPAK.Extract(index: integer; NewName: string);
var
   s: TFileStream;
begin
   if NewName = '' then
...
procedure TVKVfsPAK.Extract(FileName, NewName: string);
begin
   if Self.FileExists(FileName) then
