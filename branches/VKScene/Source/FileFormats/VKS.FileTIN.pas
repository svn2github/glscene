...
uses
  VKS.VectorGeometry, VKS.Utils, VKS.Types;
// ------------------
// ------------------ TVKTINVectorFile ------------------
// ------------------
...
// LoadFromStream
//
procedure TVKTINVectorFile.LoadFromStream(aStream : TStream);
var
   i, j : Integer;
   sl, tl : TStringList;
   mesh : TVKMeshObject;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if tl.Count = 9 then
        begin
            SetVector(v1, VKS.Utils.StrToFloatDef(tl[0],0), VKS.Utils.StrToFloatDef(tl[1],0), VKS.Utils.StrToFloatDef(tl[2],0));
            SetVector(v2, VKS.Utils.StrToFloatDef(tl[3],0), VKS.Utils.StrToFloatDef(tl[4],0), VKS.Utils.StrToFloatDef(tl[5],0));
            SetVector(v3, VKS.Utils.StrToFloatDef(tl[6],0), VKS.Utils.StrToFloatDef(tl[7],0), VKS.Utils.StrToFloatDef(tl[8],0));
...
// ------------------------------------------------------------------
   ...
   RegisterVectorFileFormat('tin', 'Triangular Irregular Network', TVKTINVectorFile);
