...
uses
  ...
  VKS.CrossPlatform, VKS.OpenGLTokens, VKS.Context, VKS.Graphics, VKS.TextureFormat,
  VKS.RGBE, VKS.ApplicationFileIO, VKS.VectorGeometry, VKS.Strings;
...
begin
  ...
  begin
    ...
    raise EInvalidRasterFile.CreateFmt(vksFileNotFound, [filename]);
end;
...
procedure TVKDDSImage.SaveToStream(stream: TStream);
const
...
procedure TVKDDSImage.AssignFromTexture(textureContext: TVKContext;
  const textureHandle: TGLuint;
  textureTarget: TVKTextureTarget;
  const CurrentFormat: Boolean;
...
procedure TVKDDSImage.flipSurface(chgData: PGLubyte; w, h, d: integer);
var
  lineSize: integer;
  sliceSize: integer;
  tempBuf: PGLubyte;
...
initialization
  ...
  RegisterRasterFormat('dds', 'Direct Draw Surface', TVKDDSImage);
