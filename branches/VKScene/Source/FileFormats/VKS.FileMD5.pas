...
procedure TVKMD5VectorFile.LoadFromStream(aStream: TStream);
  ...
  begin
    ...
    pos.X := VKS.Utils.StrToFloatDef(FTempString[2]);
    pos.Y := VKS.Utils.StrToFloatDef(FTempString[4]);
    pos.Z := VKS.Utils.StrToFloatDef(FTempString[3]);

    quat := QuaternionMakeFromImag(VKS.Utils.StrToFloatDef(FTempString[5]),
      VKS.Utils.StrToFloatDef(FTempString[7]),
      VKS.Utils.StrToFloatDef(FTempString[6]));
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            mesh.TexCoords[vnum] :=
              AffineVectorMake(VKS.Utils.StrToFloatDef(FTempString[2]),
              1 - VKS.Utils.StrToFloatDef(FTempString[3]), 0);
        ...
        begin
          ...
          begin
            ...
            VertexWeight[wnum] := VKS.Utils.StrToFloatDef(FTempString[3]);
            VertexWeighted[wnum] :=
              AffineVectorMake(VKS.Utils.StrToFloatDef(FTempString[4]),
              VKS.Utils.StrToFloatDef(FTempString[6]),
              VKS.Utils.StrToFloatDef(FTempString[5]));
          end;
        end;
      end;
    until temp = '}';
  ...
  begin
    ...
    repeat
      ...
      if FTempString.Count >= 6 then
      begin
        pos := AffineVectorMake(VKS.Utils.StrToFloatDef(FTempString[0]),
          VKS.Utils.StrToFloatDef(FTempString[1]),
          VKS.Utils.StrToFloatDef(FTempString[2]));
        quat := QuaternionMakeFromImag(VKS.Utils.StrToFloatDef(FTempString[3]),
          VKS.Utils.StrToFloatDef(FTempString[4]),
          VKS.Utils.StrToFloatDef(FTempString[5]));
        FFramePositions.Add(pos);
        FFrameQuaternions.Add(quat);
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          pos.X := VKS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 2 > 0 then
        begin
          pos.Y := VKS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 4 > 0 then
        begin
          pos.Z := VKS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;

        if FJointFlags[i] and 8 > 0 then
        begin
          quat.ImagPart.X := VKS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 16 > 0 then
        begin
          quat.ImagPart.Y := VKS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 32 > 0 then
          quat.ImagPart.Z := VKS.Utils.StrToFloatDef(FTempString[j]);
      end;
  ...
  begin
    for i := 0 to Owner.MeshObjects.Count - 1 do
      TVKSkeletonMeshObject(Owner.MeshObjects[i])
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if Owner is TVKActor then
            begin
              with TVKActor(Owner).Animations.Add do
              begin
