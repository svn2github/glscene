  ...
  try
    ...
    begin
      ...
      begin
        ...
        with Owner.Skeleton do
          if (tl[2] <> '-1') then
            bone := TVKSkeletonBone.CreateOwned
              (RootBones.BoneByID(StrToInt(tl[2])))
          else
            bone := TVKSkeletonBone.CreateOwned(RootBones);
    ...
    begin
      ...
      begin
        ...
        frame.Position.Add(VKS.Utils.StrToFloatDef(tl[1]),
          VKS.Utils.StrToFloatDef(tl[2]), VKS.Utils.StrToFloatDef(tl[3]));
        v := AffineVectorMake(VKS.Utils.StrToFloatDef(tl[4]),
          VKS.Utils.StrToFloatDef(tl[5]), VKS.Utils.StrToFloatDef(tl[6]));
          ...
          begin
            ...
            begin
              ...
              nVert := FindOrAdd(boneIDs,
                AffineVectorMake(VKS.Utils.StrToFloatDef(tl[1]),
                VKS.Utils.StrToFloatDef(tl[2]), VKS.Utils.StrToFloatDef(tl[3])),
                AffineVectorMake(VKS.Utils.StrToFloatDef(tl[4]),
                VKS.Utils.StrToFloatDef(tl[5]),
                VKS.Utils.StrToFloatDef(tl[6])));
              nTex := TexCoords.FindOrAdd
                (AffineVectorMake(VKS.Utils.StrToFloatDef(tl[7]),
                VKS.Utils.StrToFloatDef(tl[8]), 0));
...
var
  ...
  procedure GetNodesFromBonesRecurs(bone: TVKSkeletonBone; ParentID: Integer;
    bl: TStrings);
  var
    i: Integer;
  begin
    ...
    begin
      ...
      for i := 0 to Owner.MeshObjects.Count - 1 do
        if Owner.MeshObjects[i] is TVKSkeletonMeshObject then
          with TVKSkeletonMeshObject(Owner.MeshObjects[i]) do
          begin
