...
uses
  ...
  VKS.VectorFileObjects, VKS.Texture, VKS.ApplicationFileIO,
  VKS.VectorTypes, VKS.VectorGeometry, VKS.Material;
...
// LoadFromStream
//
procedure TVKSMDVectorFile.LoadFromStream(aStream : TStream);

   procedure AllocateMaterial(const name : String);
   var
      matLib : TVKMaterialLibrary;
   begin
      if Owner is TVKBaseMesh then begin
   ...
   try
      ...
      while sl[i]<>'end' do begin
         ...
         while Pos(Copy(sl[i], 1, 1), ' 1234567890')>0 do begin
            ...
            frame.Position.Add(VKS.Utils.StrToFloatDef(tl[1]), VKS.Utils.StrToFloatDef(tl[2]), VKS.Utils.StrToFloatDef(tl[3]));
            v:=AffineVectorMake(VKS.Utils.StrToFloatDef(tl[4]), VKS.Utils.StrToFloatDef(tl[5]), VKS.Utils.StrToFloatDef(tl[6]));
      ...
      if Owner is TVKActor then with TVKActor(Owner).Animations.Add do begin
         k:=Pos('.', ResourceName);
         if k>0 then
            ...
            for k:=1 to 3 do with mesh do begin
                 ...
                 then begin
                   ...
                   for j := 0 to weightCount - 1 do begin
                     ...
                     BoneIDs[j].Weight := VKS.Utils.StrToFloatDef(tl[11+j*2]);
                   end;

                   nVert:=FindOrAdd(boneIDs, AffineVectorMake(VKS.Utils.StrToFloatDef(tl[1]), VKS.Utils.StrToFloatDef(tl[2]), VKS.Utils.StrToFloatDef(tl[3])),                                AffineVectorMake(VKS.Utils.StrToFloatDef(tl[4]), VKS.Utils.StrToFloatDef(tl[5]), VKS.Utils.StrToFloatDef(tl[6])));
                   nTex:=TexCoords.FindOrAdd(AffineVectorMake(VKS.Utils.StrToFloatDef(tl[7]), VKS.Utils.StrToFloatDef(tl[8]), 0));
                 ...
                 begin
                   ...
                   nVert:=FindOrAdd(boneID,AffineVectorMake(VKS.Utils.StrToFloatDef(tl[1]), VKS.Utils.StrToFloatDef(tl[2]), VKS.Utils.StrToFloatDef(tl[3])),                                AffineVectorMake(VKS.Utils.StrToFloatDef(tl[4]), VKS.Utils.StrToFloatDef(tl[5]), VKS.Utils.StrToFloatDef(tl[6])));
                   nTex:=TexCoords.FindOrAdd(AffineVectorMake(VKS.Utils.StrToFloatDef(tl[7]), VKS.Utils.StrToFloatDef(tl[8]), 0));
                   faceGroup.Add(nVert, nVert, nTex);
                   Inc(i);
                 end;
...
// SaveToStream
//
procedure TVKSMDVectorFile.SaveToStream(aStream : TStream);
var
   str,
   nodes     : TStrings;
   i,j,k,l,b : Integer;
...
// ------------------------------------------------------------------
   ...
   RegisterVectorFileFormat('smd', 'Half-Life SMD files', TVKSMDVectorFile);
