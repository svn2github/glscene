...
uses
  System.Classes, VKS.VectorFileObjects, VKS.ApplicationFileIO, VKS.CrossPlatform;
...
uses
  System.SysUtils,
  //VKS
  TypesSTL, VKS.VectorGeometry, VKS.VectorLists,  VKS.Utils;
...
// Capabilities
//
class function TVKSTLVectorFile.Capabilities : TDataFileCapabilities;
begin
...
var
   ...
   begin
      ...
      if sl.Count<>5 then
         ...
         aNormal.V[0]:=VKS.Utils.StrToFloatDef(sl[2], 0);
         aNormal.V[1]:=VKS.Utils.StrToFloatDef(sl[3], 0);
         aNormal.V[2]:=VKS.Utils.StrToFloatDef(sl[4], 0);
      end;
   end;
   ...
   begin
      ...
      if (sl.Count<>4) or (CompareText(sl[0], cVERTEX_LABEL)<>0) then
         ...
         aVertex.V[0]:=VKS.Utils.StrToFloatDef(sl[1], 0);
         aVertex.V[1]:=VKS.Utils.StrToFloatDef(sl[2], 0);
         aVertex.V[2]:=VKS.Utils.StrToFloatDef(sl[3], 0);
      end;
   end;
