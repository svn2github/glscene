...
{: VKS.FileOBJ<p>
 ...
 <b>History : </b><font size=-1><ul>
      ...
      <li>03/10/00 - Egg - Fixed TVKOBJVectorFile.LoadFromStream.ReadFace
      <li>08/10/00 - Egg - Added standard header, basic htmlification of old header,
                           removed specific trim funcs,
                           renamed TMBAGLOBJVectorFile to TVKOBJVectorFile
   </ul><p>
  ...
  License:<br>
    ...
    Contributed to Eric Grange and VKS.Scene,
...
{ - based on TFGVertexNormalTexIndexList (VKS.VectorFileObjects.pas)
...
procedure TVKOBJVectorFile.ReadLine;
var
  j: Integer;
...
procedure TVKOBJVectorFile.Error(const msg: string);
var
  E: EGLOBJFileError;
begin
...
procedure TVKOBJVectorFile.LoadFromStream(aStream: TStream);
var
          ...
          begin
            ...
            try
              ...
              begin
                ...
                Diffuse.Alpha := VKS.Utils.StrToFloatDef(objMtl.MaterialStringProperty(matName, 'd'), 1);
...
{$IFDEF STATS}
    t2 := GLGetTickCount;
    InformationDlg(Format('TVKOBJVectorFile Loaded in %dms'#13 +
      #13 +
...
procedure TVKOBJVectorFile.SaveToStream(aStream: TStream);
var
...
begin
  Assert(Owner is TVKFreeForm, 'Can only save FreeForms.');
...
function TVKMTLFile.MaterialStringProperty(const materialName, propertyName: string): string;
var
  i, n: Integer;
  buf, line: string;
begin
...
function TVKMTLFile.MaterialVectorProperty(const materialName, propertyName: string;
  const defaultValue: TVector): TVector;
var
  i: Integer;
  sl: TStringList;
begin
  ...
  try
    ...
    begin
      ...
      for i := 0 to 3 do
        if sl.Count > i then
          Result.V[i] := VKS.Utils.StrToFloatDef(sl[i], 0)
        else
          Break;
    end
    else
...
initialization
  ...
  VKS.VectorFileObjects.RegisterVectorFileFormat('obj', 'WaveFront model file', TVKOBJVectorFile);
  VKS.VectorFileObjects.RegisterVectorFileFormat('objf', 'Stripe model file', TVKOBJVectorFile);
