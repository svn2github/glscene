...
uses
  System.Classes, VKS.HeightData, VKS.Graphics, VKS.VectorGeometry, VKS.Texture,
  VKS.VectorTypes, VKS.Coordinates, VKS.Material;
   ...
   { An Height Data Source that generates terrain shadow maps automatically. 
	 ...
	 TVKShadowHDS = class (TVKHeightDataSourceFilter)
	   ...
	   protected
         ...
         function  FindUnusedMaterial:TVKLibMaterial;
	   ...
	   published
         ...
         property LightVector : TVKCoordinates read FLightVector write SetLightVector;
...
uses
  System.SysUtils,
  Winapi.OpenGL, Winapi.OpenGLext,  VKS.VectorLists;

// Create
//
constructor TVKShadowHDS.Create(AOwner: TComponent);
begin
 	inherited Create(AOwner);
  FLightVector := TVKCoordinates.CreateInitialized(Self, VectorMake(1, 0,-1));
  FLightVector.Style:=csVector; //csPoint;
  FScale := TVKCoordinates.CreateInitialized(Self, VectorMake(1,1,1));
...
// Destroy
//
destructor TVKShadowHDS.Destroy;
begin
  self.Active:=false;
...
// Notification
//
procedure TVKShadowHDS.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
procedure TVKShadowHDS.Trim(MaxTextureCount:integer); //internal use only
var matLib: TVKMaterialLibrary;
...
function TVKShadowHDS.FindUnusedMaterial:TVKLibMaterial;
var matLib: TVKMaterialLibrary;
    i:integer;
    cnt:integer;
begin
...
//  SetLightVector
//
procedure TVKShadowHDS.SetLightVector(AValue: TVKCoordinates);
begin
  With OwnerHDS.Data.LockList do try
...
// CalcStep
//
function TVKShadowHDS.CalcStep:TAffineVector;
var L:single;
    v:TAffineVector;
begin
...
begin
  ...
  HD:=HeightData;
  MatName:='ShadowHDS_x'+IntToStr(HD.XLeft)+'y'+IntToStr(HD.YTop)+'.'; //name contains xy coordinates of the current tile
  Hold:=TVKUpdateAbleObject.Create(self);
  ...
  if LibMat=nil then begin
    ...
    with libMat.Material.Texture do begin
      ...
      bmp32:=(Image as TVKBlankImage).GetBitmap32(GL_TEXTURE_2D);
      GenerateShadowMap(HD , bmp32, 1);
    end;
    libMat.Name:=MatName;
    //----------------------------------------------------
...
function TVKShadowHDS.WrapDist(Lx,Ly:single):integer;
var x,y:single;
    size:integer;
    sx,sy:single;
begin
...
procedure TVKShadowHDS.SetShadowmapLibrary(const val : TVKMaterialLibrary);
begin
...
// SetBumpScale
//
procedure TVKShadowHDS.SetScale(AValue: TVKCoordinates);
begin
  with OwnerHDS.Data.LockList do try
...
//SetSoftRange
//
procedure TVKShadowHDS.SetSoftRange(AValue:Cardinal);
begin
  with OwnerHDS.Data.LockList do try
...
//SetDiffuse
//
procedure TVKShadowHDS.SetDiffuse(AValue: Single);
begin
  with OwnerHDS.Data.LockList do try
...
//SetAmbient
//
procedure TVKShadowHDS.SetAmbient(AValue: Single);
begin
  with OwnerHDS.Data.LockList do try
