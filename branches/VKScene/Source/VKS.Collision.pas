...
uses
  ...
  VKS.VectorFileObjects, VKS.GeometryBB, VKS.CrossPlatform,
  VKS.Manager , VKS.VectorTypes;


type
   TVKBCollision = class;
   TObjectCollisionEvent = procedure (Sender : TObject; object1, object2 : TVKBaseSceneObject) of object;
...
function FastCheckPointVsPoint(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckPointVsSphere(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckPointVsEllipsoid(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckPointVsCube(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckSphereVsPoint(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckSphereVsSphere(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckSphereVsEllipsoid(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckSphereVsCube(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckEllipsoidVsPoint(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckEllipsoidVsSphere(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckEllipsoidVsEllipsoid(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckEllipsoidVsCube(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckCubeVsPoint(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckCubeVsSphere(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckCubeVsEllipsoid(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckCubeVsCube(obj1, obj2 : TVKBaseSceneObject) : Boolean;
function FastCheckCubeVsFace(obj1, obj2 : TVKBaseSceneObject) : Boolean;   //experimental
...
function FastCheckCubeVsFace(obj1, obj2 : TVKBaseSceneObject) : Boolean;
//var
//   triList : TAffineVectorList;
//   m1to2, m2to1 : TMatrix;
//   i:integer;
begin
   if (obj2 is TVKFreeForm) then begin
      //check if we are initialized correctly
      if not Assigned(TVKFreeForm(obj2).Octree) then
         TVKFreeForm(obj2).BuildOctree;

      Result:=TVKFreeForm(obj2).OctreeAABBIntersect(obj1.AxisAlignedBoundingBoxUnscaled,obj1.AbsoluteMatrix,obj1.InvAbsoluteMatrix)
...
function FastCheckFaceVsCube(obj1, obj2 : TVKBaseSceneObject) : Boolean;
begin
...
begin
   result:= false;
   if (obj1 is TVKFreeForm) and (obj2 is TVKFreeForm) then begin
      //check if we are initialized correctly
      if not Assigned(TVKFreeForm(obj1).Octree) then
         TVKFreeForm(obj1).BuildOctree;
      if not Assigned(TVKFreeForm(obj2).Octree) then
         TVKFreeForm(obj2).BuildOctree;
      ...
      triList:=TVKFreeForm(obj1).Octree.GetTrianglesFromNodesIntersectingCube(AABB2, m1to2, m2to1);
      ...
      try
         ...
         while i<TriList.Count-2 do begin
            ...
            if TVKFreeForm(obj2).OctreeTriangleIntersect(Tri[0], Tri[1], Tri[2]) then begin
               Result:= true;
...
//
function IntersectCubes(obj1, obj2 : TVKBaseSceneObject) : Boolean;
var
  aabb1, aabb2 : TAABB;
  m1To2, m2To1 : TMatrix;
begin
...
// Create
//
constructor TVKCollisionManager.Create(AOwner: TComponent);
begin
...
// Destroy
//
destructor TVKCollisionManager.Destroy;
begin
...
// RegisterClient
//
procedure TVKCollisionManager.RegisterClient(aClient : TVKBCollision);
begin
...
// DeRegisterClient
//
procedure TVKCollisionManager.DeRegisterClient(aClient : TVKBCollision);
begin
...
// DeRegisterAllClients
//
procedure TVKCollisionManager.DeRegisterAllClients;
var
   i : Integer;
begin
   // Fast deregistration
   for i:=0 to FClients.Count-1 do
      TVKBCollision(FClients[i]).FManager:=nil;
   FClients.Clear;
end;
...
var
   obj1, obj2 : TVKBaseSceneObject;
   cli1, cli2 : TVKBCollision;
...
type
   ...
   TCollisionNode = class
     public
      Collision:TVKBCollision;
      AABB:TAABB;
      constructor Create(Collision:TVKBCollision; AABB:TAABB);
   end;

constructor TCollisionNode.Create(Collision:TVKBCollision; AABB:TAABB);
begin
  inherited Create();
...
procedure TVKCollisionManager.CheckCollisions;
var
  NodeList:TList;
...
// Destroy
//
destructor TVKBCollision.Destroy;
begin
   Manager:=nil;
   inherited Destroy;
end;
...
// FriendlyName
//
class function TVKBCollision.FriendlyName : String;
begin
   Result:='Collision';
end;
...
// FriendlyDescription
//
class function TVKBCollision.FriendlyDescription : String;
begin
...
// WriteToFiler
//
procedure TVKBCollision.WriteToFiler(writer : TWriter);
begin
...
// ReadFromFiler
//
procedure TVKBCollision.ReadFromFiler(reader : TReader);
var
...
// Loaded
//
procedure TVKBCollision.Loaded;
var
   mng : TComponent;
begin
   inherited;
   if FManagerName<>'' then begin
      mng:=FindManager(TVKCollisionManager, FManagerName);
      if Assigned(mng) then
         Manager:=TVKCollisionManager(mng);
      FManagerName:='';
   end;
end;
...
// Assign
//
procedure TVKBCollision.Assign(Source: TPersistent);
begin
   if Source is TVKBCollision then begin
      Manager:=TVKBCollision(Source).Manager;
      BoundingMode:=TVKBCollision(Source).BoundingMode;
   end;
...
// SetManager
//
procedure TVKBCollision.SetManager(const val : TVKCollisionManager);
begin
...
// SetGroupIndex
//
procedure TVKBCollision.SetGroupIndex(const value : Integer);
begin
   FGroupIndex:=value;
end;

// GetOrCreateCollision (TVKBehaviours)
//
function GetOrCreateCollision(behaviours : TVKBehaviours) : TVKBCollision;
var
	i : Integer;
begin
	i:=behaviours.IndexOfClass(TVKBCollision);
	if i>=0 then
		Result:=TVKBCollision(behaviours[i])
	else Result:=TVKBCollision.Create(behaviours);
end;

// GetOrCreateCollision (TVKBaseSceneObject)
//
function GetOrCreateCollision(obj : TVKBaseSceneObject) : TVKBCollision;
begin
...
// ------------------------------------------------------------------
	...
	// class registrations
	RegisterXCollectionItemClass(TVKBCollision);

finalization

	UnregisterXCollectionItemClass(TVKBCollision);
