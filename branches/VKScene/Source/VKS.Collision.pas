...
type
  ...
  TObjectCollisionEvent = procedure(Sender: TObject;
    object1, object2: TVKBaseSceneObject) of object;
...
function GetOrCreateCollision(behaviours: TVKBehaviours)
  : TVKBCollision; overload;
...
begin
  Result := false;
  if (obj1 is TVKFreeForm) and (obj2 is TVKFreeForm) then
  begin
    // check if we are initialized correctly
    ...
    triList := TVKFreeForm(obj1).Octree.GetTrianglesFromNodesIntersectingCube
      (AABB2, m1to2, m2to1);
    ...
    try
      ...
      begin
        ...
        if TVKFreeForm(obj2).OctreeTriangleIntersect(tri[0], tri[1], tri[2])
        then
        begin
          Result := true;
...
begin
  if Source is TVKBCollision then
  begin
...
begin
  ...
  if i >= 0 then
    ...
    Result := TVKBCollision.Create(behaviours);
end;
