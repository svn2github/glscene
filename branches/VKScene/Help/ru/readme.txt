...
текстовых сообщений и подсказок с английского языка на русский язык с использованием GNU Gettext tools for Delphi & C++Builder. 
...
- Извлеките из вашей папки в VKScene с файлами pas, cpp и dfm текстовые заголовки и строки во временный файл default.po. Для этого щелкните правой кнопкой мыши по соответствующей папке, открытой 
...
- Объедините ваш временный файл default.po с помощью команды Merge template  с основным файлом перевода готовых примеров samples.po, 
  с ранее извлечеными и переведенными текстовыми строками из папки VKScene\Samples. Для вызова команды слияния щелкните правой кнопкой мыши по вашему po файлу 
  Переименуйте полученный файл, например, в samples.po (также можно объединить временный файл с vkscene.po, 
  который обновляется разработчиками). 
