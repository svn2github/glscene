The directory for VKScene's help manuals and po files with translations of captions, text strings and hints 
...
- Merge your project's templete default.po file with main translated samples.po file 
  if it was earlier extracted from VKScene\Samples directory. Right click on default.po file to execute the command 
  (the same could be done to merge a template with glscene.po updated by VKScene's developers).
...
- Choose the necessary language to build your application and execute your software with localized interface,
  that includes all VKScene's text strings, hints and translated messages.
