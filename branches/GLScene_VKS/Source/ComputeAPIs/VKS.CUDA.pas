...
uses
  ...
  VKS.PersistentClasses, VKS.BaseClasses, VKS.CrossPlatform, VKS.Context,
  VKS.VectorGeometry, VKS.VectorTypes, VKS.VectorLists, VKS.Graphics,
  VKS.CLPlatform, VKS.CUDAApi, VKS.CUDARuntime, VKS.CUDAParser,
  VKS.CUDAFourierTransform, VKS.CUDACompiler, VKS.CUDAContext,
  VKS.CUDADataAccess, VKS.Strings, VKS.Utils, VKS.Log;
...
type
  ...
  private
    ...
    FCompiler: TVKSCUDACompiler;
    procedure SetCode(const Value: TStringList);
    procedure SetCompiler(const Value: TVKSCUDACompiler);
  ...
  public
    ...
    property Context: TCUDAContext read GetContext;
    property CodeType: TVKSCUDACompilerOutput read FCodeType;
  ...
  protected
    ...
    FResourceType: TVKResourceType;
    FGLContextHandle: TVKVirtualHandle;
    FMapCounter: Integer;
    function GetIsAllocated: Boolean; override;
    procedure OnGLHandleAllocate(Sender: TVKVirtualHandle;
      var Handle: Cardinal);
    procedure OnGLHandleDestroy(Sender: TVKVirtualHandle; var Handle: Cardinal);
  ...
  public
    ...
    procedure CopyFrom(const AGLImage: TVKBitmap32); overload;
  ...
  private
    ...
    procedure SetDevice(const Value: TVKSCUDADevice);
...
constructor TVKSCUDA.Create(AOwner: TComponent);
begin
...
procedure TVKSCUDA.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
procedure TVKSCUDA.SetDevice(const Value: TVKSCUDADevice);
begin
...
procedure TVKSCUDA.SetOnOpenGLInteropInit(AEvent: TOnOpenGLInteropInit);
begin
  FOnOpenGLInteropInit := AEvent;
  CuNotifyChange(cuchContext);
end;

function TVKSCUDA.GetContext: TCUDAContext;
begin
...
function TVKSCUDA.GetIsAllocated: Boolean;
begin
  Result := FContext.IsValid;
end;

function TVKSCUDA.GetModule(const i: Integer): TCUDAModule;
var
  j, k: Integer;
begin
  Result := nil;
...
procedure TCUDAModule.SetCompiler(const Value: TVKSCUDACompiler);
begin
...
begin
  if Assigned(FMaster) and (FMaster is TVKSCUDA) then
    Result := TVKSCUDA(FMaster).Context
  else
  begin
    Result := nil;
...
begin
  if not(csLoading in ComponentState) and (Sender is TVKSCUDACompiler) then
  begin
    AllocateHandles;
  end;
end;
...
begin
  if Self is TVKSCUDA then
    Result := TVKSCUDA(Self).Context
  else
    Result := TVKSCUDA(FMaster).Context;
end;
...
begin
  Context.Requires;
  MemPerBlock := TVKSCUDA(TCUDAModule(FMaster).FMaster)
...
procedure TCUDAMemData.CopyTo(const AGLImage: TVKBitmap32);
var
  copyParam2D: TCUDA_MEMCPY2D;
  // copyParam3D: TCUDA_MEMCPY3D;
begin
...
procedure TCUDAMemData.CopyFrom(const AGLImage: TVKBitmap32);
var
  copyParam2D: TCUDA_MEMCPY2D;
  // copyParam3D: TCUDA_MEMCPY3D;
begin
...
procedure TCUDAGraphicResource.OnGLHandleAllocate(Sender: TVKVirtualHandle;
  var Handle: Cardinal);
begin
...
procedure TCUDAGraphicResource.OnGLHandleDestroy(Sender: TVKVirtualHandle;
  var Handle: Cardinal);
begin
  DestroyHandles;
end;
...
// ------------------------------------------------------------------
  ...
  RegisterClasses([TVKSCUDA, TVKSCUDACompiler, TCUDAModule, TCUDAFunction,
