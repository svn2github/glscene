...
uses
  ...
  VKS.CUDAParser, VKS.ApplicationFileIO, VKS.Log;
...
type
  ...
  TVKSCUDAVirtArch = (compute_10, compute_11, compute_12, compute_13, compute_20);
  ...
  TVKSCUDARealArch = (sm_10, sm_11, sm_12, sm_13, sm_20, sm_21);
  TVKSCUDARealArchs = set of TVKSCUDARealArch;

  TVKSCUDACompiler = class(TComponent)
  private
    ...
    FOutputCodeType: TVKSCUDACompilerOutput;
    FVirtualArch: TVKSCUDAVirtArch;
    FRealArch: TVKSCUDARealArchs;
    FMaxRegisterCount: Integer;
    FModuleInfo: TCUDAModuleInfo;
    procedure SetMaxRegisterCount(Value: Integer);
    procedure SetOutputCodeType(const Value: TVKSCUDACompilerOutput);
    function StoreProjectModule: Boolean;
    procedure SetRealArch(AValue: TVKSCUDARealArchs);
...
constructor TVKSCUDACompiler.Create(AOwner: TComponent);
var
  path: string;
begin
...
destructor TVKSCUDACompiler.Destroy;
begin
  FModuleInfo.Destroy;
  inherited;
end;

procedure TVKSCUDACompiler.Loaded;
var
  LStr: string;
begin
  inherited;
...
procedure TVKSCUDACompiler.Assign(Source: TPersistent);
var
  compiler: TVKSCUDACompiler;
begin
  if Source is TVKSCUDACompiler then
  begin
    compiler := TVKSCUDACompiler(Source);
...
procedure TVKSCUDACompiler.SetCppCompilerPath(const AValue: string);
begin
  if FileExists(AValue + 'cl.exe') then
    FCppCompilerPath := AValue;
end;

procedure TVKSCUDACompiler.setMaxRegisterCount(Value: Integer);
begin
...
procedure TVKSCUDACompiler.SetNVCCPath(const AValue: string);
begin
  if FileExists(AValue + 'nvcc.exe') then
    FNVCCPath := AValue;
end;

procedure TVKSCUDACompiler.setOutputCodeType(const Value
  : TVKSCUDACompilerOutput);
begin
...
procedure TVKSCUDACompiler.SetRealArch(AValue: TVKSCUDARealArchs);
begin
  if AValue = [] then
    AValue := [sm_10];
  FRealArch := AValue;
...
procedure TVKSCUDACompiler.SetSourceCodeFile(const AFileName: string);
begin
  if FileStreamExists(AFileName) then
    FSourceCodeFile := AFileName;
end;

function TVKSCUDACompiler.StoreProjectModule: Boolean;
begin
