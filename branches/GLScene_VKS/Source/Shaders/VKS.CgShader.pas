...
uses
  ...
  //VKS
  VKS.VectorGeometry, VKS.VectorLists, VKS.VectorTypes, VKS.Texture, VKS.Strings,
  VKS.Cadencer, VKS.OpenGLTokens, VKS.CrossPlatform, VKS.Context, VKS.BaseClasses,
  VKS.RenderContextInfo, VKS.Material, VKS.TextureFormat,
...
type
  ...
  TCgProgram = class (TVKUpdateAbleObject)
  private
  ...
  public
    ...
    procedure SetToTextureOf(LibMaterial  : TVKLibMaterial);
  ...
  // TCustomCgShader
  //
  TCustomCgShader = class (TVKShader)
  private
    { Private Declarations }
...
// SetToTextureOf
//
procedure TCgParameter.SetToTextureOf(LibMaterial: TVKLibMaterial);
var TexType : TCGtype;
begin
...
begin
  ...
  begin
    Enabled := False;
    raise EGLCGShaderException.CreateFmt(vksErrorEx + glsCadencerNotDefinedEx, [ClassName]);
  end
  else
    inherited;
end;
...
begin
  inherited;
  if (AComponent is TVKCadencer) and (Operation = opRemove) then
  begin
    FCadencer := nil;
    Enabled := False;
  end;
...
// SetCadencer
//
procedure TCadencableCustomCgShader.SetCadencer(const Value: TVKCadencer);
begin
