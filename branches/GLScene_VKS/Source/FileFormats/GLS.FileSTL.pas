...
uses
  System.SysUtils,
  //GLS
  TypesSTL, GLS.VectorGeometry, GLS.VectorLists,  GLS.Utils;
...
var
   ...
   begin
      ...
      if sl.Count<>5 then
         ...
         aNormal.V[0]:=GLS.Utils.StrToFloatDef(sl[2], 0);
         aNormal.V[1]:=GLS.Utils.StrToFloatDef(sl[3], 0);
         aNormal.V[2]:=GLS.Utils.StrToFloatDef(sl[4], 0);
      end;
   end;
   ...
   begin
      ...
      if (sl.Count<>4) or (CompareText(sl[0], cVERTEX_LABEL)<>0) then
         ...
         aVertex.V[0]:=GLS.Utils.StrToFloatDef(sl[1], 0);
         aVertex.V[1]:=GLS.Utils.StrToFloatDef(sl[2], 0);
         aVertex.V[2]:=GLS.Utils.StrToFloatDef(sl[3], 0);
      end;
   end;
