...
uses
  System.Classes, System.SysUtils, VKS.VectorFileObjects, VKS.Utils, 
  VKS.ApplicationFileIO, VKS.VectorTypes, VKS.VectorGeometry, VKS.VectorLists;
...
procedure TVKMD5VectorFile.LoadFromStream(aStream : TStream);
  ...
  begin
    ...
    pos.V[0]:=VKS.Utils.StrToFloatDef(FTempString[2]);
    pos.V[1]:=VKS.Utils.StrToFloatDef(FTempString[4]);
    pos.V[2]:=VKS.Utils.StrToFloatDef(FTempString[3]);
        ...
        if temp = 'shader' then begin
          ...
          if FTempString.Count>=6 then begin
            ...
            mesh.TexCoords[vnum]:=AffineVectorMake(
              ...
              1-VKS.Utils.StrToFloatDef(FTempString[3]),
              0);
    ...
    repeat
      ...
      if FTempString.Count>=6 then begin
        pos:=AffineVectorMake(
          VKS.Utils.StrToFloatDef(FTempString[0]),
          VKS.Utils.StrToFloatDef(FTempString[1]),
          VKS.Utils.StrToFloatDef(FTempString[2]));
        quat:=QuaternionMakeFromImag(
          VKS.Utils.StrToFloatDef(FTempString[3]),
          VKS.Utils.StrToFloatDef(FTempString[4]),
          VKS.Utils.StrToFloatDef(FTempString[5]));
        FFramePositions.Add(pos);
        FFrameQuaternions.Add(quat);
    ...
    for i:=0 to FJointFlags.Count-1 do begin
      ...
      if FJointFlags[i] > 0 then begin
        ...
        if FJointFlags[i] and 1 > 0 then begin
          pos.V[0]:=VKS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 2 > 0 then begin
          pos.V[1]:=VKS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 4 > 0 then begin
          pos.V[2]:=VKS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;

        if FJointFlags[i] and 8 > 0 then begin
          quat.ImagPart.V[0]:=VKS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 16 > 0 then begin
          quat.ImagPart.V[1]:=VKS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 32 > 0 then
          quat.ImagPart.V[2]:=VKS.Utils.StrToFloatDef(FTempString[j]);
      end;
