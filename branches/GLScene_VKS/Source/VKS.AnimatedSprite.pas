...
function TSpriteAnimation.GetMaterialLibrary: TVKAbstractMaterialLibrary;
begin
  if not (Owner is TSpriteAnimationList) then
    Result := nil
  else
  begin
    if not (TSpriteAnimationList(Owner).Owner is TVKAnimatedSprite) then
      Result := nil
    else
      Result :=
        TVKAnimatedSprite(TSpriteAnimationList(Owner).Owner).FMaterialLibrary;
  end;
end;
...
procedure TSpriteAnimation.SetLibMaterialName(const val: TVKLibMaterialName);
begin
...
function TSpriteAnimation.GetLibMaterialCached: TVKLibMaterial;
begin
  Result := nil;
...
procedure TVKAnimatedSprite.SetAnimationMode(const val: TSpriteAnimationMode);
begin
