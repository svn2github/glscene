...
uses
   ...
   VKS.Scene, VKS.Material, VKS.VectorGeometry, VKS.VectorLists,
   VKS.OpenGLTokens, VKS.VectorFileObjects, VKS.ApplicationFileIO,
   VKS.RenderContextInfo, VKS.XOpenGL, VKS.Context, VKS.VectorTypes;
...
type
   ...
   TVKTreeBranch = class
      private
         ...
         FRight : TVKTreeBranch;
         FParent : TVKTreeBranch;
         FBranchID : Integer;
         FParentID : Integer;
         FMatrix : TMatrix;
         FLower : TIntegerList;
         ...
         procedure BuildBranch(branchNoise : TVKTreeBranchNoise; const Matrix : TMatrix;
      ...
      public
         ...
         property Right : TVKTreeBranch read FRight;
         property Parent : TVKTreeBranch read FParent;
   ...
   TVKTree = class (TVKImmaterialSceneObject)
      ...
      published
         ...
         property LeafMaterialName : TVKLibMaterialName read FLeafMaterialName write SetLeafMaterialName;
         property LeafBackMaterialName : TVKLibMaterialName read FLeafBackMaterialName write SetLeafBackMaterialName;
         property BranchMaterialName : TVKLibMaterialName read FBranchMaterialName write SetBranchMaterialName;
   end;
...
// Create
//
constructor TVKTreeBranch.Create(AOwner : TVKTreeBranches; AParent : TVKTreeBranch);
begin
   FOwner:=AOwner;
   FParent:=AParent;
...
// Destroy
//
destructor TVKTreeBranch.Destroy;
begin
   FUpper.Free;
   FLower.Free;
   FLeft.Free;
...
// BuildBranch
//
procedure TVKTreeBranch.BuildBranch(branchNoise : TVKTreeBranchNoise; const Matrix : TMatrix;
                                    TexCoordY, Twist : Single; Level : Integer);
var
   i : Integer;
   Tree : TVKTree;
   Branches : TVKTreeBranches;
   Facets : Integer;
   t,c,s : Single;
   ...
   LeftBranchNoise : TVKTreeBranchNoise;
   CenterBranchNoise : TVKTreeBranchNoise;
   RightBranchNoise : TVKTreeBranchNoise;
   LeftMatrix, RightMatrix, CenterMatrix : TMatrix;
   central_leader : Boolean;
begin
   Assert(Assigned(FOwner),'Incorrect use of TVKTreeBranch');
   Assert(Assigned(FOwner.FOwner),'Incorrect use of TVKTreeBranches');
   ...
   if (((Level+1)>=Tree.Depth) or (LeftRadius<Tree.LeafThreshold)) then begin
      ...
      Inc(Branches.FCount);
      FLeft:=TVKTreeBranch.Create(Owner,Self);
   ...
   if (((Level+1)>=Tree.Depth) or (CenterRadius<Tree.LeafThreshold)) then begin
      ...
      Inc(Branches.FCount);
      FCenter:=TVKTreeBranch.Create(Owner,Self);
   ...
   if (((Level+1)>=Tree.Depth) or (RightRadius<Tree.LeafThreshold)) then begin
      ...
      Inc(Branches.FCount);
      FRight:=TVKTreeBranch.Create(Owner,Self);
...
// Create
//
constructor TVKTreeBranches.Create(AOwner: TVKTree);
begin
   FOwner:=AOwner;
...
// Destroy
//
destructor TVKTreeBranches.Destroy;
begin
   FSinList.Free;
   FCosList.Free;
   FVertices.Free;
...
// BuildBranches
//
procedure TVKTreeBranches.BuildBranches;
var
   i : Integer;
   u : Single;
...
begin
   ...
   Inc(FCount);
   FRoot:=TVKTreeBranch.Create(Self,nil);
...
// Clear
//
procedure TVKTreeBranches.Clear;
begin
   FSinList.Clear;
   FCosList.Clear;
   FVertices.Clear;
...
// Create
//
constructor TVKTreeBranchNoise.Create;
begin
   FBranchNoise:=Random;
end;
...
// Destroy
//
destructor TVKTreeBranchNoise.Destroy;
begin
   FLeft.Free;
   FRight.Free;
   inherited;
...
// GetLeft
//
function TVKTreeBranchNoise.GetLeft: TVKTreeBranchNoise;
begin
   if not Assigned(FLeft) then
      FLeft:=TVKTreeBranchNoise.Create;
   Result:=FLeft;
end;

// GetRight
//
function TVKTreeBranchNoise.GetRight: TVKTreeBranchNoise;
begin
   if not Assigned(FRight) then
      FRight:=TVKTreeBranchNoise.Create;
   Result:=FRight;
end;

function TVKTreeBranchNoise.GetCenter: TVKTreeBranchNoise;
begin
   if not Assigned(FCenter) then
      FCenter:=TVKTreeBranchNoise.Create;
   Result:=FCenter;
end;
...
// Create
//
constructor TVKTree.Create(AOwner: TComponent);
begin
   inherited;
   ...
   FLeaves:=TVKTreeLeaves.Create(Self);
   FBranches:=TVKTreeBranches.Create(Self);
   FNoise:=TVKTreeBranchNoise.Create;
end;

// Destroy
//
destructor TVKTree.Destroy;
begin
   FLeaves.Free;
   FBranches.Free;
   FNoise.Free;
...
// Notification
//
procedure TVKTree.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
// SetBranchAngle
//
procedure TVKTree.SetBranchAngle(const Value: Single);
begin
...
// SetBranchAngleBias
//
procedure TVKTree.SetBranchAngleBias(const Value: Single);
begin
...
// SetBranchNoise
//
procedure TVKTree.SetBranchNoise(const Value: Single);
begin
...
// SetBranchRadius
//
procedure TVKTree.SetBranchRadius(const Value: Single);
begin
...
// SetBranchSize
//
procedure TVKTree.SetBranchSize(const Value: Single);
begin
...
// SetBranchTwist
//
procedure TVKTree.SetBranchTwist(const Value: Single);
begin
...
// SetDepth
//
procedure TVKTree.SetDepth(const Value: Integer);
begin
...
// SetBranchFacets
//
procedure TVKTree.SetBranchFacets(const Value: Integer);
begin
...
// SetLeafSize
//
procedure TVKTree.SetLeafSize(const Value: Single);
begin
...
// SetLeafThreshold
//
procedure TVKTree.SetLeafThreshold(const Value: Single);
begin
...
//
procedure TVKTree.SetCentralLeaderBias(const Value: Single);
begin
...
// SetCentralLeader
//
procedure TVKTree.SetCentralLeader(const Value: Boolean);
begin
...
// SetSeed
//
procedure TVKTree.SetSeed(const Value: Integer);
begin
...
//
procedure TVKTree.SetCenterBranchConstant(const Value : Single);
begin
...
//
procedure TVKTree.SetBranchMaterialName(const Value: TVKLibMaterialName);
begin
...
//
procedure TVKTree.SetLeafBackMaterialName(const Value: TVKLibMaterialName);
begin
...
// SetLeafMaterialName
//
procedure TVKTree.SetLeafMaterialName(const Value: TVKLibMaterialName);
begin
...
// SetMaterialLibrary
//
procedure TVKTree.SetMaterialLibrary(const Value: TVKMaterialLibrary);
begin
...
// ForceTotalRebuild
//
procedure TVKTree.ForceTotalRebuild;
begin
   Clear;
   FNoise.Free;
   RandSeed:=FSeed;
...
// LoadFromStream
//
procedure TVKTree.LoadFromStream(aStream: TStream);
var
...
// SaveToStream
procedure TVKTree.SaveToStream(aStream: TStream);
var
   StrList : TStringList;
begin
   StrList:=TStringList.Create;
...
// LoadFromFile
//
procedure TVKTree.LoadFromFile(aFileName: String);
var
   stream : TStream;
begin
...
// SaveToFile
//
procedure TVKTree.SaveToFile(aFileName: String);
var
   stream : TStream;
begin
...
// GetExtents
procedure TVKTree.GetExtents(var min, max : TAffineVector);
var
  lmin, lmax,
...
//
function TVKTree.AxisAlignedDimensionsUnscaled : TVector;
var
...
// SetAutoCenter
//
procedure TVKTree.SetAutoCenter(const Value: Boolean);
begin
...
// SetAutoRebuild
//
procedure TVKTree.SetAutoRebuild(const Value: Boolean);
begin
