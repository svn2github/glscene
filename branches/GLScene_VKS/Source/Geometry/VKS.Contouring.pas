...
uses
  ...
  //VKS
  VKS.VectorGeometry, VKS.VectorLists, VKS.Objects, VKS.MultiPolygon,  VKS.Coordinates,
...
type
  ...
  TVKByteVector = array of Byte;

  TVKMatrix = array of array of Single;
  TVKByteMatrix = array of array of Byte;

  TVectorL4D = array [0 .. 4] of Single;
  TVectorL4I = array [0 .. 4] of Integer;
  TCastArray = array [0 .. 2, 0 .. 2, 0 .. 2] of Integer;

  TVKIsoline2D = array [0 .. 32767] of TVKPoint2D;
  PGLIsoline2D = ^TVKIsoline2D;

  TVKIsoline = class (TObject)
    NP: Integer;
    Line: PGLIsoline2D;
    constructor Create(LineSize: integer); virtual;
    destructor Destroy; override;
  end;
  PGLIsoline = ^TVKIsoline;

  TVKIsolineState = (ilsEmpty, ilsCalculating, ilsReady);

  TVKIsolines = class (TVKLines)
  public
    IsoVertix: TAffineVector;
    GLSpaceTextSF: array of TVKSpaceText;
    procedure MakeIsolines(var Depths: TVKMatrix; bmSize: Integer;
  ...
  { CONREC is a contouring routine for rectangular spaced data or regular 2D grids
   ...
   private
     CoordRange: Integer;
     LineList: TList;
     IsolineState: TVKIsolineState;
  end;

procedure Initialize_Contouring(var DataGrid: TVKMatrix;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Isoline := TVKIsoline.Create(np1 + np2);
            for k := 0 to np2 - 1 do
            begin
...
destructor TVKIsolines.Destroy;
begin
  FreeList;
  Nodes.Free;
  inherited;
...
procedure TVKIsolines.FreeList;
var
  i: integer;
begin
 if LineList<>nil then
  begin
    ...
    begin
      with TVKIsoline(LineList.Items[i]) do
        Free;
    end;
    LineList.Clear;
    IsolineState := ilsEmpty;
...
procedure TVKIsolines.MakeIsolines(var Depths: TVKMatrix; bmSize: integer;
  StartDepth, EndDepth: Single; Interval: Integer);
var
  Isoline: TVKIsoline;
...
constructor TVKIsoline.Create(LineSize: Integer);
begin
  inherited Create;
  NP := LineSize;
