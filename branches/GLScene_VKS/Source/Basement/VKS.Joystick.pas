...
// Create
//
constructor TVKJoystick.Create(AOwner: TComponent);
begin
   inherited;
...
// Destroy
//
destructor TVKJoystick.Destroy;
begin
...
// WndProc
//
procedure TVKJoystick.WndProc(var Msg: TMessage);
begin
   with Msg do begin
      case FJoystickID of
...
// Assign
//
procedure TVKJoystick.Assign(Source: TPersistent);
begin
   if Source is TVKJoystick then begin
      FInterval := TVKJoystick(Source).FInterval;
      FThreshold := TVKJoystick(Source).FThreshold;
      FCapture := TVKJoystick(Source).FCapture;
      FJoystickID := TVKJoystick(Source).FJoystickID;
      try
         ReapplyCapture(FJoystickID);
      except
         FJoystickID := jidNoJoystick;
...
// MakeJoyButtons
//
function TVKJoystick.MakeJoyButtons(Param: UINT): TJoystickButtons;
begin
   Result := [];
...
// ReapplyCapture
//
procedure TVKJoystick.ReapplyCapture(AJoystick: TJoystickID);
var
   jc : TJoyCaps;
begin
...
// DoJoystickCapture
//
procedure TVKJoystick.DoJoystickCapture(AHandle: HWND; AJoystick: TJoystickID);
var
   res : Cardinal;
begin
...
// DoJoystickRelease
//
procedure TVKJoystick.DoJoystickRelease(AJoystick: TJoystickID);
begin
...
// SetCapture
//
procedure TVKJoystick.SetCapture(AValue: Boolean);
begin
...
// SetInterval
//
procedure TVKJoystick.SetInterval(AValue: Cardinal);
begin
...
// SetJoystickID
//
procedure TVKJoystick.SetJoystickID(AValue: TJoystickID);
begin
...
procedure TVKJoystick.SetThreshold(AValue: Cardinal);
...
// DoXYMove
//
procedure TVKJoystick.DoXYMove(Buttons: Word; XPos, YPos: Integer);
var
   I: Integer;
   dX, dY: Integer;
begin
...
// DoZMove
//
procedure TVKJoystick.DoZMove(Buttons: Word; ZPos: Integer);
begin
   if FLastZ = -1 then
