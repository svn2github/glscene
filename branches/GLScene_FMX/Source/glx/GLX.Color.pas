...
{: GLX.Color<p>
  ...
  <b>History : </b><font size=-1><ul>
    <li>01/04/14 - PW - Updated to support FireMonkey platform
...
uses
  ...
  // GLX
  GLX.VectorTypes, GLX.VectorGeometry,  GLX.CrossPlatform,
  GLX.PersistentClasses, GLX.BaseClasses;
...
const
  ...
  clMask = TColors.White;
  clDontMask = TColors.Black;
...
procedure InitGLSceneColors;
begin
  clrScrollBar := ConvertWinColor(TColors.SysScrollBar);
  clrActiveCaption := ConvertWinColor(TColors.SysActiveCaption);
  clrInactiveCaption := ConvertWinColor(TColors.SysInactiveCaption);
  clrMenu := ConvertWinColor(TColors.SysMenu);
  clrWindow := ConvertWinColor(TColors.SysWindow);
  clrWindowFrame := ConvertWinColor(TColors.SysWindowFrame);
  clrMenuText := ConvertWinColor(TColors.SysMenuText);
  clrWindowText := ConvertWinColor(TColors.SysWindowText);
  clrCaptionText := ConvertWinColor(TColors.SysCaptionText);
  clrActiveBorder := ConvertWinColor(TColors.SysActiveBorder);
  clrInactiveBorder := ConvertWinColor(TColors.SysInactiveBorder);
  clrAppWorkSpace := ConvertWinColor(TColors.SysAppWorkSpace);
  clrHighlightText := ConvertWinColor(TColors.SysHighlightText);
  clrBtnFace := ConvertWinColor(TColors.SysBtnFace);
  clrBtnShadow := ConvertWinColor(TColors.SysBtnShadow);
  clrGrayText := ConvertWinColor(TColors.SysGrayText);
  clrBtnText := ConvertWinColor(TColors.SysBtnText);
  clrInactiveCaptionText := ConvertWinColor(TColors.SysInactiveCaptionText);
  clrBtnHighlight := ConvertWinColor(TColors.SysBtnHighlight);
  clr3DDkShadow := ConvertWinColor(TColors.Sys3DDkShadow);
  clr3DLight := ConvertWinColor(TColors.Sys3DLight);
  clrInfoText := ConvertWinColor(TColors.SysInfoText);
  clrInfoBk := ConvertWinColor(TColors.SysInfoBk);
  {$ifndef LCLGTK2}
