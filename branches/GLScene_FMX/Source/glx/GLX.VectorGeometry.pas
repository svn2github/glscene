...
begin
   if VectorEquals(v1, v2) then begin
      ...
      q1:=QuaternionFromEuler(GLX.VectorGeometry.RadToDeg(v1.X), GLX.VectorGeometry.RadToDeg(v1.Y), GLX.VectorGeometry.RadToDeg(v1.Z), eulZYX);
      q2:=QuaternionFromEuler(GLX.VectorGeometry.RadToDeg(v2.X), GLX.VectorGeometry.RadToDeg(v2.Y), GLX.VectorGeometry.RadToDeg(v2.Z), eulZYX);
...
function InterpolateTan(const Start, Stop, Delta: Single): Single;
begin
  Result := (Stop - Start) * GLX.VectorGeometry.Tan(Delta * Pi / 4) + Start;
end;
...
begin
  ...
  begin
    ...
    Result := (Stop - Start) * GLX.VectorGeometry.Power(Delta, i) + Start;
  end
  else
    Result := (Stop - Start) * GLX.VectorGeometry.Power(Delta, DistortionDegree) + Start;
end;
...
var
   c, s, v0 : Single;
begin
   GLX.VectorGeometry.SinCos(alpha, s, c);
   v0:=v.X;
   v.X:=c*v0+s*v.Z;
   v.Z:=c*v.Z-s*v0;
end;
...
var
   s, c : Single;
begin
   GLX.VectorGeometry.SinCos(angle, s, c);
...
begin
   GLX.VectorGeometry.SinCos(angle, sine, cosine);
...
begin
  ...
  Tran[ttRotateY]:=GLX.VectorGeometry.ArcSin(-row0.V[Z]);
  if cos(Tran[ttRotateY]) <> 0 then begin
    Tran[ttRotateX]:=GLX.VectorGeometry.ArcTan2(row1.V[Z], row2.V[Z]);
    Tran[ttRotateZ]:=GLX.VectorGeometry.ArcTan2(row0.V[Y], row0.V[X]);
  end else begin
    tran[ttRotateX]:=GLX.VectorGeometry.ArcTan2(row1.V[X], row1.V[Y]);
    tran[ttRotateZ]:=0;
  end;
  // All done!
  Result:=True;
...
begin
  ...
  y:= ZNear * GLX.VectorGeometry.Tan(GLX.VectorGeometry.DegToRad(FOV) * 0.5);
  x:= y * Aspect;
...
var
   f, s, c : Single;
begin
   GLX.VectorGeometry.SinCos(GLX.VectorGeometry.DegToRad(angle*cOneDotFive), s, c);
	Result.RealPart:=c;
...
begin
   ...
   if High(s)-Low(s)<1000 then begin
      ...
      beta:=Sin(d);
      GLX.VectorGeometry.SinCos(startAngle*cPIdiv180, s[Low(s)], c[Low(s)]);
      ...
      for i:=Low(s) to High(s) do
         GLX.VectorGeometry.SinCos((i-Low(s))*d+startAngle, s[i], c[i]);
   end;
end;
...
begin
   Result:= GLX.VectorGeometry.ArcTan2(Sqrt(1 - Sqr(X)), X);
end;
...
begin
   Result:= GLX.VectorGeometry.ArcTan2(X, Sqrt(1 - Sqr(X)))
end;
...
begin
   ...
   t:=2*PI*Random;
   w:=Sqrt(1-p.Z*p.Z);
   GLX.VectorGeometry.SinCos(t, w, p.Y, p.X);
end;
...
begin
   ...
   SetVector(Result, sqrt((M.X.X-cost) / cost1),
                     ...
                     GLX.VectorGeometry.arccos(cost));
...
begin
  ...
  cost:=VectorAngleCosine(QStart.ImagPart, QEnd.ImagPart);
   ...
   if (1 - cost) < EPSILON then
      ...
      // normal case
      theta:=GLX.VectorGeometry.arccos(cost);
      phi:=theta + Spin * Pi;
      sint:=sin(theta);
...
begin
   ...
   if ((1.0-cosom)>EPSILON2) then begin // standard case (slerp)
      omega:=GLX.VectorGeometry.ArcCos(cosom);
      sinom:=1/Sin(omega);
...
begin
  ...
  Result.X := GLX.VectorGeometry.RadToDeg(-pitchangledif);
  Result.Y := GLX.VectorGeometry.RadToDeg(turnangledif);
end;
...
begin
    ...
    pitchNow := GLX.VectorGeometry.ArcCos(VectorDotProduct(AMovingObjectUp, normalT2C));
    pitchNow := ClampValue(pitchNow + GLX.VectorGeometry.DegToRad(pitchDelta), 0 + 0.025, PI -
      0.025);
    ...
    RotateVector(normalT2C, AMovingObjectUp, -GLX.VectorGeometry.DegToRad(turnDelta));
...
begin
  Result := GLX.VectorGeometry.ArcCos(VectorAngleCosine(
