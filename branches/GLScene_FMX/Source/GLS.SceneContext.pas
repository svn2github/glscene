...
{ : GLS.SceneContext<p>
  ...
  <li>22/12/14 - PW - Adapted  "GLWin32Context.pas" to "GLSceneContextFMX.pas"
...
resourcestring
  ...
  cBackwardContextFailed =
    'Can not create backward compatible context: #%X, %s';
  cFailHWRC =
    'Unable to create rendering context with hardware acceleration - down to software';
  ...
  glsDriverNotSupportOESRC =
    'Driver not support creating of OpenGL ES 2.0 context';
  glsDriverNotSupportDebugRC = 'Driver not support creating of debug context';
  glsOESvsForwardRC =
    'OpenGL ES 2.0 context incompatible with Forward context - flag ignored';
  glsFRC_created = 'Forward core context seccussfuly created';
  glsOESRC_created = 'OpenGL ES 2.0 context seccussfuly created';
  glsPBufferRC_created =
    'Backward compatible core PBuffer context successfully created';
...
const
  ...
  begin
    if not FGL.WChoosePixelFormatARB(DC, @FiAttribs[0], @FfAttribs[0], 32,
...
begin
      ...
      try
        ...
        try
          ...
          begin
            ...
            if nbFormats > 0 then
            begin
              if FGL.W_ARB_multisample and (AntiAliasing in [aaNone, aaDefault])
              then
              begin
                ...
                begin
                  ...
                  iValue := GL_FALSE;
                  FGL.WGetPixelFormatAttribivARB(ADeviceHandle, pixelFormat, 0,
                    1, @iAttrib, @iValue);
  ...
  if not FLegacyContextsOnly and FGL.W_ARB_create_context and
    (FAcceleration = chaHardware) then
...
begin
  try
    ...
    on E: Exception do
    begin
      raise Exception.Create(cUnableToCreateLegacyContext + #13#10 + E.ClassName
        + ': ' + E.message);
    end;
  end;
end;
          ...
          try
            ...
            try
              ...
              begin
                ...
                localRC := FGL.WCreateContextAttribsARB(localDC, 0,
                  @FiAttribs[0]);
                if localRC = 0 then
{$IFDEF GLS_LOGGING}
                begin
