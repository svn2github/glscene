...
type
  ...
  private
    ...
    procedure SetShader(AShader: TGLGLSLShader);
  ...
  protected
    ...
    property Shader: TGLGLSLShader read FShader write SetShader;
    { Primitive type. }
...
{$IFDEF VKS_REGION}{$REGION 'TGLVertexAttribute'}{$ENDIF}
...
function TGLVertexAttribute.GetLocation: GLint;
begin
  if FLocation < 0 then
...
{$IFDEF VKS_REGION}{$REGION 'TGLVertexAttributes'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLCustomFeedBackMesh'}{$ENDIF}
...
procedure TGLCustomFeedBackMesh.SetShader(AShader: TGLGLSLShader);
begin
