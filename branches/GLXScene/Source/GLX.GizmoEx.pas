...
type
  ...
  private
    ...
    FInterfaceRender: TGLDirectVulkan;
    FInternalRender: TGLDirectVulkan;
...
begin
  ...
  //BoundingBoxes...
  FInternalRender := TGLDirectVulkan(FUIBaseGizmo.AddNewChild(TGLDirectVulkan));
  FInternalRender.OnRender := InternalRender;

  FUIRootHelpers := TGLDummyCube(FUIBaseGizmo.AddNewChild(TGLDummyCube));

  //Canvas...
  FInterfaceRender := TGLDirectVulkan(FUIBaseGizmo.AddNewChild(TGLDirectVulkan));
...
procedure TGLGizmoEx.InternalRender(Sender: TObject; var rci: TGLRenderContextInfo);
  ...
  begin
    if not Assigned(FLabelFont) and (Text = '') then
      Exit;
    rci.VKStates.Enable(stDepthTest);
    FLayout := GLX.CrossPlatform.tlCenter;
    FAlignment := taCenter;
...
procedure TGLGizmoEx.SetVKGizmoExThickness(const Value: Single);
begin
