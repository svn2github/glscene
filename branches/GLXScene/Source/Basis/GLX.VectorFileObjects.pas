...
uses
  GLX.Strings, GLX.XOpenGL, GLX.CrossPlatform, GLX.MeshUtils, GLX.State, GLX.Utils,
  GLX.BaseMeshSilhouette, GLX.VectorTypes;
...
begin
  ...
  begin
    glBegin(GL_LINES);
    glVertex3fv(@TGLSkeletonBone(Owner).GlobalMatrix.W.X);
    glVertex3fv(@GlobalMatrix.W.X);
    glEnd;
  end;
  // render sub-bones
...
procedure TGLMeshObject.GetTriangleData(tri: Integer;
...
procedure TGLMeshObject.SetTriangleData(tri: Integer;
...
procedure TGLMeshObject.BuildTangentSpace(
  buildBinormals: Boolean = True;
  buildTangents: Boolean = True);
var
  i, j: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      frameIndex1 := GLX.VectorGeometry.Trunc(lerpFactor);
      frameIndex2 := frameIndex1 + 1;
      lerpFactor := GLX.VectorGeometry.Frac(lerpFactor);
    end;
    weight := 1;
...
// ------------------------------------------------------------------
  ...
  RegisterVectorFileFormat('glsm', 'VKScene Mesh', TGLGLSMVectorFile);

  RegisterClasses([TGLFreeForm, TGLActor, TGLSkeleton, TGLSkeletonFrame,
    TGLSkeletonBone, TGLSkeletonMeshObject, TGLMeshObject, TGLSkeletonFrameList, TGLMeshMorphTarget,
    TGLMorphableMeshObject, TGLFaceGroup, TFGVertexIndexList, TFGVertexNormalTexIndexList,
    TGLAnimationControler, TFGIndexTexCoordList, TGLSkeletonCollider, TGLSkeletonColliderList]);
