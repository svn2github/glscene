...
type
  ...
  private
    ...
    procedure DoAllocate(Sender: TGLVirtualHandle; var handle: GLuint);
    procedure DoDeallocate(Sender: TGLVirtualHandle; var handle: GLuint);
  public
  ...
  private
    ...
    FHandle: TGLVertexProgramHandle;
    FSource: TStringList;
    FSourceFile: string;
    FInfoLog: string;
    procedure SetSource(AValue: TStringList);
    procedure SetSourceFile(AValue: string);
    function GetHandle: TGLVertexProgramHandle;
  public
    ...
    procedure NotifyChange(Sender: TObject); override;
    property Handle: TGLVertexProgramHandle read GetHandle;
  published
...
const
  ...
  cTextureSwizzle: array[TGLTextureSwizzle] of GLEnum =
    (
    GL_RED,
    GL_GREEN,
    GL_BLUE,
...
const
  cTextureMode: array[TGLTextureMode] of GLEnum =
...
{$IFDEF VKS_REGION}{$REGION 'TGLBaseMaterialCollectionItem'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLFixedFunctionProperties'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLAbstractTexture'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLTextureImageEx'}{$ENDIF}
...
begin
  ...
  try
    ...
    if not IsDesignTime then
    begin
      FUseStreaming := FUseStreaming and (TGLUnpackPBOHandle.IsSupported > False) ;
...
procedure TGLTextureImageEx.SetInternalFormat(const AValue: GLinternalFormat);
begin
...
{$IFDEF VKS_REGION}{$REGION 'TGLTextureSampler'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLTextureCombiner'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLLibMaterialEx'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLMultitexturingProperties'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLTextureProperties'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLShaderEx'}{$ENDIF}
...
procedure TGLShaderEx.SetGeometryVerticesOut(AValue: GLint);
begin
  if AValue < 1 then
    AValue := 1
...
{$IFDEF VKS_REGION}{$REGION 'TGLLibMaterialProperty'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLLibMaterialsEx'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLBaseShaderModel'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLMatLibComponents'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLMaterialLibraryEx'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLShaderUniformTexture'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLAbstractShaderUniform'}{$ENDIF}
...
function TGLAbstractShaderUniform.GetInt: GLint;
begin
  FillChar(Result, SizeOf(Result), $00);
end;
...
function TGLAbstractShaderUniform.GetUInt: GLuint;
begin
  FillChar(Result, SizeOf(Result), $00);
end;
...
procedure TGLAbstractShaderUniform.SetFloat(const Value: GLfloat);
begin
end;
...
{$IFDEF VKS_REGION}{$REGION 'TGLShaderUniform'}{$ENDIF}
...
function TGLShaderUniform.GetInt: GLint;
begin
...
function TGLShaderUniform.GetProgram: GLuint;
begin
...
function TGLShaderUniform.GetUInt: GLuint;
begin
...
procedure TGLShaderUniform.SetFloat(const Value: GLfloat);
begin
  PushProgram;
...
{$IFDEF VKS_REGION}{$REGION 'TGLShaderUniformDSA'}{$ENDIF}

procedure TGLShaderUniformDSA.SetFloat(const Value: GLfloat);
begin
...
{$IFDEF VKS_REGION}{$REGION 'TGLTextureSwizzling'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLFrameBufferAttachment'}{$ENDIF}
...
{$IFDEF VKS_REGION}{$REGION 'TGLASMVertexProgram'}{$ENDIF}
...
begin
  inherited;
  FHandle := TGLVertexProgramHandle.Create;
  FHandle.OnPrapare := DoOnPrepare;
  FSource := TStringList.Create;
  FSource.OnChange := NotifyChange;
...
function TGLASMVertexProgram.GetHandle: TGLVertexProgramHandle;
begin
  Result := FHandle;
end;
