...
uses
  ...
  GLX.OpenGLAdapter,
  GLX.Strings, GLX.CrossPlatform, GLX.BaseClasses,
  GLX.VectorGeometry, GLX.Graphics, GLX.Context, GLX.State, GLX.Color, GLX.Coordinates,
  GLX.RenderContextInfo, GLX.TextureFormat, GLX.ApplicationFileIO, GLX.Utils;
...
type
  ...
  //
  TGLTextureCompression = VKinternalCompression;
 ...
 TGLCubeMapTarget = Integer;
  ...
  // TGLTextureExItem
  //
  TGLTextureExItem = class(TCollectionItem, IVKTextureNotifyAble)
  private
...
procedure RegisterTextureImageClass(textureImageClass: TGLTextureImageClass);
// Finds a registerer TGLTextureImageClass using its classname
function FindTextureImageClass(const className: string): TGLTextureImageClass;
...
// TODO: remove dependancy on GLX.Scene.pas unit (related to tmmCubeMapLight0)
...
{$IFDEF VKS_REGIONS}{$REGION 'TGLTextureImage'}{$ENDIF}
...
procedure TGLTextureImage.LoadTexture(AInternalFormat: GLinternalFormat);
begin
end;
...
{$IFDEF VKS_REGIONS}{$REGION 'TGLBlankImage'}{$ENDIF}
...
{$IFDEF VKS_REGIONS}{$REGION 'TGLPictureImage'}{$ENDIF}
...
{$IFDEF VKS_REGIONS}{$REGION 'TGLPersistentImage'}{$ENDIF}
...
{$IFDEF VKS_REGIONS}{$REGION 'TGLPicFileImage'}{$ENDIF}
...
{$IFDEF VKS_REGIONS}{$REGION 'TGLCubeMapImage'}{$ENDIF}
...
{$IFDEF VKS_REGIONS}{$REGION 'TGLTexture'}{$ENDIF}
...
procedure TGLTexture.SetTextureFormatEx(const val: GLinternalFormat);
begin
...
function TGLTexture.AllocateHandle: GLuint;
var
...
function TGLTexture.GetHandle: GLuint;
var
  target: GLuint;
  LBinding: array[TGLTextureTarget] of GLuint;
...
function TGLTexture.VulkanTextureFormat: Integer;
var
...
procedure TGLTexture.PrepareImage(target: GLuint);
var
  bitmap32: TGLImage;
  texComp: TGLTextureCompression;
  glFormat: GLEnum;
...
procedure TGLTexture.PrepareParams(target: GLuint);
const
...
{$IFDEF VKS_REGIONS}{$REGION 'TGLTextureExItem'}{$ENDIF}
...
var
  intf: IVKTextureNotifyAble;
begin
  if Supports(TObject(TGLTextureEx(Collection).FOwner), IVKTextureNotifyAble,
    intf) then
...
{$IFDEF VKS_REGIONS}{$REGION 'TGLTextureEx'}{$ENDIF}
...
initialization
  ...
  RegisterTextureImageClass(TGLBlankImage);
  RegisterTextureImageClass(TGLPersistentImage);
  RegisterTextureImageClass(TGLPicFileImage);
  RegisterTextureImageClass(TGLCubeMapImage);
