...
//
{
  ...
  Ru:
  TGLLanguage создан для локализации вашего приложения
...
type
  ...
  { **
    * Eng
    *   Class TGLLanguage is used for downloading and translation, as in the final product it's no need for text processing.
    * Ru
    *   Класс TGLLanguage используется толко для загрузки и перевода текста, так как в конечном
