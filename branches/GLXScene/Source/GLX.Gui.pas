...
uses
  ...
  GLX.Scene, GLX.BitmapFont, GLX.Material, GLX.CrossPlatform, Winapi.OpenGL, Winapi.OpenGLext,  GLX.Context,
  GLX.PersistentClasses, GLX.VectorGeometry, GLX.Coordinates, GLX.BaseClasses;
...
procedure TGLBaseGuiObject.SetLeft(const Value: GLfloat);
var
  NewPosX: GLfloat;
  i: integer;
  Diff: GLfloat;
...
function TGLBaseGuiObject.GetLeft: GLfloat;
begin
  if Assigned(Parent) and (Parent is TGLBaseGuiObject) then
...
procedure TGLBaseGuiObject.SetTop(const Value: GLfloat);
var
  NewPosY: GLfloat;
  i: integer;
  Diff: GLfloat;
...
function TGLBaseGuiObject.GetTop: GLfloat;
begin
  if Assigned(Parent) and (Parent is TGLBaseGuiObject) then
...
procedure TGLBaseGuiObject.SetWidth(const val: GLfloat);
begin
  if FWidth <> val then
  begin
    FWidth := val;
...
procedure TGLBaseGuiObject.SetHeight(const val: GLfloat);
begin
  if FHeight <> val then
  begin
    FHeight := val;
...
procedure TGLGuiComponent.RenderToArea(X1, Y1, X2, Y2: GLfloat; var Res:
