...
unit DGLCrossPlatform;

interface

{$INCLUDE DGLEngine.inc}
...
type
  ...
  TDGLPicture = TPicture;
  TDGLGraphic = TGraphic;
  TDGLBitmap = TBitmap;
  TGraphicClass = class of TGraphic;

  TDGLTextLayout = (tlTop, tlCenter, tlBottom); // idem TTextLayout;

  TDGLMouseButton = (mbLeft, mbRight, mbMiddle); // idem TMouseButton;
  TDGLMouseEvent = procedure(Sender: TObject; Button: TDGLMouseButton;
    Shift: TShiftState; X, Y: Integer) of object;
  TDGLMouseMoveEvent = TMouseMoveEvent;
  TDGLKeyEvent = TKeyEvent;
  TDGLKeyPressEvent = TKeyPressEvent;
  ...
  TDGLComponent = class(TComponent);
...
function GLPoint(const x, y: Integer): TDGLPoint;
{ @HTML ( Builds a TColor from Red Green Blue components. }
function RGB(const r, g, b: Byte): TColor; {$NODEFINE RGB}
function GLRect(const aLeft, aTop, aRight, aBottom: Integer): TDGLRect;{$NODEFINE GLRect}
{ @HTML ( Increases or decreases the width and height of the specified rectangle.<p>
   Adds dx units to the left and right ends of the rectangle and dy units to
   the top and bottom. }
procedure InflateGLRect(var aRect: TDGLRect; dx, dy: Integer);
procedure IntersecTDGLRect(var aRect: TDGLRect; const rect2: TDGLRect);
function PtInRect(const Rect: TDGLRect; const P: TPoint): Boolean;

procedure RaiseLastOSError;

{ @HTML ( Number of pixels per logical inch along the screen width for the device.<p>
   Under Win32 awaits a HDC and returns its LOGPIXELSX. }
function GetDeviceLogicalPixelsX(device: HDC): Integer;
{ @HTML ( Number of bits per pixel for the current desktop resolution. }
function GetCurrentColorDepth: Integer;
{ @HTML ( Returns the number of color bits associated to the given pixel format. }
function PixelFormatToColorBits(aPixelFormat: TPixelFormat): Integer;

{ @HTML ( Returns the bitmap's scanline for the specified row. }
function BitmapScanLine(aBitmap: TDGLBitmap; aRow: Integer): Pointer;

{ @HTML ( Replace path delimiter to delimiter of the current platform. }
procedure FixPathDelimiter(var S: string);
{ @HTML ( Remove if possible part of path witch leads to project executable. }
function RelativePath(const S: string): string;
{ @HTML ( Returns the current value of the highest-resolution counter.<p>
...
{ @HTML ( Returns the frequency of the counter used by QueryPerformanceCounter.<p>
...
{ @HTML ( Starts a precision timer.<p>
...
{ @HTML ( Computes time elapsed since timer start.<p>
   Return time lap in seconds. }
function PrecisionTimerLap(const precisionTimer: Int64): Double;
{ @HTML ( Computes time elapsed since timer start and stop timer.<p>
   Return time lap in seconds. }
...
{ @HTML ( Returns the number of CPU cycles since startup.<p>
...
{ @HTML ( Determine if the directory is writable.<p> }
...
function GLRect(const aLeft, aTop, aRight, aBottom: Integer): TDGLRect;
begin
  Result.Left := aLeft;
  Result.Top := aTop;
  Result.Right := aRight;
