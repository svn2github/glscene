...
type
  ...
  { @HTML ( Parses a TC text description and setups combiners accordingly.<p>
     *experimental*<br>
...
begin
  ...
  begin
    TCAssertCheck(dglCheckExtension('ARB_texture_env_crossbar') , 'Requires GL_ARB_texture_env_crossbar');
    n := StrToIntDef(Copy(arg, 4, MaxInt), -1);
    if n in [0..7] then
      sourceValue := GL_TEXTURE0_ARB + n;
  end;
  TCAssertCheck((operandValue > 0) and (sourceValue > 0),'invalid argument : "' + origArg + '"');
...
//      operEnum := GL_ADD;
  ...
  begin
    // MODULATE operator
    operEnum :=GL_COMBINE_EXT;// GL_MODULATE;
    arg1 := Copy(line, 1, p - 1);
    arg2 := Copy(line, p + 1, MaxInt);
    line := '';
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        TCAssertCheck(dglCheckExtension('ARB_texture_env_dot3'), 'Requires GL_ARB_texture_env_dot3');
...
begin
  vCommandCache := nil;
  TCAssertCheck(dglCheckExtension('ARB_texture_env_combine'), 'Requires GL_ARB_texture_env_combine support');
  sl := TStringList.Create;
  try
    sl.Assign(tcCode);
