...
procedure Swap(var A, B: Integer); {$IFDEF GLS_INLINE} inline; {$ENDIF}
var
  C: Integer;
begin
  C := A;
...
const
  cConvertTable: array [0 .. 35] of TConvertTableRec = ((type_: GL_UNSIGNED_BYTE; proc1: UbyteToImf; proc2: ImfToUbyte),
  ...
  // ---------------
  { Process Functions }
  {$IFDEF GLS_REGIONS}{$REGION 'Process Functions'}{$ENDIF}
...
begin
  ...
  try
    ...
    begin
      ...
      // Back to native image format
      ConvertFromIntermediateFormat(tempBuf1, ADst[level], AColorFormat, ASrcWidth, ASrcHeight);
    end;
  finally
...
begin
  ...
  try
    ...
    // Back to native image format
    ConvertFromIntermediateFormat(tempBuf1, ASrc, AColorFormat, ASrcWidth, ASrcHeight);
