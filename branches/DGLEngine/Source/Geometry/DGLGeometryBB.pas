...
const
  NullBoundingBox: THmgBoundingBox = (BBox: ((X: 0; Y: 0; Z: 0; W: 1), (X: 0; Y: 0; Z: 0; W: 1), (X: 0; Y: 0; Z: 0; W: 1), (X: 0; Y: 0; Z: 0; W: 1), (X: 0; Y: 0; Z: 0; W: 1), (X: 0; Y: 0; Z: 0; W: 1), (X: 0; Y: 0; Z: 0; W: 1), (X: 0; Y: 0; Z: 0; W: 1)));
...
function AABBToClipRect(const Aabb: TAABB; ModelViewProjection: TMatrix; ViewportSizeX, ViewportSizeY: Integer): TClipRect;

{ : Finds the intersection between a ray and an axis aligned bounding box. }
function RayCastAABBIntersect(const RayOrigin, RayDirection: TVector; const Aabb: TAABB; out TNear, TFar: Single): Boolean; overload;
function RayCastAABBIntersect(const RayOrigin, RayDirection: TVector; const Aabb: TAABB; IntersectPoint: PVector = nil): Boolean; overload;
...
const
  ...
  CBBPlans: TPlanBB       = ((0, 1, 2, 3), (4, 5, 6, 7), (0, 4, 7, 3), (1, 5, 6, 2), (0, 1, 5, 4), (2, 3, 7, 6));
...
begin
  ...
  begin
    ...
    if IsInRange(Pt.V[0], Aabb1.Min.V[0], Aabb1.Max.V[0]) and IsInRange(Pt.V[1], Aabb1.Min.V[1], Aabb1.Max.V[1]) and IsInRange(Pt.V[2], Aabb1.Min.V[2], Aabb1.Max.V[2]) then
    begin
      Result := True;
      Exit;
    end;
  ...
  begin
    ...
    if IsInRange(Pt1[I].V[0], Aabb2.Min.V[0], Aabb2.Max.V[0]) and IsInRange(Pt1[I].V[1], Aabb2.Min.V[1], Aabb2.Max.V[1]) and IsInRange(Pt1[I].V[2], Aabb2.Min.V[2], Aabb2.Max.V[2]) then
    begin
      Result := True;
      Exit;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        // check point in Wire
        if IsInRange(P.V[0], Pt1[CWires[I, 0]].V[0], Pt1[CWires[I, 1]].V[0]) and IsInRange(P.V[1], Pt1[CWires[I, 0]].V[1], Pt1[CWires[I, 1]].V[1]) and IsInRange(P.V[2], Pt1[CWires[I, 0]].V[2], Pt1[CWires[I, 1]].V[2]) then
        begin
          // check point in Plane
          if IsInRange(P.V[0], Pt2[CPlanes[T, 0]].V[0], Pt2[CPlanes[T, 2]].V[0]) and IsInRange(P.V[1], Pt2[CPlanes[T, 0]].V[1], Pt2[CPlanes[T, 2]].V[1]) and IsInRange(P.V[2], Pt2[CPlanes[T, 0]].V[2], Pt2[CPlanes[T, 2]].V[2]) then
          begin
            Result := True;
            Exit;
          end;
...
begin
  ...
  if (Aabb2.Min.V[0] > Aabb1.Max.V[0]) or (Aabb2.Min.V[1] > Aabb1.Max.V[1]) then
    Exit
  else if (Aabb2.Max.V[0] < Aabb1.Min.V[0]) or (Aabb2.Max.V[1] < Aabb1.Min.V[1]) then
    Exit
  else
    Result := True;
...
function IntersectAABBsAbsoluteXZ(const Aabb1, Aabb2: TAABB): Boolean;
begin
  Result := ((Aabb1.Min.V[0] < Aabb2.Max.V[0]) and (Aabb1.Min.V[2] < Aabb2.Max.V[2]) and

    (Aabb2.Min.V[0] < Aabb1.Max.V[0]) and (Aabb2.Min.V[2] < Aabb1.Max.V[2]));
end;
...
function IntersectAABBsAbsolute(const Aabb1, Aabb2: TAABB): Boolean;
begin
  Result := not((Aabb1.Min.V[0] > Aabb2.Max.V[0]) or (Aabb1.Min.V[1] > Aabb2.Max.V[1]) or (Aabb1.Min.V[2] > Aabb2.Max.V[2]) or

    (Aabb2.Min.V[0] > Aabb1.Max.V[0]) or (Aabb2.Min.V[1] > Aabb1.Max.V[1]) or (Aabb2.Min.V[2] > Aabb1.Max.V[2]));
end;
...
begin
  ...
  Result := (Aabb1.Min.V[0] >= Aabb2.Min.V[0]) and (Aabb1.Min.V[1] >= Aabb2.Min.V[1]) and (Aabb1.Min.V[2] >= Aabb2.Min.V[2]) and

    (Aabb1.Max.V[0] <= Aabb2.Max.V[0]) and (Aabb1.Max.V[1] <= Aabb2.Max.V[1]) and (Aabb1.Max.V[2] <= Aabb2.Max.V[2]);
end;
...
begin
  Result := (P.V[0] <= Aabb.Max.V[0]) and (P.V[0] >= Aabb.Min.V[0]) and (P.V[1] <= Aabb.Max.V[1]) and (P.V[1] >= Aabb.Min.V[1]) and (P.V[2] <= Aabb.Max.V[2]) and (P.V[2] >= Aabb.Min.V[2]);
end;
...
var
  ...
  function EdgesStripPlaneIntersection(const pt0, Pt1, pt4, pt7: TVector3f; const plane: THmgPlane; var inter: TVector3f): Boolean;
  begin
    Result := True;
...
procedure ExtractAABBCorners(const Aabb: TAABB; var AABBCorners: TAABBCorners);
begin
  MakeVector(AABBCorners[0], Aabb.Min.V[0], Aabb.Min.V[1], Aabb.Min.V[2]);
  MakeVector(AABBCorners[1], Aabb.Min.V[0], Aabb.Min.V[1], Aabb.Max.V[2]);
  MakeVector(AABBCorners[2], Aabb.Min.V[0], Aabb.Max.V[1], Aabb.Min.V[2]);
  MakeVector(AABBCorners[3], Aabb.Min.V[0], Aabb.Max.V[1], Aabb.Max.V[2]);

  MakeVector(AABBCorners[4], Aabb.Max.V[0], Aabb.Min.V[1], Aabb.Min.V[2]);
  MakeVector(AABBCorners[5], Aabb.Max.V[0], Aabb.Min.V[1], Aabb.Max.V[2]);
  MakeVector(AABBCorners[6], Aabb.Max.V[0], Aabb.Max.V[1], Aabb.Min.V[2]);
  MakeVector(AABBCorners[7], Aabb.Max.V[0], Aabb.Max.V[1], Aabb.Max.V[2]);
end;
...
begin
  ...
  if ((MainAABB.Min.V[0] < TestAABB.Max.V[0]) and (MainAABB.Min.V[1] < TestAABB.Max.V[1]) and (MainAABB.Min.V[2] < TestAABB.Max.V[2]) and

    (TestAABB.Min.V[0] < MainAABB.Max.V[0]) and (TestAABB.Min.V[1] < MainAABB.Max.V[1]) and (TestAABB.Min.V[2] < MainAABB.Max.V[2])) then
  begin
    if (TestAABB.Min.V[0] >= MainAABB.Min.V[0]) and (TestAABB.Min.V[1] >= MainAABB.Min.V[1]) and (TestAABB.Min.V[2] >= MainAABB.Min.V[2]) and

      (TestAABB.Max.V[0] <= MainAABB.Max.V[0]) and (TestAABB.Max.V[1] <= MainAABB.Max.V[1]) and (TestAABB.Max.V[2] <= MainAABB.Max.V[2]) then
...
begin
  ...
  begin
    ...
    begin
      if PlaneEvaluatePoint(TPlaneArray(Frustum)[IPlane], AABBCorners[ICorner]) < 0 then
      begin
        PointIn := False;
        Dec(InCount);
      end;
...
begin
  ...
  begin
    ...
    begin
      if ((RayOrigin.V[P] < Aabb.Min.V[P]) or (RayOrigin.V[P] > Aabb.Max.V[P])) then
        Exit;
    end
    else
    begin
