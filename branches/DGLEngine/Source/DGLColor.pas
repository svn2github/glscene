...
uses
  ...
  // DGLE
  DGLCrossPlatform, dglOpenGL,
  DGLPersistentClasses, DGLBaseClasses,
  DGLVectorTypes, DGLVectorMaths;
...
type
  ...
  TRGBColor = TVector3b;
   ...
   TDGLColor = class(TDGLUpdateAbleObject)
      private
         { Private Properties }
			FColor : TColorVector; FPDefaultColor : PColorVector;
      ...
      procedure SetHSVA(const hsva : TVector);
		...
		public
         { Public Properties }
			...
			constructor CreateInitialized(AOwner : TPersistent; const color : TColorVector; changeEvent : TNotifyEvent = nil);
      destructor Destroy; override;
      procedure NotifyChange(Sender : TObject); override;
			procedure Assign(Source : TPersistent); override;
			procedure Initialize(const color : TColorVector);
			function AsAddress : PGLFloat; //PSingle;
  ...
  published
    ...
    property Red: Single index 0 read GetColorComponent write SetColorComponent stored False;
    property Green: Single index 1 read GetColorComponent write SetColorComponent stored False;
    property Blue: Single index 2 read GetColorComponent write SetColorComponent stored False;
    property Alpha: Single index 3 read GetColorComponent write SetColorComponent stored False;
	end;
...
function ColorManager: TDGLColorManager;
...
var
   // Specifies if TDGLColor should allocate memory for
...
var
	vColorManager : TDGLColorManager;
...
begin
	if not Assigned(vColorManager) then begin
		vColorManager:=TDGLColorManager.Create;
...
// ------------------
{ TDGLColor }
{$IFDEF GLS_REGIONS}{$REGION 'TDGLColor'}{$ENDIF}

constructor TDGLColor.Create(AOwner: TPersistent);
begin
   inherited;
   Initialize(clrBlack);
end;

constructor TDGLColor.CreateInitialized(AOwner : TPersistent; const color : TColorVector; changeEvent : TNotifyEvent = nil);
begin
   Create(AOwner);
   Initialize(color);
...
destructor TDGLColor.Destroy;
begin
   if assigned(FPDefaultColor) then Dispose(FPDefaultColor);
   inherited;
end;

procedure TDGLColor.Initialize(const color : TColorVector);
begin
...
procedure TDGLColor.SetColorVector(const aColor : TColorVector);
begin
   SetVector(FColor, AColor);
	NotifyChange(Self);
end;

procedure TDGLColor.SetDirectColorVector(const AColor: TColorVector);
begin
  SetVector(FColor, AColor);
end;

procedure TDGLColor.SetColorComponent(index : Integer; value : Single);
begin
...
procedure TDGLColor.SetAsWinColor(const val : TColor);
begin
	FColor:=ConvertWinColor(val);
	NotifyChange(Self);
end;

function TDGLColor.GetAsWinColor : TColor;
begin
	Result:=ConvertColorVector(FColor);
end;

function TDGLColor.GetColorComponent(const Index: Integer): Single;
begin
  Result := FColor.V[Index];
end;

procedure TDGLColor.Assign(Source: TPersistent);
begin
   if Assigned(Source) and (Source is TDGLColor) then
   begin
		FColor:=TDGLColor(Source).FColor;
      NotifyChange(Self);
   end else inherited;
end;

procedure TDGLColor.DefineProperties(Filer: TFiler);
begin
  inherited;
  Filer.DefineBinaryProperty('Color', ReadData, WriteData, not (Assigned(FPDefaultColor) and VectorEquals(FColor, FPDefaultColor^)));
end;

procedure TDGLColor.ReadData(Stream: TStream);
begin
   Stream.Read(FColor, SizeOf(FColor));
end;

procedure TDGLColor.WriteData(Stream: TStream);
begin
   Stream.Write(FColor, SizeOf(FColor));
end;

procedure TDGLColor.NotifyChange(Sender : TObject);
var
  intf: IDGLNotifyable;
begin
   if Assigned(Owner) then
   begin
      if Supports(Owner,IDGLNotifyable, intf) then
...
function TDGLColor.AsAddress: PGLfloat;
begin
	Result:=@FColor;
end;

procedure TDGLColor.RandomColor;
begin
   Red:=Random;
   Green:=Random;
   Blue:=Random;
...
procedure TDGLColor.SetColor(red, green, blue : Single; alpha : Single = 1);
begin
   FColor.V[0]:=red;
   FColor.V[1]:=Green;
   FColor.V[2]:=blue;
...
function TDGLColor.GetHSVA : TVector;
var
   delta, min : Single;
const
   H = 0;
...
procedure TDGLColor.SetHSVA(const hsva : TVector);
var
...
// ------------------
{ TDGLColorManager }
{$IFDEF GLS_REGIONS}{$REGION 'TDGLColorManager'}{$ENDIF}

function TDGLColorManager.FindColor(const aName: String): TColorVector;
var
   i : Integer;
begin
   Result:=clrBlack;
...
function TDGLColorManager.GetColor(const aName : String): TColorVector;
var
   workCopy  : String;
   delimiter : Integer;
begin
...
function TDGLColorManager.GetColorName(const aColor: TColorVector): String;
const MinDiff = 1e-6;
var I : Integer;
begin
  for I:=0 to Count-1 do
  ...
  if I < Count then
    ...
    Result:=Format('<%.3f %.3f %.3f %.3f>',[AColor.V[0],AColor.V[1], AColor.V[2],AColor.V[3]]);
end;

destructor TDGLColorManager.Destroy;
var
   i : Integer;
begin
   for i:=0 to Count-1 do
...
procedure TDGLColorManager.AddColor(const aName: String; const aColor: TColorVector);
var
...
procedure TDGLColorManager.EnumColors(Proc: TGetStrProc);
var
   i : Integer;
begin
   for i:=0 to Count-1 do
...
procedure TDGLColorManager.EnumColors(AValues: TStrings);
var
   i : Integer;
begin
   for i:=0 to Count-1 do
...
procedure TDGLColorManager.RegisterDefaultColors;
begin
   Capacity:=150;
...
procedure TDGLColorManager.RemoveColor(const aName: String);
var
   i : Integer;
begin
   for i:=0 to Count-1 do
