...
{ @HTML ( DGLFileDDS<p>
 ...
 <b>History : </b><font size=-1><ul>
        ...
        <li>20/01/10 - JD - Import from GLScene
   </ul><p>
}
unit DGLFileDDS;
...
uses
  ...
  //DGLE
  DGLCrossPlatform, DGLTypes, dglOpenGL, DGLContext, DGLGraphics, DGLTextureFormat,
  DGLSrgbe, DGLApplicationFileIO, DGLVectorMaths, DGLResStrings;
...
type
  ...
  TDGLDDSDetailLevels = (ddsHighDet, ddsMediumDet, ddsLowDet);

  // TDGLDDSImage
  //

  TDGLDDSImage = class(TDGLBaseImage)
  private
...
var
  ...
  vDDSDetailLevel: TDGLDDSDetailLevels = ddsHighDet;
...
// ------------------
{ TDGLDDSImage }
{$IFDEF GLS_REGION}{$REGION 'TDGLDDSImage'}{$ENDIF}

procedure TDGLDDSImage.LoadFromFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TDGLDDSImage.SaveToFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TDGLDDSImage.LoadFromStream(stream: TStream);
var
  header: TDDSHeader;
...
procedure TDGLDDSImage.SaveToStream(stream: TStream);
const
...
procedure TDGLDDSImage.AssignFromTexture(textureContext: TDGLContext;
  const textureHandle: TGLuint;
  textureTarget: TDGLTextureTarget;
  const CurrentFormat: Boolean;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if dglCheckExtension('NV_texture_compression_vtc') and (FLOD[level].Depth > 0)
...
procedure TDGLDDSImage.flipSurface(chgData: PGLubyte; w, h, d: integer);
var
  lineSize: integer;
  sliceSize: integer;
  tempBuf: PGLubyte;
...
initialization
  ...
  RegisterRasterFormat('dds', 'Direct Draw Surface', TDGLDDSImage);
