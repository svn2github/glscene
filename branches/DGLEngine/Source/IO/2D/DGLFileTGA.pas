...
type
  ...
  TDGLTGAImage = class(TDGLBaseImage)
  public
    { Public Declarations }
...
// ------------------
{ TDGLTGAImage }
{$IFDEF GLS_REGION}{$REGION 'TDGLTGAImage'}{$ENDIF}

procedure TDGLTGAImage.LoadFromFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TDGLTGAImage.SaveToFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TDGLTGAImage.LoadFromStream(stream: TStream);
var
...
procedure TDGLTGAImage.SaveToStream(stream: TStream);
begin
{$MESSAGE Hint 'TDGLTGAImage.SaveToStream not yet implemented' }
end;

procedure TDGLTGAImage.AssignFromTexture(textureContext: TDGLContext;
...
initialization
  ...
  RegisterRasterFormat('tga', 'TARGA Image File', TDGLTGAImage);
