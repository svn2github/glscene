...
{: DGLFileHDR<p>
 ...
 <b>History : </b><font size=-1><ul>
        <li>28/12/15 - Yar - Imported From GLScene
   </ul><p>
}
unit DGLFileHDR;
...
type
  ...
  TDGLHDRImage = class(TDGLBaseImage)
  private
...
procedure TDGLHDRImage.LoadFromFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TDGLHDRImage.LoadFromStream(stream: TStream);
const
...
begin
  ...
  FLOD[0].Depth := 0;
  fColorFormat := GL_RGB;
  fInternalFormat := tfRGBA32F;
  fDataType := GL_FLOAT;
  fCubeMap := false;
  fTextureArray := false;
  fElementSize := GetTextureElementSize(tfRGB32F);
...
function TDGLHDRImage.GetProgramType: Ansistring;
begin
  Result := fProgramType;
end;

procedure TDGLHDRImage.SetProgramType(aval: Ansistring);
var
  i: integer;
begin
...
procedure TDGLHDRImage.AssignFromTexture(textureContext: TDGLContext;
  const textureHandle: TGLuint;
  textureTarget: TDGLTextureTarget;
  const CurrentFormat: Boolean;
...
initialization
  ...
  RegisterRasterFormat('hdr', 'High Dynamic Range Image', TDGLHDRImage);
