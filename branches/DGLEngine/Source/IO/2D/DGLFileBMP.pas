...
{: GLFileBMP<p>
 ...
 <b>History : </b><font size=-1><ul>
        <li>04/04/11 - JD - Imported from GLScene
   </ul></font>
}
...
type
  ...
  TDGLBMPImage = class(TDGLBaseImage)
  private
    ...
    function ExpandColor(Value: longword): TDGLPixel32;
...
// ------------------
{ TDGLBMPImage }
{$IFDEF GLS_REGIONS}{$REGION 'TDGLBMPImage'}{$ENDIF}

procedure TDGLBMPImage.LoadFromFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TDGLBMPImage.SaveToFile(const filename: string);
var
  fs: TStream;
begin
...
function TDGLBMPImage.CountBits(Value: byte): shortint;
var
  i, bits: shortint;
begin
  bits := 0;
...
function TDGLBMPImage.ShiftCount(Mask: longword): shortint;
var
  tmp: shortint;
begin
  tmp := 0;
...
function TDGLBMPImage.ExpandColor(Value: longword): TDGLPixel32;
var
...
function TDGLBMPImage.Monochrome(N: Integer): Integer;
begin
  Result := (FLineBuffer[N div 8] shr (7 - (N and 7))) and 1;
end;

function TDGLBMPImage.Quadrochrome(N: Integer): Integer;
begin
  Result := (FLineBuffer[N div 2] shr (((N + 1) and 1) * 4)) and $0F;
end;

function TDGLBMPImage.Octochrome(N: Integer): Integer;
begin
  Result := FLineBuffer[N];
end;

procedure TDGLBMPImage.LoadFromStream(stream: TStream);
type
...
begin
  ...
  begin
    ...
    if LInfo.ClrUsed > 0 then
      Stream.Read(LPalette[0], LInfo.ClrUsed * SizeOf(TDGLPixel32))
    else // Seems to me that this is dangerous.
      Stream.Read(LPalette[0], nPalette * SizeOf(TDGLPixel32));
  end
...
procedure TDGLBMPImage.ExpandRLE4ScanLine(Row: Integer; Stream: TStream);
var
  i, j, tmpsize: integer;
  b0, b1: byte;
  nibline: PByteArray;
...
procedure TDGLBMPImage.ExpandRLE8ScanLine(Row: Integer; Stream: TStream);
var
  i, j: integer;
  b0, b1: byte;
begin
...
procedure TDGLBMPImage.SaveToStream(stream: TStream);
begin
  {$Message Hint 'TDGLBMPImage.SaveToStream not yet implemented' }
end;

procedure TDGLBMPImage.AssignFromTexture(textureContext: TDGLContext;
...
initialization
  ...
  RegisterRasterFormat('bmp', 'Bitmap Image File', TDGLBMPImage);
