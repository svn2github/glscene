...
uses
  ...
  //DGLE
  DGLCrossPlatform, DGLTypes, dglOpenGL, DGLContext, DGLGraphics,
  DGLTextureFormat, DGLApplicationFileIO;
...
{ TDGLPNGImage }
{$IFDEF GLS_REGION}{$REGION 'TDGLPNGImage'}{$ENDIF}

procedure TDGLPNGImage.LoadFromFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TDGLPNGImage.SaveToFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TDGLPNGImage.LoadFromStream(stream: TStream);
var
...
procedure TDGLPNGImage.SaveToStream(stream: TStream);
var
  png_ptr: png_structp;
  info_ptr: png_infop;
...
begin
  ...
  try
    ...
    case fColorFormat of
      GL_RED: color_type := PNG_COLOR_TYPE_GRAY;
      GL_RG: color_type := PNG_COLOR_TYPE_GRAY_ALPHA;
...
procedure TDGLPNGImage.AssignFromTexture(textureContext: TDGLContext;
  const textureHandle: TGLuint;
  textureTarget: TDGLTextureTarget;
  const CurrentFormat: Boolean;
...
initialization
  ...
  RegisterRasterFormat('png', 'Portable Network Graphic', TDGLPNGImage);
