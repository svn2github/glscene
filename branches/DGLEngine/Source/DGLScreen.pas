...
uses
  ...
  DGLVectorMaths, DGLTypes, DGLCrossPlatform;
...
procedure GLGetCursorPos(var point: TDGLPoint);
...
const
  ...
  {$IFDEF MSWINDOWS}
  LowResModes: array [0 .. NumberLowResModes - 1] of TLowResMode = ((Width: 320; Height: 200; ColorDepth: 8), (Width: 320; Height: 200; ColorDepth: 15), (Width: 320; Height: 200; ColorDepth: 16), (Width: 320; Height: 200; ColorDepth: 24),
    (Width: 320; Height: 200; ColorDepth: 32), (Width: 400; Height: 300; ColorDepth: 8), (Width: 400; Height: 300; ColorDepth: 15), (Width: 400; Height: 300; ColorDepth: 16), (Width: 400; Height: 300; ColorDepth: 24), (Width: 400; Height: 300;
    ColorDepth: 32), (Width: 512; Height: 384; ColorDepth: 8), (Width: 512; Height: 384; ColorDepth: 15), (Width: 512; Height: 384; ColorDepth: 16), (Width: 512; Height: 384; ColorDepth: 24), (Width: 512; Height: 384; ColorDepth: 32));
  {$ENDIF}
...
begin
  ...
  for I := 1 to vNumberVideoModes - 1 do
    ...
    begin
      if (Width >= XRes) and ((Width - XRes) <= XDiff) and (Height >= YRes) and ((Height - YRes) <= YDiff) and (ColorDepth >= BPP) and ((ColorDepth - BPP) <= CDiff) then
      begin
        XDiff  := Width - XRes;
        YDiff  := Height - YRes;
  ...
  with vVideoModes[I]^ do
  begin
    if (hDisplay >= XRes) and ((hDisplay - XRes) <= XDiff) and (vDisplay >= YRes) and ((vDisplay - YRes) <= YDiff) then
    begin
      XDiff  := hDisplay - XRes;
      YDiff  := vDisplay - YRes;
      Result := I;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          I := 0;
          while (I < NumberLowResModes - 1) and (vNumberVideoModes < MaxVideoModes) do
          begin
      ...
      begin
        ...
        if not vScreenModeChanged then
          if XF86VidModeGetModeLine(vDisplay, vCurrentVideoMode, @vDesktop.dotclock, PXF86VidModeModeLine(PtrUInt(@vDesktop) + Sizeof(vDesktop.dotclock))) then
            TryToAddToList;
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        ...
                        point.y := 0;
                        dpy     := XOpenDisplay(nil);
                        Result  := LongBool(XQueryPointer(dpy, XDefaultRootWindow(dpy), @root, @child, @rootX, @rootY, @winX, @winY, @xstate));
                        If Result then
                        begin
                          point.x := rootX;
                          point.y := rootY;
