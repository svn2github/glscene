...
{ : GLScene inner logger.
    ...
    which is not what one might want. This also allows to create a DGLSLogger with
...
procedure UseCustomDGLSLogger(const ALogger: TLogSession);
...
function DGLSLogger(): TLogSession;
begin
  if v_DGLSLogger = nil then
  begin
  {$IFDEF GLS_LOGGING}
    v_DGLSLogger := TLogSession.Init(Copy(ExtractFileName(ParamStr(0)), 1,
    Length(ExtractFileName(ParamStr(0))) - Length(ExtractFileExt(ParamStr(0)))) +
    '.log', lfElapsed, llMax);
  {$ELSE}
    v_DGLSLogger := TLogSession.OnlyCreate;
  {$ENDIF}
  end;
...
procedure UseCustomDGLSLogger(const ALogger: TLogSession);
begin
  if (v_DGLSLogger <> nil) then v_DGLSLogger.Destroy;
  v_DGLSLogger := ALogger;
end;
...
{$ENDIF}
  ...
  // IDELogProc.
  if (Self = v_DGLSLogger) and Assigned(vIDELogProc) then
    vIDELogProc('');
end;
