...
type
  ...
  TDGLSMWaveOut = class(TDGLSoundManager)
  private
...
procedure PlayOnWaveOut(pcmData: Pointer; lengthInBytes: Integer; sampling: TDGLSoundSampling); overload;
function PlayOnWaveOut(pcmData: Pointer; lengthInBytes: Integer; waveFormat: TWaveFormatEx): HWaveOut; overload;
...
procedure _waveOutCallBack2(hwo: HWaveOut; uMsg: Cardinal; dwInstance, dwParam1, dwParam2: Integer); stdcall;
begin
...
//
function PlayOnWaveOut(pcmData: Pointer; lengthInBytes: Integer; waveFormat: TWaveFormatEx): HWaveOut;
var
  hwo:   HWaveOut;
  wh:    wavehdr;
  mmres: MMRESULT;
begin
  mmres := waveOutOpen(@hwo, WAVE_MAPPER, @waveFormat, Cardinal(@_waveOutCallBack2), 0, CALLBACK_FUNCTION);
...
//
procedure PlayOnWaveOut(pcmData: Pointer; lengthInBytes: Integer; sampling: TDGLSoundSampling);
var
  wfx: TWaveFormatEx;
begin
...
procedure _waveOutCallBack(hwo: HWaveOut; uMsg: Cardinal; dwInstance, dwParam1, dwParam2: Integer); stdcall;
begin
  if uMsg = WOM_DONE then
  begin
    PWaveOutPlayingRec(TDGLSoundSource(dwInstance).ManagerTag).CurrentState := ssFinished;
  end;
end;
...
// ------------------
{ TDGLSMWaveOut }
{$IFDEF GLS_REGION}{$REGION 'TDGLSMWaveOut'}{$ENDIF}

constructor TDGLSMWaveOut.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  MaxChannels := 4;
end;

destructor TDGLSMWaveOut.Destroy;
begin
  inherited Destroy;
end;

function TDGLSMWaveOut.DoActivate: Boolean;
begin
  Result := True;
end;

procedure TDGLSMWaveOut.DoDeActivate;
var
  i: Integer;
begin
...
procedure TDGLSMWaveOut.KillSource(aSource: TDGLBaseSoundSource);
var
...
procedure TDGLSMWaveOut.UpdateSources;
var
  i, n:  Integer;
  wfx:   TWaveFormatEx;
  smp:   TDGLSoundSample;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          mmres := waveOutOpen(@hwo, WAVE_MAPPER, @wfx, Cardinal(@_waveOutCallBack), Integer(Sources[i]), CALLBACK_FUNCTION);
          Assert(mmres = MMSYSERR_NOERROR, IntToStr(mmres));
