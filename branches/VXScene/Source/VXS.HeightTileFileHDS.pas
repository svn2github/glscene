...
uses
  ...
  VXS.HeightData, VXS.HeightTileFile;

type

	// TVXHeightTileFileHDS
	//
   { An Height Data Source for the HTF format.  }
	TVXHeightTileFileHDS = class (TVXHeightDataSource)
	   private
	      
         FInfiniteWrap : Boolean;
         FInverted     : Boolean;
         FHTFFileName : String;
         FHTF : TVXHeightTileFile;
	   ...
	   public
         ...
         function OpenHTF:TVXHeightTileFile; //gives you direct access to the HTF object

	   published
	      

         { FileName of the HTF file. 
            Note that it is accessed via the services of VXS.ApplicationFileIO,
...
// Create
//
constructor TVXHeightTileFileHDS.Create(AOwner: TComponent);
begin
...
// Destroy
//
destructor TVXHeightTileFileHDS.Destroy;
begin
   FHTF.Free;
	inherited Destroy;
end;
...
// SetHTFFileName
//
procedure TVXHeightTileFileHDS.SetHTFFileName(const val : String);
begin
...
// SetInfiniteWrap
//
procedure TVXHeightTileFileHDS.SetInfiniteWrap(val : Boolean);
begin
...
// SetInverted
//
procedure TVXHeightTileFileHDS.SetInverted(val : Boolean);
begin
...
// SetMinElevation
//
procedure TVXHeightTileFileHDS.SetMinElevation(val : Integer);
begin
...
//
function TVXHeightTileFileHDS.OpenHTF:TVXHeightTileFile;
begin
  if not Assigned(FHTF) then begin
    if FHTFFileName='' then FHTF:=nil
      else FHTF:=TVXHeightTileFile.Create(FHTFFileName);
  end;
  result:=FHTF;
end;
...
// StartPreparingData
//
procedure TVXHeightTileFileHDS.StartPreparingData(HeightData : TVXHeightData);
var
...
function TVXHeightTileFileHDS.Width :integer;
begin
  if OpenHTF=nil then result:=0
                 else result:=FHTF.SizeX;
end;

function TVXHeightTileFileHDS.Height:integer;
begin
  if OpenHTF=nil then result:=0
...
// ------------------------------------------------------------------
	...
	// class registrations
   RegisterClasses([TVXHeightTileFileHDS]);
