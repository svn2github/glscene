...
type
  ...
  // TVXWindowsBitmapFont
  //
  { A bitmap font automatically built from a TFont. 
     It works like a TVXBitmapfont, you set ranges and which chars are assigned
  ...
  TVXWindowsBitmapFont = class(TVXCustomBitmapFont)
  private
  ...
  protected
    ...
    procedure PrepareImage(var ARci: TVXRenderContextInfo); override;
...
constructor TVXWindowsBitmapFont.Create(AOwner: TComponent);
begin
  inherited;
  FFont := TFont.Create;
...
destructor TVXWindowsBitmapFont.Destroy;
begin
  FFont.Free;
  Ranges.Clear; 
  inherited;
...
function TVXWindowsBitmapFont.FontTextureWidth: Integer;
begin
...
function TVXWindowsBitmapFont.FontTextureHeight: Integer;
begin
...
procedure TVXWindowsBitmapFont.SetFont(value: TFont);
begin
  FFont.Assign(value);
end;
...
procedure TVXWindowsBitmapFont.NotifyChange(Sender: TObject);
begin
  StreamlineRanges;
  FreeTextureHandle;
  InvalidateUsers;
...
procedure TVXWindowsBitmapFont.LoadWindowsFont;
...
function TVXWindowsBitmapFont.StoreRanges: Boolean;
begin
  Result := (Ranges.Count <> 1) or (Ranges[0].StartASCII[1] <> ' ') or (Ranges[0].StopASCII[1] <> cDefaultLast);
end;

type
  TFriendlyRange = class(TVXBitmapFontRange);

procedure TVXWindowsBitmapFont.StreamlineRanges;
var
  I, C: Integer;
begin
  C := 0;
...
procedure TVXWindowsBitmapFont.SetList(const AList: TIntegerList);
var
  i : integer;
  f, n, s : integer;
begin
...
procedure TVXWindowsBitmapFont.EnsureChars(const AStart, AEnd: widechar);
var
  c : WideChar;
...
//add characters to internal list
procedure TVXWindowsBitmapFont.EnsureString(const s: String);
var
  i : Integer;
  ACharList : TIntegerList;
begin
...
procedure TVXWindowsBitmapFont.PrepareImage(var ARci: TVXRenderContextInfo);
begin
  LoadWindowsFont;
...
function TVXWindowsBitmapFont.TextureFormat: Integer;
begin
  Result := GL_ALPHA;
end;
...
initialization
  ...
  Win32PlatformIsUnicode := (Win32Platform = VER_PLATFORM_WIN32_NT);
   ...
   // class registrations
  RegisterClasses([TVXWindowsBitmapFont]);
