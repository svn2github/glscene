...
procedure ECAddFreeForm(var MovePack: TECMovePack; FreeForm: TVXBaseSceneObject;
  Solid: Boolean; ObjectID: Integer);

//Add a TriMesh box
procedure ECAddBox(var MovePack: TECMovePack;
  BoxObj: TVXBaseSceneObject; BoxSize: TAffineVector;
  Solid: Boolean; ObjectID: Integer);

//Add the terrain as a TriMesh
procedure ECAddTerrain(var MovePack: TECMovePack;
  TerrainRenderer: TVXTerrainRenderer; Resolution: Single;
  Solid: Boolean; ObjectID: Integer);
...
begin
  Master := FreeForm;
  if Master is TVXFreeFormProxy then
    Master := TVXFreeFormProxy(Master).MasterObject;
  if Master is TVXMultiProxy then
    if TVXMultiProxy(Master).MasterObjects.Count > 0 then
      Master := TVXMultiProxy(Master).MasterObjects[0].MasterObject;
  Assert((Master is TVXFreeForm), 'Object must be freeform, freeformproxy or freeformbased Multiproxy.');
  Assert(Assigned(TVXFreeForm(Master).Octree), 'Octree must have been prepared and setup before use.');
  ...
  with TVXFreeForm(Master).Octree do
  begin
    ...
    MovePack.Freeforms[count].InvertedNormals := TVXFreeForm(Master).NormalsOrientation = mnoInvert;
  end;
end;
