...
type
  ...
  TVXGLMVectorFile = class (TVXVectorFile)
    public
      class function Capabilities : TVXDataFileCapabilities; override;
      procedure LoadFromStream(aStream : TStream); override;
  end;

  TVXGLAVectorFile = class (TVXVectorFile)
    public
      class function Capabilities : TVXDataFileCapabilities; override;
      procedure LoadFromStream(aStream : TStream); override;
  end;
...
class function TVXGLMVectorFile.Capabilities : TVXDataFileCapabilities;
begin
  Result:=[dfcRead];
end;

procedure TVXGLMVectorFile.LoadFromStream(aStream : TStream);
var
  GLMFile     : TFileGLM;
  i,j,k,s,c,d : integer;
...
// Capabilities
//
class function TVXGLAVectorFile.Capabilities : TVXDataFileCapabilities;
begin
  Result:=[dfcRead];
end;
...
// LoadFromStream
//
procedure TVXGLAVectorFile.LoadFromStream(aStream : TStream);
var
  GLAFile  : TFileGLA;
  i,j      : Integer;
  frame    : TVXSkeletonFrame;
  CompBone : TVXACompQuatBone;
  quat     : TQuaternion;
  pos      : TAffineVector;
  basepose : TVXSkeletonFrame;
...
begin
  ...
  try
    if not (Owner is TVXActor) then exit;
    ...
    while bonelist.count>0 do begin
      if GLAFile.Skeleton[bonelist[0]].parent = -1 then
        ...
        if Assigned(bone) then
          bone:=TVXSkeletonBone.CreateOwned(bone)
      end;
    ...
    // Build the base pose
    basepose:=TVXSkeletonFrame.CreateOwned(TVXActor(Owner).Skeleton.Frames);
    ...
    for i:=0 to GLAFile.AnimHeader.numFrames-1 do begin
      // Create the frame
      frame:=TVXSkeletonFrame.CreateOwned(TVXActor(Owner).Skeleton.Frames);
    ...
    for i:=0 to Owner.MeshObjects.Count-1 do
      TVXSkeletonMeshObject(Owner.MeshObjects[i]).PrepareBoneMatrixInvertedMeshes;
...
// ------------------------------------------------------------------
   ...
   RegisterVectorFileFormat('glm', 'Ghoul2 (GLM) model files', TVXGLMVectorFile);
   RegisterVectorFileFormat('glx', 'Ghoul2 (GLX) model files', TVXGLMVectorFile);
   RegisterVectorFileFormat('gla', 'Ghoul2 (GLA) animation files', TVXGLAVectorFile);
