...
//
{
  Support-Code to load DXF (Drawing eXchange Files) TVXFreeForm or
  TVXActor Components in GLScene.  
...
type
  TVXDXFVectorFile = class(TVXVectorFile)
  private
  ...
  protected
    ...
    // procedure DoProgress (Stage: TVXProgressStage; PercentDone: single; RedrawNow: Boolean; const Msg: string);
    function NeedMesh(basemesh: TVXBaseMesh; layer: STRING): TVXMeshObject;
    function NeedFaceGroup(m: TVXMeshObject; fgmode: TVXFaceGroupMeshMode;
    ...
    procedure ReadInsert(basemesh: TVXBaseMesh);
    procedure ReadEntity3Dface(basemesh: TVXBaseMesh);
    procedure ReadEntityPolyLine(basemesh: TVXBaseMesh);
    procedure ReadEntities(basemesh: TVXBaseMesh);
  public
...
procedure BuildNormals(m: TVXMeshObject); FORWARD;
...
const
  ...
  class function TVXDXFVectorFile.Capabilities: TVXDataFileCapabilities;
  begin
    result := [dfcRead];
  end;

  function TVXDXFVectorFile.ReadLine: STRING;
  var
    j: Integer;
    FLine: STRING;
    NewlineChar: CHAR;
...
{
  procedure TVXDXFVectorFile.DoProgress (Stage: TVXProgressStage; PercentDone: single; RedrawNow: Boolean; const Msg: string);
  var perc:BYTE;
  begin
  ...
  procedure TVXDXFVectorFile.PushCode(code: Integer);
  begin
    PushedCode := code;
    HasPushedCode := True;
  end;

  function TVXDXFVectorFile.GetCode: Integer;
  var
    S: STRING;
  begin
  ...
  function TVXDXFVectorFile.ReadDouble: double;
  var
    S: String;
    c: CHAR;
  begin
  ...
  function TVXDXFVectorFile.ReadInt: Integer;
  var
    S: String;
  begin
    S := Trim(ReadLine);
  ...
  procedure TVXDXFVectorFile.SkipSection;
  var
    S: String;
    code: Integer;
  begin
  ...
  procedure TVXDXFVectorFile.SkipTable;
  var
    S: String;
    code: Integer;
  begin
  ...
  procedure TVXDXFVectorFile.ReadLayer;
  var
  ...
  procedure TVXDXFVectorFile.ReadLayerTable;
  var
    S: STRING;
    code: Integer;
  begin
  ...
  procedure TVXDXFVectorFile.ReadTables;
  var
    S: String;
    code: Integer;
  begin
    ...
    procedure TVXDXFVectorFile.ReadBlocks;
    var
      S: String;
      code: Integer;
      blockname: String;
      blockmesh: TVXFreeForm;

    begin
      // This code reads blocks into orphaned TVXFreeForms.
      ...
      repeat
        ...
        begin
          blockmesh := TVXFreeForm.create(owner);
    ...
    procedure TVXDXFVectorFile.ReadInsert(basemesh: TVXBaseMesh);
    var
      code, idx, indexoffset: Integer;
      i, j, k: Integer;
      blockname, S: STRING;
      pt, insertpoint, scale: TAffineVector;
      blockmesh: TVXBaseMesh;
      // blockproxy  :TVXProxyObject;
      mo_block: TVXMeshObject;
      mo_base: TVXMeshObject;
    ...
    begin
      ...
      if idx >= 0 then
      begin
        blockmesh := FBlocks.Objects[idx] as TVXBaseMesh;
        ...
        (*
          ...
          begin
          blockproxy:=TVXproxyObject.CreateAsChild(basemesh);
    ...
    function TVXDXFVectorFile.NeedMesh(basemesh: TVXBaseMesh; layer: STRING)
      : TVXMeshObject;
    var
      i: Integer;
    begin
      ...
      begin
        result := TVXMeshObject.CreateOwned(basemesh.MeshObjects);
        result.mode := momFaceGroups;
        result.name := layer;
      end;
    end;

    function TVXDXFVectorFile.NeedFaceGroup(m: TVXMeshObject;
      fgmode: TVXFaceGroupMeshMode; fgmat: STRING): TFGVertexIndexList;
    var
      i: Integer;
      acadcolor: LONGINT;
    ...
    procedure TVXDXFVectorFile.NeedMeshAndFaceGroup(basemesh: TVXBaseMesh;
      layer: STRING; fgmode: TVXFaceGroupMeshMode; fgmat: STRING;
      var m: TVXMeshObject; var fg: TFGVertexIndexList);
    begin
      m := NeedMesh(basemesh, layer);
      fg := NeedFaceGroup(m, fgmode, fgmat);
    end;

    procedure TVXDXFVectorFile.ReadEntity3Dface(basemesh: TVXBaseMesh);
    var
      code, i: Integer;
    ...
    procedure TVXDXFVectorFile.ReadEntityPolyLine(basemesh: TVXBaseMesh);

      procedure ReadPolylineVertex(m: TVXMeshObject; vertexindexbase: Integer);
      var
        color: STRING;
        pt: TAffineVector;
        fg: TFGVertexIndexList;
    ...
    procedure TVXDXFVectorFile.ReadEntities(basemesh: TVXBaseMesh);
    var
      code: Integer;
      S: STRING;
    begin
    ...
    // build normals
    procedure BuildNormals(m: TVXMeshObject);
    var
      i, j: Integer;
    ...
    procedure TVXDXFVectorFile.LoadFromStream(aStream: TStream);
    var
      S: STRING;
      code, i: Integer;
    begin
      ...
      for i := FBlocks.count - 1 downto 0 do
        (FBlocks.Objects[i] as TVXFreeForm).free;
      FBlocks.free;
...
RegisterVectorFileFormat('dxf', 'AutoCAD Exchange Format', TVXDXFVectorFile);
