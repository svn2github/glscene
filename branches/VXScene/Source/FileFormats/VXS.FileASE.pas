...
//
{
	 ASE (ASCI Scene Export) file format support for VXScene
}
unit VXS.FileASE;
...
type
  ...
  TVXASEFaceTexure = record
    Idx0, Idx1, Idx2: Integer;
  end;

  { face texture channels }
  TVXASEFaceTexureChannels = record
    Count: Integer;
    ChanelTexture: array [0..GL_ASE_MAX_TEXURE_CHANNELS - 1] of TVXASEFaceTexure;
  end;

  TVXASESmoothingGroups = record
    Count: Integer;
    Groups: array [0..GL_ASE_MAX_SMOOTH_GROUPS] of Integer;
  end;

  { ASE mesh face }
  TVXASEFace = class(TObject)
  private
    FV: array [0..2] of Integer;
    FNormal: TAffineVector;
    FN: array [0..2] of TAffineVector;
    FSmoothing: TVXASESmoothingGroups;
    FSubMaterialID: Integer;                    // submaterial ID
    FTextChannels: TVXASEFaceTexureChannels;
  public
    constructor Create;
    ...
    property TextChannels: TVXASEFaceTexureChannels read FTextChannels; // channels texture coordinates
    property Smoothing: TVXASESmoothingGroups read FSmoothing;   // smoothing group list
    property SubMaterialID: Integer read FSubMaterialID; // submaterial ID
  end;

  { ASE mesh faces list }
  TVXASEFaceList = class(TObject)
  private
    FItems: TList;
    function GetFace(Index: Integer): TVXASEFace;
    function GetCount: Integer;
  public
    constructor Create;
    destructor Destroy; override;
    function Add: TVXASEFace;
    procedure Delete(aIndex: Integer);
    procedure Clear;
    property Face[Index: Integer]: TVXASEFace read GetFace; default;
  ...
  { ASE geom object, represents single mesh object;
   ...
   piece of it into TVXMeshObject }
  TVXASEMeshObject = class(TObject)
  private
    FFaces: TVXASEFaceList;
  ...
  public
    constructor Create;
    destructor Destroy; override;
    property Faces: TVXASEFaceList read FFaces;
  ...
  TVXASEMaterialTextureMap = record
    Kind: string;
    Name: string;
    _Class: string;
    No: Integer;
  ...
  TVXASEMaterialTextureMaps = record
    Map: array [0..GL_ASE_MAX_TEXTURE_MAPS - 1] of TVXASEMaterialTextureMap;
    Count: Integer;
  end;

  TVXASESubMaterial = record
    Name: string;
    ...
    TextureMaps: TVXASEMaterialTextureMaps;
  end;

  TVXASESubMaterialList = record
    SubMaterial: array [0..GL_ASE_MAX_SUBMATERIALS - 1] of TVXASESubMaterial;
    Count: Integer;
  end;

  TVXASEMaterial = class(TObject)
  private
    FWireSize: Single;
    ...
    FSubMaterials: TVXASESubMaterialList;
    FTextureMaps: TVXASEMaterialTextureMaps;
  public
    constructor Create;
    ...
    property TextureMaps: TVXASEMaterialTextureMaps read FTextureMaps;
    property SubMaterials: TVXASESubMaterialList read FSubMaterials;
  end;

  TVXASEMaterialList = class(TObject)
  private
    FItems: TList;
    function GetCount: Integer;
    function GetMaterial(Index: Integer): TVXASEMaterial;
  public
    constructor Create;
    destructor Destroy; override;
    function Add: TVXASEMaterial;
    procedure Delete(aIndex: Integer);
    procedure Clear;
    property Material[Index: Integer]: TVXASEMaterial read GetMaterial; default;
    property Count: Integer read GetCount;
  end;


  { ASE vector file parser }
  TVXASEVectorFile = class(TVXVectorFile)
  private
    ...
    FCastShadow: Boolean;
    FMotionBlur: Boolean;
    FMaterialList: TVXASEMaterialList;
    ...
    procedure ParseFaceString(const aData: string; var Index, A, B, C, AB, BC, CA, MatID: Integer; var Smooth: TVXASESmoothingGroups);
  private
    procedure ParseScene(var aLineIndex: Integer);
    procedure ParseGeomObject(var aLineIndex: Integer);
    procedure ParseMeshOptions(var aLineIndex: Integer; aMesh: TVXASEMeshObject);
    procedure ParseMeshGeom(var aLineIndex: Integer; aMesh: TVXASEMeshObject);
    procedure ParseMappingChannel(var aLineIndex: Integer; aMesh: TVXASEMeshObject);
    procedure ParseMeshVertices(var aLineIndex: Integer; aMesh: TVXASEMeshObject; VerticesCount: Integer);
    procedure ParseMeshFaces(var aLineIndex: Integer; aMesh: TVXASEMeshObject; FacesCount: Integer);
    procedure ParseMeshNormals(var aLineIndex: Integer; aMesh: TVXASEMeshObject; FacesCount: Integer);
    procedure ParseMeshTextureVertices(var aLineIndex: Integer; aMesh: TVXASEMeshObject; TextureVerticesCount: Integer);
    procedure ParseMeshTextureFaces(var aLineIndex: Integer; aMesh: TVXASEMeshObject; TextureFacesCount: Integer);
    procedure ParseMaterialList(var aLineIndex: Integer);
    procedure ParseMaterial(var aLineIndex: Integer; aMaterial: TVXASEMaterial);
    procedure ParseSubMaterial(var aLineIndex: Integer; aMaterial: TVXASEMaterial);
    function CheckTextureMap(var aLineIndex: Integer; var aMaps: TVXASEMaterialTextureMaps): Boolean;
    procedure ParseTextureMap(var aLineIndex: Integer; var aMaps: TVXASEMaterialTextureMaps; const aMapKind: string);
...
procedure CopyASEToMesh(aASEMesh: TVXASEMeshObject; aMesh: TVXMeshObject; aASEMaterials: TVXASEMaterialList);
  ...
  function FindTextureMap(aMaterial: TVXASEMaterial; aMapKind: string; aSubmaterialIndex: Integer;
    out TM: TVXASEMaterialTextureMap): Boolean;

    function FindInMaps(const aMaps: TVXASEMaterialTextureMaps): Boolean;
    var
      i: Integer;
    begin
      Result := False;
  ...
  var
    material : TVXASEMaterial;
    specColor : TVector;
    matLib : TVXMaterialLibrary;
    libMat : TVXLibMaterial;
    TM: TVXASEMaterialTextureMap;
  begin
...
constructor TVXASEFaceList.Create;
begin
  FItems := TList.Create;
end;

destructor TVXASEFaceList.Destroy;
begin
  Clear;
  FreeAndNil(FItems);
  inherited;
...
function TVXASEFaceList.Add: TVXASEFace;
begin
  Result := TVXASEFace.Create;
  FItems.Add(Result);
end;

procedure TVXASEFaceList.Delete(aIndex: Integer);
begin
  Face[aIndex].Free;
  FItems.Delete(aIndex);
end;

procedure TVXASEFaceList.Clear;
var
  i: Integer;
begin
...
function TVXASEFaceList.GetFace(Index: Integer): TVXASEFace;
begin
  Result := FItems[Index];
end;

function TVXASEFaceList.GetCount: Integer;
begin
  Result := FItems.Count;
end;
...
constructor TVXASEMaterial.Create;
begin
...
constructor TVXASEMeshObject.Create;
begin
  FFaces := TVXASEFaceList.Create;
...
destructor TVXASEMeshObject.Destroy;
var
  i: Integer;
begin
  FreeAndNil(FFaces);
...
function TVXASEMeshObject.AddTexChannel: TAffineVectorList;
begin
...
function TVXASEMeshObject.GetTextChannel(Channel: Integer): TAffineVectorList;
begin
...
constructor TVXASEMaterialList.Create;
begin
  FItems := TList.Create;
end;

destructor TVXASEMaterialList.Destroy;
begin
  Clear;
  FreeAndNil(FItems);
  inherited;
...
function TVXASEMaterialList.Add: TVXASEMaterial;
begin
  Result := TVXASEMaterial.Create;
  FItems.Add(Result);
end;

procedure TVXASEMaterialList.Delete(aIndex: Integer);
begin
  Material[aIndex].Free;
  FItems.Delete(aIndex);
end;

procedure TVXASEMaterialList.Clear;
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
...
function TVXASEMaterialList.GetMaterial(Index: Integer): TVXASEMaterial;
begin
  Result := FItems[Index];
end;

function TVXASEMaterialList.GetCount: Integer;
begin
  Result := FItems.Count;
end;
...
constructor TVXASEVectorFile.Create;
begin
  inherited;
  FStringData := TStringList.Create;
  FMaterialList := TVXASEMaterialList.Create;
end;

destructor TVXASEVectorFile.Destroy;
begin
  FMaterialList.Free;
  FStringData.Free;
  inherited;
...
class function TVXASEVectorFile.Capabilities: TVXDataFileCapabilities;
begin
  Result := [dfcRead];
end;

procedure TVXASEVectorFile.LoadFromStream(aStream: TStream);
begin
  FStringData.LoadFromStream(aStream);
  Parse;
end;

function TVXASEVectorFile.ContainString(const aData, aString: string): Boolean;
begin
  Result := (Length(aData) >= Length(aString)) and (Pos(AString, aData) > 0);
end;

function TVXASEVectorFile.GetDoubleValue(const aData: string): Double;
var
  Start: Integer;
  Data: string;
  Value: string;
...
function TVXASEVectorFile.GetEndOfFirstValue(const aData: string): Integer;
var
  Value: string;
begin
...
function TVXASEVectorFile.GetFirstValue(aData: string): string;
const
  sep1 = #32;
var
  Index: Integer;
...
function TVXASEVectorFile.GetPropCastShadow(const aData: string): Boolean;
begin
  Result := Pos('1', aData) > 0;
end;

function TVXASEVectorFile.GetPropMBlur(const aData: string): Boolean;
begin
  Result := Pos('1', aData) > 0;
end;

function TVXASEVectorFile.GetPropRECVShadow(const aData: string): Boolean;
begin
  Result := Pos('1', aData) > 0;
end;

function TVXASEVectorFile.GetStringValue(const aData: string): string;
const
  Breaker = '"';
var
  Start: Integer;
...
function TVXASEVectorFile.GetTagOnData(const aData: string): Integer;
var
  Index: Integer;
begin
  Result := -1;
...
function TVXASEVectorFile.GetValue3D(const aData: string): TAffineVector;
var
  Start: Integer;
  Data: string;
  Value: String;
...
function TVXASEVectorFile.GetValue4D(const aData: string; var Value0: Integer): TAffineVector;
var
  Start: Integer;
  Data: string;
  Value: String;
...
function TVXASEVectorFile.IsEndOfSection(const aData: string): Boolean;
begin
...
function TVXASEVectorFile.IsSectionBegingin(const aData: string): Boolean;
begin
  Result := Pos('{', aData) > 0;
end;

function TVXASEVectorFile.CheckUnknownData(var aLineIndex: Integer): Boolean;
begin
...
procedure TVXASEVectorFile.SkipSection(var aLineIndex: Integer);
var
  Data: string;
begin
  Inc(aLineIndex);
...
procedure TVXASEVectorFile.Parse;
var
  LineIndex: Integer;
  Data: string;
  b: Boolean;
...
procedure TVXASEVectorFile.ParseFaceString(const aData: string; var Index, A, B, C, AB, BC, CA, MatID: Integer;
  var Smooth: TVXASESmoothingGroups);
var
  Data: string;
  Start: Integer;
  Value: string;
  ...
  procedure GetNextValues(var aValues: TVXASESmoothingGroups);
...
procedure TVXASEVectorFile.ParseGeomObject(var aLineIndex: Integer);
var
  aseMesh: TVXASEMeshObject;
  obj: TVXMeshObject;
  Data: string;
  b: Boolean;
begin
  aseMesh := TVXASEMeshObject.Create;
  try
    obj := TVXMeshObject.CreateOwned(Owner.MeshObjects);
...
procedure TVXASEVectorFile.ParseMeshFaces(var aLineIndex: Integer; aMesh: TVXASEMeshObject; FacesCount: Integer);
var
  Data: string;
  Index, A, B, C, AB, BC, CA, ID: Integer;
  face: TVXASEFace;
  smooth: TVXASESmoothingGroups;
begin
  Inc(aLineIndex);
  Data := FStringData[aLineIndex];
  while not IsEndOfSection(Data) do begin
...
procedure TVXASEVectorFile.ParseMeshNormals(var aLineIndex: Integer; aMesh: TVXASEMeshObject; FacesCount: Integer);
var
  Data: string;
  Index: Integer;
  Counter: Integer;
...
procedure TVXASEVectorFile.ParseMeshTextureFaces(var aLineIndex: Integer; aMesh: TVXASEMeshObject;
...
procedure TVXASEVectorFile.ParseMeshTextureVertices(var aLineIndex: Integer; aMesh: TVXASEMeshObject;
...
procedure TVXASEVectorFile.ParseMeshGeom(var aLineIndex: Integer; aMesh: TVXASEMeshObject);
var
  Data: string;
...
procedure TVXASEVectorFile.ParseMeshVertices(var aLineIndex: Integer; aMesh: TVXASEMeshObject;
  VerticesCount: Integer);
var
  Data: string;
  VertIndex: Integer;
...
procedure TVXASEVectorFile.ParseMappingChannel(var aLineIndex: Integer; aMesh: TVXASEMeshObject);
var
  Data: string;
  TextureVerticesCount: Integer;
  TextureFacesCount: Integer;
...
procedure TVXASEVectorFile.ParseMeshOptions(var aLineIndex: Integer; aMesh: TVXASEMeshObject);
var
  Data: string;
  b: Boolean;
begin
...
procedure TVXASEVectorFile.ParseScene(var aLineIndex: Integer);
begin
  CheckUnknownData(aLineIndex);
end;

procedure TVXASEVectorFile.ParseMaterialList(var aLineIndex: Integer);
var
  Data: string;
  material: TVXASEMaterial;
begin
...
function TVXASEVectorFile.CheckTextureMap(var aLineIndex: Integer; var aMaps: TVXASEMaterialTextureMaps): Boolean;
var
  Data: string;
begin
  Result := True;
...
procedure TVXASEVectorFile.ParseMaterial(var aLineIndex: Integer; aMaterial: TVXASEMaterial);
var
  Data: string;
  b: Boolean;
begin
...
procedure TVXASEVectorFile.ParseSubMaterial(var aLineIndex: Integer; aMaterial: TVXASEMaterial);
var
  Data: string;
  Index: Integer;
  b: Boolean;
...
procedure TVXASEVectorFile.ParseTextureMap(var aLineIndex: Integer; var aMaps: TVXASEMaterialTextureMaps; const aMapKind: string);
var
  Data: string;
  Index: Integer;
  b: Boolean;
...
initialization
  RegisterVectorFileFormat('ase', 'ASCII files', TVXASEVectorFile);
