...
function TVXWAVFile.CreateCopy(AOwner: TPersistent): TVXDataFile;
begin
  Result := inherited CreateCopy(AOwner);
  if Assigned(Result) then
  begin
{$IFDEF MSWINDOWS}
    TVXWAVFile(Result).waveFormat := waveFormat;
{$ENDIF}
    TVXWAVFile(Result).data := Copy(data);
  end;
end;

class function TVXWAVFile.Capabilities: TVXDataFileCapabilities;
begin
  Result := [dfcRead, dfcWrite];
end;

procedure TVXWAVFile.LoadFromStream(Stream: TStream);
{$IFDEF MSWINDOWS}
var
  ck: TRIFFChunkInfo;
...
procedure TVXWAVFile.SaveToStream(Stream: TStream);
begin
  if Length(data) > 0 then
    Stream.Write(data[0], Length(data));
end;

procedure TVXWAVFile.PlayOnWaveOut;
begin
{$IFDEF MSWINDOWS}
...
function TVXWAVFile.WAVData: Pointer;
begin
  if Length(data) > 0 then
    Result := @data[0]
  else
...
function TVXWAVFile.WAVDataSize: Integer;
begin
  Result := Length(data);
end;

function TVXWAVFile.PCMData: Pointer;
begin
{$IFDEF MSWINDOWS}
  if Length(data) > 0 then
...
function TVXWAVFile.LengthInBytes: Integer;
begin
...
RegisterSoundFileFormat('wav', 'Windows WAV files', TVXWAVFile);
