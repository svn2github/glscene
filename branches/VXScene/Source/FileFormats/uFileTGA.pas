...
type
  ...
  TVXTGAImage = class(TVXBaseImage)
  public
...
procedure TVXTGAImage.LoadFromFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TVXTGAImage.SaveToFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TVXTGAImage.LoadFromStream(stream: TStream);
var
...
procedure TVXTGAImage.SaveToStream(stream: TStream);
begin
{$MESSAGE Hint 'TVXTGAImage.SaveToStream not yet implemented' }
end;

// AssignFromTexture
//

procedure TVXTGAImage.AssignFromTexture(textureContext: TVXContext;
  const textureHandle: GLuint; textureTarget: TVXTextureTarget;
  const CurrentFormat: boolean; const intFormat: TVXInternalFormat);
begin
{$MESSAGE Hint 'TVXTGAImage.AssignFromTexture not yet implemented' }
end;

class function TVXTGAImage.Capabilities: TVXDataFileCapabilities;
begin
...
//===========================================================
  ...
  RegisterRasterFormat('tga', 'TARGA Image File', TVXTGAImage);
