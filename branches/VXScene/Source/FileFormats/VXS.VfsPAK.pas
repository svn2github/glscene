...
//
{
   ...
   VXS.ApplicationFileIO mechanism will be pointed into PAK file.
...
// VXS.ApplicationFileIO end

function TVXVfsPAK.GetStreamNumber: integer;
begin
   Result:=FStreamList.IndexOf(FStream);
end;

procedure TVXVfsPAK.SetStreamNumber(i:integer);
begin
   FStream:=TFileStream(FStreamList[i]);
end;

constructor TVXVfsPAK.Create(AOwner : TComponent);
begin
...
constructor TVXVfsPAK.Create(AOwner : TComponent; const CbrMode: TZCompressedMode);
begin
   Self.Create(AOwner);
   FCompressionLevel := None;
...
function TVXVfsPAK.GetFileCount: integer;
begin
   Result := FHeader.DirLength div SizeOf(TFileSection);
end;

procedure TVXVfsPAK.MakeFileList;
var
   I: integer;
begin
...
procedure TVXVfsPAK.LoadFromFile(FileName: string; Mode: word);
var
   l: integer;
begin
   FFileName := FileName;
...
procedure TVXVfsPAK.ClearPakFiles;
begin
   SetLength(FHeaderList, 0);
   FPakFiles.Clear;
   // Objects are automatically freed by TObjectList
...
function TVXVfsPAK.GetFile(index: integer): TStream;
begin
...
function TVXVfsPAK.FileExists(FileName: string): boolean;
begin
   Result := (FFiles.IndexOf(FileName) > -1);
end;

function TVXVfsPAK.GetFile(FileName: string): TStream;
begin
   Result := nil;
...
function TVXVfsPAK.GetFileSize(index: integer): integer;
begin
   FStream.Seek(FHeader.DirOffset + SizeOf(TFileSection) * index, soFromBeginning);
...
function TVXVfsPAK.GetFileSize(FileName: string): integer;
begin
   Result := -1;
   if Self.FileExists(FileName) then
...
{$WARNINGS OFF}
procedure TVXVfsPAK.AddFromStream(FileName, Path: string; F: TStream);
var
   Temp: TMemoryStream;
begin
...
procedure TVXVfsPAK.AddFromFile(FileName, Path: string);
var
   F: TFileStream;
begin
...
procedure TVXVfsPAK.AddEmptyFile(FileName, Path: string);
var
   F: TMemoryStream;
begin
...
procedure TVXVfsPAK.RemoveFile(index: integer);
var
   Temp: TMemoryStream;
   i:    integer;
   f:    TFileSection;
...
procedure TVXVfsPAK.RemoveFile(FileName: string);
begin
   if Self.FileExists(FileName) then
      RemoveFile(FFiles.IndexOf(FileName));
end;

procedure TVXVfsPAK.Extract(index: integer; NewName: string);
var
   s: TFileStream;
begin
   if NewName = '' then
...
procedure TVXVfsPAK.Extract(FileName, NewName: string);
begin
   if Self.FileExists(FileName) then
