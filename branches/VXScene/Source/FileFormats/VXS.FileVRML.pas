...
type
  ...
  TVXVRMLVectorFile = class (TVXVectorFile)
    public
...
class function TVXVRMLVectorFile.Capabilities : TVXDataFileCapabilities;
begin
  Result:=[dfcRead];
end;

procedure TVXVRMLVectorFile.LoadFromStream(aStream : TStream);
var
  mesh : TVXMeshObject;
  uniqueMatID : Integer;
  currentMaterial : TVXLibMaterial;
  ...
  function AddMaterialToLibrary(VRMLMaterial : TVRMLMaterial) : TVXLibMaterial;
  var
    matname : String;
  begin
    Result:=nil;
  ...
  var
    ...
    fg : TFGVertexNormalTexIndexList;
    face : TIntegerList;
    tempLibMat : TVXLibMaterial;
    saveTransform, mat : TMatrix;
    saveMaterial : TVXLibMaterial;
    axis : TAffineVector;
    angle : Single;
  begin
...
// ------------------------------------------------------------------
   ...
   RegisterVectorFileFormat('wrl', 'VRML files', TVXVRMLVectorFile);
