...
type
  ...
  TVXMS3DVectorFile = class(TVXVectorFile)
  public
...
class function TVXMS3DVectorFile.Capabilities: TVXDataFileCapabilities;
begin
  Result := [dfcRead];
end;

procedure TVXMS3DVectorFile.LoadFromStream(aStream: TStream);
var
  ...
  Sk_MO: TVXSkeletonMeshObject;

  GroupList: TList;
  GLLibMaterial: TVXLibMaterial;
...
begin
  ...
  try
    ...
    begin
      MO := TVXSkeletonMeshObject.CreateOwned(Owner.MeshObjects);
      TVXSkeletonMeshObject(MO).BonesPerVertex := 4;
    end
    else
    ...
    for i := 0 to nNumVertices - 1 do
      ...
      begin
        ...
        if Owner is TVXActor then
          TVXSkeletonMeshObject(MO).AddWeightedBone(Byte(BoneID), 1);
      end;
    ...
    if Owner is TVXActor then
    begin
      TVXActor(Owner).Interval := trunc(1 / fAnimationFPS * 1000);
    end;
    ...
    Sk_MO := TVXSkeletonMeshObject(MO);
    if Owner is TVXActor then
    begin
      for i := 0 to nNumVertices - 1 do
      begin
    ...
    if (Owner is TVXActor) and (nNumJoints > 0) then
    begin
      ...
      begin
        ...
        if j = -1 then
          ...
          bone := TVXSkeletonBone.CreateOwned(Owner.Skeleton.RootBones.BoneByID(j));
      ...
      TVXSkeletonMeshObject(MO).PrepareBoneMatrixInvertedMeshes;
...
initialization
  ...
  RegisterVectorFileFormat('ms3d', 'MilkShape3D files', TVXMS3DVectorFile);
