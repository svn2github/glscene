...
type
  ...
  TVXHDRImage = class(TVXBaseImage)
  private
...
procedure TVXHDRImage.LoadFromFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TVXHDRImage.LoadFromStream(stream: TStream);
const
...
function TVXHDRImage.GetProgramType: Ansistring;
begin
  Result := fProgramType;
end;

procedure TVXHDRImage.SetProgramType(aval: Ansistring);
var
  i: integer;
begin
...
//-------------------------------------------------------------------
  ...
  RegisterRasterFormat('hdr', 'High Dynamic Range Image', TVXHDRImage);
