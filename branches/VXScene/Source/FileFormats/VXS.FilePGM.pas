...
uses
  ...
  VXS.Context, VXS.Graphics, VXS.TextureFormat,
  VXS.ApplicationFileIO;

type

  TVXPGMImage = class(TVXBaseImage)
  public
    ...
    procedure AssignFromTexture(textureContext: TVXContext;
      const textureHandle: GLEnum; textureTarget: TVXTextureTarget;
...
uses
  ...
  // LoadFromFile
  //
procedure TVXPGMImage.LoadFromFile(const filename: string);
var
  w, h: Integer;
...
// SaveToFile
//
procedure TVXPGMImage.SaveToFile(const filename: string);
begin
  if not IsCUTILInitialized then
    if not InitCUTIL then
    begin
...
procedure TVXPGMImage.LoadFromStream(stream: TStream);
begin
  Assert(false, 'Stream loading not supported');
end;

procedure TVXPGMImage.SaveToStream(stream: TStream);
begin
...
// AssignFromTexture
//
procedure TVXPGMImage.AssignFromTexture(textureContext: TVXContext;
  const textureHandle: GLEnum; textureTarget: TVXTextureTarget;
  const CurrentFormat: Boolean; const intFormat: TVXInternalFormat);
var
...
// Capabilities
//
class function TVXPGMImage.Capabilities: TVXDataFileCapabilities;
begin
...
RegisterRasterFormat('pgm', 'Portable Graymap', TVXPGMImage);
