...
type
  ...
  TVXDDSDetailLevels = (ddsHighDet, ddsMediumDet, ddsLowDet);

  TVXDDSImage = class(TVXBaseImage)
  private
...
procedure TVXDDSImage.LoadFromFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TVXDDSImage.SaveToFile(const filename: string);
var
  fs: TStream;
begin
...
procedure TVXDDSImage.LoadFromStream(stream: TStream);
var
  header: TDDSHeader;
...
procedure TVXDDSImage.flipSurface(chgData: PGLubyte; w, h, d: integer);
var
  lineSize: integer;
  sliceSize: integer;
  tempBuf: PGLubyte;
...
class function TVXDDSImage.Capabilities: TVXDataFileCapabilities;
begin
