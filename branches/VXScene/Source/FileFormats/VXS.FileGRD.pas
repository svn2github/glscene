...
type
  ...
  TVXGRDVectorFile = class(TVXVectorFile)
   public
    HeightField: TVXHeightField;
...
class function TVXGRDVectorFile.Capabilities: TVXDataFileCapabilities;
begin
  Result := [dfcRead];
end;

function TVXGRDVectorFile.WordPosition(const N: Integer; const S: string;
  const WordDelims: TSysCharSet): Integer;
var
  Count, I: Integer;
begin
...
function TVXGRDVectorFile.ExtractWord(N: Integer; const S: string;
  const WordDelims: TSysCharSet): string;
...
procedure TVXGRDVectorFile.LoadFromStream(aStream: TStream);
var
  I, J, K: Integer;
...
begin
  ...
  try
    ...
    HeightField := TVXHeightField.Create(Owner);
...
RegisterVectorFileFormat('grd', 'ArcInfo/Surfer grids', TVXGRDVectorFile);
