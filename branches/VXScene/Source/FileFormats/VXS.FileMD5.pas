...
type
  ...
  TVXMD5VectorFile = class(TVXVectorFile)
  private
    FMD5String, FTempString, FBoneNames: TStringList;
    FCurrentPos: Integer;
    FBasePose: TVXSkeletonFrame;
...
function TVXMD5VectorFile.ReadLine: String;
begin
  Result := '';
...
class function TVXMD5VectorFile.Capabilities: TVXDataFileCapabilities;
begin
  Result := [dfcRead];
end;

procedure TVXMD5VectorFile.LoadFromStream(aStream: TStream);
  ...
  var
    ...
    bone, parentbone: TVXSkeletonBone;
  begin
    ...
    pos.X := VXS.Utils.StrToFloatDef(FTempString[2]);
    pos.Y := VXS.Utils.StrToFloatDef(FTempString[4]);
    pos.Z := VXS.Utils.StrToFloatDef(FTempString[3]);

    quat := QuaternionMakeFromImag(VXS.Utils.StrToFloatDef(FTempString[5]),
      VXS.Utils.StrToFloatDef(FTempString[7]),
      VXS.Utils.StrToFloatDef(FTempString[6]));
    ...
    begin
      ...
      if ParentBoneID = -1 then
        bone := TVXSkeletonBone.CreateOwned(Owner.Skeleton.RootBones)
      else
      begin
        parentbone := Owner.Skeleton.RootBones.BoneByID(ParentBoneID);
        bone := TVXSkeletonBone.CreateOwned(parentbone);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            mesh.TexCoords[vnum] :=
              AffineVectorMake(VXS.Utils.StrToFloatDef(FTempString[2]),
              1 - VXS.Utils.StrToFloatDef(FTempString[3]), 0);
        ...
        begin
          ...
          begin
            ...
            VertexWeight[wnum] := VXS.Utils.StrToFloatDef(FTempString[3]);
            VertexWeighted[wnum] :=
              AffineVectorMake(VXS.Utils.StrToFloatDef(FTempString[4]),
              VXS.Utils.StrToFloatDef(FTempString[6]),
              VXS.Utils.StrToFloatDef(FTempString[5]));
          end;
        end;
      end;
    until temp = '}';
  ...
  begin
    ...
    repeat
      ...
      if FTempString.Count >= 6 then
      begin
        pos := AffineVectorMake(VXS.Utils.StrToFloatDef(FTempString[0]),
          VXS.Utils.StrToFloatDef(FTempString[1]),
          VXS.Utils.StrToFloatDef(FTempString[2]));
        quat := QuaternionMakeFromImag(VXS.Utils.StrToFloatDef(FTempString[3]),
          VXS.Utils.StrToFloatDef(FTempString[4]),
          VXS.Utils.StrToFloatDef(FTempString[5]));
        FFramePositions.Add(pos);
        FFrameQuaternions.Add(quat);
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          pos.X := VXS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 2 > 0 then
        begin
          pos.Y := VXS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 4 > 0 then
        begin
          pos.Z := VXS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;

        if FJointFlags[i] and 8 > 0 then
        begin
          quat.ImagPart.X := VXS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 16 > 0 then
        begin
          quat.ImagPart.Y := VXS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 32 > 0 then
          quat.ImagPart.Z := VXS.Utils.StrToFloatDef(FTempString[j]);
      end;
  ...
  begin
    for i := 0 to Owner.MeshObjects.Count - 1 do
      TVXSkeletonMeshObject(Owner.MeshObjects[i])
    ...
    repeat
      ...
      begin
        ...
        begin
          ...
          begin
            FBasePose := TVXSkeletonFrame.CreateOwned(Owner.Skeleton.Frames);
        ...
        begin
          ...
          if Owner is TVXActor then
            TVXActor(Owner).Reference := aarSkeleton;
        end
        ...
        begin
          ...
          begin
            ...
            for i := 1 to FNumFrames do
              TVXSkeletonFrame.CreateOwned(Owner.Skeleton.Frames);
            if Owner is TVXActor then
            begin
...
RegisterVectorFileFormat('md5mesh', 'Doom3 mesh files', TVXMD5VectorFile);
RegisterVectorFileFormat('md5anim', 'Doom3 animation files', TVXMD5VectorFile);
