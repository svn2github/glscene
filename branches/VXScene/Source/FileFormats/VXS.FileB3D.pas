...
type
  TVXB3DVectorFile = class(TVXVectorFile)
  public
...
// ------------------------------ TVXB3DVectorFile ------------------------------
class function TVXB3DVectorFile.Capabilities: TVXDataFileCapabilities;
begin
  Result := [DfcRead];
end;

procedure TVXB3DVectorFile.LoadFromStream(AStream: TStream);
...
var
  ...
  begin
    ...
    begin
      ...
      LightLib := TVXBaseMesh(GetOwner).LightmapLibrary;
...
begin
  ...
  try
    ...
    if GetOwner is TVXBaseMesh then
      (GetOwner as TVXBaseMesh).NormalsOrientation := MnoDefault;
...
RegisterVectorFileFormat('b3d', 'Blitz 3D model files', TVXB3DVectorFile);
