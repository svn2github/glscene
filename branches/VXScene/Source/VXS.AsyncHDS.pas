...
//
{
  ...
  When found, it then prepares the queued tile in its own TVXHeightDataThread.
...
constructor TVXAsyncHDS.Create(AOwner: TComponent);
begin
...
destructor TVXAsyncHDS.Destroy;
begin
  inherited Destroy;
end;

procedure TVXAsyncHDS.BeforePreparingData(HeightData: TVXHeightData);
begin
...
procedure TVXAsyncHDS.StartPreparingData(HeightData: TVXHeightData);
var
  HDThread: TVXAsyncHDThread;
  HDS: TVXHeightDataSource;
begin
...
procedure TVXAsyncHDS.ThreadIsIdle;
var
  i: integer;
  lst: TList;
  HD: TVXHeightData;
...
procedure TVXAsyncHDS.NewTilePrepared(HeightData: TVXHeightData);
var
  HD: TVXHeightData;
begin
  if assigned(HeightDataSource) then
...
{
  procedure TVXAsyncHDS.NotifyChange(Sender : TObject);
  begin
  TilesChanged:=true;
  end;
}
...
procedure TVXAsyncHDS.TilesUpdatedFlagReset;
begin
...
// -------------------HD Thread----------------
Procedure TVXAsyncHDThread.Execute;
Begin
  HDS.StartPreparingData(HeightData);
...
Procedure TVXAsyncHDThread.Sync;
begin
