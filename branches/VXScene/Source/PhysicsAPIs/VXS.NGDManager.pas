...
//
{
  A Newton Game Dynamics Manager for VXScene.
...
uses
  ...
  VXS.Objects,
  VXS.GeomObjects,
  VXS.VectorFileObjects, // cube cone freeform...
  VXS.Color,
  VXS.GeometryBB; // For show debug
...
type
  ...
  TVXNGDBehaviourList = class(TList)
  protected
    function GetBehav(index: Integer): TVXNGDBehaviour;
    procedure PutBehav(index: Integer; Item: TVXNGDBehaviour);
  public
    property ItemsBehav[index: Integer]
      : TVXNGDBehaviour read GetBehav write PutBehav; default;
  end;
  ...
  strict private
    FManager: TVXNGDManager;
    FGeomColorDyn: TVXColor; // Green
    FGeomColorStat: TVXColor; // Red
    FAABBColor: TVXColor; // Yellow
    FAABBColorSleep: TVXColor; // Orange
    FCenterOfMassColor: TVXColor; // Purple dot
    FContactColor: TVXColor; // White
    FJointAxisColor: TVXColor; // Blue
    FJointPivotColor: TVXColor; // Aquamarine
    FForceColor: TVXColor; // Black
    FAppliedForceColor: TVXColor; // Silver
    FAppliedVelocityColor: TVXColor; // Lime
    FCustomColor: TVXColor; // Aqua
  ...
  published
    property GeomColorDyn: TVXColor read FGeomColorDyn write FGeomColorDyn;
    property GeomColorStat: TVXColor read FGeomColorStat write FGeomColorStat;
    property AABBColor: TVXColor read FAABBColor write FAABBColor;
    property AABBColorSleep: TVXColor read FAABBColorSleep write FAABBColorSleep;
    property CenterOfMassColor: TVXColor read FCenterOfMassColor write FCenterOfMassColor;
    property ContactColor: TVXColor read FContactColor write FContactColor;
    property JointAxisColor: TVXColor read FJointAxisColor write FJointAxisColor;
    property JointPivotColor: TVXColor read FJointPivotColor write FJointPivotColor;
    property ForceColor: TVXColor read FForceColor write FForceColor;
    property AppliedForceColor: TVXColor read FAppliedForceColor write FAppliedForceColor;
    property AppliedVelocityColor: TVXColor read FAppliedVelocityColor write FAppliedVelocityColor;
    property CustomColor: TVXColor read FCustomColor write FCustomColor;
  ...
  TVXNGDManager = class(TComponent)
  strict private
    ...
    FWorldSizeMin: TVXCoordinates; // Default=-100, -100, -100
    FWorldSizeMax: TVXCoordinates; // Default=100, 100, 100
    FThreadCount: Integer; // Default=1
    FGravity: TVXCoordinates; // Default=(0,-9.81,0)
  ...
  private
    ...
    FNGDBehaviours: TVXNGDBehaviourList;
    FCurrentColor: TVXColor;
  protected
    ...
    procedure SetLines(const Value: TVXLines);
  ...
  published
    ...
    property Gravity: TVXCoordinates read FGravity write FGravity;
    property WorldSizeMin
      : TVXCoordinates read FWorldSizeMin write FWorldSizeMin;
    property WorldSizeMax
      : TVXCoordinates read FWorldSizeMax write FWorldSizeMax;
  ...
  private
    ...
    FOwnerBaseSceneObject: TVXBaseSceneObject;
    ...
    FHeightFieldOptions: TVXHeightField;
  protected
    ...
    procedure Loaded; override;
    procedure SetManager(Value: TVXNGDManager);
    ...
    procedure SetHeightFieldOptions(const Value: TVXHeightField);
  ...
  public
    ...
    property HeightFieldOptions: TVXHeightField read FHeightFieldOptions write
      SetHeightFieldOptions;
  published
    property Manager: TVXNGDManager read FManager write SetManager;
  ...
  TVXNGDDynamic = class(TVXNGDBehaviour)
  strict private
    FAABBmin: TVXCoordinates;
    FAABBmax: TVXCoordinates;
    FForce: TVXCoordinates;
    FTorque: TVXCoordinates;
    FCenterOfMass: TVXCoordinates;
    FAutoSleep: Boolean; // Default=True
    FLinearDamping: Single; // default=0.1
    FAngularDamping: TVXCoordinates; // Default=0.1
    ...
    FVolume: Single;
    FMass: Single;
    FAppliedForce: TVXCoordinates;
    FAppliedTorque: TVXCoordinates;
    FAppliedOmega: TVXCoordinates;
    FAppliedVelocity: TVXCoordinates;
  ...
  published
    property Force: TVXCoordinates read FForce write FForce;
    property Torque: TVXCoordinates read FTorque write FTorque;
    property Velocity: TVector read GetVelocity write SetVelocity;
    property Omega: TVector read GetOmega write SetOmega;
    property CenterOfMass: TVXCoordinates read FCenterOfMass write FCenterOfMass;
    ...
    property AngularDamping: TVXCoordinates read FAngularDamping write FAngularDamping;
    ...
    // Read Only
    property AppliedOmega: TVXCoordinates read FAppliedOmega;
    property AppliedVelocity: TVXCoordinates read FAppliedVelocity;
    property AppliedForce: TVXCoordinates read FAppliedForce;
    property AppliedTorque: TVXCoordinates read FAppliedTorque;
    property Volume: Single read FVolume;
    property Mass: Single read FMass;
  end;

  TVXNGDStatic = class(TVXNGDBehaviour)
  protected
    procedure Render; override;
  public
    class function FriendlyName: string; override;
  ...
  private
    FManager: TVXNGDManager;
    FPivotPoint: TVXCoordinates;
    FOuter: TNGDJoint;
  public
    constructor Create(AOwner: TComponent; aOuter: TNGDJoint); virtual;
    destructor Destroy; override;
  published
    property PivotPoint: TVXCoordinates read FPivotPoint write FPivotPoint;
  end;

  TNGDJointPin = class(TNGDJointPivot)
  private
    FPinDirection: TVXCoordinates;
  public
    constructor Create(AOwner: TComponent; aOuter: TNGDJoint); override;
    destructor Destroy; override;
  published
    property PinDirection: TVXCoordinates read FPinDirection write FPinDirection;
  end;

  TNGDJointPin2 = class(TNGDJointPin)
  private
    FPinDirection2: TVXCoordinates;
  public
    constructor Create(AOwner: TComponent; aOuter: TNGDJoint); override;
    destructor Destroy; override;
  published
    property PinDirection2: TVXCoordinates read FPinDirection2 write FPinDirection2;
  end;
  ...
  private
    // Global
    FManager: TVXNGDManager;
    FParentObject: TVXBaseSceneObject;
    ...
    FChildObject: TVXBaseSceneObject;
    ...
    { nj_UpVector}
    FUPVectorDirection: TVXCoordinates;
    ...
    procedure SetChildObject(const Value: TVXBaseSceneObject);
    procedure SetCollisionState(const Value: Boolean);
    procedure SetParentObject(const Value: TVXBaseSceneObject);
  ...
  published
    ...
    property ParentObject: TVXBaseSceneObject read FParentObject write
      SetParentObject;
    property ChildObject: TVXBaseSceneObject read FChildObject write
      SetChildObject;
    ...
    property UPVectorDirection
      : TVXCoordinates read FUPVectorDirection write FUPVectorDirection;
  end;

  { Global function }
function GetNGDStatic(Obj: TVXBaseSceneObject): TVXNGDStatic;
function GetOrCreateNGDStatic(Obj: TVXBaseSceneObject): TVXNGDStatic;
function GetNGDDynamic(Obj: TVXBaseSceneObject): TVXNGDDynamic;
function GetOrCreateNGDDynamic(Obj: TVXBaseSceneObject): TVXNGDDynamic;
function GetBodyFromVXSceneObject(Obj: TVXBaseSceneObject): PNewtonBody;
...
function GetNGDStatic(Obj: TVXBaseSceneObject): TVXNGDStatic;
begin
  Result := TVXNGDStatic(Obj.Behaviours.GetByClass(TVXNGDStatic));
end;

function GetOrCreateNGDStatic(Obj: TVXBaseSceneObject): TVXNGDStatic;
begin
  Result := TVXNGDStatic(Obj.GetOrCreateBehaviour(TVXNGDStatic));
end;

function GetNGDDynamic(Obj: TVXBaseSceneObject): TVXNGDDynamic;
begin
  Result := TVXNGDDynamic(Obj.Behaviours.GetByClass(TVXNGDDynamic));
end;

function GetOrCreateNGDDynamic(Obj: TVXBaseSceneObject): TVXNGDDynamic;
begin
  Result := TVXNGDDynamic(Obj.GetOrCreateBehaviour(TVXNGDDynamic));
end;

function GetBodyFromVXSceneObject(Obj: TVXBaseSceneObject): PNewtonBody;
var
  Behaviour: TVXNGDBehaviour;
begin
  Behaviour := TVXNGDBehaviour(Obj.Behaviours.GetByClass(TVXNGDBehaviour));
...
begin
  FManager := AOwner as TVXNGDManager;
  with FManager do
  begin
    FGeomColorDyn := TVXColor.CreateInitialized(self, clrGreen, NotifyChange);
    FGeomColorStat := TVXColor.CreateInitialized(self, clrRed, NotifyChange);
    FAABBColor := TVXColor.CreateInitialized(self, clrYellow, NotifyChange);
    FAABBColorSleep := TVXColor.CreateInitialized(self, clrOrange,
      NotifyChange);
    FCenterOfMassColor := TVXColor.CreateInitialized(self, clrPurple,
      NotifyChange);
    FContactColor := TVXColor.CreateInitialized(self, clrWhite, NotifyChange);
    FJointAxisColor := TVXColor.CreateInitialized(self, clrBlue, NotifyChange);
    FJointPivotColor := TVXColor.CreateInitialized(self, clrAquamarine,
      NotifyChange);

    FForceColor := TVXColor.CreateInitialized(self, clrBlack, NotifyChange);
    FAppliedForceColor := TVXColor.CreateInitialized(self, clrSilver,
      NotifyChange);
    FAppliedVelocityColor := TVXColor.CreateInitialized(self, clrLime,
      NotifyChange);

    FCustomColor := TVXColor.CreateInitialized(self, clrAqua, NotifyChange);
  end;
  FDotAxisSize := 1;
  FNGDManagerDebugs := [];
...
{ TVXNGDManager }
// ------------------------------------------------------------------
procedure TVXNGDManager.AddNode(const Value: TVector);
begin
  if Assigned(FGLLines) then
  begin
    FGLLines.Nodes.AddNode(Value);

    with (FGLLines.Nodes.Last as TVXLinesNode) do
      Color := FCurrentColor;
  end;
end;

procedure TVXNGDManager.AddNode(const coords: TVXCustomCoordinates);
begin
  if Assigned(FGLLines) then
  begin
    FGLLines.Nodes.AddNode(coords); (FGLLines.Nodes.Last as TVXLinesNode)
    .Color := FCurrentColor;
  end;
end;

procedure TVXNGDManager.AddNode(const X, Y, Z: Single);
begin
  if Assigned(FGLLines) then
  begin
    FGLLines.Nodes.AddNode(X, Y, Z); (FGLLines.Nodes.Last as TVXLinesNode)
    .Color := FCurrentColor;
  end;
end;

procedure TVXNGDManager.AddNode(const Value: TAffineVector);
begin
  if Assigned(FGLLines) then
  begin
    FGLLines.Nodes.AddNode(Value); (FGLLines.Nodes.Last as TVXLinesNode)
    .Color := FCurrentColor;
  end;
end;

constructor TVXNGDManager.Create(AOwner: TComponent);
var
  minworld, maxworld: TVector;
begin
  inherited;
  FNGDBehaviours := TVXNGDBehaviourList.Create;
  FVisible := True;
  FVisibleAtRunTime := False;
  FSolverModel := smExact;
  FFrictionModel := fmExact;
  FMinimumFrameRate := 60;
  FWorldSizeMin := TVXCoordinates.CreateInitialized(self,
    VectorMake(-100, -100, -100, 0), csPoint);
  FWorldSizeMax := TVXCoordinates.CreateInitialized(self,
    VectorMake(100, 100, 100, 0), csPoint);

  // Using Events because we need to call API Function when
  // theses TVXCoordinates change.
  FWorldSizeMin.OnNotifyChange := NotifyWorldSizeChange;
  FWorldSizeMax.OnNotifyChange := NotifyWorldSizeChange;

  FThreadCount := 1;
  FGravity := TVXCoordinates3.CreateInitialized(self,
...
destructor TVXNGDManager.Destroy;
begin
...
procedure TVXNGDManager.Loaded;
begin
  inherited;
...
function TVXNGDManager.GetBodyCount: Integer;
begin
...
function TVXNGDManager.GetConstraintCount: Integer;
begin
  if (csDesigning in ComponentState) then
...
procedure TVXNGDManager.NotifyChange(Sender: TObject);
var
  I: Integer;
begin
  // This event is raise
...
procedure TVXNGDManager.SetFrictionModel(const Value: TNGDFrictionModels);
begin
...
procedure TVXNGDManager.SetLines(const Value: TVXLines);
begin
  if Assigned(FGLLines) then
    FGLLines.Nodes.Clear;
...
procedure TVXNGDManager.SetMinimumFrameRate(const Value: Integer);
begin
...
procedure TVXNGDManager.SetSolverModel(const Value: TNGDSolverModels);
begin
  FSolverModel := Value;
  if not(csDesigning in ComponentState) then
...
procedure TVXNGDManager.SetThreadCount(const Value: Integer);
begin
  if Value > 0 then
    FThreadCount := Value;
...
procedure TVXNGDManager.SetVisible(const Value: Boolean);
begin
  FVisible := Value;
  if (csDesigning in ComponentState) then
    NotifyChange(self);
...
procedure TVXNGDManager.SetVisibleAtRunTime(const Value: Boolean);
begin
...
procedure TVXNGDManager.NotifyWorldSizeChange(Sender: TObject);
begin
  if not(csDesigning in ComponentState) then
...
procedure TVXNGDManager.RebuildAllJoint(Sender: TObject);
    ...
    with Joint do
      ...
      begin
        FNewtonJoint := NewtonConstraintCreateBall(FNewtonWorld,
          ...
          GetBodyFromVXSceneObject(FChildObject),
          GetBodyFromVXSceneObject(FParentObject));
  ...
  begin
    ...
    with Joint do
      if Assigned(FParentObject) and Assigned(FChildObject) then
      begin
        bso := TVXBaseSceneObject.Create(FManager);
  ...
  begin
    ...
    with Joint do
      ...
      begin
        ...
        FNewtonUserJoint := CreateCustomSlider(@pinAndPivot, GetBodyFromVXSceneObject(FChildObject), GetBodyFromVXSceneObject(FParentObject));
...
begin
  ...
  if not(csDesigning in ComponentState) and not(csLoading in ComponentState)
    then
  begin
    if Sender is TVXNGDManager then
      for i := 0 to NewtonJoint.Count - 1 do
    ...
    if Sender is TVXCoordinates then
      BuildOneJoint(((Sender as TVXCoordinates).Owner as TNGDJoint));
...
procedure TVXNGDManager.RebuildAllMaterial;
...
procedure TVXNGDManager.Step(deltatime: Single);
begin
  if not(csDesigning in ComponentState) then
...
destructor TVXNGDBehaviour.Destroy;
begin
...
procedure TVXNGDBehaviour.Finalize;
var
  i: integer;
begin
  FInitialized := False;
...
function TVXNGDBehaviour.GetBBoxCollision: PNewtonCollision;
var
...
function TVXNGDBehaviour.GetBSphereCollision: PNewtonCollision;
var
...
function TVXNGDBehaviour.GetConvexCollision: PNewtonCollision;
var
  I, J: Integer;
  vertexArray: array of TVertex;
begin
  if FOwnerBaseSceneObject is TVXBaseMesh then
  begin
    with (FOwnerBaseSceneObject as TVXBaseMesh) do
    begin
...
function TVXNGDBehaviour.GetHeightFieldCollision: PNewtonCollision;
var
  I: Integer;
...
function TVXNGDBehaviour.GetMeshCollision: PNewtonCollision;
var
...
function TVXNGDBehaviour.GetNewtonBodyMatrix: TMatrix;
begin
  if Assigned(FManager) then
...
function TVXNGDBehaviour.GetNewtonBodyAABB: TAABB;
begin
  if Assigned(FManager) then
    NewtonBodyGetAABB(FNewtonBody, @(Result.min), @(Result.max));
end;

function TVXNGDBehaviour.GetNGDFileCollision: PNewtonCollision;
var
  MyFile: TFileStream;
begin
  ...
  begin
    ...
    Result := NewtonCreateCollisionFromSerialization(FManager.FNewtonWorld,
      @TVXNGDBehaviour.NewtonDeserialize, Pointer(MyFile));
...
function TVXNGDBehaviour.GetNullCollision: PNewtonCollision;
begin
  Result := NewtonCreateNull(FManager.FNewtonWorld);
end;

function TVXNGDBehaviour.GetPrimitiveCollision: PNewtonCollision;
var
  collisionOffsetMatrix: TMatrix; // For cone capsule and cylinder
begin
  collisionOffsetMatrix := IdentityHmgMatrix;

  if (FOwnerBaseSceneObject is TVXCube) then
  begin
    with (FOwnerBaseSceneObject as TVXCube) do
      Result := NewtonCreateBox(FManager.FNewtonWorld, CubeWidth, CubeHeight,
        CubeDepth, 0, @collisionOffsetMatrix);
  end

  else if (FOwnerBaseSceneObject is TVXSphere) then
  begin
    with (FOwnerBaseSceneObject as TVXSphere) do
      Result := NewtonCreateSphere(FManager.FNewtonWorld, Radius, Radius,
        Radius, 0, @collisionOffsetMatrix);
  end

  else if (FOwnerBaseSceneObject is TVXCone) then
  begin
    collisionOffsetMatrix := MatrixMultiply(collisionOffsetMatrix,
      CreateRotationMatrixZ(Pi / 2.0));
    with (FOwnerBaseSceneObject as TVXCone) do
      Result := NewtonCreateCone(FManager.FNewtonWorld, BottomRadius, Height,
        0, @collisionOffsetMatrix);
  end

  else if (FOwnerBaseSceneObject is TVXCapsule) then
  begin
    collisionOffsetMatrix := MatrixMultiply(collisionOffsetMatrix,
      CreateRotationMatrixY(Pi / 2.0));
    with (FOwnerBaseSceneObject as TVXCapsule) do
  ...
  else if (FOwnerBaseSceneObject is TVXCylinder) then
  begin
    collisionOffsetMatrix := MatrixMultiply(collisionOffsetMatrix,
      CreateRotationMatrixZ(Pi / 2.0));
    with (FOwnerBaseSceneObject as TVXCylinder) do
...
function TVXNGDBehaviour.GetTreeCollision: PNewtonCollision;
var
...
procedure TVXNGDBehaviour.Initialize;
begin
  FInitialized := True;
...
procedure TVXNGDBehaviour.Loaded;
var
  mng: TComponent;
begin
  inherited;
  ...
  begin
    mng := FindManager(TVXNGDManager, FManagerName);
    if Assigned(mng) then
      Manager := TVXNGDManager(mng);
    FManagerName := '';
  end;
...
class procedure TVXNGDBehaviour.NewtonCollisionIterator
  (const userData: Pointer; vertexCount: Integer; const faceArray: PNGDFloat;
  faceId: Integer)cdecl;
begin
  TVXNGDBehaviour(userData).FCollisionIteratorEvent(userData, vertexCount,
    faceArray, faceId);
end;
...
// loading time
class procedure TVXNGDBehaviour.NewtonDeserialize(serializeHandle,
  buffer: Pointer; size: Cardinal)cdecl;
begin
  TFileStream(serializeHandle).read(buffer^, size);
end;

class procedure TVXNGDBehaviour.NewtonSerialize(serializeHandle: Pointer;
  const buffer: Pointer; size: Cardinal)cdecl;
begin
  TFileStream(serializeHandle).write(buffer^, size);
end;

procedure TVXNGDBehaviour.OnCollisionIteratorEvent(const userData: Pointer;
  vertexCount: Integer; const cfaceArray: PNGDFloat; faceId: Integer);
var
  I: Integer;
...
procedure TVXNGDBehaviour.Reinitialize;
begin
  if Initialized then
  begin
...
procedure TVXNGDBehaviour.Render;
var
  M: TMatrix;
begin
...
procedure TVXNGDBehaviour.SetContinuousCollisionMode(const Value: Boolean);
begin
...
procedure TVXNGDBehaviour.SetHeightFieldOptions(const Value: TVXHeightField);
begin
  FHeightFieldOptions := Value;
  Reinitialize;
end;

procedure TVXNGDBehaviour.SetManager(Value: TVXNGDManager);
begin
...
procedure TVXNGDBehaviour.SetNewtonBodyMatrix(const Value: TMatrix);
begin
...
procedure TVXNGDBehaviour.SetNGDNewtonCollisions
...
procedure TVXNGDBehaviour.SetNGDSurfaceItem(const Value: TNGDSurfaceItem);
begin
  FNGDSurfaceItem := Value;
  FManager.RebuildAllMaterial;
end;

function TVXNGDBehaviour.StoredTolerance: Boolean;
begin
  Result := not SameValue(FConvexCollisionTolerance, 0.01, epsilon);
end;

class function TVXNGDBehaviour.UniqueItem: Boolean;
begin
  Result := True;
end;

procedure TVXNGDBehaviour.ReadFromFiler(reader: TReader);
var
  version: Integer;
begin
  inherited;
...
procedure TVXNGDBehaviour.WriteToFiler(writer: TWriter);
begin
  inherited;
  with writer do
  begin
...
procedure TVXNGDBehaviour.Serialize(filename: string);
var
  MyFile: TFileStream;
begin
  ...
  NewtonCollisionSerialize(FManager.FNewtonWorld, FCollision,
    @TVXNGDBehaviour.NewtonSerialize, Pointer(MyFile));

  MyFile.Free;
end;

procedure TVXNGDBehaviour.DeSerialize(filename: string);
var
  MyFile: TFileStream;
  collisionInfoRecord: TNewtonCollisionInfoRecord;
begin
...
procedure TVXNGDDynamic.AddImpulse(const veloc, pointposit: TVector);
begin
...
begin
  ...
  FAutoSleep := True;
  FLinearDamping := 0.1;
  FAngularDamping := TVXCoordinates.CreateInitialized(self,
    VectorMake(0.1, 0.1, 0.1, 0), csPoint);
  FAngularDamping.OnNotifyChange := NotifyAngularDampingChange;
  FDensity := 1;
  FVolume := 1;
  FForce := TVXCoordinates.CreateInitialized(self, NullHmgVector, csVector);
  FTorque := TVXCoordinates.CreateInitialized(self, NullHmgVector, csVector);
  FCenterOfMass := TVXCoordinates.CreateInitialized(self, NullHmgVector,
    csPoint);
  FCenterOfMass.OnNotifyChange := NotifyCenterOfMassChange;
  FAABBmin := TVXCoordinates.CreateInitialized(self, NullHmgVector, csPoint);
  FAABBmax := TVXCoordinates.CreateInitialized(self, NullHmgVector, csPoint);
  FAppliedOmega := TVXCoordinates.CreateInitialized(self, NullHmgVector,
    csVector);
  FAppliedVelocity := TVXCoordinates.CreateInitialized(self, NullHmgVector,
    csVector);
  FAppliedForce := TVXCoordinates.CreateInitialized(self, NullHmgVector,
    csVector);
  FAppliedTorque := TVXCoordinates.CreateInitialized(self, NullHmgVector,
    csVector);
  FUseGravity := True;
...
destructor TVXNGDDynamic.Destroy;
begin
  // Clean up everything
...
procedure TVXNGDDynamic.Finalize;
begin
  if not(csDesigning in FOwnerBaseSceneObject.ComponentState) then
    if Assigned(FManager) then
    begin
...
class function TVXNGDDynamic.FriendlyName: string;
begin
  Result := 'NGD Dynamic';
end;


procedure TVXNGDDynamic.Initialize;
begin
  inherited;
  if not(csDesigning in FOwnerBaseSceneObject.ComponentState) then
    if Assigned(FManager) then
    begin
      ...
      NewtonBodySetForceAndTorqueCallback(FNewtonBody,
        @TVXNGDDynamic.NewtonApplyForceAndTorque);
      NewtonBodySetTransformCallback(FNewtonBody,
        @TVXNGDDynamic.NewtonSetTransform);
    end;
end;

procedure TVXNGDDynamic.Render;

  procedure DrawAABB(min, max: TVXCoordinates3);
  begin
...
procedure TVXNGDDynamic.SetAutoSleep(const Value: Boolean);
begin
  FAutoSleep := Value;
  if not(csDesigning in FOwnerBaseSceneObject.ComponentState) then
    if Assigned(FManager) then
...
procedure TVXNGDDynamic.SetDensity(const Value: Single);
var
  inertia: TVector;
  origin: TVector;
begin
...
procedure TVXNGDDynamic.SetLinearDamping(const Value: Single);
begin
...
function TVXNGDDynamic.GetOmega: TVector;
begin
  NewtonBodyGetOmega(FNewtonBody, @Result);
end;

procedure TVXNGDDynamic.SetOmega(const Omega: TVector);
begin
  NewtonBodySetOmega(FNewtonBody, @Omega);
end;

function TVXNGDDynamic.GetVelocity: TVector;
begin
  NewtonBodyGetVelocity(FNewtonBody, @Result);
end;

procedure TVXNGDDynamic.SetVelocity(const Velocity: TVector);
begin
  NewtonBodySetVelocity(FNewtonBody, @Velocity);
end;

function TVXNGDDynamic.StoredDensity: Boolean;
begin
  Result := not SameValue(FDensity, 1, epsilon);
end;

function TVXNGDDynamic.StoredLinearDamping: Boolean;
begin
  Result := not SameValue(FLinearDamping, 0.1, epsilon);
end;

function TVXNGDDynamic.StoredNullCollisionVolume: Boolean;
begin
...
// WriteToFiler
//
procedure TVXNGDDynamic.WriteToFiler(writer: TWriter);
begin
  inherited;
  with writer do
  begin
...
// ReadFromFiler
//
procedure TVXNGDDynamic.ReadFromFiler(reader: TReader);
var
  version: Integer;
begin
  inherited;
...
procedure TVXNGDDynamic.Loaded;
begin
  inherited;
  if Assigned(FManager) then
  begin
...
class procedure TVXNGDDynamic.NewtonApplyForceAndTorque
  (const body: PNewtonBody; timestep: NGDFloat; threadIndex: Integer); cdecl;
begin
  TVXNGDDynamic(NewtonBodyGetUserData(body)).FApplyForceAndTorqueEvent(body,
    timestep, threadIndex);
end;

class procedure TVXNGDDynamic.NewtonSetTransform(const body: PNewtonBody;
  const matrix: PNGDFloat; threadIndex: Integer); cdecl;
begin
  TVXNGDDynamic(NewtonBodyGetUserData(body)).FSetTransformEvent(body, matrix,
    threadIndex);
end;

procedure TVXNGDDynamic.NotifyAngularDampingChange(Sender: TObject);
begin
...
procedure TVXNGDDynamic.NotifyCenterOfMassChange(Sender: TObject);
begin
...
procedure TVXNGDDynamic.OnApplyForceAndTorqueEvent(const cbody: PNewtonBody;
  timestep: NGDFloat; threadIndex: Integer);
var
...
procedure TVXNGDDynamic.OnSetTransformEvent(const cbody: PNewtonBody;
  const cmatrix: PNGDFloat; threadIndex: Integer);
var
  epsi: Single;
begin
  // The Newton API does not support scale [scale modifie value in matrix],
  // so this line reset scale of the VXSceneObject to (1,1,1)
...
class function TVXNGDStatic.FriendlyName: string;
begin
  Result := 'NGD Static';
end;
...
begin
  ...
  FManager := TVXNGDManager(Collection.Owner);
  FManager.RebuildAllMaterial;
end;
...
begin
  ...
  FUPVectorDirection := TVXCoordinates.CreateInitialized(self, YHmgVector,
    csVector);
...
procedure TNGDJoint.SetChildObject(const Value: TVXBaseSceneObject);
begin
  FChildObject := Value;
...
procedure TNGDJoint.SetParentObject(const Value: TVXBaseSceneObject);
begin
  FParentObject := Value;
  FManager.RebuildAllJoint(self);
end;
...
begin
  FManager := AOwner as TVXNGDManager;
  FOuter := aOuter;
  FPivotPoint := TVXCoordinates.CreateInitialized(aOuter, NullHMGPoint,
    csPoint);
...
begin
  inherited;
  FPinDirection := TVXCoordinates.CreateInitialized(aOuter, NullHmgVector,
    csVector);
...
begin
  inherited;
  FPinDirection2 := TVXCoordinates.CreateInitialized(aOuter, NullHmgVector,
    csVector);
...
{ TVXNGDBehaviourList }
// ------------------------------------------------------------------
function TVXNGDBehaviourList.GetBehav(index: Integer): TVXNGDBehaviour;
begin
  Result := Items[index];
end;

procedure TVXNGDBehaviourList.PutBehav(index: Integer; Item: TVXNGDBehaviour);
begin
...
RegisterXCollectionItemClass(TVXNGDDynamic);
RegisterXCollectionItemClass(TVXNGDStatic);
...
UnregisterXCollectionItemClass(TVXNGDDynamic);
UnregisterXCollectionItemClass(TVXNGDStatic);
