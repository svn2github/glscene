...
//
{
  TVXRagdoll extended using Open Dynamics Engine (ODE).  
}
...
type
  ...
  TODERagdollDummyJoint = class(TVXRagdolJoint)
  end;


  TODERagdollHingeJoint = class(TVXRagdolJoint)
  private
    FParamHiStop: Single;
    FParamLoStop: Single;
    FAxis: TAffineVector;
  ...
  public
	...
	TODERagdollBone = class (TVXRagdolBone)
  private
  ...
  TODERagdoll = class(TVXRagdoll)
  private
    FODEWorld: TODERagdollWorld;
    FVXSceneRoot: TVXBaseSceneObject;
    FShowBoundingBoxes: Boolean;
  public
    constructor Create(AOwner : TVXBaseMesh);
    property ODEWorld: TODERagdollWorld read FODEWorld write FODEWorld;
    property VXSceneRoot: TVXBaseSceneObject read FVXSceneRoot write FVXSceneRoot;
...
begin
  ...
  FGeom.data := FRagdoll.VXSceneRoot.AddNewChild(TODERagdollCube);
...
begin
  ...
  begin
       o:=TVXBaseSceneObject(FGeom.data);
       FRagdoll.VXSceneRoot.Remove(o, false);
       o.free;
  end;
...
begin
  PositionSceneObject(TVXBaseSceneObject(PdxGeom(FGeom.data)), FGeom);
  RagDoll.Owner.Skeleton.BoneByID(BoneID).SetGlobalMatrixForRagDoll(TVXBaseSceneObject(PdxGeom(FGeom.data)).AbsoluteMatrix);
end;
...
constructor TODERagdoll.Create(AOwner : TVXBaseMesh);
begin
