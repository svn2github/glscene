...
type
   ...
   { Basic sound manager based on WinMM <i>waveOut</i> function. 
	...
	TVXSMWaveOut = class (TVXSoundManager)
	   private
...
procedure PlayOnWaveOut(pcmData : Pointer; lengthInBytes : Integer;
                        sampling : TVXSoundSampling); overload;
...
// Create
//
constructor TVXSMWaveOut.Create(AOwner : TComponent);
begin
...
// Destroy
//
destructor TVXSMWaveOut.Destroy;
begin
	inherited Destroy;
end;
...
// DoActivate
//
function TVXSMWaveOut.DoActivate : Boolean;
begin
   Result:=True;
end;
...
// DoDeActivate
//
procedure TVXSMWaveOut.DoDeActivate;
var
   i : Integer;
begin
...
// KillSource
//
procedure TVXSMWaveOut.KillSource(aSource : TVXBaseSoundSource);
var
...
begin
   if uMsg=WOM_DONE then begin
      PWaveOutPlayingRec(TVXSoundSource(dwInstance).ManagerTag).CurrentState:=ssFinished;
   end;
end;

// UpdateSource
//
procedure TVXSMWaveOut.UpdateSources;
var
   i, n : Integer;
   wfx : TWaveFormatEx;
   smp : TVXSoundSample;
