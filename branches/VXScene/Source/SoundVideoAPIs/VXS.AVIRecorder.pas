...
type
  ...
  TVXAVIRecorder = class(TComponent)
  private
    AVIBitmap: TBitmap;
  ...
  protected
    ...
    FVKNonVisualViewer: TVXNonVisualViewer;
  ...
  published
    ...
    property VXSceneViewer: TVXSceneViewer read FVXSceneViewer
      write SetVXSceneViewer;
    property VKNonVisualViewer: TVXNonVisualViewer read FVKNonVisualViewer
...
//
procedure TVXAVIRecorder.AddAVIFrame(bmp: TBitmap);
begin
  if RecorderState <> rsRecording then
    raise Exception.Create('Cannot add frame to AVI. AVI file not created.');
  { TODO : E2003 Undeclared identifier: 'Draw' }
...
function TVXAVIRecorder.CreateAVIFile(DPI: integer = 0): boolean;
var
...
procedure TVXAVIRecorder.CloseAVIFile(UserAbort: boolean = false);
begin
...
// Recording
//
function TVXAVIRecorder.Recording: boolean;
begin
  Result := (RecorderState = rsRecording);
end;

procedure TVXAVIRecorder.SetVXSceneViewer(const Value: TVXSceneViewer);
begin
  FVXSceneViewer := Value;
  if Assigned(FVXSceneViewer) then
    FBuffer := FVXSceneViewer.Buffer
  else
    FBuffer := nil;
end;

procedure TVXAVIRecorder.SetVKNonVisualViewer(const Value: TVXNonVisualViewer);
begin
