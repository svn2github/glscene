...
type
  ...
  TCUDADimensions = class(TVXUpdateAbleObject)
  private
  ...
  TOnOpenGLInteropInit = procedure(out Context: TVXContext) of object;
  ...
  TVXSCUDADevice = class(TComponent)
  private
...
constructor TVXSCUDADevice.Create(AOwner: TComponent);
var
  LDevice: TCUDADevice;
begin
  inherited Create(AOwner);
...
destructor TVXSCUDADevice.Destroy;
var
  Device: TCUDADevice;
begin
  inherited;
...
function TVXSCUDADevice.GetDevice: TCUDADevice;
begin
  Result := CUDAContextManager.GetDeviceByName(FSelectDeviceName);
end;

function TVXSCUDADevice.Suitable: Boolean;
var
  LDevice: TCUDADevice;
begin
  LDevice := GetDevice;
...
procedure TVXSCUDADevice.SetDevice(AValue: TCUDADevice);
begin
end;

procedure TVXSCUDADevice.SetDeviceName(const AName: string);
begin
...
// ------------------------------------------------------------------
  ...
  RegisterClasses([TVXSCUDADevice]);
