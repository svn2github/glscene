...
type
  TVXSCUDACompilerOutput = (codeUndefined, codePtx, codeCubin, codeGpu);
  ...
  TVXSCUDAVirtArch = (compute_10, compute_11, compute_12, compute_13, compute_20);

  // TVXSCUDAGPUGeneration
  //
  ...
  TVXSCUDARealArch = (sm_10, sm_11, sm_12, sm_13, sm_20, sm_21);
  TVXSCUDARealArchs = set of TVXSCUDARealArch;

  TVXSCUDACompiler = class(TComponent)
  private
    ...
    FOutputCodeType: TVXSCUDACompilerOutput;
    FVirtualArch: TVXSCUDAVirtArch;
    FRealArch: TVXSCUDARealArchs;
    FMaxRegisterCount: Integer;
    FModuleInfo: TCUDAModuleInfo;
    procedure SetMaxRegisterCount(Value: Integer);
    procedure SetOutputCodeType(const Value: TVXSCUDACompilerOutput);
    function StoreProjectModule: Boolean;
    procedure SetRealArch(AValue: TVXSCUDARealArchs);
  ...
  published
    ...
    { Output code type for module kernel
      ...
      - Cubin - CUDA Binary }
    property OutputCodeType: TVXSCUDACompilerOutput read FOutputCodeType
    ...
    property RealArchitecture: TVXSCUDARealArchs read FRealArch
      write SetRealArch default [sm_13];
    { Virtual architecture. }
    property VirtualArchitecture: TVXSCUDAVirtArch read FVirtualArch
...
constructor TVXSCUDACompiler.Create(AOwner: TComponent);
var
  path: string;
begin
...
destructor TVXSCUDACompiler.Destroy;
begin
  FModuleInfo.Destroy;
  inherited;
end;

procedure TVXSCUDACompiler.Loaded;
var
  LStr: string;
begin
  inherited;
...
procedure TVXSCUDACompiler.Assign(Source: TPersistent);
var
  compiler: TVXSCUDACompiler;
begin
  if Source is TVXSCUDACompiler then
  begin
    compiler := TVXSCUDACompiler(Source);
...
procedure TVXSCUDACompiler.SetCppCompilerPath(const AValue: string);
begin
  if FileExists(AValue + 'cl.exe') then
    FCppCompilerPath := AValue;
end;

procedure TVXSCUDACompiler.setMaxRegisterCount(Value: Integer);
begin
...
procedure TVXSCUDACompiler.SetNVCCPath(const AValue: string);
begin
  if FileExists(AValue + 'nvcc.exe') then
    FNVCCPath := AValue;
end;

procedure TVXSCUDACompiler.setOutputCodeType(const Value
  : TVXSCUDACompilerOutput);
begin
...
procedure TVXSCUDACompiler.SetRealArch(AValue: TVXSCUDARealArchs);
begin
  if AValue = [] then
    AValue := [sm_10];
  FRealArch := AValue;
...
procedure TVXSCUDACompiler.SetSourceCodeFile(const AFileName: string);
begin
  if FileStreamExists(AFileName) then
    FSourceCodeFile := AFileName;
end;

function TVXSCUDACompiler.StoreProjectModule: Boolean;
begin
