...
type
  ...
  private
    ...
    FHandle: PCUmodule;
    FCode: TStringList;
    FCodeType: TVXSCUDACompilerOutput;
    FCompiler: TVXSCUDACompiler;
    procedure SetCode(const Value: TStringList);
    procedure SetCompiler(const Value: TVXSCUDACompiler);
  ...
  public
    ...
    property Context: TCUDAContext read GetContext;
    property CodeType: TVXSCUDACompilerOutput read FCodeType;
  ...
  published
    ...
    property Compiler: TVXSCUDACompiler read FCompiler write SetCompiler;
  end;

  TVXResourceType = (rtTexture, rtBuffer);
  ...
  protected
    ...
    FResourceType: TVXResourceType;
    FGLContextHandle: TVXVirtualHandle;
    FMapCounter: Integer;
    function GetIsAllocated: Boolean; override;
    procedure OnGLHandleAllocate(Sender: TVXVirtualHandle;
      var Handle: Cardinal);
    procedure OnGLHandleDestroy(Sender: TVXVirtualHandle; var Handle: Cardinal);
  ...
  public
    ...
    procedure CopyTo(const AGLImage: TVXImage); overload;
    ...
    procedure CopyFrom(const AGLImage: TVXBitmap32); overload;
  ...
  TVXSCUDA = class(TCUDAComponent)
  private
    
    fDevice: TVXSCUDADevice;
    fContext: TCUDAContext;
    FOnOpenGLInteropInit: TOnOpenGLInteropInit;
    procedure SetDevice(const Value: TVXSCUDADevice);
  ...
  published
    ...
    property ComputingDevice: TVXSCUDADevice read fDevice write SetDevice;
...
constructor TVXSCUDA.Create(AOwner: TComponent);
begin
...
procedure TVXSCUDA.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
procedure TVXSCUDA.SetDevice(const Value: TVXSCUDADevice);
begin
...
procedure TVXSCUDA.SetOnOpenGLInteropInit(AEvent: TOnOpenGLInteropInit);
begin
  FOnOpenGLInteropInit := AEvent;
  CuNotifyChange(cuchContext);
end;

function TVXSCUDA.GetContext: TCUDAContext;
begin
...
function TVXSCUDA.GetIsAllocated: Boolean;
begin
  Result := FContext.IsValid;
end;

function TVXSCUDA.GetModule(const i: Integer): TCUDAModule;
var
  j, k: Integer;
begin
  Result := nil;
...
procedure TCUDAModule.SetCompiler(const Value: TVXSCUDACompiler);
begin
...
begin
  if Assigned(FMaster) and (FMaster is TVXSCUDA) then
    Result := TVXSCUDA(FMaster).Context
  else
  begin
    Result := nil;
...
begin
  if not(csLoading in ComponentState) and (Sender is TVXSCUDACompiler) then
  begin
    AllocateHandles;
  end;
end;
...
begin
  if Self is TVXSCUDA then
    Result := TVXSCUDA(Self).Context
  else
    Result := TVXSCUDA(FMaster).Context;
end;
...
begin
  Context.Requires;
  MemPerBlock := TVXSCUDA(TCUDAModule(FMaster).FMaster)
...
procedure TCUDAMemData.CopyTo(const AGLImage: TVXBitmap32);
var
  copyParam2D: TCUDA_MEMCPY2D;
  // copyParam3D: TCUDA_MEMCPY3D;
begin
...
procedure TCUDAMemData.CopyFrom(const AGLImage: TVXBitmap32);
var
  copyParam2D: TCUDA_MEMCPY2D;
  // copyParam3D: TCUDA_MEMCPY3D;
begin
...
procedure TCUDAGraphicResource.OnGLHandleAllocate(Sender: TVXVirtualHandle;
  var Handle: Cardinal);
begin
...
procedure TCUDAGraphicResource.OnGLHandleDestroy(Sender: TVXVirtualHandle;
  var Handle: Cardinal);
begin
  DestroyHandles;
end;
...
// ------------------------------------------------------------------
  ...
  RegisterClasses([TVXSCUDA, TVXSCUDACompiler, TCUDAModule, TCUDAFunction,
