...
type
  ...
  TVXROAMPatch = class(TObject)
  private
    FID: Integer;
    FHeightData: TVXHeightData; // Referred, not owned
    ...
    FNorth, FSouth, FWest, FEast: TVXROAMPatch; // neighbours
    FHighRes: Boolean;
    FMaxDepth: Integer;
  ...
  protected
    procedure SetHeightData(Val: TVXHeightData);
  ...
  public
    ...
    procedure ConnectToTheWest(WestPatch: TVXROAMPatch);
    procedure ConnectToTheNorth(NorthPatch: TVXROAMPatch);
    ...
    class procedure FlushAccum(Vertices: TAffineVectorList;
      VertexIndices: TIntegerList; TexCoords: TTexPointList);
    property HeightData: TVXHeightData read FHeightData write SetHeightData;
...
var
  FVBOVertHandle, FVBOTexHandle: TVXVBOArrayBufferHandle;
  FVBOIndicesHandle: TVXVBOElementArrayHandle;
...
constructor TVXROAMPatch.Create;
begin
  inherited Create;
  FID := vNextPatchID;
  Inc(vNextPatchID);
...
destructor TVXROAMPatch.Destroy;
begin
  FListHandle.Free;
...
procedure TVXROAMPatch.SetHeightData(Val: TVXHeightData);
begin
  FHeightData := Val;
...
procedure TVXROAMPatch.SetOcclusionSkip(Val: Integer);
begin
  if Val < 0 then
    Val := 0;
...
procedure TVXROAMPatch.ConnectToTheWest(WestPatch: TVXROAMPatch);
begin
...
// RenderAsStrips
//
procedure TVXROAMPatch.RenderAsStrips(vertices: TAffineVectorList;
  VertexIndices: TIntegerList; TexCoords: TTexPointList);
...
FVBOTexHandle := TVXVBOArrayBufferHandle.Create;
FVBOIndicesHandle := TVXVBOElementArrayHandle.Create;
