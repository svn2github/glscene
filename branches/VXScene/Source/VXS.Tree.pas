...
type
  TVXTree = class;
  TVXTreeBranches = class;
  TVXTreeBranchNoise = class;
  ...
  TVXTreeBranch = class
  private
    FOwner: TVXTreeBranches;
    FLeft: TVXTreeBranch;
    FCenter: TVXTreeBranch;
    FRight: TVXTreeBranch;
    FParent: TVXTreeBranch;
    FBranchID: Integer;
    FParentID: Integer;
    FMatrix: TMatrix;
    FLower: TIntegerList;
    ...
    procedure BuildBranch(branchNoise: TVXTreeBranchNoise;
      const matrix: TMatrix; TexCoordY, Twist: Single; Level: Integer);
  public
    constructor Create(AOwner: TVXTreeBranches; AParent: TVXTreeBranch);
    destructor Destroy; override;
    property Owner: TVXTreeBranches read FOwner;
    property Left: TVXTreeBranch read FLeft;
    property Center: TVXTreeBranch read FCenter;
    property Right: TVXTreeBranch read FRight;
    property Parent: TVXTreeBranch read FParent;
  ...
  private
    FBranchNoise: Single;
    FLeft, FRight, FCenter: TVXTreeBranchNoise;
    function GetLeft: TVXTreeBranchNoise;
    function GetCenter: TVXTreeBranchNoise;
    function GetRight: TVXTreeBranchNoise;
  public
    constructor Create;
    destructor Destroy; override;
    property Left: TVXTreeBranchNoise read GetLeft;
    property Center: TVXTreeBranchNoise read GetCenter;
    property Right: TVXTreeBranchNoise read GetRight;
    property branchNoise: Single read FBranchNoise;
  end;

  TVXTree = class(TVXImmaterialSceneObject)
  private
    FDepth: Integer;
    ...
    FLeaves: TVXTreeLeaves;
    FBranches: TVXTreeBranches;
    FNoise: TVXTreeBranchNoise;
    FMaterialLibrary: TVXMaterialLibrary;
    FLeafMaterialName: TVXLibMaterialName;
    FLeafBackMaterialName: TVXLibMaterialName;
    FBranchMaterialName: TVXLibMaterialName;
  ...
  protected
    ...
    procedure SetLeafMaterialName(const Value: TVXLibMaterialName);
    procedure SetLeafBackMaterialName(const Value: TVXLibMaterialName);
    procedure SetBranchMaterialName(const Value: TVXLibMaterialName);
  ...
  public
    ...
    procedure BuildMesh(GLBaseMesh: TVXBaseMesh);
    ...
    property Leaves: TVXTreeLeaves read FLeaves;
    property Branches: TVXTreeBranches read FBranches;
    property Noise: TVXTreeBranchNoise read FNoise;
  published
    ...
    property LeafMaterialName: TVXLibMaterialName read FLeafMaterialName
      write SetLeafMaterialName;
    property LeafBackMaterialName: TVXLibMaterialName read FLeafBackMaterialName
      write SetLeafBackMaterialName;
    property BranchMaterialName: TVXLibMaterialName read FBranchMaterialName
...
constructor TVXTreeLeaves.Create(AOwner: TVXTree);
begin
  FOwner := AOwner;
  FCount := 0;
...
destructor TVXTreeLeaves.Destroy;
begin
  FVertices.Free;
  FNormals.Free;
  FTexCoords.Free;
...
procedure TVXTreeLeaves.AddNew(matrix: TMatrix);
var
  radius: Single;
  pos: TVector;
begin
...
procedure TVXTreeLeaves.BuildList(var rci: TVXRenderContextInfo);
var
  i: Integer;
  n: TAffineVector;
...
var
  ...
  LeftBranchNoise: TVXTreeBranchNoise;
  CenterBranchNoise: TVXTreeBranchNoise;
  RightBranchNoise: TVXTreeBranchNoise;
  LeftMatrix, RightMatrix, CenterMatrix: TMatrix;
  central_leader: Boolean;
begin
  Assert(Assigned(FOwner), 'Incorrect use of TVXTreeBranch');
  ...
  begin
    Inc(Branches.FCount);
    FLeft := TVXTreeBranch.Create(Owner, Self);
  ...
  begin
    Inc(Branches.FCount);
    FCenter := TVXTreeBranch.Create(Owner, Self);
  ...
  begin
    Inc(Branches.FCount);
    FRight := TVXTreeBranch.Create(Owner, Self);
...
begin
  ...
  Inc(FCount);
  FRoot := TVXTreeBranch.Create(Self, nil);
...
begin
  if not Assigned(FLeft) then
    FLeft := TVXTreeBranchNoise.Create;
  Result := FLeft;
end;
...
begin
  if not Assigned(FRight) then
    FRight := TVXTreeBranchNoise.Create;
  Result := FRight;
end;

function TVXTreeBranchNoise.GetCenter: TVXTreeBranchNoise;
begin
  if not Assigned(FCenter) then
    FCenter := TVXTreeBranchNoise.Create;
  Result := FCenter;
end;
...
begin
  ...
  FLeaves := TVXTreeLeaves.Create(Self);
  FBranches := TVXTreeBranches.Create(Self);
...
procedure TVXTree.BuildMesh(GLBaseMesh: TVXBaseMesh);
  ...
  begin
    ...
    if Assigned(Branch.Left) then
      RecursBranches(Branch.Left, TVXSkeletonBone.CreateOwned(bone), Frame);
    if Assigned(Branch.Right) then
      RecursBranches(Branch.Right, TVXSkeletonBone.CreateOwned(bone), Frame);
  end;

var
  // SkelMesh : TVXSkeletonMeshObject;
...
begin
  ...
  // if GLBaseMesh is TVXActor then
  // TVXSkeletonMeshObject.CreateOwned(GLBaseMesh.MeshObjects)
  // else
  TVXMeshObject.CreateOwned(GLBaseMesh.MeshObjects);
  ...
  { if GLBaseMesh is TVXActor then begin
    TVXActor(GLBaseMesh).Reference:=aarSkeleton;
    RecursBranches(Branches.FRoot,
    TVXSkeletonBone.CreateOwned(GLBaseMesh.Skeleton.RootBones),
    TVXSkeletonFrame.CreateOwned(GLBaseMesh.Skeleton.Frames));
    SkelMesh:=TVXSkeletonMeshObject(GLBaseMesh.MeshObjects[0]);
