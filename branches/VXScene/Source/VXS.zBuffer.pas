...
type
  ...
  public
    SceneViewer: TVXSceneViewer;
    MemoryViewer: TVXMemoryViewer;
    Buffer: TVXSceneBuffer;
    Normal: TAffineVector; //Absolute direction of camera
    constructor Create;
    destructor Destroy; override;
    procedure LinkToViewer(viewer: TVXSceneViewer); overload;
    procedure LinkToViewer(viewer: TVXMemoryViewer); overload;
  ...
  TVXZShadows = class(TVXBaseSceneObject)
  private
    FViewer: TVXSceneViewer;
    FCaster: TVXMemoryViewer;
    FDepthFade: Boolean;
  ...
  protected
    ...
    function GetViewer: TVXSceneViewer;
    procedure SetViewer(const val: TVXSceneViewer);
    function GetCaster: TVXMemoryViewer;
    procedure SetCaster(const val: TVXMemoryViewer);
    procedure CalcShadowTexture(var rci: TVXRenderContextInfo);
  ...
  public
    ViewerZBuf: TVXzBuffer;
    CasterZBuf: TVXzBuffer;
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure DoRender(var ARci: TVXRenderContextInfo; ARenderSelf,
      ARenderChildren: Boolean); override;
  published
    property Viewer: TVXSceneViewer read GetViewer write SetViewer;
    property Caster: TVXMemoryViewer read GetCaster write SetCaster;
...
procedure TVXzBuffer.LinkToViewer(viewer: TVXSceneViewer); // overload;
begin
...
procedure TVXzBuffer.LinkToViewer(viewer: TVXMemoryViewer); // overload;
begin
...
destructor TVXzBuffer.Destroy;
begin
  FreeMem(FData);
  inherited Destroy;
end;

procedure TVXzBuffer.PrepareBufferMemory;
var
  i: Integer;
begin
...
procedure TVXzBuffer.SetWidth(val: Integer);
begin
  if val <> FWidth then
  begin
    Assert(val >= 0);
...
procedure TVXzBuffer.SetHeight(const val: Integer);
begin
  if val <> FHeight then
  begin
    Assert(val >= 0);
...
function TVXzBuffer.GetDepthBuffer(CalcVectors: Boolean; ContextIsActive:
  boolean): PZArray;
begin
...
function TVXzBuffer.GetPixelzDepth(x, y: integer): Single;
begin
...
function TVXzBuffer.PixelToDistance_OLD(x, y: integer): Single;
var
...
function TVXzBuffer.PixelToDistance(x, y: integer): Single;
var
  z, dst: single;
  xx, yy, zz: single;
  fy: integer;
...
procedure TVXzBuffer.Refresh;
begin
  if assigned(Buffer) then
    GetDepthBuffer(True, False);
end;

procedure TVXzBuffer.DoCalcVectors;
var
  axs: TAffineVector;
  Hnorm, hcvec: TVector;
  vec: TAffineVector;
...
function TVXzBuffer.FastScreenToVector(x, y: integer): TAffineVector;
var
  w, h: integer;
  Rlerp, Ulerp: single;
begin
...
function TVXzBuffer.FastVectorToScreen(Vec: TAffineVector): TAffineVector;
var
  v0, v1, x, y, z: Single;
begin
  x := vec.X;
...
function TVXzBuffer.PixelToWorld(const x, y: Integer): TAffineVector;
var
  z, dst: single;
  fy: integer;
  camvec: TVector;
...
function TVXzBuffer.WorldToPixel(const aPoint: TAffineVector; out pixX, pixY:
...
function TVXzBuffer.WorldToPixelZ(const aPoint: TAffineVector; out pixX, pixY:
...
function TVXzBuffer.OrthWorldToPixelZ(const aPoint: TAffineVector; out pixX,
...
constructor TVXZShadows.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TVXZShadows.Destroy;
begin
  ViewerZBuf.Free;
  CasterZBuf.Free;
  FColor.Free;
...
procedure TVXZShadows.BindTexture;
begin
...
procedure TVXZShadows.DoRender(var ARci: TVXRenderContextInfo;
...
begin
  ...
  if TVXSceneBuffer(ARci.buffer).Camera <> FViewer.Camera then
    exit;
  if not assigned(ViewerZBuf) then
  begin //Create viewer zbuffer
    ViewerZBuf := TVXZBuffer.Create;
...
procedure TVXZShadows.CalcShadowTexture(var rci: TVXRenderContextInfo);
var
...
function TVXZShadows.HardSet(const x, y: integer): Byte;
var
  pix: Byte;
  coord: TAffineVector;
...
function TVXZShadows.SoftTest(const x, y: integer): Byte;
begin
  result := FDataInvIdx[y]^[x];
end;

function TVXZShadows.GetViewer: TVXSceneViewer;
begin
  result := FViewer;
end;

procedure TVXZShadows.SetViewer(const val: TVXSceneViewer);
begin
  FViewer := Val;
...
function TVXZShadows.GetCaster: TVXMemoryViewer;
begin
  result := FCaster;
end;

procedure TVXZShadows.SetCaster(const val: TVXMemoryViewer);
begin
  FCaster := Val;
end;

function TVXZShadows.CastShadow: Boolean;
begin
  if Caster <> nil then
  begin
    if not assigned(CasterZBuf) then
    begin
      CasterZBuf := TVXZBuffer.Create;
...
procedure TVXZShadows.SetWidth(const val: integer);
begin
  FWidth := val;
  SetXRes(val);
end;

procedure TVXZShadows.SetHeight(const val: integer);
begin
  FHeight := val;
  SetYRes(val);
end;

procedure TVXZShadows.SetXRes(const val: integer);
var
  i: integer;
begin
  i := 2;
...
procedure TVXZShadows.SetYRes(const val: integer);
var
  i: integer;
begin
  i := 2;
...
procedure TVXZShadows.SetSoft(const val: boolean);
begin
  FSoft := val;
  NotifyChange(Self);
end;
