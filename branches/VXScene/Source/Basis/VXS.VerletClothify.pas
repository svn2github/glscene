...
//
{ 
   Methods for turning a TVXBaseMesh into a Verlet cloth / jelly 
    
}

unit VXS.VerletClothify;
...
type
  ...
  public
    ...
    MeshObject : TVXMeshObject;
    Active : boolean;

    procedure UpdateNormal;

    constructor Create(aMeshObject : TVXMeshObject);
  end;
  ...
  protected
    procedure ProcessMeshObject(const MeshObject : TVXMeshObject); virtual;
  public
    procedure ExtractFacesFromVertexIndexList(
      const FaceGroup : TFGVertexIndexList; const MeshObject : TVXMeshObject);
    ...
    property GLBaseMesh : TVXBaseMesh read FGLBaseMesh;

    property NodeList : TVerletNodeList read FNodeList;

    function AddFace(const Vi0, Vi1, Vi2 : integer; const MeshObject : TVXMeshObject) : TFace; virtual;

    constructor Create(const aGLBaseMesh : TVXBaseMesh); virtual;
  ...
  public
    ...
    property MeshObject : TVXMeshObject read FMeshObject write FMeshObject;
    ...
    constructor Create(const AOwner: TEdgeDetector; AVi0, AVi1 : integer;
      AFace0, AFace1 : TFace; AMeshObject : TVXMeshObject; ASolid : boolean);
  end;
  ...
  public
    ...
    function AddEdge(const Vi0, Vi1 : integer; const Face : TFace; const AMeshObject : TVXMeshObject) : TEdge;
    function AddFace(const Vi0, Vi1, Vi2 : integer; const MeshObject : TVXMeshObject) : TFace; override;
    function AddNode(const VerletWorld : TVXVerletWorld; const MeshObject : TVXMeshObject; const VertexIndex : integer) : TVerletNode; virtual;

    procedure AddNodes(const VerletWorld : TVXVerletWorld);
    procedure AddEdgesAsSticks(const VerletWorld : TVXVerletWorld; const Slack : single);
    procedure AddEdgesAsSprings(const VerletWorld : TVXVerletWorld; const Strength, Damping, Slack : single);
    procedure AddEdgesAsSolidEdges(const VerletWorld : TVXVerletWorld);
    procedure AddOuterEdgesAsSolidEdges(const VerletWorld : TVXVerletWorld);

    procedure RenderEdges(var rci : TVXRenderContextInfo);
    ...
    constructor Create(const aGLBaseMesh : TVXBaseMesh); override;
    destructor Destroy; override;
  end;

  TVXMeshObjectVerletNode = class(TVerletNode)
  private
    MeshObject : TVXMeshObject;
...
constructor TFaceExtractor.Create(const aGLBaseMesh : TVXBaseMesh);
begin
...
procedure TFaceExtractor.ProcessMeshObject(const MeshObject : TVXMeshObject);
var
 iFaceGroup : integer;
begin
...
function TFaceExtractor.AddFace(const Vi0, Vi1, Vi2: integer; const MeshObject : TVXMeshObject) : TFace;
var
  Face : TFace;
begin
...
{ TVXMeshObjectVerletNode }

constructor TVXMeshObjectVerletNode.CreateOwned(const aOwner: TVXVerletWorld);
begin
  inherited;
  VertexIndices := TIntegerList.Create;
end;

destructor TVXMeshObjectVerletNode.Destroy;
begin
  VertexIndices.Free;
  inherited;
end;

procedure TVXMeshObjectVerletNode.AfterProgress;
var
  i : integer;
begin
...
constructor TEdgeDetector.Create(const aGLBaseMesh: TVXBaseMesh);
begin
...
function TEdgeDetector.AddEdge(const Vi0, Vi1: integer; const Face: TFace; const AMeshObject : TVXMeshObject): TEdge;
var
  i : integer;
  Edge : TEdge;
begin
...
function TEdgeDetector.AddFace(const Vi0, Vi1, Vi2: integer;
  const MeshObject: TVXMeshObject): TFace;
var
  Face : TFace;
begin
  Face := TFace.Create(MeshObject);
...
procedure TEdgeDetector.AddNodes(const VerletWorld : TVXVerletWorld);
var
  i : integer;
  MO : TVXMeshObject;
begin
...
procedure TEdgeDetector.AddEdgesAsSprings(const VerletWorld : TVXVerletWorld;
...
procedure TEdgeDetector.AddEdgesAsSticks(const VerletWorld : TVXVerletWorld;
  const Slack : single);
var
  i : integer;
  Edge : TEdge;
...
procedure TEdgeDetector.AddEdgesAsSolidEdges(
  const VerletWorld: TVXVerletWorld);
var
  i : integer;
  Edge : TEdge;
begin
...
procedure TEdgeDetector.RenderEdges(var rci: TVXRenderContextInfo);
var
  i : integer;
  Edge : TEdge;
...
function TEdgeDetector.AddNode(const VerletWorld : TVXVerletWorld; const MeshObject: TVXMeshObject;
  const VertexIndex: integer): TVerletNode;
var
  Location : TAffineVector;
  aNode : TVXMeshObjectVerletNode;
  i : integer;
begin
  ...
  begin
    aNode := TVXMeshObjectVerletNode(FNodeList[i]);
  ...
  aNode := TVXMeshObjectVerletNode.CreateOwned(VerletWorld);
...
constructor TFace.Create(aMeshObject: TVXMeshObject);
begin
