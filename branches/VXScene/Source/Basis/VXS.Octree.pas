...
type
  ...
  TOctreeNode = record
    ...
    // TextureHandle:TVXTextureHandle;
    ListHandle: TVXListHandle;
...
destructor TVXOctree.Destroy;
begin
  DisposeTree;
  inherited Destroy;
end;

procedure TVXOctree.DisposeTree;
...
procedure TVXOctree.CreateTree(Depth: Integer);
begin
...
function TVXOctree.GetMidPoint(Min, Max: Single): Single;
begin
  Result := Max / 2 + Min / 2;
  // This formula is non-quadrant specific; ie: good.
end;

function TVXOctree.GetExtent(const Flags: array of Byte; ParentNode: POctreeNode)
  : TAffineFLTVector;
var
...
procedure TVXOctree.InitializeTree(const AWorldMinExtent, AWorldMaxExtent
  : TAffineVector; const ATriangles: TAffineVectorList;
  const ATreeDepth: Integer);
var
  N: Integer;
...
procedure TVXOctree.Refine(ParentNode: POctreeNode; Level: Integer);
var
  N, X, Z: Integer;
...
procedure TVXOctree.ConvertR4(ONode: POctreeNode; const Scale: TAffineFLTVector);
var
  N: Smallint;
begin
...
function TVXOctree.PointInNode(const Min, Max, APoint: TAffineFLTVector): BOOLEAN;
begin
...
procedure TVXOctree.WalkPointToLeaf(Onode: POctreeNode; const P: TAffineVector);
begin
  Finalize(Resultarray);
  WalkPointToLeafx(Onode, P);
end;

procedure TVXOctree.WalkPointToLeafx(Onode: POctreeNode; const P: TAffineVector);
var
  N: Integer;
begin
...
function TVXOctree.SphereInNode(const MinExtent, MaxExtent: TAffineVector;
  const C: TVector; Radius: Single): Boolean;
...
procedure TVXOctree.WalkSphereToLeaf(Onode: POctreeNode; const P: TVector;
  Radius: Single);
begin
  Finalize(Resultarray);
...
procedure TVXOctree.WalkSphereToLeafx(Onode: POctreeNode; const P: TVector;
  Radius: Single);
var
  N: Integer;
begin
...
procedure TVXOctree.WalkRayToLeaf(Onode: POctreeNode; const P, V: TVector);
begin
  Finalize(Resultarray);
...
procedure TVXOctree.WalkRayToLeafx(Onode: POctreeNode; const P, V: TVector);
var
  N: Integer;
  Coord: TVector;
begin
...
function TVXOctree.TriIntersectNode(const MinExtent, MaxExtent, V1, V2,
...
procedure TVXOctree.WalkTriToLeaf(Onode: POctreeNode;
  const V1, V2, V3: TAffineFLTVector);
begin
  Finalize(Resultarray);
...
procedure TVXOctree.WalkTriToLeafx(Onode: POctreeNode;
  const V1, V2, V3: TAffineFLTVector);
var
  M: Integer;
begin
...
function TVXOctree.RayCastIntersect(const RayStart, RayVector: TVector;
  IntersectPoint: PVector = nil; IntersectNormal: PVector = nil;
  TriangleInfo: POctreeTriangleInfo = nil): Boolean;
const
...
//
function TVXOctree.SphereSweepIntersect(const RayStart, RayVector: TVector;
  const Velocity, Radius: Single; IntersectPoint: PVector = nil;
  IntersectNormal: PVector = nil): Boolean;
const
...
function TVXOctree.TriangleIntersect(const V1, V2, V3: TAffineVector): Boolean;
var
  I, T, K: Integer;
  P: POctreeNode;
  P1, P2, P3: PAffineVector;
...
function TVXOctree.AABBIntersect(const AABB: TAABB; M1to2, M2to1: TMatrix;
  Triangles: TAffineVectorList = nil): Boolean;
var
...
function TVXOctree.GetTrianglesFromNodesIntersectingAABB(const ObjAABB: TAABB)
  : TAffineVectorList;
var
  AABB1: TAABB;
...
function TVXOctree.GetTrianglesFromNodesIntersectingCube(const ObjAABB: TAABB;
  const ObjToSelf, SelfToObj: TMatrix): TAffineVectorList;
var
  AABB1: TAABB;
  M1To2, M2To1: TMatrix;
