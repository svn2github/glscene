...
constructor TVXAsyncTimer.Create(AOwner: TComponent);
begin
...
destructor TVXAsyncTimer.Destroy;
begin
  Enabled := False;
...
procedure TVXAsyncTimer.DoTimer;
begin
  if Enabled and Assigned(FOnTimer) then
    FOnTimer(Self);
end;

procedure TVXAsyncTimer.SetEnabled(Value: Boolean);
begin
...
function TVXAsyncTimer.GetInterval: Word;
begin
  Result := TTimerThread(FTimerThread).FInterval;
end;

procedure TVXAsyncTimer.SetInterval(Value: Word);
begin
...
function TVXAsyncTimer.GetThreadPriority: TThreadPriority;
begin
  Result := FTimerThread.Priority;
end;

procedure TVXAsyncTimer.SetThreadPriority(Value: TThreadPriority);
begin
