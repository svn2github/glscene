...
type
  TVXVerletEdgeList = class;
  TVXVerletWorld = class;
  ...
  public
    constructor CreateOwned(const aOwner: TVXVerletWorld); virtual;
    ...
    { The TVerletNode inherits from TSpacePartitionLeaf, and it needs to
      know how to publish itself. The owner ( a TVXVerletWorld ) has a spatial
      partitioning object }
    procedure UpdateCachedAABBAndBSphere; override;
    { The VerletWorld that owns this verlet }
    property Owner: TVXVerletWorld read FOwner;
  ...
  private
    ...
    FTag: Integer;
  public
    constructor Create(const aOwner: TVXVerletWorld); virtual;
    destructor Destroy; override;
  ...
  TVXVerletDualConstraint = class(TVerletConstraint)
  private
  ...
  public
    constructor Create(const aOwner: TVXVerletWorld); override;
    destructor Destroy; override;
    procedure RemoveNode(const aNode: TVerletNode); override;
  ...
  TVXVerletEdge = class(TSpacePartitionLeaf)
  private
    FNodeA: TVerletNode;
    FNodeB: TVerletNode;
  public
    { The TVXVerletEdge inherits from TSpacePartitionLeaf, and it needs to
      know how to publish itself. The owner ( a TVXVerletWorld ) has a spatial
      partitioning object }
    procedure UpdateCachedAABBAndBSphere; override;
  ...
  TVXVerletEdgeList = class(TList)
  private
    function GetItems(i: Integer): TVXVerletEdge;
    procedure SetItems(i: Integer; const Value: TVXVerletEdge);
  public
    property Items[i: Integer]: TVXVerletEdge read GetItems write SetItems; default;
  end;

  TVXVerletGlobalConstraint = class(TVerletConstraint)
  private
  ...
  public
    ...
    procedure SatisfyConstraintForEdge(const aEdge: TVXVerletEdge; const iteration, maxIterations: Integer); virtual;
    property Location: TAffineVector read FLocation write SetLocation;
  ...
  TVXVerletGlobalFrictionConstraint = class(TVXVerletGlobalConstraint)
  private
  ...
  TVXVerletGlobalFrictionConstraintSP = class(TVXVerletGlobalFrictionConstraint)
  public
    procedure SatisfyConstraint(const iteration, maxIterations: Integer); override;
    procedure PerformSpaceQuery; virtual; abstract;
  end;

  TVXVerletGlobalFrictionConstraintSphere = class(TVXVerletGlobalFrictionConstraintSP)
  private
  ...
  TVXVerletGlobalFrictionConstraintBox = class(TVXVerletGlobalFrictionConstraintSP)
  private
    FCachedAABB: TAABB;
    procedure SetLocation(const Value: TAffineVector); override;
  public
  ...
  TVXVerletDualForce = class(TVXVerletForce)
  private
    FNodeA, FNodeB: TVerletNode;
  public
    procedure RemoveNode(const aNode: TVerletNode); override;
  ...
  TVerletGroupForce = class(TVXVerletForce)
  private
    FNodes: TVerletNodeList;
  public
    constructor Create(const aOwner: TVXVerletWorld); override;
  ...
  TVXVerletGlobalForce = class(TVXVerletForce)
  public
    procedure RemoveNode(const aNode: TVerletNode); override;
  ...
  TVXVerletForceList = class(TList)
  private
    function GetItems(i: Integer): TVXVerletForce;
    procedure SetItems(i: Integer; const Value: TVXVerletForce);
  public
    property Items[i: Integer]: TVXVerletForce read GetItems write SetItems; default;
  end;
  ...
  public
    ...
    function AddForce(const aForce: TVXVerletForce): Integer;
    procedure RemoveForce(const aForce: TVXVerletForce);
    ...
    property SolidEdges: TVXVerletEdgeList read FSolidEdges write FSolidEdges;
  ...
  TVFGravity = class(TVXVerletGlobalForce)
  private
  ...
  TVFAirResistance = class(TVXVerletGlobalForce)
  private
    FDragCoeff: Single;
  ...
  TVFSpring = class(TVXVerletDualForce)
  private
    FRestLength: Single;
    FStrength: Single;
    FDamping: Single;
  ...
  TVCFloor = class(TVXVerletGlobalFrictionConstraintSP)
  private
  ...
  TVCStick = class(TVXVerletDualConstraint)
  private
    FSlack: Single;
    FRestLength: Single;
  public
  ...
  TVCSlider = class(TVXVerletDualConstraint)
  private
  ...
  TVCSphere = class(TVXVerletGlobalFrictionConstraintSphere)
  private
    FRadius: Single;
  public
    function GetBSphere: TBSphere; override;
    procedure SatisfyConstraintForNode(const aNode: TVerletNode; const iteration, maxIterations: Integer); override;
    procedure SatisfyConstraintForEdge(const aEdge: TVXVerletEdge; const iteration, maxIterations: Integer); override;
    property Radius: Single read FRadius write FRadius;
  end;

  { Cylinder collision constraint. The cylinder is considered infinite by this constraint. }
  TVCCylinder = class(TVXVerletGlobalFrictionConstraint)
  private
    FAxis: TAffineVector;
  ...
  TVCCube = class(TVXVerletGlobalFrictionConstraintBox)
  private
  ...
  TVCCapsule = class(TVXVerletGlobalFrictionConstraintSphere)
  private
    FAxis: TAffineVector;
  ...
  public
    ...
    property VXBaseSceneObject : TVXBaseSceneObject read FBaseSceneObject write SetBaseSceneObject;
...
constructor TVerletNode.CreateOwned(const aOwner: TVXVerletWorld);
begin
...
constructor TVerletConstraint.Create(const aOwner: TVXVerletWorld);
begin
  inherited Create;
  if Assigned(aOwner) then
...
// ------------------
// ------------------ TVXVerletDualConstraint ------------------
// ------------------

procedure TVXVerletDualConstraint.RemoveNode(const aNode: TVerletNode);
begin
...
constructor TVerletGroupConstraint.Create(const aOwner: TVXVerletWorld);
begin
...
// ------------------
// ------------------ TVXVerletGlobalConstraint ------------------
// ------------------

procedure TVXVerletGlobalConstraint.AddKickbackForceAt(const Pos: TAffineVector; const Force: TAffineVector);
var
  dPos: TAffineVector;
begin
  // Sum forces
...
function TVXVerletGlobalConstraint.TranslateKickbackTorque(const TorqueCenter: TAffineVector): TAffineVector;
begin
...
procedure TVXVerletGlobalConstraint.BeforeIterations;
begin
  inherited;
...
procedure TVXVerletGlobalConstraint.RemoveNode(const aNode: TVerletNode);
begin
  // nothing to do here
end;

procedure TVXVerletGlobalConstraint.SetLocation(const Value: TAffineVector);
begin
  FLocation := Value;
end;

procedure TVXVerletGlobalConstraint.SatisfyConstraint(const iteration, maxIterations: Integer);
var
  i: Integer;
  node: TVerletNode;
begin
...
procedure TVXVerletGlobalConstraint.SatisfyConstraintForEdge(const aEdge: TVXVerletEdge;
...
// ------------------
// ------------------ TVXVerletGlobalFrictionConstraint ------------------
// ------------------

constructor TVXVerletGlobalFrictionConstraint.Create(const aOwner: TVXVerletWorld);
begin
  inherited;
...
// ------------------
// ------------------ TVXVerletGlobalFrictionConstraintSP ------------------
// ------------------

procedure TVXVerletGlobalFrictionConstraintSP.SatisfyConstraint(const iteration, maxIterations: Integer);
var
  i: Integer;
  node: TVerletNode;
  edge: TVXVerletEdge;
...
begin
  ...
  begin
    ...
    else if Leaf is TVXVerletEdge then
    begin
      if cctEdge in Owner.CollisionConstraintTypes then
      begin
        edge := Leaf as TVXVerletEdge;
...
constructor TVXVerletForce.Create(const aOwner: TVXVerletWorld);
begin
  inherited Create;
  if Assigned(aOwner) then
...
destructor TVXVerletForce.Destroy;
begin
  if Assigned(FOwner) then
...
constructor TVerletGroupForce.Create(const aOwner: TVXVerletWorld);
begin
  inherited Create(aOwner);
  FNodes := TVerletNodeList.Create;
end;
...
// ------------------
// ------------------ TVXVerletGlobalForce ------------------
// ------------------

procedure TVXVerletGlobalForce.RemoveNode(const aNode: TVerletNode);
begin
  // nothing to do here
end;

procedure TVXVerletGlobalForce.AddForce;
var
  i: Integer;
  node: TVerletNode;
begin
...
procedure TVXVerletDualForce.RemoveNode(const aNode: TVerletNode);
begin
  if FNodeA = aNode then
    FNodeA := nil;
  if FNodeB = aNode then
...
function TVXVerletForceList.GetItems(i: Integer): TVXVerletForce;
begin
  Result := Get(i);
end;

procedure TVXVerletForceList.SetItems(i: Integer; const Value: TVXVerletForce);
begin
  Put(i, Value);
end;
...
constructor TVXVerletWorld.Create;
begin
  inherited;
  FDrag := G_DRAG;
  FNodes := TVerletNodeList.Create;
  ...
  FForces := TVXVerletForceList.Create;
  FMaxDeltaTime := 0.02;
  FIterations := 3;
  FSolidEdges := TVXVerletEdgeList.Create;
  FCurrentStepCount := 0;
...
destructor TVXVerletWorld.Destroy;
var
  i: Integer;
begin
  // Delete all nodes
...
procedure TVXVerletWorld.AccumulateForces(const vpt: TVerletProgressTimes);
var
  i: Integer;
begin
...
function TVXVerletWorld.AddNode(const aNode: TVerletNode): Integer;
begin
...
procedure TVXVerletWorld.RemoveNode(const aNode: TVerletNode);
var
  i: Integer;
begin
...
function TVXVerletWorld.AddConstraint(const aConstraint: TVerletConstraint): Integer;
begin
...
procedure TVXVerletWorld.RemoveConstraint(const aConstraint: TVerletConstraint);
begin
...
function TVXVerletWorld.AddForce(const aForce: TVXVerletForce): Integer;
begin
...
procedure TVXVerletWorld.RemoveForce(const aForce: TVXVerletForce);
begin
...
procedure TVXVerletWorld.AddSolidEdge(const aNodeA, aNodeB: TVerletNode);
var
  VerletEdge: TVXVerletEdge;
begin
  VerletEdge := TVXVerletEdge.CreateEdgeOwned(aNodeA, aNodeB);
  SolidEdges.Add(VerletEdge);
end;

function TVXVerletWorld.FirstNode: TVerletNode;
begin
  Assert(FNodes.Count > 0, 'There are no nodes in the assembly!');
  Result := FNodes[0];
end;

function TVXVerletWorld.LastNode: TVerletNode;
begin
  Assert(FNodes.Count > 0, 'There are no nodes in the assembly!');
  Result := FNodes[FNodes.Count - 1];
end;

function TVXVerletWorld.CreateOwnedNode(const Location: TAffineVector; const aRadius: Single = 0; const aWeight: Single = 1)
  : TVerletNode;
begin
...
function TVXVerletWorld.CreateStick(const aNodeA, aNodeB: TVerletNode; const Slack: Single = 0): TVCStick;
begin
...
function TVXVerletWorld.CreateSpring(const aNodeA, aNodeB: TVerletNode; const aStrength, aDamping: Single;
...
function TVXVerletWorld.CreateSlider(const aNodeA, aNodeB: TVerletNode; const aSlideDirection: TAffineVector): TVCSlider;
begin
...
procedure TVXVerletWorld.Initialize;
var
  i: Integer;
begin
  for i := 0 to FNodes.Count - 1 do
...
function TVXVerletWorld.Progress(const deltaTime, newTime: Double): Integer;
var
  i: Integer;
  ticks: Integer;
  myDeltaTime: Single;
...
procedure TVXVerletWorld.DoUpdateSpacePartition;
var
  i: Integer;
begin
...
procedure TVXVerletWorld.SatisfyConstraints(const vpt: TVerletProgressTimes);
var
  i, j: Integer;
...
procedure TVXVerletWorld.Verlet(const vpt: TVerletProgressTimes);
var
  i: Integer;
begin
...
constructor TVFGravity.Create(const aOwner: TVXVerletWorld);
begin
  inherited;
  FGravity.X := 0;
  FGravity.Y := -9.81;
...
constructor TVFAirResistance.Create(const aOwner: TVXVerletWorld);
begin
  inherited;
...
constructor TVCFloor.Create(const aOwner: TVXVerletWorld);
begin
  inherited;
...
procedure TVCSphere.SatisfyConstraintForEdge(const aEdge: TVXVerletEdge; const iteration, maxIterations: Integer);
var
...
// BROKEN AND VERY SLOW!
procedure TVCCube.SatisfyConstraintForEdge(const aEdge: TVXVerletEdge; const iteration, maxIterations: Integer);
var
...
procedure TVCCapsule.SatisfyConstraintForEdge(const aEdge: TVXVerletEdge; const iteration, maxIterations: Integer);
var
...
constructor TVXVerletEdge.CreateEdgeOwned(const aNodeA, aNodeB: TVerletNode);
begin
  FNodeA := aNodeA;
  FNodeB := aNodeB;
...
procedure TVXVerletEdge.UpdateCachedAABBAndBSphere;
begin
...
function TVXVerletEdgeList.GetItems(i: Integer): TVXVerletEdge;
begin
  Result := Get(i);
end;

procedure TVXVerletEdgeList.SetItems(i: Integer; const Value: TVXVerletEdge);
begin
  Put(i, Value);
end;
...
procedure TVXVerletWorld.CreateOctree(const OctreeMin, OctreeMax: TAffineVector; const LeafThreshold, MaxTreeDepth: Integer);
var
...
procedure TVXVerletWorld.PauseInertia(const IterationSteps: Integer);
begin
...
// ------------------
// ------------------ TVXVerletGlobalFrictionConstraintBox ------------------
// ------------------

procedure TVXVerletGlobalFrictionConstraintBox.PerformSpaceQuery;
begin
  Owner.SpacePartition.QueryAABB(FCachedAABB);
end;

procedure TVXVerletGlobalFrictionConstraintBox.SetLocation(const Value: TAffineVector);
begin
  inherited;
...
procedure TVXVerletGlobalFrictionConstraintBox.UpdateCachedAABB;
begin
  FCachedAABB := GetAABB;
end;

{ TVXVerletGlobalFrictionConstraintSphere }

procedure TVXVerletGlobalFrictionConstraintSphere.PerformSpaceQuery;
begin
  Owner.SpacePartition.QueryBSphere(FCachedBSphere);
end;

procedure TVXVerletGlobalFrictionConstraintSphere.SetLocation(const Value: TAffineVector);
begin
  inherited;
  UpdateCachedBSphere;
end;

procedure TVXVerletGlobalFrictionConstraintSphere.UpdateCachedBSphere;
begin
  FCachedBSphere := GetBSphere;
end;

constructor TVXVerletGlobalConstraint.Create(const aOwner: TVXVerletWorld);
begin
  inherited;
  if Assigned(aOwner) then
...
destructor TVXVerletGlobalConstraint.Destroy;
begin
