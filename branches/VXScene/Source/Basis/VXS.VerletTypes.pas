...
{ 
   ...
   This unit is generic, specific sub-classes are in VKVerletClasses. 
...
type
   ...
   TVXVerletGlobalConstraint = class (TVerletConstraint)
      ...
      public
         ...
         procedure SatisfyConstraintForEdge(const aEdge : TVXVerletEdge;
...
function TVXVerletGlobalConstraint.TranslateKickbackTorque(
...
//
procedure TVXVerletGlobalConstraint.SatisfyConstraintForEdge(
  const aEdge: TVXVerletEdge; const iteration, maxIterations: Integer);
begin
...
// SatisfyConstraint
//
procedure TVXVerletGlobalFrictionConstraintSP.SatisfyConstraint(
...
begin
   ...
   for i:=0 to SP.QueryResult.Count-1 do begin
      ...
      end else if Leaf is TVXVerletEdge then begin
...
function TVXVerletWorld.CreateOwnedNode(const location : TAffineVector;
...
// CreateSpring
//
function TVXVerletWorld.CreateSpring(const aNodeA, aNodeB : TVerletNode;
              const aStrength, aDamping : Single; const aSlack : Single = 0) : TVFSpring;
begin
...
// CreateSlider
//
function TVXVerletWorld.CreateSlider(const aNodeA, aNodeB : TVerletNode;
                                   const aSlideDirection : TAffineVector) : TVCSlider;
begin
...
procedure TVCSphere.SatisfyConstraintForEdge(const aEdge: TVXVerletEdge;
  const iteration, maxIterations: Integer);
var
...
// BROKEN AND VERY SLOW!
procedure TVCCube.SatisfyConstraintForEdge(const aEdge: TVXVerletEdge;
  const iteration, maxIterations: Integer);
var
...
procedure TVCCapsule.SatisfyConstraintForEdge(const aEdge: TVXVerletEdge;
  const iteration, maxIterations: Integer);
var
...
procedure TVXVerletWorld.CreateOctree(const OctreeMin,
...
procedure TVXVerletGlobalFrictionConstraintBox.SetLocation(
...
procedure TVXVerletGlobalFrictionConstraintSphere.SetLocation(
  const Value: TAffineVector);
begin
  inherited;
  UpdateCachedBSphere;
