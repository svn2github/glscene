...
type
  ...
  TVXSeparateTextureWrap = (twRepeat, twClampToEdge, twClampToBorder,
  ...
  TVXTextureCompareMode = (tcmNone, tcmCompareRtoTexture);

  { Filtering quality }
  TVXTextureFilteringQuality = (tfIsotropic, tfAnisotropic);
  ...
  TVXTextureSwizzle = (tswRed, tswGreen, tswBlue, tswAlpha, tswZero, tswOne);
  TSwizzleVector = array[0..3] of TVXTextureSwizzle;
  ...
  { Texture compression option.
     If OpenVX supports it, this will activate a compressed texture format:
      ...
      . }
  TVXInternalCompression = (tcDefault, tcNone, tcStandard, tcHighQuality,
    tcHighSpeed);

var
  vDefaultTextureFormat: TVXInternalFormat = tfRGBA8;
  vDefaultTextureCompression: TVXInternalCompression = tcNone;

const
  cDefaultSwizzleVector: TSwizzleVector = (tswRed, tswGreen, tswBlue, tswAlpha);

{Give a OpenVX texture format from texture format. }
function InternalFormatToOpenVXFormat(intFormat: TVXInternalFormat): Cardinal;
{Give a VXScene texture format from OpenVX texture format. }
function OpenVXFormatToInternalFormat(glFormat: Cardinal): TVXInternalFormat;
{Give a pixel size in bytes from texture format or data format. }
function GetTextureElementSize(intFormat: TVXInternalFormat): Integer; overload;
function GetTextureElementSize(colorFormat: Cardinal; dataType: Cardinal):
  Integer; overload;
{Give compatible OpenVX image format and data type. }
procedure FindCompatibleDataFormat(intFormat: TVXInternalFormat; out dFormat:
...
function CompressedInternalFormatToOpenVX(intFormat: TVXInternalFormat):
  Integer;
{True if texture target supported. }
function IsTargetSupported(glTarget: Cardinal): Boolean; overload;
function IsTargetSupported(target: TVXTextureTarget): Boolean; overload;
{True if texture format is supported by hardware or software. }
function IsFormatSupported(intFormat: TVXInternalFormat): Boolean;
{True if texture format is float. }
function IsFloatFormat(intFormat: TVXInternalFormat): Boolean; overload;
function IsFloatFormat(glFormat: Cardinal): Boolean; overload;
{True if depth texture. }
function IsDepthFormat(intFormat: TVXInternalFormat): boolean; overload;
function IsDepthFormat(glFormat: Cardinal): Boolean; overload;
{True if texture compressed. }
function IsCompressedFormat(intFormat: TVXInternalFormat): Boolean; overload;
function IsCompressedFormat(glFormat: Cardinal): Boolean; overload;
{Give generic compressed OpenVX texture format. }
function GetGenericCompressedFormat(const intFormat: TVXInternalFormat;
  const colorFormat: Cardinal; out internalFormat: Cardinal): Boolean;
{Give uncompressed texture format and OpenVX color format. }
function GetUncompressedFormat(const intFormat: TVXInternalFormat;
  out internalFormat: TVXInternalFormat; out colorFormat: Cardinal): Boolean;

function DecodeTextureTarget(const TextureTarget: TVXTextureTarget): Cardinal;
function EncodeTextureTarget(const glTarget: Cardinal): TVXTextureTarget;
function IsTargetSupportMipmap(const TextureTarget: TVXTextureTarget): Boolean; overload;
...
const
  ...
  cTextureFormatToOpenVX: array[low(TVXInternalFormat)..high(TVXInternalFormat)] of TFormatDesc =
  (
...
function GetTextureElementSize(intFormat: TVXInternalFormat): Integer;
begin
...
function IsTargetSupported(target: TVXTextureTarget): Boolean;
begin
...
function IsFloatFormat(intFormat: TVXInternalFormat): boolean;
begin
...
function IsDepthFormat(intFormat: TVXInternalFormat): boolean;
begin
...
function IsCompressedFormat(intFormat: TVXInternalFormat): boolean;
begin
...
function DecodeTextureTarget(const TextureTarget: TVXTextureTarget): Cardinal;
const
  cTargetToEnum: array[TVXTextureTarget] of Cardinal =
  (
    0,
    GL_TEXTURE_1D,
    GL_TEXTURE_2D,
...
function IsTargetSupportMipmap(const TextureTarget: TVXTextureTarget): Boolean;
begin
