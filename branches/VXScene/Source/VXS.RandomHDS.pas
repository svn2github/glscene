...
unit VXS.RandomHDS;
{
  ...
  that can be used with TVXTerrainRenderer. General properties are defined in
  TVXBaseRandomHDS, but the main object is TVXCustomRandomHDS,
  which defines all the basic functionalities; however, it is an abstract class.
  So far, it has only one descendent, TVXFractalHDS, which implements the fractal
  ...
  particular to the algorithm added (see TVXFractalHDS implementation). The
  ...
  infinite landscape through TVXTiledRndLandscape. The function of this class
  is to maintain a list of landscapes (called thereafter "landtiles"), to build
  and free them when needed.
  The TVXFractalArchipelago is an example of such a landscape generating an
  infinite landscape made of fractal islands.

  Although this structure may appear complex, the programmer just need to
  instanciate a TVXFractalArchipelago and to set its properties to get it running
  ...
  create this component manually in your code and link it to a TVXTerrainRenderer.
...
type
  ...
  TLandTileInfo = record
    ...
    State: TVXHeightDataState; // Preparation status of the landtile
  end;
  ...
  TVXBaseRandomHDS = class;

  // Function type to use for topography-based texture
  TOnDrawTexture = function(const Sender: TVXBaseRandomHDS; x, y: integer; z: double; Normal: TVector): TColorVector of object;
  ...
  { This class introduces all the basic properties of random landscape. No method
    ...
    - TVXCustomRandomLandscape: one tile landscape (cyclic or not)
    - TVXTiledRndLandscape: "infinite" landscapes (grids of TVXCustomRandomLandscape) }
  TVXBaseRandomHDS = class(TVXHeightDataSource)
  private
    FSteps: TSteps;
    FLandCover: boolean;
  ...
  protected
    ...
    FTerrainRenderer: TVXTerrainRenderer;
    ...
    function OnDrawTextureDefault(const Sender: TVXBaseRandomHDS; x, y: integer; z: double; Normal: TVector): TColorVector;
    ...
    procedure SetTerrainRenderer(const Value: TVXTerrainRenderer); virtual; abstract;
  ...
  published
    ...
    property TerrainRenderer: TVXTerrainRenderer read FTerrainRenderer write SetTerrainRenderer;
  ...
  TVXCustomRandomHDS = class(TVXBaseRandomHDS)
  private
    FSlave: boolean;
    FMaxHeight: single;
    FMinHeight: single;
  ...
  protected
    procedure SetTerrainRenderer(const Value: TVXTerrainRenderer); override;
    ...
    // TVXTerrainRenderer event handler
    procedure GetTerrainBounds(var l, t, r, b: single);
    // This procedure MUST be called by the descendent of TVXBaseRandomHDS
  ...
  public
    ...
    { x and y are range-checked and constrained into the array. This slows down
      ...
      (this is a protected field, therefore only accessible from TVXFractalHDS
      descendents. }
    ...
    { Property used by TVXTiledRndLandscape to know where the landtile is located
  ...
  TVXFractalHDS = class(TVXCustomRandomHDS)
  private
    FAmplitude: integer;
    FDepth: integer;
    FRoughness: single;
  ...
  (* TMapOfLandscapes	:array of array of TVXBaseRandomHDS; *)

  TLandTile = TVXCustomRandomHDS;
  ...
  TVXTiledRndLandscape = class(TVXBaseRandomHDS)
  private
    ...
    procedure fDefaultOnCreateDefaultTile(HeightData: TVXHeightData);
  ...
  protected
    ...
    procedure SetCyclic(const Value: boolean); override;
    { This procedure MUST be called by the descendent of TVXRandomArchipelago }
    procedure SetSize(const aSize: integer);
  ...
  published
    ...
    property TerrainRenderer: TVXTerrainRenderer read FTerrainRenderer
      write SetTerrainRenderer;
  end;

  TVXFractalArchipelago = class(TVXTiledRndLandscape)
  private
    FDepth: integer;
    ...
    procedure FPostRenderSeaStatic(var rci: TVXRenderContextInfo; var HeightDatas: TList);
    { Sea with waves.
      Borrowed from Eric's Archipelago GLScene advanced demo }
    procedure FPostRenderSeaDynamic(var rci: TVXRenderContextInfo; var HeightDatas: TList);
  ...
  protected
    ...
    procedure fOnCreateDefaultTile(heightData: TVXHeightData);
  public
...
// TVXBaseRandomHDS
//
constructor TVXBaseRandomHDS.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TVXBaseRandomHDS.Destroy;
begin
  inherited;
end;

function TVXBaseRandomHDS.GetSeaLevel: single;
begin
  Result := FSeaLevel / VSF; // factor used in tTerrainRender
end;

function TVXBaseRandomHDS.GetSeaTransparency: single;
begin
  Result := FSeaTransparency / VSF; // factor used in tTerrainRender
end;

function TVXBaseRandomHDS.GetErosionByRain: TRainErosion;
begin
  Result := FErosionByRain;
end;

function TVXBaseRandomHDS.GetLandTileInfo: TLandTileInfo;
begin
  Result := FLandTileInfo;
end;

function TVXBaseRandomHDS.OnDrawTextureDefault(const Sender: TVXBaseRandomHDS; x, y: integer; z: double; Normal: TVector)
  : TColorVector;
begin
...
procedure TVXBaseRandomHDS.SetAmbientLight(const Value: single);
begin
  FAmbientLight := Value;
end;

procedure TVXBaseRandomHDS.SetErosionByFraction(const Value: TFractionErosion);
begin
  FErosionByFraction := Value;
end;

procedure TVXBaseRandomHDS.SetErosionByLife(const Value: TLifeErosion);
begin
  FErosionByLife := Value;
end;

procedure TVXBaseRandomHDS.SetErosionByRain(const Value: TRainErosion);
begin
  FErosionByRain := Value;
end;

procedure TVXBaseRandomHDS.SetErosionBySea(const Value: TSeaErosion);
begin
  FErosionBySea := Value;
end;

procedure TVXBaseRandomHDS.SetLandCover(const Value: boolean);
begin
  FLandCover := Value;
end;

procedure TVXBaseRandomHDS.SetLandTileInfo(const Value: TLandTileInfo);
begin
  FLandTileInfo := Value;
end;

procedure TVXBaseRandomHDS.SetLightColor(const Value: TColorVector);
begin
  FLightColor := Value;
end;

procedure TVXBaseRandomHDS.SetLightDirection(const Value: TVector);
var
  v: TVector;
begin
  v := Value;
...
procedure TVXBaseRandomHDS.SetLighting(const Value: boolean);
begin
  FLighting := Value;
end;

procedure TVXBaseRandomHDS.SetLightSmoothing(const Value: boolean);
begin
  FLightSmoothing := Value;
end;

procedure TVXBaseRandomHDS.SetMaterialName(const Value: string);
begin
  FMaterialName := Value;
end;

procedure TVXBaseRandomHDS.SetOnDrawTexture(const Value: TOnDrawTexture);
begin
  if @Value <> nil then
...
procedure TVXBaseRandomHDS.SetPrimerLandscape(const Value: boolean);
begin
  FPrimerLandscape := Value;
end;

procedure TVXBaseRandomHDS.SetSea(const Value: boolean);
begin
  FSea := Value;
end;

procedure TVXBaseRandomHDS.SetSeaLevel(const Value: single);
begin
  FSeaLevel := Value * VSF; // factor used in tTerrainRender
end;

procedure TVXBaseRandomHDS.SetSeaTransparency(const Value: single);
begin
  FSeaTransparency := Value * VSF; // factor used in tTerrainRender
end;

procedure TVXBaseRandomHDS.SetSeed(const Value: integer);
begin
  FSeed := Value;
end;

procedure TVXBaseRandomHDS.SetShadows(const Value: boolean);
begin
  FShadows := Value;
end;

procedure TVXBaseRandomHDS.SetSteps(const Value: TSteps);
begin
  FSteps := Value;
end;

procedure TVXBaseRandomHDS.SetTextureScale(const Value: integer);
begin
  FTextureScale := Value;
end;
...
// TVXCustomRandomHDS
//
procedure TVXCustomRandomHDS.BoundaryClamp(var x, y: single);
begin
  ClampValue(x, 0, FSize);
  ClampValue(y, 0, FSize);
end;

procedure TVXCustomRandomHDS.BoundaryClamp(var x, y: integer);
begin
  if x < 0 then
    x := 0
...
function TVXCustomRandomHDS.BoundaryX: integer;
begin
  Result := Round(FSize * Scale.x);
end;

function TVXCustomRandomHDS.BoundaryZ: integer;
begin
  Result := Round(FSize * Scale.z);
end;

procedure TVXCustomRandomHDS.BuildLandscape;
begin
...
procedure TVXCustomRandomHDS.BuildLightMap;
var
  i, j, k, m, n: integer;
  x, y: single;
  t: single;
...
procedure TVXCustomRandomHDS.BuildLightMap(const aLightDirection: TVector);
begin
  FLightDirection := aLightDirection;
  BuildLightMap;
end;

procedure TVXCustomRandomHDS.BuildNormals;
var
  i, j: integer;
  z0: single;
  v1, v2: TVector;
...
procedure TVXCustomRandomHDS.BuildTexture;
type
...
(* procedure TVXCustomRandomHDS.BuildTexture2;
var
  Bmp	:tBitmap;
  Mat	:TVXLibMaterial;
  x,y	:integer;
...
procedure TVXCustomRandomHDS.ClearHeightField;
begin
  PrimerNull(FHeight);
end;

procedure TVXCustomRandomHDS.ClearLightMap;
var
  x, y: integer;
begin
  SetLength(FLightMap, (FSize + 1) * TextureScale, (FSize + 1) * TextureScale);
...
procedure TVXCustomRandomHDS.ConstrainCoordinates(var x, y: integer);
begin
  FIntegerConstrain(x, y);
end;

procedure TVXCustomRandomHDS.ConstrainCoordinates(var x, y: single);
begin
  FSingleConstrain(x, y);
end;

constructor TVXCustomRandomHDS.Create(AOwner: TComponent);
begin
  inherited;
  FLandCover := True;
...
procedure TVXCustomRandomHDS.CyclicClamp(var x, y: single);
var
  ix, iy: integer;
  sx, sy: single;
begin
...
procedure TVXCustomRandomHDS.CyclicClamp(var x, y: integer);
begin
  x := (FSize + x) mod FSize;
  y := (FSize + y) mod FSize;
end;

destructor TVXCustomRandomHDS.Destroy;
var
  x, y: integer;
  Mat: TVXLibMaterial;
begin
  ...
  if (FSlave) and (Owner <> nil) then
    with LandTileInfo do
      TVXTiledRndLandscape(Owner).MarkDirty(x * FSize, z * FSize, (x + 1) * FSize - 1, (z + 1) * FSize - 1);
  inherited;
end;

procedure TVXCustomRandomHDS.DoCyclicBoundaries;
var
  i: integer;
begin
  for i := 0 to FSize do
...
procedure TVXCustomRandomHDS.DoErosionByFraction;
begin

end;

procedure TVXCustomRandomHDS.DoErosionByLife;
var
  x, y, i: integer;
  z, z1: single;
begin
...
procedure TVXCustomRandomHDS.DoErosionByRain;
...
(*
  ...
  procedure TVXBaseRandomHDS.DoErosionByRain(const Intensity: single);
  const
...
procedure TVXCustomRandomHDS.DoErosionBySea;
var
  i, j: integer;
begin
  for i := 0 to FSize do
...
procedure TVXCustomRandomHDS.DoSea;
var
  i, j: integer;
begin
  for i := 0 to FSize do
...
procedure TVXCustomRandomHDS.DoSteps;
var
  i, j: integer;
  Stp: single;
begin
...
function TVXCustomRandomHDS.GetHeight(x, y: integer): single;
begin
  FIntegerConstrain(x, y);
  Result := FHeight[x, y];
end;

procedure TVXCustomRandomHDS.GetTerrainBounds(var l, t, r, b: single);
begin
  l := 0;
  b := 0;
  t := FSize;
...
function TVXCustomRandomHDS.Interpolate(x, y: single): single;
...
function TVXCustomRandomHDS.PointInMap(const x, y: single): boolean;
begin
  Result := (x >= 0) and (x <= FSize) and (y >= 0) and (y <= FSize);
end;

function TVXCustomRandomHDS.Normal(const Position: TVector): TVector;
var
  x, y: integer;
begin
  if (FNormal <> nil) then
...
function TVXCustomRandomHDS.PointInMap(const x, y: integer): boolean;
begin
  Result := (x >= 0) and (x <= FSize) and (y >= 0) and (y <= FSize);
end;

function TVXCustomRandomHDS.Scale: TVXCoordinates;
begin
  try
...
procedure TVXCustomRandomHDS.SetCyclic(const Value: boolean);
begin
  FCyclic := Value;
  if FCyclic then
  begin
...
procedure TVXCustomRandomHDS.SetHeight(x, y: integer; const Value: single);
begin
  FIntegerConstrain(x, y);
  FHeight[x, y] := Value;
end;

procedure TVXCustomRandomHDS.SetSize(const aSize: integer);
var
  Tile: integer;
begin
  FSize := aSize;
...
procedure TVXCustomRandomHDS.SetTerrainRenderer(const Value: TVXTerrainRenderer);
begin
...
function TVXCustomRandomHDS.StandardisedHeight(const x, y: integer): single;
begin
  Result := (Heights[x, y] - FMinHeight) / FRangeHeight * 1000;
end;

procedure TVXCustomRandomHDS.StartPreparingData(heightData: TVXHeightData);
var
  x, y, x0, y0: integer;
...
function TVXCustomRandomHDS.XMoveBoundary: single;
begin
  Result := FSize * Scale.x * 0.95;
end;

function TVXCustomRandomHDS.ZMoveBoundary: single;
begin
  Result := FSize * Scale.y * 0.95;
end;

procedure TVXCustomRandomHDS.SetKeepNormals(const Value: boolean);
begin
  FKeepNormals := Value;
end;
...
procedure TVXFractalHDS.BuildHeightField(const aDepth, aSeed, aAmplitude: integer);
begin
  fDepth := aDepth;
  FSeed := aSeed;
...
procedure TVXFractalHDS.BuildHeightField;
begin
...
constructor TVXFractalHDS.Create(AOwner: TComponent);
begin
  inherited;
  Depth := 4;
  FSea := True;
...
procedure TVXFractalHDS.SetAmplitude(const Value: integer);
begin
  fAmplitude := Value;
...
procedure TVXFractalHDS.SetDepth(const Value: integer);
begin
  fDepth := Value;
  SetSize(Round(IntPower(2, fDepth)));
end;

procedure TVXFractalHDS.SetRoughness(const Value: single);
begin
  fRoughness := Value;
end;
...
procedure TVXTiledRndLandscape.ApplyLighting(var aLandTile: TLandTile);
begin
  with aLandTile do
  begin
...
procedure TVXTiledRndLandscape.ApplyTexture(var aLandTile: TLandTile);
begin
  with aLandTile do
  begin
...
procedure TVXTiledRndLandscape.ApplyTopography(var aLandTile: TLandTile);
begin
  with aLandTile do
  begin
...
procedure TVXTiledRndLandscape.BoundaryClamp(var x, z: single);
begin
  ClampValue(x, 0, FExtentX * fLandTileSize);
  ClampValue(z, 0, FExtentZ * fLandTileSize);
end;

procedure TVXTiledRndLandscape.BoundaryClamp(var x, z: integer);
begin
  if x < 0 then
    x := 0
...
procedure TVXTiledRndLandscape.CameraPosition(var TileX, TileZ: integer);
begin
  FindLandTile(-Camera.Position.x, Camera.Position.z, TileX, TileZ);
end;

procedure TVXTiledRndLandscape.CleanUp;
var
  i: integer;
begin
...
procedure TVXTiledRndLandscape.ComputeLandTile(const aX, aZ: integer; var NewLandTile: TLandTile);
var
  sx, sz: string;
begin
...
procedure TVXTiledRndLandscape.ConstrainCoordinates(var x, z: single);
begin
  FSingleConstrain(x, z);
end;

procedure TVXTiledRndLandscape.ConstrainCoordinates(var x, z: integer);
begin
  FIntegerConstrain(x, z);
end;

constructor TVXTiledRndLandscape.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TVXTiledRndLandscape.CyclicClamp(var x, z: integer);
begin
  exit;
...
procedure TVXTiledRndLandscape.CyclicClamp(var x, z: single);
var
  ix, iz: integer;
  sx, sz: single;
begin
...
destructor TVXTiledRndLandscape.Destroy;
begin
  fLandTiles.Free;
  inherited;
end;

function TVXTiledRndLandscape.fDefaultIsDefaultTile(x, z: integer): boolean;
begin
  InitializeRandomGenerator(LandTileSeed(x, z));
  Result := (random >= FLandTileDensity);
end;

procedure TVXTiledRndLandscape.fDefaultOnCreateDefaultTile(heightData: TVXHeightData);
begin
  heightData.DataState := hdsNone;
  // raise EAccessViolation.Create('No DefaultStartPreparingDefaultTile procedure supplied.');
end;

procedure TVXTiledRndLandscape.FindLandTile(const x, z: single; var TileX, TileZ: integer);
begin
...
function TVXTiledRndLandscape.fSortLandscapes(Item1, Item2: Pointer): integer;
...
function TVXTiledRndLandscape.GetTask: string;
begin
...
function TVXTiledRndLandscape.GetTaskProgress: integer;
begin
  if fComputedLandTile <> nil then
...
procedure TVXTiledRndLandscape.GetTerrainBounds(var l, t, r, b: single);
begin
  l := 0;
  b := 0;
...
procedure TVXTiledRndLandscape.Initialize(const aX, aZ: single);
var
  cx, cz: integer;
...
function TVXTiledRndLandscape.LandTileSeed(x, z: integer): integer;
{ Generates a unique seed from the tile coordinates }
begin
  Result := fBaseSeed + z * ExtentX + x;
end;

function TVXTiledRndLandscape.LandtileCount: integer;
begin
  Result := fLandTiles.Count;
end;

{ procedure TVXTiledRndLandscape.PrepareLandTileData(HeightData: tHeightData;
  LandTile: tLandTile);
  var
  x,y,x0,y0	:integer;
...
procedure TVXTiledRndLandscape.SetCamera(const Value: TVXCamera);
begin
  FCamera := Value;
end;

procedure TVXTiledRndLandscape.SetCyclic(const Value: boolean);
begin
  FCyclic := Value;
  if FCyclic then
  begin
...
procedure TVXTiledRndLandscape.SetExtentX(const Value: integer);
begin
  FExtentX := Value;
  FExtentXhalf := FExtentX div 2;
end;

procedure TVXTiledRndLandscape.SetExtentZ(const Value: integer);
begin
  FExtentZ := Value;
  FExtentZhalf := FExtentZ div 2;
end;

procedure TVXTiledRndLandscape.SetGenerationRadius(const Value: integer);
var
  x, z, i: integer;
begin
...
procedure TVXTiledRndLandscape.SetIsDefaultTile(const Value: TIsDefaultTile);
begin
  FIsDefaultTile := Value;
end;

procedure TVXTiledRndLandscape.SetLandTileCapacity(const Value: integer);
begin
  FLandTileCapacity := Value;
end;

procedure TVXTiledRndLandscape.SetLandTileDensity(const Value: single);
begin
  FLandTileDensity := Value;
end;

procedure TVXTiledRndLandscape.SetOnCreateDefaultTile(const Value: TStartPreparingDataEvent);
begin
  fOnCreateDefaultTile := Value;
end;

procedure TVXTiledRndLandscape.SetOnCreateLandTile(const Value: TOnCreateLandTile);
begin
  fOnCreateLandTile := Value;
end;

procedure TVXTiledRndLandscape.SetSeed(const Value: integer);
begin
  FSeed := Value;
  InitializeRandomGenerator(FSeed);
end;

procedure TVXTiledRndLandscape.SetSize(const aSize: integer);
begin
  fLandTileSize := aSize;
end;

procedure TVXTiledRndLandscape.SetTerrainRenderer(const Value: TVXTerrainRenderer);
begin
  FTerrainRenderer := Value;
  FTerrainRenderer.HeightDataSource := Self;
end;

procedure TVXTiledRndLandscape.StartPreparingData(heightData: TVXHeightData);
var
  i: integer;
  tx, tz: integer;
begin
...
function TVXTiledRndLandscape.TileDistance(const x1, z1, x2, z2: integer): single;
begin
  Result := sqrt(sqr(FExtentXhalf - abs(abs(x1 - x2) - FExtentXhalf)) + sqr(FExtentZhalf - abs(abs(z1 - z2) - FExtentZhalf)));
end;

function TVXTiledRndLandscape.TileDistanceSquared(const x1, z1, x2, z2: integer): integer;
begin
  Result := sqr(FExtentXhalf - abs(abs(x1 - x2) - FExtentXhalf)) + sqr(FExtentZhalf - abs(abs(z1 - z2) - FExtentZhalf));
end;

procedure TVXTiledRndLandscape.Update;
var
  i, j, maxi: integer;
  maxd, d: integer;
  cx, cz: integer;
...
function TVXTiledRndLandscape.XMoveBoundary: single;
begin
  Result := ExtentX * LandTileSize * 0.95;
end;

function TVXTiledRndLandscape.ZMoveBoundary: single;
begin
  Result := ExtentZ * LandTileSize * 0.95;
end;

//
// TVXFractalArchipelago
//
procedure TVXFractalArchipelago.ComputeLandTile(const aX, aZ: integer; var NewLandTile: TLandTile);
begin
  NewLandTile := TVXFractalHDS.Create(Self);
  NewLandTile.FSlave := True;
  inherited ComputeLandTile(aX, aZ, NewLandTile);
end;

constructor TVXFractalArchipelago.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TVXFractalArchipelago.fOnCreateDefaultTile(heightData: TVXHeightData);
var
  x, y: integer;
...
procedure TVXFractalArchipelago.fOnCreateLandTile(aX, aZ, aSeed: integer; var aLandscape: TLandTile);
begin
  InitializeRandomGenerator(aSeed);
  with TVXFractalHDS(aLandscape) do
  begin
    { Initialize the tile }
    Seed := random(MaxInt);
    Depth := Self.fDepth;
...
procedure TVXFractalArchipelago.FPostRenderSeaDynamic(var rci: TVXRenderContextInfo; var HeightDatas: TList);
// Code borrowed from Eric's Archipelago GLScene advanced demo
var
  i, x, y, s, s2: integer;
  t: single;
...
begin
  ...
  repeat
    // if not WasAboveWater then InverTVXFrontFace;
    ...
    begin
      hd := TVXHeightData(HeightDatas.List[i]);
...
procedure TVXFractalArchipelago.FPostRenderSeaStatic(var rci: TVXRenderContextInfo; var HeightDatas: TList);
var
  i, x, y, s, s2: integer;
  hd: TVXHeightData;
  t: single;
...
function TVXFractalArchipelago.GetIslandDensity: single;
begin
  Result := FLandTileDensity;
end;

procedure TVXFractalArchipelago.SetAmplitudeMax(const Value: integer);
begin
  FAmplitudeMax := Value;
end;

procedure TVXFractalArchipelago.SetAmplitudeMin(const Value: integer);
begin
  FAmplitudeMin := Value;
end;

procedure TVXFractalArchipelago.SetDepth(const Value: integer);
begin
  fDepth := Value;
  SetSize(Round(IntPower(2, fDepth)));
end;

procedure TVXFractalArchipelago.SetIslandDensity(const Value: single);
begin
  LandTileDensity := Value;
end;

procedure TVXFractalArchipelago.SetRoughnessMax(const Value: single);
begin
  FRoughnessMax := Value;
end;

procedure TVXFractalArchipelago.SetRoughnessMin(const Value: single);
begin
  FRoughnessMin := Value;
end;

procedure TVXFractalArchipelago.SetSeaDynamic(const Value: boolean);
begin
  FSeaDynamic := Value;
  if FSeaDynamic then
...
procedure TVXFractalArchipelago.SetSeaMaterialName(const Value: string);
begin
  FSeaMaterialName := Value;
end;

procedure TVXFractalArchipelago.SetTerrainRenderer(const Value: TVXTerrainRenderer);
begin
  inherited;
  SeaDynamic := FSeaDynamic; // Called to hook the PostRender event handler
end;

procedure TVXFractalArchipelago.SetWaveAmplitude(const Value: single);
begin
  FWaveAmplitude := Value;
end;

procedure TVXFractalArchipelago.SetWaveSpeed(const Value: single);
begin
  FWaveSpeed := Value;
end;
