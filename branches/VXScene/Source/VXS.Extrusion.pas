...
type
  ...
  public
    ...
    property Items[index: Integer]: TVXPipeNode read GetItems
...
begin
        ...
        case TexCoordMode of
          ...
          ptcmManual:
            begin
              k := Trunc(T);
              if k < Nodes.Count - 1 then
                rows[curRow].textcoordT := Lerp(TVXPipeNode(Nodes[k]).TexCoordT,
                  TVXPipeNode(Nodes[k + 1]).TexCoordT, Frac(T))
              else
    ...
    begin
      ...
      begin
        ...
        CalculateRow(@rows[curRow], PAffineVector(@Nodes[i].AsVector)^,
          Nodes.Vector(i), TVXPipeNode(Nodes[i]).RadiusFactor);
  ...
  begin
    ...
    CalculateRow(@rows[0], PAffineVector(@Nodes[i].AsVector)^, normal,
      TVXPipeNode(Nodes[i]).RadiusFactor);
