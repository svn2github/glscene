...
type
  TVXSpriteAnimFrame = class;
  TVXSpriteAnimFrameList = class;
  TVXSpriteAnimation = class;
  TVXSpriteAnimationList = class;
  ...
  protected
    ...
    procedure DoChanged;
  public
    constructor Create(Animation: TVXSpriteAnimation);
    property Owner: TVXSpriteAnimation read FOwner;
  published
  ...
  private
    ...
    FFrameHeight,
    FInterval: Integer;
    FFrames: TVXSpriteAnimFrameList;
    FLibMaterialName: TVXLibMaterialName;
    FLibMaterialCached: TVXLibMaterial;
    FDimensions: TVXSpriteFrameDimensions;
    FMargins: TVXSpriteAnimMargins;
    procedure DoChanged;
  protected
    ...
    procedure WriteToFiler(writer: TWriter); override;
    procedure ReadFromFiler(reader: TReader); override;
    procedure SetDimensions(const Value: TVXSpriteFrameDimensions);
    procedure SetLibMaterialName(const val: TVXLibMaterialName);
    function GetLibMaterialCached: TVXLibMaterial;
  ...
  public
    ...
    class function FriendlyName: string; override;
    class function FriendlyDescription: string; override;
    property LibMaterialCached: TVXLibMaterial read GetLibMaterialCached;
  published
    ...
    { The name of the lib material the sprites associated material library
       for this animation. }
    property LibMaterialName: TVXLibMaterialName read FLibMaterialName write
      SetLibMaterialName;
    { Manual dimension animation frames. Stores the offsets and dimensions
       for each frame in the animation. }
    property Frames: TVXSpriteAnimFrameList read FFrames;
    // Automatic or manual texture coordinate generation.
    property Dimensions: TVXSpriteFrameDimensions read FDimensions write
      SetDimensions;
  ...
  private
    ...
    FMirrorU,
      FMirrorV: Boolean;
    FAnimationMode: TVXSpriteAnimationMode;
  ...
  protected
    ...
    procedure SetAnimationMode(const val: TVXSpriteAnimationMode);
  ...
  published
    { A collection of animations. Stores the settings for animating
       then sprite. }
    property Animations: TVXSpriteAnimationList read FAnimations;
    ...
    property AnimationMode: TVXSpriteAnimationMode read FAnimationMode write
      SetAnimationMode;
...
procedure TVXSpriteAnimFrame.DoChanged;
begin
  if Assigned(Owner) then
  begin
    if Assigned(Owner.Owner) then
      if Owner.Owner is TVXSpriteAnimation then
        TVXSpriteAnimation(Owner.Owner).DoChanged;
  end;
end;

class function TVXSpriteAnimFrame.FriendlyName: string;
begin
  Result := 'Frame';
end;

class function TVXSpriteAnimFrame.FriendlyDescription: string;
begin
  Result := 'Sprite Animation Frame';
end;

procedure TVXSpriteAnimFrame.WriteToFiler(writer: TWriter);
begin
  inherited;
...
procedure TVXSpriteAnimFrame.ReadFromFiler(reader: TReader);
var
...
procedure TVXSpriteAnimFrame.SetOffsetX(const Value: Integer);
begin
...
procedure TVXSpriteAnimFrame.SetOffsetY(const Value: Integer);
begin
...
procedure TVXSpriteAnimFrame.SetWidth(const Value: Integer);
begin
...
procedure TVXSpriteAnimFrame.SetHeight(const Value: Integer);
begin
...
// ----------
// ---------- TVXSpriteAnimFrameList ----------
// ----------

constructor TVXSpriteAnimFrameList.Create(aOwner: TPersistent);
begin
  inherited;
end;
...
procedure TVXSpriteAnimMargins.SetLeft(const Value: Integer);
begin
  if Value <> FLeft then
  begin
    FLeft := Value;
...
procedure TVXSpriteAnimMargins.SetTop(const Value: Integer);
begin
  if Value <> FTop then
  begin
    FTop := Value;
...
procedure TVXSpriteAnimMargins.SetRight(const Value: Integer);
begin
...
procedure TVXSpriteAnimMargins.SetBottom(const Value: Integer);
begin
...
procedure TVXSpriteAnimMargins.DoChanged;
begin
  if Assigned(Owner) then
    Owner.DoChanged;
end;
...
begin
  ...
  FMargins := TVXSpriteAnimMargins.Create(Self);
end;

destructor TVXSpriteAnimation.Destroy;
begin
  FFrames.Free;
  FMargins.Free;
  inherited;
...
function TVXSpriteAnimation.GetMaterialLibrary: TVXAbstractMaterialLibrary;
begin
  if not (Owner is TVXSpriteAnimationList) then
    Result := nil
  else
  begin
    if not (TVXSpriteAnimationList(Owner).Owner is TVXAnimatedSprite) then
      Result := nil
    else
      Result :=
        TVXAnimatedSprite(TVXSpriteAnimationList(Owner).Owner).FMaterialLibrary;
  end;
end;

class function TVXSpriteAnimation.FriendlyName: string;
begin
  Result := 'Animation';
end;

class function TVXSpriteAnimation.FriendlyDescription: string;
begin
  Result := 'Sprite Animation';
end;

procedure TVXSpriteAnimation.WriteToFiler(writer: TWriter);
begin
  inherited;
...
procedure TVXSpriteAnimation.ReadFromFiler(reader: TReader);
var
  archiveVersion: Integer;
begin
  inherited;
  ...
  begin
    ...
    Dimensions := TVXSpriteFrameDimensions(ReadInteger);
...
procedure TVXSpriteAnimation.DoChanged;
begin
  if Assigned(Owner) then
  begin
    if Assigned(Owner.Owner) then
      if Owner.Owner is TVXBaseSceneObject then
        TVXBaseSceneObject(Owner.Owner).NotifyChange(Self);
  end;
end;

procedure TVXSpriteAnimation.SetCurrentFrame(const Value: Integer);
begin
...
procedure TVXSpriteAnimation.SetFrameWidth(const Value: Integer);
begin
...
procedure TVXSpriteAnimation.SetFrameHeight(const Value: Integer);
begin
...
procedure TVXSpriteAnimation.SetDimensions(
  const Value: TVXSpriteFrameDimensions);
begin
...
procedure TVXSpriteAnimation.SetLibMaterialName(const val: TVXLibMaterialName);
begin
...
function TVXSpriteAnimation.GetLibMaterialCached: TVXLibMaterial;
begin
  Result := nil;
  ...
  if not Assigned(FLibMaterialCached) then
    if Assigned(Owner) then
      if Assigned(Owner.Owner) then
        if Owner.Owner is TVXAnimatedSprite then
          if Assigned(TVXAnimatedSprite(Owner.Owner).MaterialLibrary) then
            FLibMaterialCached :=
              TVXAnimatedSprite(Owner.Owner).MaterialLibrary.Materials.GetLibMaterialByName(FLibMaterialName);

  Result := FLibMaterialCached;
end;

procedure TVXSpriteAnimation.SetInterval(const Value: Integer);
begin
...
procedure TVXSpriteAnimation.SetFrameRate(const Value: Single);
begin
  if Value > 0 then
...
function TVXSpriteAnimation.GetFrameRate: Single;
begin
  if Interval > 0 then
...
// ----------
// ---------- TVXSpriteAnimationList ----------
// ----------

constructor TVXSpriteAnimationList.Create(aOwner: TPersistent);
begin
  inherited;
end;
...
constructor TVXAnimatedSprite.Create(AOwner: TComponent);
begin
  inherited;

  FAnimations := TVXSpriteAnimationList.Create(Self);
  FAnimationIndex := -1;
  FInterval := 100;
  FPixelRatio := 100;
  FRotation := 0;
...
destructor TVXAnimatedSprite.Destroy;
begin
  FAnimations.Free;
  inherited;
end;
...
procedure TVXAnimatedSprite.BuildList(var rci: TVXRenderContextInfo);
var
  vx, vy: TAffineVector;
  w, h, temp: Single;
  mat: TMatrix;
...
begin
  ...
  if (AnimationIndex = -1) then
    exit;
  intr := TVXSpriteAnimation(Animations[AnimationIndex]).Interval;
  if intr = 0 then
    intr := Interval;
...
procedure TVXAnimatedSprite.DefineProperties(Filer: TFiler);
begin
  inherited;
...
procedure TVXAnimatedSprite.WriteAnimations(Stream: TStream);
var
  writer: TWriter;
begin
...
procedure TVXAnimatedSprite.ReadAnimations(Stream: TStream);
var
  reader: TReader;
begin
...
procedure TVXAnimatedSprite.NextFrame;
var
  currentFrame,
    startFrame,
    endFrame: Integer;
...
procedure TVXAnimatedSprite.SetInterval(const val: Integer);
begin
...
function TVXAnimatedSprite.GetFrameRate: Single;
begin
  if Interval > 0 then
    Result := 1000 / Interval
  else
...
procedure TVXAnimatedSprite.SetAnimationIndex(const val: Integer);
begin
  ...
  begin
    ...
    if FAnimationIndex < 0 then
      FAnimationIndex := -1;
    if (FAnimationIndex <> -1) and (FAnimationIndex < Animations.Count) then
      with TVXSpriteAnimation(Animations[FAnimationIndex]) do
        case AnimationMode of
...
procedure TVXAnimatedSprite.SetAnimationMode(const val: TVXSpriteAnimationMode);
begin
...
procedure TVXAnimatedSprite.SetMaterialLibrary(const val: TVXMaterialLibrary);
var
  i: Integer;
begin
  ...
  begin
    ...
    for i := 0 to Animations.Count - 1 do
      TVXSpriteAnimation(Animations[i]).FLibMaterialCached := nil;
    NotifyChange(Self);
  end;
end;

procedure TVXAnimatedSprite.SetPixelRatio(const val: Integer);
begin
...
procedure TVXAnimatedSprite.SetRotation(const val: Integer);
begin
...
procedure TVXAnimatedSprite.SetMirrorU(const val: Boolean);
begin
...
procedure TVXAnimatedSprite.SetMirrorV(const val: Boolean);
begin
...
// -----------------------------------------------------------------------------
  ...
  RegisterClasses([TVXAnimatedSprite,
    TVXSpriteAnimFrame, TVXSpriteAnimFrameList,
      TVXSpriteAnimation, TVXSpriteAnimationList]);

  RegisterXCollectionItemClass(TVXSpriteAnimFrame);
  RegisterXCollectionItemClass(TVXSpriteAnimation);

finalization

  UnregisterXCollectionItemClass(TVXSpriteAnimFrame);
  UnregisterXCollectionItemClass(TVXSpriteAnimation);
