...
constructor TVXTrail.Create(AOwner: TComponent);
begin
 inherited Create(AOwner);
 vertices.Clear;    // inherited TVXmesh makes a triangle... remove it.
...
procedure TVXTrail.CreateMark(obj: TVXBaseSceneObject; width: single;CurrentTime : Double);
var
...
function TVXTrail.CreateMark(p1,p2: TVector3f; CurrentTime : Double): boolean;
var
   diff: integer;
   uv1,uv2: TTexPoint;
...
procedure TVXTrail.CreateMark(APos,ADir,AUp: TVector3f; AWidth: single;ACurrentTime : Double);
var
...
procedure TVXTrail.SetTrailObject(const Value: TVXBaseSceneObject);
begin
...
procedure TVXTrail.SetMarkStyle(const Value: TMarkStyle);
begin
  FMarkStyle := Value;
end;

procedure TVXTrail.Notification(AComponent: TComponent;
...
procedure TVXTrail.SetAlpha(const Value: single);
begin
  FAlpha := Value;
end;

procedure TVXTrail.SetAlphaFade(const Value: Boolean);
begin
  FAlphaFade := Value;
end;

procedure TVXTrail.SetMinDistance(const Value: single);
begin
  FMinDistance := Value;
end;

procedure TVXTrail.SetTimeLimit(const Value: single);
begin
  FTimeLimit := Value;
end;

procedure TVXTrail.SetUVScale(const Value: single);
begin
  FUVScale := Value;
end;

procedure TVXTrail.SetVertLimit(const Value: integer);
begin
  FVertLimit := Value;
end;


procedure TVXTrail.SetMarkWidth(const Value: single);
begin
  FMarkWidth := Value;
end;

procedure TVXTrail.SetEnabled(const Value: boolean);
begin
  FEnabled := Value;
end;

function TVXTrail.StoreAntiZFightOffset: Boolean;
begin
