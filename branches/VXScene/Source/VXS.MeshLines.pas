...
type
   ...
   { Specialized collection for Nodes in TVXMeshLines objects.
  ...
  TLightmapBounds = class(TVXCustomCoordinates)
  private
  ...
  TVXMeshLines = class(TVXFreeForm)
  private
    ...
    procedure DrawNode(var rci : TVXRenderContextInfo; Node: TLineNode; LineWidth: Single);
...
begin
   if (GetOwner<>nil) then
      TVXMeshLines((GetOwner as TLineItem).Collection.Owner).StructureChanged;
end;
...
begin
  if TVXMeshLines(Collection.Owner).SelectedLineItem = self then
    TVXMeshLines(Collection.Owner).DeSelectLineItem;
  FNodes.Free;
  inherited;
end;
...
begin
  ...
  if (GetOwner<>nil) then
    TVXMeshLines(Collection.Owner).NotifyChange(Self);
end;
...
// TVXMeshLine
//--------------------------------
constructor TVXMeshLines.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TVXMeshLines.Destroy;
begin
  FLines.Free;
...
procedure TVXMeshLines.Loaded;
begin
  DoChanged;
end;

procedure TVXMeshLines.BeginUpdate;
begin
  inc(FUpdating);
end;

procedure TVXMeshLines.EndUpdate;
begin
  Dec(FUpdating);
  if FUpdating < 1 then
  begin
...
procedure TVXMeshLines.BuildList(var rci : TVXRenderContextInfo);
var
  i,j: Integer;
begin
  inherited;
...
procedure TVXMeshLines.DoRender(var rci : TVXRenderContextInfo; renderSelf, renderChildren : Boolean);
begin
  if FNoZWrite then
  begin
...
procedure TVXMeshLines.SetShowNodes(const Value: Boolean);
begin
  FShowNodes := Value;
  DoChanged;
end;

procedure TVXMeshLines.SetNoZWrite(const Value: Boolean);
begin
  FNoZWrite := Value;
  DoChanged;
end;

procedure TVXMeshLines.SetLightmapIndex(const value: Integer);
begin
  FLightmapIndex := Value;
  DoChanged;
end;

procedure TVXMeshLines.SetLightmapMaterialName(const value: String);
var
  lLibMaterial: TVXLibMaterial;
begin
  if Value <> '' then
  begin
...
procedure TVXMeshLines.SetLightmapBounds( const value: TLightmapBounds );
begin
  FLightmapBounds.SetVector(value.X, value.Y,value.Z,value.W);
  DoChanged;
end;

procedure TVXMeshLines.DoChanged;
begin
...
procedure TVXMeshLines.BuildGeometry;
var
  i: Integer;
...
begin
  ...
  FMesh := TVXMeshObject.CreateOwned(FMeshObjects);
...
procedure TVXMeshLines.DrawNode(var rci : TVXRenderContextInfo; Node: TLineNode; LineWidth: Single);
var
  lNodeSize: Single;
begin
...
procedure TVXMeshLines.DrawCircle(Radius: Single);
var
...
function TVXMeshLines.SelectNode(LineItem: TLineItem; X,Z: Single): TLineNode;
var
  i: Integer;
  lRange: Single;
  length: single;
...
function TVXMeshLines.SelectLineItem(LineItem: TLineItem): TLineItem;
begin
  Result := nil;
...
function TVXMeshLines.SelectLineItem(LineNode: TLineNode): TLineItem;
begin
...
procedure TVXMeshLines.DeselectLineItem;
begin
  FSelectedLineItem := nil;
  FSelectedNode := nil;
  DoChanged;
...
procedure TVXMeshLines.DeselectLineNode;
begin
  FSelectedNode := nil;
  DoChanged;
end;

function TVXMeshLines.SelectLineItem(const X,Z: Single; Tolerance: single = 1): TLineItem;
var
  i: Integer;
  lStartPoint: Integer;
  lNode: TLineNode;
...
function TVXMeshLines.GetUpdating: Boolean;
begin
  Result := FUpdating > 0;
end;

function TVXMeshLines.PointNearLine(const LineItem: TLineItem; const X,Z: Single; Tolerance: single = 1): boolean;
var
  i: Integer;
...
function TVXMeshLines.PointNearSegment(const StartNode, EndNode: TLineNode; const X,Z: Single; LineWidth: single; Tolerance: single = 1): boolean;
var
  xt, yt, u, len: single;
  xp, yp: single;
  lDist: Single;
...
procedure TVXMeshLines.StitchStrips(idx: TIntegerList);
var
  i: integer;
  i0, i1, i2: integer;
begin
...
procedure TVXMeshLines.AddStitchMarker(idx: TIntegerList);
begin
  idx.Add(-1);
  idx.Add(-2);
  idx.Add(-2);
...
procedure TVXMeshLines.NotifyChange(Sender : TObject);
begin
  inherited;
  DoChanged;
end;

procedure TVXMeshLines.AddIndex;
begin
  FFaceGroup.Add(FIndex);
  inc(FIndex);
end;

procedure TVXMeshLines.AddVertices(Up,Inner,Outer: TAffineVector; S: Single; Correction: Single; UseDegenerate: Boolean; LineItem: TLineItem);
begin
...
procedure TVXMeshLines.BuildLineItem(LineItem: TLineItem);
var
  Seg1: TAffineVector;
  Seg2: TAffineVector;
  NSeg1: TAffineVector;
