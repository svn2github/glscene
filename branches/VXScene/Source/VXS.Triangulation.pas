...
{ TVXDelaunay2D is a class for Delaunay triangulation of arbitrary points
...
constructor TVXDelaunay2D.Create;
begin
...
destructor TVXDelaunay2D.Destroy;
begin
  SetLength(Vertex, 0);
...
function TVXDelaunay2D.InCircle(Xp, Yp, X1, Y1, X2, Y2, X3, Y3: Single;
...
function TVXDelaunay2D.Triangulate(nvert: Integer): Integer;
...
procedure TVXDelaunay2D.Mesh(sort: Boolean);
begin
  if sort then
...
procedure TVXDelaunay2D.AddPoint(X, Y, Z, U, V: Single; MatIndex: Integer);
var
  i, AE: Integer;
begin
...
procedure TVXDelaunay2D.AddPointNoCheck(X, Y, Z, U, V: Single; MatIndex: Integer);
begin
  Vertex[tPoints].X := X;
  Vertex[tPoints].Y := Y;
  Vertex[tPoints].Z := Z;
...
procedure TVXDelaunay2D.RemoveLastPoint;
begin
  tPoints := tPoints - 1;
end;

procedure TVXDelaunay2D.QuickSort(var A: TDVertex; Low, High: Integer);
