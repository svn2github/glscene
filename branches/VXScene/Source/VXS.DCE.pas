...
//
{
  ...
  - You can choose the shape of your static object
    - csEllipsoid, csBox
    - csFreeform MUST BE A TVXFreeform, otherwise will raise errors
    - csTerrain MUST BE A TVXTerrainRenderer, same condition above
...
type
  ...
  TDCECollisionEvent = procedure (Sender : TObject; object1, object2 : TVXBaseSceneObject;
                                       CollisionInfo: TDCECollision) of object;
  TDCEObjectCollisionEvent = procedure (Sender : TObject; ObjectCollided : TVXBaseSceneObject;
                                       CollisionInfo: TDCECollision) of object;
  ...
  private
    ...
    FDynamics : TList;
    FGravity: Single;
    FWorldDirection: TVXCoordinates; //Used to calculate jumps f.i.
    FWorldScale: Single;
    FMovimentScale: Single;
    FStandardiseLayers : TDCECollisionSelection;
    FManualStep: Boolean;
    FOnCollision : TDCECollisionEvent;
    procedure SetWorldDirection(const Value: TVXCoordinates);
    procedure SetWorldScale(const Value: Single);
    function GetDynamicCount: Integer;
    function GetStaticCount: Integer;
  protected
    
    procedure RegisterStatic(aClient : TVXDCEStatic);
    procedure DeRegisterStatic(aClient : TVXDCEStatic);
    procedure DeRegisterAllStatics;
    procedure RegisterDynamic(aClient : TVXDCEDynamic);
  ...
  public
    ...
    function MoveByDistance(var Body: TVXDCEDynamic; deltaS, deltaAbsS: TAffineVector): Single;
  ...
  published
    ...
    property Size : TVXCoordinates read FSize write SetSize;
  end;
...
function GetOrCreateDCEStatic(behaviours : TVXBehaviours) : TVXDCEStatic; overload;
function GetOrCreateDCEStatic(obj : TVXBaseSceneObject) : TVXDCEStatic; overload;
...
begin
  ...
  FGravity:=0;
  FWorldDirection:=TVXCoordinates.CreateInitialized(Self, YHmgVector, csVector);
  FWorldScale := 1;
  FMovimentScale := 1;
...
function TVXDCEManager.GetDynamicCount: Integer;
begin
  result := FDynamics.Count;
end;

function TVXDCEManager.GetStaticCount: Integer;
begin
  result := FStatics.Count;
end;

function TVXDCEManager.MoveByDistance(var Body: TVXDCEDynamic; deltaS, deltaAbsS: TAffineVector): Single;
var
    //Friction and bounce
...
begin
  ...
  while deltaCR>0 do begin
    ...
    with TVXDCEStatic(FStatics[i]) do
    begin
      ...
      begin
        case Shape of
          ...
          csTerrain: ECAddTerrain(MP,TVXTerrainRenderer(OwnerBaseSceneObject),FWorldScale*2,Solid,i);
        end;
      end;
    ...
    with TVXDCEDynamic(FDynamics[i]) do
    begin
      CanCollide := False;
      if (Active) and (TVXDCEDynamic(FDynamics[i]) <> Body) then
      case FStandardiseLayers of
        ccsDCEStandard: CanCollide := (Layer <= Body.Layer);
        ccsCollisionStandard: CanCollide := (layer = 0) or (body.layer = 0) or (layer <> body.layer);
        ccsHybrid: CanCollide := ( (layer = 0) or (body.layer = 0) or (Layer <= Body.Layer) ) and (layer>=0) and (body.layer>=0);
  ...
  begin
    ...
    if oi < 0 then
    begin
      tFriction := TVXDCEDynamic(FDynamics[abs(oi) - 1]).Friction;
      tBounceFactor := TVXDCEDynamic(FDynamics[abs(oi) - 1]).BounceFactor;
      tObject := TVXDCEDynamic(FDynamics[abs(oi) - 1]).OwnerBaseSceneObject;
    end else
    begin
      tFriction := TVXDCEStatic(FStatics[oi]).Friction;
      tBounceFactor := TVXDCEStatic(FStatics[oi]).BounceFactor;
      tObject := TVXDCEStatic(FStatics[oi]).OwnerBaseSceneObject;
    end;
    ...
    if (oi >= 0) and Assigned(TVXDCEStatic(FStatics[oi]).FOnCollision) then
      TVXDCEStatic(FStatics[oi]).FOnCollision(Self,Body.OwnerBaseSceneObject,ColInfo);
    end;
  finally
    ContactList.Free;
  end;
...
procedure TVXDCEManager.Step(deltaTime: Double);
var i: Integer;
begin
...
procedure TVXDCEManager.SetWorldDirection(const Value: TVXCoordinates);
begin
  FWorldDirection := Value;
  FWorldDirection.Normalize;
end;

procedure TVXDCEManager.SetWorldScale(const Value: Single);
begin
...
procedure TVXDCEManager.RegisterStatic(aClient : TVXDCEStatic);
begin
...
procedure TVXDCEManager.DeRegisterStatic(aClient : TVXDCEStatic);
begin
...
procedure TVXDCEManager.DeRegisterAllStatics;
var
   i : Integer;
begin
   // Fast deregistration
...
procedure TVXDCEManager.DeRegisterDynamic(aClient : TVXDCEDynamic);
begin
   if Assigned(aClient) then begin
      aClient.FManager:=nil;
...
procedure TVXDCEManager.DeRegisterAllDynamics;
var
   i : Integer;
begin
   // Fast deregistration
   for i:=0 to FDynamics.Count-1 do
      TVXDCEDynamic(FDynamics[i]).FManager:=nil;
   FDynamics.Clear;
end;

{ TVXDCEStatic }

procedure TVXDCEStatic.Assign(Source: TPersistent);
begin
   if Source is TVXDCEStatic then begin
      Active := TVXDCEStatic(Source).Active;
      Manager:=TVXDCEStatic(Source).Manager;
      Shape := TVXDCEStatic(Source).Shape;
      Layer := TVXDCEStatic(Source).Layer;
      Solid := TVXDCEStatic(Source).Solid;
      Size.Assign(TVXDCEStatic(Source).Size);
      Friction := TVXDCEStatic(Source).Friction;
      BounceFactor := TVXDCEStatic(Source).BounceFactor;
   end;
...
begin
   inherited Create(aOwner);
   FActive := True;
   FSize:=TVXCoordinates.CreateInitialized(Self, XYZHmgVector, csVector);
   FShape := csEllipsoid;
   FSolid := True;
   FFriction := 1;
   FBounceFactor := 0;
...
destructor TVXDCEStatic.Destroy;
begin
   Manager:=nil;
   FSize.Free;
   inherited Destroy;
...
class function TVXDCEStatic.FriendlyDescription: String;
begin
   Result:='Static Collision-detection registration';
end;

class function TVXDCEStatic.FriendlyName: String;
begin
   Result:='DCE Static Collider';
end;

procedure TVXDCEStatic.Loaded;
var
   mng : TComponent;
begin
   inherited;
   if FManagerName<>'' then begin
      mng:=FindManager(TVXDCEManager, FManagerName);
      if Assigned(mng) then
         Manager:=TVXDCEManager(mng);
      FManagerName:='';
   end;
end;

procedure TVXDCEStatic.WriteToFiler(writer: TWriter);
begin
...
procedure TVXDCEStatic.ReadFromFiler(reader: TReader);
var
...
procedure TVXDCEStatic.SetBounceFactor(const Value: Single);
begin
  FBounceFactor := Value;
...
procedure TVXDCEStatic.SetFriction(const Value: Single);
begin
  FFriction := Value;
...
procedure TVXDCEStatic.SetManager(const val: TVXDCEManager);
begin
...
procedure TVXDCEStatic.SetShape(const Value: TDCEShape);
begin
  FShape := Value;
end;

procedure TVXDCEStatic.SetSize(const Value: TVXCoordinates);
begin
  FSize.Assign(Value);
...
procedure TVXDCEDynamic.ApplyAccel(NewAccel: TAffineVector);
begin
  AddVector(FAccel, NewAccel);
end;

procedure TVXDCEDynamic.ApplyAccel(x,y,z: Single);
begin
  AddVector(FAccel, AffineVectorMake(x,y,z));
end;

procedure TVXDCEDynamic.ApplyAbsAccel(NewAccel: TAffineVector);
begin
  AddVector(FAbsAccel, NewAccel);
end;

procedure TVXDCEDynamic.ApplyAbsAccel(x,y,z: Single);
begin
  AddVector(FAbsAccel, AffineVectorMake(x,y,z));
end;

procedure TVXDCEDynamic.StopAccel;
begin
  SetVector(FAccel, NullVector);
end;

procedure TVXDCEDynamic.StopAbsAccel;
begin
  SetVector(FAbsAccel, NullVector);
end;

procedure TVXDCEDynamic.Assign(Source: TPersistent);
begin
  if Source is TVXDCEDynamic then begin
    Manager:=TVXDCEDynamic(Source).Manager;
    Active := TVXDCEDynamic(Source).Active;
    UseGravity := TVXDCEDynamic(Source).UseGravity;
    Layer := TVXDCEDynamic(Source).Layer;
    Solid := TVXDCEDynamic(Source).Solid;
    Size.Assign(TVXDCEDynamic(Source).Size);
    Friction := TVXDCEDynamic(Source).Friction;
    BounceFactor := TVXDCEDynamic(Source).BounceFactor;
    SlideOrBounce := TVXDCEDynamic(Source).SlideOrBounce;
    MaxRecursionDepth := TVXDCEDynamic(Source).MaxRecursionDepth;
  end;
  inherited Assign(Source);
end;
...
destructor TVXDCEDynamic.Destroy;
begin
  Manager:=nil;
  FSize.Free;
  inherited Destroy;
...
procedure TVXDCEDynamic.DoMove(deltaTime: Double);
...
class function TVXDCEDynamic.FriendlyDescription: String;
begin
  Result:='Dynamic Collision-detection registration';
end;

class function TVXDCEDynamic.FriendlyName: String;
begin
  Result:='DCE Dynamic Collider';
end;

procedure TVXDCEDynamic.Jump(jHeight, jSpeed: Single);
begin
...
procedure TVXDCEDynamic.Loaded;
var
   mng : TComponent;
begin
   inherited;
...
procedure TVXDCEDynamic.Move(deltaS: TAffineVector; deltaTime: Double);
begin
  ScaleVector(deltaS, deltaTime);
  FManager.MoveByDistance(Self, NullVector, deltaS);
end;

procedure TVXDCEDynamic.MoveTo(Position: TAffineVector; Amount: Single);
begin
...
procedure TVXDCEDynamic.WriteToFiler(writer: TWriter);
begin
   with writer do begin
      // ArchiveVersion 1, added inherited call
      WriteInteger(1);
...
procedure TVXDCEDynamic.ReadFromFiler(reader: TReader);
var
   archiveVersion : Integer;
begin
   with reader do begin
...
procedure TVXDCEDynamic.SetBounceFactor(const Value: Single);
begin
  FBounceFactor := Value;
  if FBounceFactor < 0 then FBounceFactor := 0;
  if FBounceFactor > 1 then FBounceFactor := 1;
...
procedure TVXDCEDynamic.SetFriction(const Value: Single);
begin
  FFriction := Value;
  if FFriction < 0 then FFriction := 0;
  if FFriction > 100 then FFriction := 100;
...
procedure TVXDCEDynamic.SetManager(const val: TVXDCEManager);
begin
   if val<>FManager then begin
      if Assigned(FManager) then
...
procedure TVXDCEDynamic.SetSize(const Value: TVXCoordinates);
begin
  FSize.Assign(Value);
  if FSize.X <= 0 then FSize.X := 0.1;
  if FSize.Y <= 0 then FSize.Y := 0.1;
...
function GetOrCreateDCEStatic(behaviours : TVXBehaviours) : TVXDCEStatic;
var
	i : Integer;
begin
	i:=behaviours.IndexOfClass(TVXDCEStatic);
	if i>=0 then
		Result:=TVXDCEStatic(behaviours[i])
	else Result:=TVXDCEStatic.Create(behaviours);
end;

function GetOrCreateDCEStatic(obj : TVXBaseSceneObject) : TVXDCEStatic;
begin
	Result:=GetOrCreateDCEStatic(obj.Behaviours);
end;

function GetOrCreateDCEDynamic(behaviours : TVXBehaviours) : TVXDCEDynamic;
var
	i : Integer;
begin
	i:=behaviours.IndexOfClass(TVXDCEDynamic);
	if i>=0 then
		Result:=TVXDCEDynamic(behaviours[i])
	else Result:=TVXDCEDynamic.Create(behaviours);
end;

function GetOrCreateDCEDynamic(obj : TVXBaseSceneObject) : TVXDCEDynamic;
begin
...
// ------------------------------------------------------------------
	...
	// class registrations
	RegisterXCollectionItemClass(TVXDCEStatic);
	RegisterXCollectionItemClass(TVXDCEDynamic);

finalization

	UnregisterXCollectionItemClass(TVXDCEStatic);
	UnregisterXCollectionItemClass(TVXDCEDynamic);
