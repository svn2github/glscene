...
type
  TVXProcTextureNoise = class(TVXTextureImage)
  private
    FNoiseMap: TVXBitmap32;
...
constructor TVXProcTextureNoise.Create(AOwner: TPersistent);
...
destructor TVXProcTextureNoise.Destroy;
begin
  ReleaseBitmap32;
  inherited;
end;
...
function TVXProcTextureNoise.GetBitmap32: TVXBitmap32;
begin
  if not Assigned(FNoiseMap) then
  begin
    FNoiseMap := TVXBitmap32.Create;
...
class function TVXProcTextureNoise.FriendlyName: string;
begin
...
class function TVXProcTextureNoise.FriendlyDescription: string;
begin
  Result := 'Procedural Noise (Animated)';
end;

procedure TVXProcTextureNoise.SetSeamless(const val: Boolean);
begin
...
procedure TVXProcTextureNoise.LoadFromFile(const fileName: string);
begin
  Assert(False, 'TVXProcTextureNoise.LoadFromFile not implemented');
end;

procedure TVXProcTextureNoise.ReleaseBitmap32;
begin
...
procedure TVXProcTextureNoise.SaveToFile(const fileName: string);
begin
  {Nothing here}
end;

function TVXProcTextureNoise.GetHeight: Integer;
begin
  Result := FHeight;
end;

function TVXProcTextureNoise.GetWidth: Integer;
begin
  Result := FWidth;
end;

function TVXProcTextureNoise.GetDepth: Integer;
begin
  Result := 1;
end;
...
function TVXProcTextureNoise.GetTextureTarget: TVXTextureTarget;
begin
  Result := ttTexture2D;
end;

procedure TVXProcTextureNoise.SetHeight(const val: Integer);
begin
  if val <> FHeight then
  begin
    FHeight := val;
...
procedure TVXProcTextureNoise.SetWidth(const val: Integer);
begin
  if val <> FWidth then
  begin
    FWidth := val;
...
procedure TVXProcTextureNoise.SetMinCut(const val: Byte);
begin
  if val <> FMinCut then
  begin
    FMinCut := val;
...
procedure TVXProcTextureNoise.SetNoiseSharpness(const val: Single);
begin
...
procedure TVXProcTextureNoise.SetNoiseRandSeed(const val: Longint);
var
  i: Integer;
  seedBackup: Longint;
  z, r, theta: Single;
...
procedure TVXProcTextureNoise.Assign(Source: TPersistent);
begin
  if Assigned(Source) and (Source is TVXProcTextureNoise) then
  begin
    FWidth := TVXProcTextureNoise(Source).FWidth;
    FHeight := TVXProcTextureNoise(Source).FHeight;
    FMinCut := TVXProcTextureNoise(Source).FMinCut;
    FNoiseSharpness := TVXProcTextureNoise(Source).FNoiseSharpness;
    FNoiseRandSeed := TVXProcTextureNoise(Source).FNoiseRandSeed;
    Invalidate;
  end
  else
    inherited;
...
procedure TVXProcTextureNoise.NoiseAnimate(speed: Single);
begin
  FNoiseAnimate := FNoiseAnimate + speed;
  Invalidate;
end;

function TVXProcTextureNoise.Noise(x, y: Single): Single;
var
  ix, iy, iz: Integer;
...
procedure TVXProcTextureNoise.SetPermFromData(inPERM: array of Byte);
var
  I: Integer;
begin
  for I := 0 to 255 do
...
procedure TVXProcTextureNoise.SetPermToDefault;
begin
...
initialization
  ...
  RegisterTextureImageClass(TVXProcTextureNoise);
