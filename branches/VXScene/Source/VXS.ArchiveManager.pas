...
Type
  ...
  TVXBaseArchiveClass = class of TVXBaseArchive;
  ...
  public
    ...
    function FindFromFileName(const fileName: string): TVXBaseArchiveClass;
    procedure Remove(AClass: TVXBaseArchiveClass);
  end;
  ...
  function GetArchiveFileFormats: TVXArchiveFileFormatsList;

  //Регистрация архиватора.
  procedure RegisterArchiveFormat(const AExtension, ADescription: string;
    AClass: TVXBaseArchiveClass);
  procedure UnregisterArchiveFormat(AClass: TVXBaseArchiveClass);
...
var
  vArchiveFileFormats: TVXArchiveFileFormatsList;
  vArchiveManager: TVXSArchiveManager;

function GetArchiveFileFormats: TVXArchiveFileFormatsList;
begin
  if not Assigned(vArchiveFileFormats) then
    vArchiveFileFormats := TVXArchiveFileFormatsList.Create;
...
destructor TVXArchiveFileFormatsList.Destroy;
begin
  Clean;
  inherited Destroy;
end;

procedure TVXArchiveFileFormatsList.Add(const Ext, Desc: string; DescID: Integer;
  AClass: TVXBaseArchiveClass);
var
...
function TVXArchiveFileFormatsList.FindExt(ext: string): TVXBaseArchiveClass;
var
  i: Integer;
begin
...
function TVXArchiveFileFormatsList.FindFromFileName(const fileName: string
  ): TVXBaseArchiveClass;
var
  ext: string;
begin
...
procedure TVXArchiveFileFormatsList.Remove(AClass: TVXBaseArchiveClass);
var
  i: Integer;
begin
  for i := Count - 1 downto 0 do
...
procedure TVXBaseArchive.SetCompressionLevel(aValue: TCompressionLevel);
begin
  if FCompressionLevel <> aValue then
     FCompressionLevel := aValue;
end;

constructor TVXBaseArchive.Create(AOwner: TPersistent);
begin
...
destructor TVXBaseArchive.Destroy;
begin
  FContentList.Free;
  inherited Destroy;
end;
...
constructor TVXSArchiveManager.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TVXSArchiveManager.Destroy;
begin
  vArchiveManager := nil;
  FArchives.Free;
  inherited Destroy;
...
procedure TVXSArchiveManager.SetArchives(aValue: TLibArchives);
begin
  FArchives.Assign(aValue);
end;

function TVXSArchiveManager.GetArchiveByFileName(const aName: string): TLibArchive;
begin
  Result := FArchives.GetArchiveByFileName(AName);
end;

function TVXSArchiveManager.GetFileNameOfArchive(const aArchive: TLibArchive): string;
begin
  Result := FArchives.GetFileNameOfArchive(aArchive)
end;

function TVXSArchiveManager.GetContent(aContentName: string): TStream;
var
  i: integer;
begin
  Result := nil;
...
function TVXSArchiveManager.ContentExists(aContentName: string): boolean;
var
  i: integer;
begin
    Result := false;
...
function TVXSArchiveManager.OpenArchive(aFileName: string): TLibArchive;
begin
  Result := FArchives.Add;
  Result.LoadFromFile(aFileName);
end;

function TVXSArchiveManager.OpenArchive(aFileName, aAchiverType: string
  ): TLibArchive;
begin
  Result := FArchives.Add;
...
procedure TVXSArchiveManager.CloseArchive(aArchive: TLibArchive);
begin
