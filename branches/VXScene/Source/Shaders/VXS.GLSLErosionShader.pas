...
//TVXCustomGLSLSimpleErosionShader
//
{ Custom class for GLSLSimpleErosionShader. 
 A shader that Erode surface object }
Type
  TVXCustomGLSLSimpleErosionShader = class(TVXCustomGLSLShader)
  private
    ...
    procedure SetErosionTexTexture(const Value: TVXTexture);
    ...
    function GetErosionTexName: TVXLibMaterialName;
    procedure SetErosionTexName(const Value: TVXLibMaterialName);
...
//    property Color1: TVXColor read FColor1;
//    property Color2: TVXColor read FColor2;
    ...
    property ErosionTexture: TVXTexture read FErosionTex write SetErosionTexTexture;
    property ErosionTextureName: TVXLibMaterialName read GetErosionTexName write SetErosionTexName;
  ...
  TVXSLSimpleErosionShader = class(TVXCustomGLSLSimpleErosionShader)
  published
//    property Color1;
//    property Color2;
...
constructor TVXCustomGLSLSimpleErosionShader.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TVXCustomGLSLSimpleErosionShader.Destroy;
begin
  FAmbientColor.Free;
  FSpecularColor.Free;
  inherited;
...
procedure TVXCustomGLSLSimpleErosionShader.DoApply(var rci : TVXRenderContextInfo; Sender : TObject);
begin
...
function TVXCustomGLSLSimpleErosionShader.DoUnApply(var rci: TVXRenderContextInfo): Boolean;
begin
...
function TVXCustomGLSLSimpleErosionShader.GetMaterialLibrary: TVXAbstractMaterialLibrary;
begin
  Result := FMaterialLibrary;
end;

procedure TVXCustomGLSLSimpleErosionShader.SetMaterialLibrary(const Value: TVXAbstractMaterialLibrary);
begin
...
procedure TVXCustomGLSLSimpleErosionShader.SetMainTexTexture(const Value: TVXTexture);
begin
...
procedure TVXCustomGLSLSimpleErosionShader.SetNoiseTexTexture(const Value: TVXTexture);
begin
...
procedure TVXCustomGLSLSimpleErosionShader.SetErosionTexTexture(const Value: TVXTexture);
begin
...
function TVXCustomGLSLSimpleErosionShader.GetNoiseTexName: TVXLibMaterialName;
begin
  Result := TVXMaterialLibrary(FMaterialLibrary).GetNameOfTexture(FNoiseTex);
  if Result = '' then Result := FNoiseTexName;
end;

procedure TVXCustomGLSLSimpleErosionShader.SetNoiseTexName(const Value: TVXLibMaterialName);
begin
...
function TVXCustomGLSLSimpleErosionShader.GetMainTexName: TVXLibMaterialName;
begin
  Result := TVXMaterialLibrary(FMaterialLibrary).GetNameOfTexture(FMainTex);
  if Result = '' then Result := FMainTexName;
end;

procedure TVXCustomGLSLSimpleErosionShader.SetMainTexName(const Value: TVXLibMaterialName);
begin
 // Assert(not(assigned(FMaterialLibrary)),'You must set Material Library Before');
...
function TVXCustomGLSLSimpleErosionShader.GetErosionTexName: TVXLibMaterialName;
begin
  Result := TVXMaterialLibrary(FMaterialLibrary).GetNameOfTexture(FErosionTex);
  if Result = '' then Result := FErosionTexName;
end;

procedure TVXCustomGLSLSimpleErosionShader.SetErosionTexName(const Value: TVXLibMaterialName);
begin
 // Assert(not(assigned(FMaterialLibrary)),'You must set Material Library Before');
  if FErosionTexName = Value then Exit;
  FErosionTexName  := Value;

  FErosionTex := TVXMaterialLibrary(FMaterialLibrary).TextureByName(FErosionTexName);
  NotifyChange(Self);
end;

procedure TVXCustomGLSLSimpleErosionShader.SetAmbientColor(AValue: TVXColor);
begin
  FAmbientColor.DirectColor := AValue.Color;
end;

procedure TVXCustomGLSLSimpleErosionShader.SetSpecularColor(AValue: TVXColor);
begin
  FSpecularColor.DirectColor := AValue.Color;
end;


procedure TVXCustomGLSLSimpleErosionShader.Notification(AComponent: TComponent; Operation: TOperation);
var
  Index: Integer;
begin
  inherited;
  if Operation = opRemove then
    if AComponent = FMaterialLibrary then
      ...
      begin
        ...
        begin
          Index := TVXMaterialLibrary(FMaterialLibrary).Materials.GetTextureIndex(FErosionTex);
          if Index <> -1 then
