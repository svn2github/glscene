...
type
  ...
  TVXGLSLShaderEventEx = procedure(Shader: TVXCustomGLSLShader;
    Sender: TObject) of object;

  TVXActiveAttrib = record
    Name: string;
    Size: GLInt;
    AType: TVXSLDataType;
    Location: Integer;
  ...
  TVXActiveAttribArray = array of TVXActiveAttrib;

  TVXCustomGLSLShader = class(TVXCustomShader)
  private
    FGLSLProg: TVXProgramHandle;
    FParam: TVXGLSLShaderParameter;
    FActiveVarying: TStrings;
    FTransformFeedBackMode: TVXTransformFeedBackMode;
    FOnInitialize: TVXGLSLShaderEvent;
    FOnApply: TVXGLSLShaderEvent;
    FOnUnApply: TVXGLSLShaderUnApplyEvent;
    FOnInitializeEx: TVXGLSLShaderEventEx;
    FOnApplyEx: TVXGLSLShaderEventEx;
    function GetParam(const Index: string): TVXGLSLShaderParameter;
    function GetDirectParam(const Index: Cardinal): TVXGLSLShaderParameter;
    procedure OnChangeActiveVarying(Sender: TObject);
  protected
    property OnApply: TVXGLSLShaderEvent read FOnApply write FOnApply;
    property OnUnApply: TVXGLSLShaderUnApplyEvent read FOnUnApply write FOnUnApply;
    property OnInitialize: TVXGLSLShaderEvent read FOnInitialize write FOnInitialize;
    property OnInitializeEx: TVXGLSLShaderEventEx read FOnInitializeEx write FOnInitializeEx;
    property OnApplyEx: TVXGLSLShaderEventEx read FOnApplyEx write FOnApplyEx;
    function GetGLSLProg: TVXProgramHandle; virtual;
    function GetCurrentParam: TVXGLSLShaderParameter; virtual;
    procedure SetActiveVarying(const Value: TStrings);
    procedure SetTransformFeedBackMode(const Value: TVXTransformFeedBackMode);
  ...
  public
    ...
    function GetActiveAttribs: TVXActiveAttribArray;
    property Param[const Index: string]: TVXGLSLShaderParameter read GetParam;
    property DirectParam[const Index: Cardinal]: TVXGLSLShaderParameter read GetDirectParam;
    property ActiveVarying: TStrings read FActiveVarying write SetActiveVarying;
    property TransformFeedBackMode: TVXTransformFeedBackMode read FTransformFeedBackMode write SetTransformFeedBackMode default tfbmInterleaved;
  end;

  { Wrapper around a parameter of a GLSL program. }
  TVXGLSLShaderParameter = class(TVXCustomShaderParameter)
  private
    FGLSLProg: TVXProgramHandle;
    FParameterID: GLInt;
  protected
    ...
    function GetAsCustomTexture(const TextureIndex: Integer;
      TextureTarget: TVXTextureTarget): Cardinal; override;
    procedure SetAsCustomTexture(const TextureIndex: Integer;
      TextureTarget: TVXTextureTarget; const Value: Cardinal); override;
  ...
  TVXGLSLShader = class(TVXCustomGLSLShader)
  published
...
procedure TVXCustomGLSLShader.DoApply(var rci: TVXRenderContextInfo; Sender: TObject);
begin
...
procedure TVXCustomGLSLShader.DoInitialize(var rci: TVXRenderContextInfo; Sender: TObject);
const
    ...
    try
      ...
      begin
        ...
        if VertexProgram.Enabled then
          FGLSLProg.AddShader(TVXVertexShaderHandle, VertexProgram.Code.Text, FDebugMode);
        if FragmentProgram.Enabled then
          FGLSLProg.AddShader(TVXFragmentShaderHandle, FragmentProgram.Code.Text, FDebugMode);
        if GeometryProgram.Enabled then
          FGLSLProg.AddShader(TVXGeometryShaderHandle, GeometryProgram.Code.Text, FDebugMode);
...
function TVXCustomGLSLShader.DoUnApply(var rci: TVXRenderContextInfo): Boolean;
begin
  Result := False;
...
function TVXCustomGLSLShader.ShaderSupported: Boolean;
begin
  Result := (GL_ARB_shader_objects and GL_ARB_vertex_program and
             GL_ARB_vertex_shader and GL_ARB_fragment_shader);
end;

function TVXCustomGLSLShader.GetActiveAttribs: TVXActiveAttribArray;
var
  LRci: TVXRenderContextInfo;
  i, j: Integer;
...
procedure TVXCustomGLSLShader.Assign(Source: TPersistent);
begin
  inherited Assign(Source);

  if Source is TVXCustomGLSLShader then
  begin
    FreeAndNil(FGLSLProg); //just free the handle for it to be recreated on next initialization
  end;
end;

procedure TVXCustomGLSLShader.DoFinalize;
begin
  inherited;
...
function TVXCustomGLSLShader.GetGLSLProg: TVXProgramHandle;
begin
  Result := FGLSLProg;
end;

function TVXCustomGLSLShader.GetParam(
  const Index: string): TVXGLSLShaderParameter;
begin
  FParam.FParameterID := FGLSLProg.GetUniformLocation(Index);
  Result := FParam;
end;

function TVXCustomGLSLShader.GetDirectParam(
  const Index: Cardinal): TVXGLSLShaderParameter;
begin
  FParam.FParameterID := Index;
  Result := FParam;
end;

function TVXCustomGLSLShader.GetCurrentParam: TVXGLSLShaderParameter;
begin
  Result := FParam;
end;

constructor TVXCustomGLSLShader.Create(AOwner: TComponent);
begin
  inherited;
  FGLSLProg := TVXProgramHandle.Create;
  FParam := TVXGLSLShaderParameter.Create;
...
destructor TVXCustomGLSLShader.Destroy;
begin
...
procedure TVXCustomGLSLShader.SetActiveVarying(const Value: TStrings);
begin
  FActiveVarying.Assign(Value);
  NotifyChange(Self);
end;

procedure TVXCustomGLSLShader.SetTransformFeedBackMode(const Value: TVXTransformFeedBackMode);
begin
...
procedure TVXCustomGLSLShader.OnChangeActiveVarying(Sender: TObject);
begin
  NotifyChange(Self);
end;

//------------------------------------------------------------
{ TVXGLSLShaderParameter }
//------------------------------------------------------------

function TVXGLSLShaderParameter.GetAsCustomTexture(
  const TextureIndex: Integer; TextureTarget: TVXTextureTarget): Cardinal;
begin
  glGetUniformiv(FGLSLProg.Handle, TextureIndex, @Result);
end;

function TVXGLSLShaderParameter.GetAsMatrix2f: TMatrix2f;
begin
  glGetUniformfv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsMatrix3f: TMatrix3f;
begin
  glGetUniformfv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsMatrix4f: TMatrix4f;
begin
  glGetUniformfv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsVector1f: Single;
begin
  glGetUniformfv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsVector1i: Integer;
begin
  glGetUniformiv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsVector2f: TVector2f;
begin
  glGetUniformfv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsVector2i: TVector2i;
begin
  glGetUniformiv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsVector3f: TVector3f;
begin
  glGetUniformfv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsVector3i: TVector3i;
begin
  glGetUniformiv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsVector4f: TVector;
begin
  glGetUniformfv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsVector4i: TVector4i;
begin
  glGetUniformiv(FGLSLProg.Handle, FParameterID, @Result);
end;

procedure TVXGLSLShaderParameter.SetAsCustomTexture(
...
procedure TVXGLSLShaderParameter.SetAsVector1i(const Value: Integer);
begin
  glUniform1i(FParameterID, Value);
end;

procedure TVXGLSLShaderParameter.SetAsVector2f(const Value: TVector2f);
begin
  glUniform2f(FParameterID, Value.X, Value.Y);
end;

procedure TVXGLSLShaderParameter.SetAsVector2i(const Value: TVector2i);
begin
  glUniform2i(FParameterID, Value.X, Value.Y);
end;

procedure TVXGLSLShaderParameter.SetAsVector3f(const Value: TVector3f);
begin
  glUniform3f(FParameterID, Value.X, Value.Y, Value.Z);
end;

procedure TVXGLSLShaderParameter.SetAsVector3i(const Value: TVector3i);
begin
  glUniform3i(FParameterID, Value.X, Value.Y, Value.Z);
end;

procedure TVXGLSLShaderParameter.SetAsVector4f(const Value: TVector4f);
begin
  glUniform4f(FParameterID, Value.X, Value.Y, Value.Z, Value.W);
end;

procedure TVXGLSLShaderParameter.SetAsVector4i(const Value: TVector4i);
begin
  glUniform4i(FParameterID, Value.X, Value.Y, Value.Z, Value.W);
end;

function TVXGLSLShaderParameter.GetAsUniformBuffer: GLenum;
begin
  glGetUniformiv(FGLSLProg.Handle, FParameterID, @Result);
end;

function TVXGLSLShaderParameter.GetAsVector1ui: GLuint;
begin
  glGetUniformuiv(FGLSLProg.Handle, FParameterID, @Result);
end;

procedure TVXGLSLShaderParameter.SetAsVector1ui(const Value: GLuint);
begin
  glUniform1ui(FParameterID, Value);
end;

function TVXGLSLShaderParameter.GetAsVector2ui: TVector2ui;
begin
  glGetUniformiv(FGLSLProg.Handle, FParameterID, @Result);
end;

procedure TVXGLSLShaderParameter.SetAsVector2ui(const Value: TVector2ui);
begin
  glUniform2ui(FParameterID, Value.X, Value.Y);
end;

function TVXGLSLShaderParameter.GetAsVector3ui: TVector3ui;
begin
  glGetUniformiv(FGLSLProg.Handle, FParameterID, @Result);
end;

procedure TVXGLSLShaderParameter.SetAsVector3ui(const Value: TVector3ui);
begin
  glUniform3ui(FParameterID, Value.X, Value.Y, Value.Z);
end;

function TVXGLSLShaderParameter.GetAsVector4ui: TVector4ui;
begin
  glGetUniformiv(FGLSLProg.Handle, FParameterID, @Result);
end;

procedure TVXGLSLShaderParameter.SetAsVector4ui(const Value: TVector4ui);
begin
...
initialization
//=======================================================
  RegisterClasses([TVXCustomGLSLShader, TVXGLSLShader]);
