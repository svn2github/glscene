...
unit VXS.GLSLVertexDisplacementShader;
...
//TVXCustomGLSLSimpleVertexDisplacementShader
//
{ Custom class for GLSLVertexDisplacementShader. 
 VertexDisplacement Shader : Spherical Environment Mapping }
Type
  TVXCustomGLSLVertexDisplacementShader = class(TVXCustomGLSLShader)
  private
...
//    property DiffuseColor : TVXColor read FDiffuseColor Write setDiffuseColor;
  ...
  TVXSLVertexDisplacementShader = class(TVXCustomGLSLVertexDisplacementShader)
  published
...
constructor TVXCustomGLSLVertexDisplacementShader.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TVXCustomGLSLVertexDisplacementShader.Destroy;
begin
...
procedure TVXCustomGLSLVertexDisplacementShader.DoApply(var rci: TVXRenderContextInfo; Sender: TObject);
begin
...
function TVXCustomGLSLVertexDisplacementShader.DoUnApply(var rci: TVXRenderContextInfo): Boolean;
begin
...
function TVXCustomGLSLVertexDisplacementShader.GetMaterialLibrary: TVXAbstractMaterialLibrary;
begin
  Result := FMaterialLibrary;
end;

procedure TVXCustomGLSLVertexDisplacementShader.SetMaterialLibrary(const Value: TVXAbstractMaterialLibrary);
begin
...
procedure TVXCustomGLSLVertexDisplacementShader.SetMainTexTexture(const Value: TVXTexture);
begin
...
function TVXCustomGLSLVertexDisplacementShader.GetMainTexName: TVXLibMaterialName;
begin
  Result := TVXMaterialLibrary(FMaterialLibrary).GetNameOfTexture(FMainTexture);
  if Result = '' then Result := FMainTexName;
end;

procedure TVXCustomGLSLVertexDisplacementShader.SetMainTexName(const Value: TVXLibMaterialName);
begin
...
//procedure TVXCustomGLSLVertexDisplacementShader.SetDiffuseColor(AValue: TVXColor);
//begin
//  FDiffuseColor.DirectColor := AValue.Color;
//end;

procedure TVXCustomGLSLVertexDisplacementShader.SetAmbientColor(AValue: TVXColor);
begin
  FAmbientColor.DirectColor := AValue.Color;
end;

procedure TVXCustomGLSLVertexDisplacementShader.SetSpecularColor(AValue: TVXColor);
begin
  FSpecularColor.DirectColor := AValue.Color;
end;

procedure TVXCustomGLSLVertexDisplacementShader.Notification(AComponent: TComponent; Operation: TOperation);
var
  Index: Integer;
begin
  inherited;
