...
procedure UVPlanarMapping(MeshObject: TMeshObject; min, max: TAffineVector;
  Axis: TAxis = 2);
procedure UVCubicMapping(MeshObject: TMeshObject; min, max: TAffineVector;
  Axis: TAxis = 2);
procedure UVCylindricalMapping(MeshObject: TMeshObject; min, max: TAffineVector;
  Axis: TAxis = 2; MaxUDiff: Single = 0.75);
procedure UVSphericalMapping(MeshObject: TMeshObject; min, max: TAffineVector;
  Axis: TAxis = 2; MaxUDiff: Single = 0.75);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for k := 0 to 2 do
        begin
          P := VectorSubtract(Vertices.Items[fg.VertexIndices.Items[j * 3 + k]
            ], center);
          ...
          fg.TexCoordIndices.Add
            (TexCoords.FindOrAdd(AffineVectorMake((u / 3 + uoff),
            (v / 4 + voff), 0)));
        end;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (j >= 3) and (j mod 3 = 0) then
        begin
          if (triU[1] - triU[0] > MaxUDiff) or (triU[2] - triU[0] > MaxUDiff)
          then
          begin
            k := TexCoords.FindOrAdd(AffineVectorMake(triU[0] + 1, v, 0));
            fg.TexCoordIndices.Items[j - 3] := k;
          end;
          if (triU[0] - triU[1] > MaxUDiff) or (triU[2] - triU[1] > MaxUDiff)
          then
          begin
            k := TexCoords.FindOrAdd(AffineVectorMake(triU[1] + 1, v, 0));
            fg.TexCoordIndices.Items[j - 2] := k;
          end;
          if (triU[0] - triU[2] > MaxUDiff) or (triU[1] - triU[2] > MaxUDiff)
          then
          begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        P := VectorNormalize
          (VectorSubtract(Vertices.Items[fg.VertexIndices.Items[j]], center));
...
begin
  ...
  with Form1.GLFreeForm1.MeshObjects.Items[0] do
  begin
    for i := 0 to FaceGroups.Count - 1 do
    begin
      fg := TFGVertexNormalTexIndexList(FaceGroups.Items[i]);
      for j := 0 to fg.TexCoordIndices.Count - 1 do
      begin
        if j = 0 then
        begin
          x0 := round(TexCoords.Items[fg.TexCoordIndices.Items[j]].X *
            Form1.Image1.Width);
          y0 := round(TexCoords.Items[fg.TexCoordIndices.Items[j]].Y *
            Form1.Image1.Height);
          Image1.Canvas.MoveTo(x0, Image1.Height - y0);
        end
        else
        begin
          X := round(TexCoords.Items[fg.TexCoordIndices.Items[j]].X *
            Form1.Image1.Width);
          Y := round(TexCoords.Items[fg.TexCoordIndices.Items[j]].Y *
            Form1.Image1.Height);
...
begin
  ...
  case ComboBox2.ItemIndex of
    0:
      UVPlanarMapping(mo, min, max, ComboBox1.ItemIndex);
    1:
      UVCubicMapping(mo, min, max, ComboBox1.ItemIndex);
    2:
      UVCylindricalMapping(mo, min, max, ComboBox1.ItemIndex, 0.8);
    3:
      UVSphericalMapping(mo, min, max, ComboBox1.ItemIndex, 0.8);
  end;
...
begin
  for m := 0 to GLFreeForm1.MeshObjects.Count - 1 do
    with GLFreeForm1.MeshObjects.Items[m] do
    begin
      for i := 0 to FaceGroups.Count - 1 do
      begin
        ...
        begin
          ...
          fg.VertexIndices.Assign(TFGVertexIndexList(FaceGroups.Items[i])
            .VertexIndices);
