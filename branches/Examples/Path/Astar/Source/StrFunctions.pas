...
interface
  ...
  function IfThen(b : boolean; s_true, s_false : string) : string;
  function BeginsWith(sShortString, sLongString : string) : boolean;

implementation

function BeginsWith(sShortString, sLongString : string) : boolean;
begin
  result := RightStr(sLongString, Length(sShortString))=sShortString;
end;
