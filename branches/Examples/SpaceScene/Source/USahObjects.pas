...
procedure BuildCube(const Tile:double=1); // Cube of radius 1 centred on the origin
...
begin
  ...
  Inr:=InV; rr:=DegToRadian(rv);
  Ino:=InN; ro:=DegToRadian(rn);
  dWu:=(Moment-(Inr-Ino)*rr*ro)/InU;
  dWn:=(-(Ini-Inr)*ri*rr)/InN;
  {Rotor axis: N, Output axis: V}
  Inr:=InN; rr:=DegToRadian(rn);
  Ino:=InV; ro:=DegToRadian(rv);
