...
procedure TSMaster.FormCreate(Sender: TObject);
begin
  inherited;
  GetCurrentDir;
...
procedure TSMaster.FormShow(Sender: TObject);
var
  i: Integer;
begin
  if not nthShow then
  begin
    dmSMaster.OpenDialog.Filter := VectorFileFormatsFilter;
    dmSMaster.SaveDialog.Filter := VectorFileFormatsSaveFilter;
    ApplyFSAA;
    ApplyFaceCull;
    ApplyFPS;
...
procedure TSMaster.acFileExitExecute(Sender: TObject);
begin
  Close;
end;

procedure TSMaster.acFileOpenExecute(Sender: TObject);
begin
  NavCube.ActiveMouse := False;
  if dmSMaster.OpenDialog.Execute then
    DoOpen(dmSMaster.OpenDialog.fileName);
end;

procedure TSMaster.acFileOpenTexLibExecute(Sender: TObject);
var
  I: Integer;
begin
  if dmSMaster.ODTextures.Execute then
    with MaterialLib do
    begin
      LoadFromFile(dmSMaster.ODTextures.fileName);
...
procedure TSMaster.acFilePickExecute(Sender: TObject);
begin
  if dmSMaster.opDialog.Execute then
  begin
...
procedure TSMaster.acFileSaveAsExecute(Sender: TObject);
var
  ext : String;
begin
  if dmSMaster.SaveDialog.Execute then
  begin
    ext := ExtractFileExt(dmSMaster.SaveDialog.fileName);
    if ext = '' then
      dmSMaster.SaveDialog.fileName :=
        ChangeFileExt(dmSMaster.SaveDialog.fileName,
        '.' + GetVectorFileFormats.FindExtByIndex
        (dmSMaster.SaveDialog.FilterIndex, False, True));
    if GetVectorFileFormats.FindFromFileName(dmSMaster.SaveDialog.fileName) = nil
    then
      ShowMessage(_('Unsupported or unspecified file extension.'))
    else
      ffObject.SaveToFile(dmSMaster.SaveDialog.fileName);
  end;
end;

procedure TSMaster.acFileSaveTexturesExecute(Sender: TObject);
begin
  if dmSMaster.SDTextures.Execute then
    MaterialLib.SaveToFile(dmSMaster.SDTextures.fileName);
end;


procedure TSMaster.snViewerBeforeRender(Sender: TObject);
begin
...
procedure TSMaster.snViewerAfterRender(Sender: TObject);
begin
  ApplyFSAA;
  Screen.Cursor := crDefault;
end;

procedure TSMaster.DoResetCamera;
var
  objSize: Single;
begin
...
procedure TSMaster.ApplyShadeModeToMaterial(aMaterial: TGLMaterial);
begin
...
procedure TSMaster.ApplyShadeMode;
var
  i: Integer;
begin
  with MaterialLib.Materials do
...
procedure TSMaster.ApplyFaceCull;
begin
  with snViewer.Buffer do
  begin
...
procedure TSMaster.ApplyBgColor;
var
  bmp: TBitmap;
  col: TColor;
begin
  ...
  try
    bmp.Width := 16;
    bmp.Height := 16;
    col := ColorToRGB(dmSMaster.ColorDialog.Color);
...
procedure TSMaster.ApplyTexturing;
var
  i: Integer;
begin
  with MaterialLib.Materials do
...
procedure TSMaster.SetupFreeFormShading;
var
  i: Integer;
...
procedure TSMaster.DoOpen(const FileName: String);
var
...
procedure TSMaster.snViewerMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  mx := X;
  my := Y;
...
procedure TSMaster.snViewerMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
var
  d: Single;
begin
...
procedure TSMaster.snViewerMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  md := False;
end;

procedure TSMaster.FormMouseWheel(Sender: TObject; Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
begin
...
procedure TSMaster.MaterialLibTextureNeeded(Sender: TObject;
  var textureFileName: String);
begin
...
procedure TSMaster.acInvertNormalsExecute(Sender: TObject);
var
  i: Integer;
begin
...
procedure TSMaster.acReverseRenderingOrderExecute(Sender: TObject);
var
  i, j, n: Integer;
  fg: TGLFaceGroup;
begin
...
procedure TSMaster.acSaveAsUpdate(Sender: TObject);
begin
  acFileSaveAs.Enabled := (ffObject.MeshObjects.Count > 0);
end;

procedure TSMaster.acHelpAboutExecute(Sender: TObject);
begin
  with TSAbout.Create(Self) do
    try
      ShowModal;
    finally
      Free;
...
procedure TSMaster.acAADefaultExecute(Sender: TObject);
begin
  (Sender as TAction).Checked := True;
  ApplyFSAA;
end;

procedure TSMaster.acConvertToIndexedTrianglesExecute(Sender: TObject);
var
  v: TAffineVectorList;
  i: TIntegerList;
  m: TMeshObject;
...
procedure TSMaster.acStripifyExecute(Sender: TObject);
var
  i: Integer;
  mo: TMeshObject;
  fg: TFGVertexIndexList;
...
procedure TSMaster.acViewFlatShadingExecute(Sender: TObject);
begin
  ApplyShadeMode;
end;

procedure TSMaster.acViewHiddenLinesExecute(Sender: TObject);
begin
  ApplyShadeMode;
end;

procedure TSMaster.acViewResetExecute(Sender: TObject);
begin
  DoResetCamera;
end;

procedure TSMaster.acViewFlatLinesExecute(Sender: TObject);
begin
  ApplyShadeMode;
end;

procedure TSMaster.acViewSmoothShadingExecute(Sender: TObject);
begin
  ApplyShadeMode;
end;

procedure TSMaster.acViewWireFrameExecute(Sender: TObject);
begin
  ApplyShadeMode;
end;

procedure TSMaster.acViewZoomInExecute(Sender: TObject);
var
  h: Boolean;
begin
...
procedure TSMaster.acViewZoomOutExecute(Sender: TObject);
var
  h: Boolean;
begin
  FormMouseWheel(Self, [], 120 * 4, Point(0, 0), h);
...
procedure TSMaster.acOptimizeExecute(Sender: TObject);
begin
...
procedure TSMaster.acToolsOptionsExecute(Sender: TObject);
begin
  with TSOptions.Create(Self) do
    try
      ShowModal;
    finally
      Free;
...
procedure TSMaster.acToolsFaceCullingExecute(Sender: TObject);
begin
  acToolsFaceCulling.Checked := not acToolsFaceCulling.Checked;
  ApplyFaceCull;
end;

procedure TSMaster.acToolsInfoExecute(Sender: TObject);
begin
  with TSDialog.Create(Self) do
  try
...
procedure TSMaster.acToolsLightingExecute(Sender: TObject);
begin
...
procedure TSMaster.acToolsShowFPSExecute(Sender: TObject);
begin
  acToolsShowFPS.Checked := not acToolsShowFPS.Checked;
  ApplyFPS;
end;

procedure TSMaster.acToolsTexturingExecute(Sender: TObject);
begin
...
procedure TSMaster.acObjectsExecute(Sender: TObject);
var
  i: Integer;
  Color : TVector3f;
  Points: TGLPoints;
...
procedure TSMaster.CadencerProgress(Sender: TObject; const deltaTime, newTime: Double);
begin
...
procedure TSMaster.acNavCubeExecute(Sender: TObject);
begin
...
procedure TSMaster.TimerTimer(Sender: TObject);
begin
  StatusBar.Panels[3].Text := Format('%.1f  FPS', [snViewer.FramesPerSecond]);
  snViewer.ResetPerformanceMonitor;
end;

procedure TSMaster.ReadIniFile;
begin
  inherited;
...
procedure TSMaster.WriteIniFile;
begin
  IniFile := TIniFile.Create(ChangeFileExt(Application.ExeName, '.ini'));
  with IniFile do
    try
