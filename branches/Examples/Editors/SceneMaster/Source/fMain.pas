...
begin
  ...
  aFolder := BinPath;
  Delete(aFolder, Length(aFolder) - 4, 4);
  AssetsPath := aFolder + 'Assets\';
  if not DirectoryExists(AssetsPath) then ForceDirectories(AssetsPath);
...
//  acNavCubeExecute(Self);
  acNavCube.Enabled := True;
  DoResetCamera;
end;
