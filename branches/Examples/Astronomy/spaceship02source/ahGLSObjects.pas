unit ahGLSobjects;
{Collection of OpenGL objects}

interface

uses
  System.Math,
  OpenGL1x, GLVectorGeometry, GLVectorFileObjects, GLScene,
  OpenGLTokens;

procedure BuildCube(const Tile:double=1); // Cube of radius 1 centred on the origine
procedure BuildStarField(const nbStars:integer); {Stars randomly placed on a sphere of radius 1}
...
type
  ...
  public
    ...
    procedure Move(const du,dv,dn:single);// Translation in the camera system
    ...
    procedure Translate(const dx,dy,dz:single);// Translation in the world system
    procedure UpdateAll(const Time:double); // Compute new coordinates and angles
...
begin
  ...
  glBegin(GL_QUADS);
    ...
    glTexCoord2f(Tile, 0.0); glVertex3f( 1, -1,  1);
    glTexCoord2f(Tile, Tile); glVertex3f( 1,  1,  1);
    glTexCoord2f(0.0, Tile); glVertex3f(-1,  1,  1);
  glEnd();
    // Back Face
  glBegin(GL_QUADS);
    glNormal3f( 0.0, 0.0,-1.0);
...
begin
  ...
  glBegin(GL_POINTS);
    glColor3d(1,1,1);
    for i:=1 to Round(0.75*nbStars) do begin
      a:=random*2*pi;
      b:=random*pi-pi/2;
      SinCos(a,s,c);
      SinCos(b,sb,cb);
  ...
  glBegin(GL_POINTS);
    glColor3d(1,1,1);
    for i:=1 to Round(0.25*nbStars) do begin
      a:=random*2*pi;
      b:=random*pi-pi/2;
      SinCos(a,s,c);
      SinCos(b,sb,cb);
...
begin
  s:=1/Speed;
  SpeedMatrix[0]:=ux;	SpeedMatrix[4]:=vx; SpeedMatrix[8]:=sx*s;	SpeedMatrix[12]:=x;
  SpeedMatrix[1]:=uy;  	SpeedMatrix[5]:=vy; SpeedMatrix[9]:=sy*s;	SpeedMatrix[13]:=y;
  SpeedMatrix[2]:=uz;	SpeedMatrix[6]:=vz;	SpeedMatrix[10]:=sz*s;	SpeedMatrix[14]:=z;
  SpeedMatrix[3]:=0;	SpeedMatrix[7]:=0;  SpeedMatrix[11]:=0;		SpeedMatrix[15]:=1;
...
begin
  ...
  Inr:=InV; rr:=DegToRad(rv);
  Ino:=InN; ro:=DegToRad(rn);
  dWu:=(Moment-(Inr-Ino)*rr*ro)/InU;
  dWn:=(-(Ini-Inr)*ri*rr)/InN;
  {Rotor axis: N, Output axis: V}
  Inr:=InN; rr:=DegToRad(rn);
  Ino:=InV; ro:=DegToRad(rv);
